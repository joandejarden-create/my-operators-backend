...
Any VIC → BE sandbox patch runner must call `assertSandboxReadyForVicBePatch()` from `lib/research-engine-v2/airtable-sandbox-validation.js`. If validation is not `airtable_sandbox_validated_ready_for_vic_be_patch`, execution must throw and abort.
