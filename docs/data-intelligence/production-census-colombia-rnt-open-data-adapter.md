...
2. Run HPC match + gated plan: `npm run census:colombia-rnt-hpc-match-plan -- --year 2026 --hotels-only --max-rows 500`
3. Match report: `docs/data-intelligence/production-census-colombia-rnt-hpc-match-plan.md`
4. Only after steward review: design controlled apply with `--confirm-colombia-rnt-steward-insert` + `--confirm-no-owner-operator-writes` (not implemented yet).
5. Note: RNT `razon_social` often differs from branded commercial names — expect low exact HPC name matches until brand/URL enrichment.
