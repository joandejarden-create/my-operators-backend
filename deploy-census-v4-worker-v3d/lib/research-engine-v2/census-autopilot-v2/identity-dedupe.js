...
export function classifyAndDedupe(candidates, vicRecords) {
  ...
  for (const c of candidates) {
    ...
    for (const v of vicIndex) {
      ...
      const score = tokenSimilarity(n, v._n);
      if (!bestVic || score > bestVic.score) bestVic = { row: v, score };
    }
    ...
    // Intra-candidate exact-name dedupe key
    const key = `${country.toLowerCase()}|${n}`;
