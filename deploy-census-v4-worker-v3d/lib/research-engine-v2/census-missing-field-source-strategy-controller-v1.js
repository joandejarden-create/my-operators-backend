...
export async function runCensusMissingFieldSourceStrategyControllerV1Mission(opts = {}) {
  ...
  // Autopilot apply-guard env confirms (required when enableProductionWrites).
  if (opts.enableProductionWrites) {
    env.ALLOW_CENSUS_AUTOPILOT_APPLY = "1";
    env.CONFIRM_WRITE_TO_PRODUCTION_CENSUS = "1";
    env.CONFIRM_NO_BRAND_EXPLORER_WRITES = "1";
    env.CONFIRM_NO_OWNER_OPERATOR_WRITES = "1";
  }
  ...
  const policyReport = await runCensusAutopilotPolicyControllerV1Mission({
    ...
    args: {
      ...
      batchSize: Number(opts.args?.batchSize || process.env.CENSUS_FIELD_STRATEGY_BATCH_SIZE || 20),
