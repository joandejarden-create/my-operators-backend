...
let updated = head.replace(
  /https:\/\/cdn\.prod\.website-files\.com\/68108c29063eeb5d1bd7ae4a\/[^"]*dealality-old-home-dark\.v[^."]+\.css/g,
  NEW_CSS
);

if (!updated.includes('v20260728ag')) {
  throw new Error('CSS URL replace failed');
}

if (!updated.includes('#insights-grid,.oh-insights-grid{display:flex!important')) {
  if (!updated.includes('</style>')) throw new Error('missing </style>');
  updated = updated.replace('</style>', APPEND_CSS + '\n</style>');
}

fs.writeFileSync('tmp-old-home-head-updated.txt', updated);
fs.writeFileSync('tmp-footer-only.txt', footer);
fs.writeFileSync(
