...
import { applyBrandExplorerAiAssistedFootnote } from "../lib/partner-intelligence/brand-explorer-ai-assisted-footnote.js";
...
import { getDiscoveryBrandConfig } from "../lib/partner-intelligence/brand-explorer-brand-asset-image-governance.js";
...
const BRAND_LIBRARY_SLUG_ALIASES = Object.freeze({
  "kimpton-hotels": "kimpton",
  autograph: "autograph-collection",
  "autograph-hotels": "autograph-collection",
...
/**
 * Resolve Brand Explorer CLI slugs (e.g. design-hotels) to Airtable record IDs or canonical names.
 * Keeps GET /api/brand-library/brand working for ?id=design-hotels deep links.
 */
function resolveBrandLibraryLookupIdentifier(rawIdentifier) {
  const decoded = decodeURIComponent(String(rawIdentifier || "")).trim();
  if (!decoded) return { lookupId: "", lookupName: "", resolutionSource: "empty" };
  if (/^rec[a-zA-Z0-9]{10,}$/.test(decoded)) {
    const legacyByRecord = getLegacySeedBrand(decoded);
    if (legacyByRecord?.recordId) {
      return {
        lookupId: legacyByRecord.recordId,
        lookupName: legacyByRecord.name || "",
        resolutionSource: "legacy_approved_seed_record_id",
        slug: legacyByRecord.slug,
      };
    }
    return { lookupId: decoded, lookupName: "", resolutionSource: "record_id" };
  }
  const normalizedSlug = BRAND_LIBRARY_SLUG_ALIASES[decoded.toLowerCase()] || decoded.toLowerCase();
  const slugConfig = getActiveProfileBrandConfig(normalizedSlug);
  if (slugConfig?.recordId) {
    return {
      lookupId: slugConfig.recordId,
      lookupName: slugConfig.name || "",
      resolutionSource: "active_profile_slug",
      slug: slugConfig.slug || normalizedSlug,
    };
  }
  const legacySeed = getLegacySeedBrand(normalizedSlug) || getLegacySeedBrand(decoded);
  if (legacySeed?.recordId) {
    return {
      lookupId: legacySeed.recordId,
      lookupName: legacySeed.name || "",
      resolutionSource: "legacy_approved_seed_slug",
      slug: legacySeed.slug || normalizedSlug,
    };
  }
  const discoveryConfig = getDiscoveryBrandConfig(normalizedSlug);
  if (discoveryConfig?.recordId) {
    return {
      lookupId: discoveryConfig.recordId,
      lookupName: discoveryConfig.name || "",
      resolutionSource: "discovery_slug",
      slug: discoveryConfig.slug || normalizedSlug,
    };
  }
  return { lookupId: "", lookupName: decoded, resolutionSource: "brand_name" };
}
...
const BRAND_EXPLORER_FILTER_OPTIONS_CATALOG = {
  ...
  // Separate is multi-select-only (no dedicated checkbox column yet); keep in catalog for Explorer filters.
  acceptableAgreementTypes: [
    ...PROJECT_FIT_ACCEPTABLE_AGREEMENT_TYPES_COLUMNS.map((c) => c.formValue),
...
/**
 * Presentation rows are keyed by Brand Name text, which can diverge from Brand
 * Basics primary name (e.g. Basics "SO/" vs Presentation "SO/ Hotels & Resorts").
 * When Brand Name = Basics name returns a sparse linked subset, also merge
 * known presentation aliases so public display loads the factory cohort rows.
 */
const BRAND_EXPLORER_PRESENTATION_BRAND_NAME_ALIASES = Object.freeze({
  "SO/": Object.freeze(["SO/ Hotels & Resorts"]),
  "SO/ Hotels & Resorts": Object.freeze(["SO/"]),
});
...
async function selectAllLinkedToBrandBasics(base, tableName, brandRecordId, brandName) {
  ...
  const escapedName = (brandName || '').replace(/"/g, '\\"');
  const linkFieldNames = ['Brand', 'Brand_Basic_ID', 'Brand Setup - Brand Basics', 'Brand Basics'];
  const aliasNames = BRAND_EXPLORER_PRESENTATION_BRAND_NAME_ALIASES[String(brandName || "").trim()] || [];
  ...
  const namesForBrandNameFormula = [
    ...(escapedName ? [escapedName] : []),
    ...aliasNames.map((n) => String(n).replace(/"/g, '\\"')).filter(Boolean),
  ];
  for (const name of [...new Set(namesForBrandNameFormula)]) {
    formulaFetches.push(
      base(tableName)
        .select({ filterByFormula: `{Brand Name} = "${name}"`, maxRecords: 500 })
        .all()
        .catch(() => [])
    );
  }
  ...
  // Prefer Brand Name / Brand formula hits. If aliases contributed rows, do not
  // short-circuit away from them — merged already includes Basics-name + aliases.
...
/** Original filename from first Image attachment (for uniqueness / near-duplicate detection). */
function firstAttachmentFilenameFromFields(fields) {
  if (!fields || typeof fields !== "object") return "";
  const names = ["Image", "Images", "Scenario Image", "Attachments", "Photo", "Photos"];
  for (const name of names) {
    const val = getFieldValue(fields, name);
    if (!Array.isArray(val) || !val.length) continue;
    const fn = (val[0]?.filename || val[0]?.name || "").toString().trim();
    if (fn) return fn;
  }
  return "";
}
...
function normalizeBrandExplorerPresentationRecords(records) {
  ...
  for (const rec of list) {
    ...
    const externalDisplayStatus = (getFieldValue(f, 'External Display Status') ?? '').toString().trim();
    if (externalDisplayStatus === 'Do Not Display' || externalDisplayStatus === 'Internal Only') continue;
    ...
    const imageFilename = firstAttachmentFilenameFromFields(f);
    const summaryUrlRaw = (
...
export async function getBrandLibraryBrandById(req, res) {
  try {
    ...
    const resolvedLookup = resolveBrandLibraryLookupIdentifier(decodedId);
    const lookupId = resolvedLookup.lookupId;
    const lookupName = resolvedLookup.lookupName || decodedId;

    console.log(`Decoded identifier: ${decodedId}`);
    if (resolvedLookup.resolutionSource === "active_profile_slug") {
      console.log(`Resolved slug to record ID: ${lookupId} (${resolvedLookup.lookupName || decodedId})`);
    } else if (
      resolvedLookup.resolutionSource === "legacy_approved_seed_slug" ||
      resolvedLookup.resolutionSource === "legacy_approved_seed_record_id"
    ) {
      console.log(
        `Resolved legacy seed to record ID: ${lookupId} (${resolvedLookup.lookupName || decodedId})`
      );
    }

    // Check if it looks like a record ID (starts with 'rec')
    if (lookupId && lookupId.startsWith('rec')) {
      try {
        brandRecord = await base(F.brandBasics.table).find(lookupId);
    ...
    if (!brandRecord) {
      const brandNameLookup = lookupName;
    ...
    // Surface Active Profile Approved Date for footnote Last Reviewed fallbacks / audits.
    if (brandFields["Active Profile Approved Date"] && !brandDetails.activeProfileApprovedDate) {
      brandDetails.activeProfileApprovedDate = String(brandFields["Active Profile Approved Date"])
        .trim()
        .split("T")[0];
    }

    // Brand Explorer global requirement: always-on AI-Assisted Profile footnote
    // (Last Reviewed · Source Basis · Region). Computed fallbacks — no CV / release writes.
    try {
      applyBrandExplorerAiAssistedFootnote(brandDetails, {
        brandBasicsFields: brandFields,
      });
    } catch (footnoteErr) {
      if (process.env.NODE_ENV !== "production") {
        console.warn(
          "[Brand Library] AI-Assisted footnote enrichment failed:",
          footnoteErr?.message || footnoteErr
        );
      }
    }
    ...
    const resolvedSlug =
      resolvedLookup?.slug ||
      brandDetails.slug ||
      (typeof decodedId === "string" && !/^rec/i.test(decodedId) ? decodedId : "");
    const legacySeed =
      resolveLegacyApprovedSeed({
        slug: resolvedSlug,
    ...
    const legacyHistoricalApproved = Boolean(legacySeed) || isLegacyApprovedSeedSlug(resolvedSlug);
    if (legacySeed?.slug && !brandDetails.slug) brandDetails.slug = legacySeed.slug;
    else if (resolvedSlug && !brandDetails.slug) brandDetails.slug = resolvedSlug;
    if (legacyHistoricalApproved) brandDetails.legacyHistoricalApproved = true;

    const displayStateMeta = resolveBrandExplorerDisplayState(brandDetails, {
      brandBasics: brandFields,
      legacyHistoricalApproved,
    });
    brandDetails.brandExplorerDisplayState = displayStateMeta.brandExplorerDisplayState;
    brandDetails.shouldRenderFullProfile = displayStateMeta.shouldRenderFullProfile;
    brandDetails.shouldSuppressIncompleteExternalSections =
      displayStateMeta.shouldSuppressIncompleteExternalSections;
    brandDetails.shouldHideExternalProfile = displayStateMeta.shouldHideExternalProfile;
    brandDetails.brandExplorerDisplayCompleteness = displayStateMeta.completeness;
    brandDetails.brandExplorerDisplayBlockers = displayStateMeta.blockers;
    // Surface release gates on the API brand so downstream lock/OS checks don't re-resolve blind.
    brandDetails.founderVisualReviewPass = displayStateMeta.completeness?.founderVisualReviewPass === true;
    brandDetails.activeProfileApproved = displayStateMeta.completeness?.activeProfileApproved === true;
    brandDetails.readyForActiveProfile = displayStateMeta.completeness?.activeProfileApproved === true;

    // Factory Preview metadata only — never mutates public shouldRenderFullProfile / displayState.
    brandDetails.factoryPreview = buildFactoryPreviewApiMeta(brandDetails);
    if (!brandDetails.slug && brandDetails.factoryPreview?.slug) {
      brandDetails.slug = brandDetails.factoryPreview.slug;
    }
...
const OPERATIONAL_SUPPORT_FORM_TO_AIRTABLE = [
  ...
  { form: 'revenueManagementOther', airtable: 'Revenue Management Other' },
  { form: 'salesMarketingOther', airtable: 'Sales Marketing Other' },
  { form: 'accountingReportingOther', airtable: 'Accounting Reporting Other' },
  { form: 'procurementServicesOther', airtable: 'Procurement Services Other' },
  { form: 'hrTrainingServicesOther', airtable: 'HR Training Services Other' },
  { form: 'technologyServicesOther', airtable: 'Technology Services Other' },
  { form: 'designRenovationSupportOther', airtable: 'Design Renovation Support Other' },
  { form: 'developmentServicesOther', airtable: 'Development Services Other' },
