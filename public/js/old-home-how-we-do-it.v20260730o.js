/**
 * Old Home — How We Do It compact one-screen process (v20260730o)
 * Path-gated to /old-home.
 * Click left nav to swap the right step in place. No scroll runway.
 * Locks right panel to tallest step so CTA does not jump.
...
(function () {
  try {
    ...
    var CSS_HREF =
      "https://cdn.prod.website-files.com/68108c29063eeb5d1bd7ae4a/6a6b59cb2e0c999e6a8ef508_dealality-old-home-how-we-do-it.v20260730o.css";
    if (window.__ohHowWeDoIt >= 202607318) return;
    window.__ohHowWeDoIt = 202607318;
    var IMG_BRAND =
    ...
    function ensureCss() {
      ...
      if (existing) {
        ...
        existing.setAttribute("data-oh-how", "30o");
        return;
      }
      if (document.querySelector('link[data-oh-how="30o"]')) return;
      var link = document.createElement("link");
      link.rel = "stylesheet";
      link.href = CSS_HREF;
      link.setAttribute("data-oh-how", "30o");
    ...
    function shotHtml(step) {
      if (step.img) {
        return (
          ...
          (step.visual === "compare" ? " is-compare-shot" : "") +
          '">' +
          '<img src="' +
          esc(step.img) +
          '" alt="' +
          esc(step.alt || "") +
          '" loading="eager" decoding="async" width="960" height="96">' +
          "</div>"
        );
      }
      return '<div class="dealality-process_shot is-empty" aria-hidden="true"></div>';
    }
    ...
    function mount() {
      ...
      // Keep existing 30o mount so height lock / images are not reset by retries
      if (existing && existing.getAttribute("data-oh-how") === "30o") return true;
    ...
    function bind(section) {
      ...
      // CSS already fixes visual/shot/copy heights. Keep the content box
      // at the tallest measured step without clearing mid-switch (no bounce).
      function lockPanelHeight() {
        ...
        probe.style.cssText =
          ...
          content.clientWidth +
          "px;box-sizing:border-box;padding:14px 16px;";
        var max = content.offsetHeight || 540;
        ...
        // Measure one clone at a time
        content.parentNode.appendChild(probe);
      ...
      Array.prototype.forEach.call(content ? content.querySelectorAll("img") : [], function (img) {
        ...
        img.addEventListener("load", lockPanelHeight, { once: true });
        img.addEventListener("error", lockPanelHeight, { once: true });
      });
      window.addEventListener("resize", function () {
        clearTimeout(section._ohHeightTimer);
        section._ohHeightTimer = setTimeout(lockPanelHeight, 120);
      });
