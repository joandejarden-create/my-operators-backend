/**
 * Old Home — How We Do It compact one-screen process (v20260730r)
...
(function () {
  try {
    ...
    var CSS_HREF =
      "https://cdn.prod.website-files.com/68108c29063eeb5d1bd7ae4a/6a6b5e07c0ab3436e27d1ca1_dealality-old-home-how-we-do-it.v20260730r.css";
    if (window.__ohHowWeDoIt >= 202607320) return;
    window.__ohHowWeDoIt = 202607320;
    var IMG_BRAND =
    ...
    function ensureCss(onReady) {
      ...
      if (existing) {
        var same = (existing.getAttribute("href") || "").indexOf("v20260730r.css") !== -1;
        existing.setAttribute("href", CSS_HREF);
        existing.setAttribute("data-oh-how", "30r");
        if (same) ready();
        else {
      ...
      if (document.querySelector('link[data-oh-how="30r"]')) {
        ready();
        return;
      }
      ...
      link.rel = "stylesheet";
      link.href = CSS_HREF;
      link.setAttribute("data-oh-how", "30r");
    ...
    function visualHtml(step) {
      ...
      if (step.visual === "paths") {
        return (
          ...
          '<div class="dealality-process_shot is-paths-shot">' +
          (step.img
            ? '<img src="' +
              esc(step.img) +
              '" alt="' +
    ...
    function mount() {
      ...
      if (existing && existing.getAttribute("data-oh-how") === "30r") return true;
