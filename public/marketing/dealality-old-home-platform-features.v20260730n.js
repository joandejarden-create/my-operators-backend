/**
 * Old Home — Features image-overlay hover tiles (v20260730n)
...
(function () {
  try {
    ...
    if (window.__ohPlatformFeaturesTiles >= 202607313) return;
    window.__ohPlatformFeaturesTiles = 202607313;
    ...
    function ensureCss() {
      ...
      for (i = 0; i < links.length; i++) {
        ...
        links[i].setAttribute("data-oh-pf", "30n");
      }
      if (document.querySelector('link[data-oh-pf="30i"]')) return;
      var link = document.createElement("link");
      link.rel = "stylesheet";
      link.href = CSS_HREF;
      link.setAttribute("data-oh-pf", "30n");
    ...
    function apply() {
      ...
      if (!grid) return;
      if (grid.getAttribute("data-oh-pf-tiles") === "30n") return;
      grid.innerHTML = TILES.map(cardHtml).join("");
      grid.setAttribute("data-oh-pf-tiles", "30n");
    }
