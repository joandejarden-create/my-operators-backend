/**
 * Old Home — How We Do It sticky-stage process story (v20260730i)
 * Path-gated to /old-home.
 * Desktop: tall scroll track + sticky stage; one step visible while scrolling.
 * Mobile: compact one-step view with nav click (no tall runway).
 * Overview tiles removed. Namespace: dealality-process_*
 */
(function () {
  try {
    ...
    var CSS_HREF =
      "https://cdn.prod.website-files.com/68108c29063eeb5d1bd7ae4a/6a6b3f281ea68ba2a786c15b_dealality-old-home-how-we-do-it.v20260730i.css";
    if (window.__ohHowWeDoIt >= 202607312) return;
    window.__ohHowWeDoIt = 202607312;
    var IMG_BRAND =
    ...
    function ensureCss() {
      ...
      if (existing) {
        ...
        existing.setAttribute("data-oh-how", "30i");
        return;
      }
      if (document.querySelector('link[data-oh-how="30i"]')) return;
      var link = document.createElement("link");
      link.rel = "stylesheet";
      link.href = CSS_HREF;
      link.setAttribute("data-oh-how", "30i");
    ...
    function bind(section) {
      ...
      var stageTrack = section.querySelector("#dealality-process-runway");
      ...
      function stepFromTrackProgress() {
        if (!stageTrack) return 0;
        var rect = stageTrack.getBoundingClientRect();
        var trackH = stageTrack.offsetHeight || 1;
        var pinH = Math.min(window.innerHeight, trackH);
        var scrolled = Math.min(
          Math.max(-rect.top, 0),
          Math.max(trackH - pinH, 1)
        );
        var progressRatio = scrolled / Math.max(trackH - pinH, 1);
        var idx = Math.min(
          STEPS.length - 1,
          Math.floor(progressRatio * STEPS.length + 0.001)
        );
        return idx;
      }
      ...
      function scrollToStep(index) {
        ...
        if (useStickyStage() && stageTrack) {
          var trackTop =
            window.pageYOffset + stageTrack.getBoundingClientRect().top;
          var trackH = stageTrack.offsetHeight;
          var pinH = Math.min(window.innerHeight, trackH);
          var scrollable = Math.max(trackH - pinH, 0);
          var target =
            trackTop +
            (STEPS.length <= 1
              ? 0
              : (index / STEPS.length) * scrollable + 4);
          window.scrollTo({
            top: target,
      ...
      function onScroll() {
        if (!useStickyStage() || scrollingTo >= 0) return;
        if (raf) return;
        raf = window.requestAnimationFrame(function () {
          raf = 0;
          var idx = stepFromTrackProgress();
