/**
 * Old Home — Request a Demo (v20260730d)
 ...
 * Hero: Request a Demo as a secondary button beside Explore Your Hotel Opportunity.
 */
(function () {
  "use strict";
  ...
  try {
    ...
    if (window.__ohRequestDemo >= 202607304) return;
    window.__ohRequestDemo = 202607304;
    ...
    function injectCss() {
      ...
      css.textContent = [
        ...
        /* Hero dual CTA: Request a Demo button directly under/beside Explore form */
        "#form-subscribe-wrap.oh-fsw-wrap,#form-subscribe-wrap{align-items:center!important;gap:0!important}",
        "#oh-hero-cta-row{display:flex!important;flex-wrap:wrap!important;align-items:center!important;justify-content:center!important;gap:12px!important;width:100%!important;max-width:630px!important;margin:12px auto 0!important}",
        "#fsw-demo-link.oh-fsw-demo-btn,#fsw-demo-link{appearance:none!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;width:100%!important;height:52px!important;min-height:52px!important;padding:0 22px!important;margin:0!important;box-sizing:border-box!important;border-radius:12px!important;border:1px solid rgba(255,255,255,.34)!important;background:linear-gradient(180deg,rgba(255,255,255,.08),rgba(255,255,255,.02))!important;color:#fff!important;font-size:15px!important;font-weight:600!important;letter-spacing:-.01em!important;line-height:1!important;white-space:nowrap!important;text-decoration:none!important;cursor:pointer!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.12)!important;transition:background .2s ease,border-color .2s ease,transform .15s ease,box-shadow .2s ease!important}",
        "#fsw-demo-link.oh-fsw-demo-btn:hover,#fsw-demo-link:hover{background:rgba(183,162,252,.2)!important;border-color:rgba(183,162,252,.65)!important;color:#fff!important;box-shadow:0 10px 24px rgba(109,92,216,.22)!important;transform:translateY(-1px)!important}",
        ...
        "#fsw-secondary-wrap.oh-fsw-secondary-wrap{margin-top:14px!important}",
        ...
        "@media (max-width:640px){.oh-demo-toast{right:12px;left:12px;min-width:0;transform:translateY(-24px)}.oh-demo-toast.show{transform:translateY(0)}}",
      ].join("");
    ...
    function enhanceCtas() {
      ...
      // Hero: Request a Demo button beside Explore (visible dual CTA row)
      ...
      if (outer && shell && !demoBtn) {
        var row = document.getElementById("oh-hero-cta-row");
        if (!row) {
          row = document.createElement("div");
          row.id = "oh-hero-cta-row";
          row.setAttribute("role", "group");
          row.setAttribute("aria-label", "Primary actions");
          // Place after the form shell so the outline button sits clearly under/beside Explore
          if (shell.nextSibling) outer.insertBefore(row, shell.nextSibling);
          else outer.appendChild(row);
        }
      ...
      if (demoBtn) {
        ...
        demoBtn.textContent = "Request a Demo";
        demoBtn.setAttribute("aria-haspopup", "dialog");
        // Ensure row wraps the demo button and sits with the form
        var rowEl = document.getElementById("oh-hero-cta-row");
        if (rowEl && shell && shell.parentNode === outer) {
          // Keep Explore form first, demo button in the row beside/under it
          if (rowEl.previousElementSibling !== shell) {
            outer.insertBefore(rowEl, shell.nextSibling);
          }
        }
