/**
 * Old Home — hero headline rotator (v20260730g)
 * Path-gated to /old-home.
 * Restores stacked words above/below the active phrase (5 desktop / 3 mobile slots).
 * Replaces the single-line 30f clip that hid the stack.
 */
(function () {
  try {
    var path = (location.pathname || "").replace(/\/+$/, "").toLowerCase() || "/";
    if (path !== "/old-home") return;
    if (window.__ohHeroRotator >= 202607307) return;
    window.__ohHeroRotator = 202607307;

    // Remove prior single-slot override from 30f
    var stale = document.getElementById("oh-hero-rotator-30f");
    if (stale && stale.parentNode) stale.parentNode.removeChild(stale);

    var STYLE_ID = "oh-hero-rotator-30g";
    if (!document.getElementById(STYLE_ID)) {
      ...
      st.textContent = [
        ...
        "width:var(--hr-w,640px)!important;",
    ...
    function start() {
      if (window.__ohHeroRotatorStarted) return true;
