/**
 * Old Home — Features 9 equal tiles + compact Radar/Fee hover copy (v20260730e)
 * Path-gated to /old-home.
 * Radar → Market Intelligence; Fee Estimator → And More…
 * Image tiles: title by default, 3-line body on hover/focus.
 */
(function () {
  try {
    var path = (location.pathname || "").replace(/\/+$/, "").toLowerCase() || "/";
    if (path !== "/old-home") return;
    if (window.__ohPlatformFeaturesTiles >= 202607305) return;
    window.__ohPlatformFeaturesTiles = 202607305;

    var CSS_HREF =
      "https://cdn.prod.website-files.com/68108c29063eeb5d1bd7ae4a/6a6b3b04bd41299ab8bbf743_dealality-old-home-platform-features.v20260730e.css";
    ...
    function ensureCss() {
      ...
      for (i = 0; i < links.length; i++) {
        ...
        links[i].setAttribute("data-oh-pf", "30e");
      }
      if (document.querySelector('link[data-oh-pf="30e"]')) return;
      var link = document.createElement("link");
      link.rel = "stylesheet";
      link.href = CSS_HREF;
      link.setAttribute("data-oh-pf", "30e");
    ...
    function apply() {
      ...
      if (!grid) return;
      if (grid.getAttribute("data-oh-pf-tiles") === "30e") return;
      grid.innerHTML = TILES.map(cardHtml).join("");
      grid.setAttribute("data-oh-pf-tiles", "30e");
    }
