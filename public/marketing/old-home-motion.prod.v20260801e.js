...
(function () {
  ...
  function decorate() {
    ...
    // Platform = live #many-futures (legacy #platform-features kept as fallback)
    markSequence("features", [
      ...
      qs("#many-futures .mf-hotel, #many-futures article.mf-hotel, #platform-features-grid > article"),
    ]);
    markLift(
      qs("#many-futures .mf-hotel, #many-futures article.mf-hotel, #platform-features-grid > article")
    );
    ...
    markSequence("ecosystem", [
      ...
      qs("#ecosystem article.oh-eco-card-1, #ecosystem .oh-eco-card-1"),
    ]);
    ...
    // Same header stack as Benefits/FAQs (not one opaque card blob)
    markSequence("cta", [
  ...
  function boot() {
    ...
    // Late-injected sections (How We Do It, Manual Process, pricing copy) —
    // re-decorate until header stacks are tagged or we give up.
    var tries = 0;
    var t = window.setInterval(function () {
      ...
      if (mf && !mf.querySelector('.oh-m-reveal[data-oh-sec="features"]')) need = true;
      var eco = document.getElementById("ecosystem");
      if (eco && !eco.querySelector('.oh-m-reveal[data-oh-sec="ecosystem"]')) need = true;
      ...
      // If How already played before lead/principle were tagged, force them in
      // (otherwise they stay opacity:0 forever under an already-in section).
      var howPlayed = document.getElementById("oh-how-we-do-it");
      if (howPlayed && howPlayed.classList.contains("oh-m-section-in")) {
        qs('.oh-m-reveal[data-oh-sec="how"]').forEach(function (el) {
