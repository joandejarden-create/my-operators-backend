/**
 * Old Home testimonials carousel (v20260731t)
 * Path-gated to /old-home.
 * - One auto-advance pass, then manual only
 * - 2 tiles visible per slide (SoT relies on #oh-tt)
 * - Joan Dejarden quotes + Justin Boutwell (replaces former Brands card)
...
(function () {
  ...
  try {
    ...
    if (window.__ohTestimonials >= 202607319) return;
  } catch (ePath) {
    return;
  }
  ...
  /* Slide 0 = Joan + Justin; slide 1 = Operators + Advisors */
  var CARDS = [
    {
      quote:
  ...
  function start(root, viewport, slides, dotsWrap) {
    if (started) return;
    started = true;
    window.__ohTestimonials = 202607319;
  ...
  function tryBoot() {
    if (started || window.__ohTestimonials >= 202607319) return;
