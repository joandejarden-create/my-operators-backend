 ...
 * Thin orange square pins; ultra-slow spin; Americas-first. No atmosphere rim.
 */
(function () {
  // Newer builds win — skip if a newer globe already booted (stale body scripts).
  var GLOBE_BUILD = 202607296;
  ...
  // Dealality gold/orange used by the rotating headline
  var PIN_ORANGE = 0xd78e2c;
  var PIN_ORANGE_CSS = "#D78E2C";

  // Latin America + Caribbean market markers
  var LATAM_PINS = [
  ...
  function makePin(pos, sharedTexture) {
    ...
    // Sit slightly above the surface along the normal
    ...
    var scale = mobile ? 0.032 : 0.026;
    // Start invisible; staggered reveal animates scale + opacity in.
  ...
  function boot() {
    // Always use curated LATAM/Caribbean set (icon markers)
    ...
    // Nudge camera left so the full sphere sits on the right without a hard crop
    camera.position.set(-0.42, 0.1, 1.85);

    var globeRoot = new THREE.Group();
    // Start with LATAM centered on the visible face (not pushed to the right limb)
    ...
    // Atmosphere rim intentionally off — keeps navy hero scrim / copy readable.

    var pinTex = makePinTexture();
    var markers = [];
    var revealStart = performance.now() + (reduce ? 0 : 500);
    var revealStagger = reduce ? 0 : 280;
    var revealDur = reduce ? 1 : 520;
    for (var i = 0; i < pins.length; i++) {
      var p = latLonToVec3(pins[i].lat, pins[i].lon, radius);
      var pin = makePin(p, pinTex);
      pin.userData.revealAt = revealStart + i * revealStagger;
      if (reduce) {
        var spr0 = pin.userData.sprite;
        var bs0 = pin.userData.baseScale;
        spr0.material.opacity = 0.92;
    ...
    // Near-imperceptible spin (~2× slower than prior 0.000045)
    ...
    function frame(now) {
      ...
      var pulse = reduce
        ? 0.92
        : 0.82 + 0.14 * Math.sin((now - t0) * 0.0012);
      for (var m = 0; m < markers.length; m++) {
        var marker = markers[m];
        var spr = marker.userData.sprite;
        if (!spr || !spr.material) continue;
        var base = marker.userData.baseScale || 0.026;
        ...
        var e = easeOutBack(t);
        var s = base * Math.max(0.001, Math.min(1.15, e));
        spr.scale.set(s, s, 1);
        spr.material.opacity = Math.min(1, t * 1.15) * pulse;
        if (t >= 1) {
  ...
  // Reclaim after any stale body-registered globe scripts finish.
