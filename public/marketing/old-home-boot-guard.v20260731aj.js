/**
 * Old Home Boot Guard (v20260731aj)
 * Path-gated to /old-home.
 * Same as 31ai + section-type e (soft body copy lock → mf-feat-benefit).
 */
(function () {
  try {
    var path = (location.pathname || "").replace(/\/+$/, "").toLowerCase();
    if (path !== "/old-home") return;
    if (window.__ohBootGuard && window.__ohBootGuard >= 202607344) return;
    window.__ohBootGuard = 202607344;
