/**
 * Old Home Boot Guard (v20260801f)
 * Path-gated to /old-home.
 * 01f: freeform-head Explore size lock + prioritize hero globe — preload THREE + 512 texture, load globe JS
 *      ahead of below-fold scripts (testimonials/FAQs/pricing).
...
(function () {
  try {
    ...
    if (window.__ohBootGuard && window.__ohBootGuard >= 202608016) return;
    window.__ohBootGuard = 202608016;
    ...
    var GLOBE_TEX = "6a6dd9f3f5f1ad337d17fcb5_dealality-globe-texture-512.v20260801.jpg";
    var GLOBE_JS = "6a6dda820ad1f528b2bf2e50_dealality-old-home-hero-globe-bg.v20260801a.js";
    ...
    // --- Hero globe priority path (before CSS/below-fold JS contention) ---
    ...
    // Kick THREE download/execute immediately so globe boot often finds window.THREE.
    ...
    // Globe JS early (async) — not deferred behind FAQs/pricing/testimonials.
    if (
      GLOBE_JS.indexOf("PLACEHOLDER") === -1 &&
      !document.querySelector('script[data-oh-globe="01a"]')
    ) {
      var globeEarly = document.createElement("script");
      globeEarly.src = b + GLOBE_JS;
      globeEarly.async = true;
      globeEarly.setAttribute("data-oh-globe", "01a");
