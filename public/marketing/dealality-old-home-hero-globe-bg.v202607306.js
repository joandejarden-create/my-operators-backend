...
(function () {
  ...
  var GLOBE_BUILD = 202607305; // Dim pins behind rotating headline after reveal
  ...
  function boot() {
    ...
    var textZone = { left: 0, top: 0, right: 0, bottom: 0, ok: false };
    var textZoneAt = 0;
    var TEXT_ZONE_REFRESH_MS = 250;
    var PIN_DIM_DELAY_MS = 850;
    var PIN_DIM_IN_TEXT = 0.2;
    var PIN_DIM_NEAR_TEXT = 0.42;
    var PIN_DIM_FULL = 1;
    ...
    function refreshTextZone(now) {
      if (now - textZoneAt < TEXT_ZONE_REFRESH_MS && textZone.ok) return;
      textZoneAt = now;
      var nodes = [
        document.getElementById("hrwrap"),
        document.getElementById("rotator"),
        document.getElementById("hstatic"),
        document.getElementById("sst-rotate"),
        document.querySelector(".oh-sst-rotate"),
        document.getElementById("sst-bg1"),
        document.getElementById("sst-bg2"),
      ];
      var left = Infinity;
      var top = Infinity;
      var right = -Infinity;
      ...
      // Pad so ghost/stack words and pin squares near the active word are covered.
      var padX = mobile ? 28 : 56;
      var padY = mobile ? 36 : 72;
      textZone.left = left - padX;
      textZone.top = top - padY;
      textZone.right = right + padX * 1.35;
      textZone.bottom = bottom + padY;
      textZone.ok = true;
    }

    function pinScreenHit(marker) {
      marker.updateWorldMatrix(true, false);
      marker.getWorldPosition(_proj);
      _proj.project(camera);
      if (_proj.z < -1 || _proj.z > 1) return null;
      var cr = canvas.getBoundingClientRect();
      if (cr.width < 1 || cr.height < 1) return null;
      return {
        x: (_proj.x * 0.5 + 0.5) * cr.width + cr.left,
        y: (-_proj.y * 0.5 + 0.5) * cr.height + cr.top,
      };
    }
    ...
    function dimTargetForPin(marker, now) {
      ...
      if (!textZone.ok) return PIN_DIM_FULL;
      var hit = pinScreenHit(marker);
      if (!hit) return PIN_DIM_FULL;
      if (
        hit.x >= textZone.left &&
        hit.x <= textZone.right &&
        hit.y >= textZone.top &&
        hit.y <= textZone.bottom
      ) {
        return PIN_DIM_IN_TEXT;
      }
      // Soft fringe around the headline so pins don't stay hot at the edge.
      var fringe = mobile ? 36 : 64;
      if (
        hit.x >= textZone.left - fringe &&
        hit.x <= textZone.right + fringe &&
        hit.y >= textZone.top - fringe &&
        hit.y <= textZone.bottom + fringe
      ) {
        return PIN_DIM_NEAR_TEXT;
      }
      return PIN_DIM_FULL;
    }
