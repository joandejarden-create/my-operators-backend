/**
 * Old Home hero — subtle full-bleed 3D globe background (v20260801d).
 * Thin orange square pins; CALA cascade → ease toward Europe → Europe cascade;
 * CALA pins stay visible; ultra-slow spin after.
 * 01d: +4 MENA pins (N Africa ×2, UAE, Saudi) after Europe cascade.
 * 01c: hi-res self-hosted texture + clearer map read (opacity/tint/anisotropy).
 * 01b: restore self-hosted earthmap1k texture (512 was too soft on large canvases).
 * 309: wait for #hero-globe DOM, start THREE immediately, reclaim retries.
 */
(function () {
  // Newer builds win — skip if a newer globe already booted (stale body scripts).
  var GLOBE_BUILD = 202608014; // MENA pins
  ...
  // Existing hi-res CDN earth map (~2.5MB) — 1k is soft on 2k+ canvases.
  var TEX =
    "https://cdn.prod.website-files.com/68108c29063eeb5d1bd7ae4a/6a691e4f3b0bf638b1052fc6_dealality-globe-texture.jpg";
  var TEX_FALLBACK =
    "https://cdn.prod.website-files.com/68108c29063eeb5d1bd7ae4a/6a6de31ad2843e12336910f9_dealality-globe-texture-1k.v20260801.jpg";
  var THREE_SRC =
    "https://cdn.prod.website-files.com/68108c29063eeb5d1bd7ae4a/6a6dd9f378d1ce1e6262fb6d_three.r125.min.js";
  var THREE_FALLBACK =
  ...
  /**
   * Major European + MENA hotel markets — inland-biased; west → east cascade
   * as the globe turns toward Europe / Middle East.
   */
  var EUROPE_PINS = [
    { lat: 53.35, lon: -6.26, name: "Dublin" },
    { lat: 38.72, lon: -9.14, name: "Lisbon" },
    // Northern Africa (west)
    { lat: 33.59, lon: -7.62, name: "Casablanca" },
    ...
    // Northern Africa (east) + Middle East — UAE + Saudi
    { lat: 30.05, lon: 31.23, name: "Cairo" },
    { lat: 24.71, lon: 46.67, name: "Riyadh" },
  ...
  function bindDom() {
    root = document.getElementById("hero-globe");
    host = document.getElementById("hero-globe-container");
    list = document.getElementById("hero-globe-list");
  ...
  function boot() {
    ...
    try {
      ...
      if (typeof console !== "undefined" && console.warn) {
        console.warn("[oh-globe-bg] WebGL", err);
      }
      try {
    ...
    function applyEarthTex(tex) {
      ...
      if (tex.minFilter != null && THREE.LinearMipmapLinearFilter != null) {
        tex.minFilter = THREE.LinearMipmapLinearFilter;
      }
      if (tex.magFilter != null && THREE.LinearFilter != null) {
        tex.magFilter = THREE.LinearFilter;
      }
      material.map = tex;
      // Soft indigo wash — keep land/ocean contrast readable (not 0x9aa0ff washout).
      material.color = new THREE.Color(0xdfe3ff);
      material.opacity = mobile ? 0.48 : 0.56;
    ...
    loader.load(
      ...
      undefined,
      function () {
        loader.load(TEX_FALLBACK, applyEarthTex);
      }
    );
  ...
  function ensureThree() {
    if (!threePromise) {
      threePromise = loadScript(THREE_SRC).catch(function () {
        return loadScript(THREE_FALLBACK);
      }).catch(function (err) {
        threePromise = null;
        throw err;
      });
    ...
    ensureThree()
      .then(function () {
        try {
          ...
          if (typeof console !== "undefined" && console.warn) {
            console.warn("[oh-globe-bg] boot", err);
          }
        }
      })
      .catch(function (err) {
  ...
  function whenDomReady(cb) {
    if (document.getElementById("hero-globe-container")) {
      cb();
      return;
    }
    if (document.readyState === "loading") {
      ...
      // Body may still be streaming — poll briefly.
      var n = 0;
      var id = setInterval(function () {
        n++;
        if (document.getElementById("hero-globe-container") || n > 40) {
          clearInterval(id);
          cb();
        }
      }, 50);
  ...
  // Warm texture cache early (parallel with THREE download).
  try {
    var imgWarm = new Image();
    imgWarm.decoding = "async";
    imgWarm.src = TEX;
  } catch (_warm) {}

  // Start THREE ASAP (do not wait on idle — FOUC/CSS work was starving the globe).
  whenDomReady(function () {
    bindDom();
    startGlobe();
  });

  // Reclaim if a stale body script wiped the canvas, or DOM arrived late.
  function reclaim() {
    if (window.__ohGlobeBuild !== GLOBE_BUILD) return;
    if (booted) {
      if (document.getElementById("oh-globe-canvas")) return;
      // Canvas was removed — allow a rebuild.
      booted = false;
    }
    startGlobe();
  }
  window.addEventListener("load", reclaim);
  setTimeout(reclaim, 800);
  setTimeout(reclaim, 2000);
  setTimeout(reclaim, 4000);
})();
