(function () {
  try {
    ...
    var cssHref =
      "https://cdn.prod.website-files.com/68108c29063eeb5d1bd7ae4a/6a6a7cb895b20766ec5595f9_dealality-old-home-freeform-head.v20260729w22.css";
    var cssRe = /dealality-old-home-freeform-head\.v20260729w\d+\.css/i;
    var w21Re = /dealality-old-home-freeform-head\.v20260729w22\.css/i;

    function pinCss() {
      var kept = null;
      document.querySelectorAll('link[rel="stylesheet"]').forEach(function (link) {
        var href = link.getAttribute("href") || "";
        if (!cssRe.test(href)) return;
        if (w21Re.test(href)) {
          kept = link;
          return;
        }
        // Older boots re-inject prior freeform-head builds; keep w22 only.
        try {
          link.disabled = true;
      ...
      if (!kept) {
        ...
        link.rel = "stylesheet";
        link.href = cssHref;
        link.setAttribute("data-oh-problem-css", "1");
        document.head.appendChild(link);
      }
    }

    function restartFragAnim() {
      var nodes = document.querySelectorAll(
        "#about-frag-scatter .about-frag-chip, #about-frag-hard, #about-frag-diffs span, #about-frag-hidden"
      );
    ...
    pinCss();
    [0, 50, 150, 400, 900, 1800].forEach(function (ms) {
      setTimeout(function () {
        pinCss();
        if (ms >= 150) restartFragAnim();
      }, ms);
    });
    ...
    visual.innerHTML =
      ...
      "</div>" +
      "</div>";
    setTimeout(restartFragAnim, 200);
  } catch (err) {
