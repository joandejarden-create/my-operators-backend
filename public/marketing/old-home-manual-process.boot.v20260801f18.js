/**
 * Old Home Manual Process boot (v20260801f18)
 * fonts → bind demo CTA → load draw JS.
 * f18: skip fonts when boot guard already loaded data-oh-fonts (no duplicate request).
 */
(function () {
  var JS_URL =
  ...
  /* Fallback only — Fraunces + shared families if boot guard did not run. */
  var FONT_HREF =
  ...
  function ensureFonts() {
    if (
      document.querySelector('link[data-oh-fonts="1"]') ||
      document.querySelector('link[data-dmp-fonts="1"]')
    ) {
      return;
    }
