/**
 * Old Home — Request a Demo (v20260730e)
...
(function () {
  ...
  try {
    ...
    if (window.__ohRequestDemo >= 202607305) return;
    window.__ohRequestDemo = 202607305;
    ...
    function injectCss() {
      ...
      css.textContent = [
        ...
        /* Hero dual CTA: Explore form + small Request a Demo box side by side */
        "#form-subscribe-wrap.oh-fsw-wrap,#form-subscribe-wrap{align-items:center!important;gap:0!important}",
        "#oh-hero-cta-row{display:flex!important;flex-direction:row!important;flex-wrap:nowrap!important;align-items:center!important;justify-content:center!important;gap:10px!important;width:100%!important;max-width:min(760px,100%)!important;margin:0 auto!important}",
        "#oh-hero-cta-row #fsw-shell,#oh-hero-cta-row .oh-fsw-form{flex:1 1 auto!important;min-width:0!important;width:auto!important;max-width:630px!important;margin:0!important}",
        "#fsw-demo-link.oh-fsw-demo-btn,#fsw-demo-link{appearance:none!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;flex:0 0 auto!important;width:auto!important;min-width:0!important;height:42px!important;min-height:42px!important;max-height:42px!important;padding:0 14px!important;margin:0!important;box-sizing:border-box!important;border-radius:10px!important;border:1px solid rgba(255,255,255,.32)!important;background:rgba(255,255,255,.05)!important;color:#fff!important;font-size:13px!important;font-weight:600!important;letter-spacing:-.01em!important;line-height:1!important;white-space:nowrap!important;text-decoration:none!important;cursor:pointer!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.1)!important;transition:background .2s ease,border-color .2s ease,transform .15s ease!important}",
        ...
        "@media (max-width:720px){#oh-hero-cta-row{flex-wrap:wrap!important;max-width:630px!important}#oh-hero-cta-row #fsw-shell{flex:1 1 100%!important;max-width:100%!important}#fsw-demo-link.oh-fsw-demo-btn,#fsw-demo-link{margin-left:auto!important;margin-right:auto!important}.oh-demo-toast{right:12px;left:12px;min-width:0;transform:translateY(-24px)}.oh-demo-toast.show{transform:translateY(0)}}",
      ].join("");
    ...
    function enhanceCtas() {
      ...
      // Hero: small Request a Demo box beside Explore (same row, not stacked)
      ...
      if (outer && shell) {
        ...
        if (shell.parentNode !== row) row.appendChild(shell);
        if (!demoBtn) {
        ...
        } else if (demoBtn.parentNode !== row) {
          row.appendChild(demoBtn);
        }
        // Keep Explore first, demo second (side by side)
        if (row.firstChild !== shell) row.insertBefore(shell, row.firstChild);
        if (demoBtn.previousSibling !== shell) row.appendChild(demoBtn);
      }
      if (demoBtn) {
        demoBtn.className = "oh-fsw-demo-btn";
        demoBtn.textContent = "Request a Demo";
