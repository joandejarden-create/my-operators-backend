/**
 * Old Home testimonials carousel (v20260731w)
...
(function () {
  ...
  try {
    ...
    if (window.__ohTestimonials >= 202607322) return;
  } catch (ePath) {
    return;
  }
  ...
  function applyCopy(root) {
    ...
    articles.forEach(function (article, i) {
      ...
      if (img) {
        ...
        img.alt = card.alt || "";
        img.loading = "lazy";
        if (card.name === "Gustavo Sarago" || card.name === "Justin Boutwell") {
  ...
  function start(root, viewport, slides, dotsWrap) {
    if (started) return;
    started = true;
    window.__ohTestimonials = 202607322;
  ...
  function tryBoot() {
    if (started || window.__ohTestimonials >= 202607322) return;
