/**
 * Old Home nav cleanup (v20260730a)
 * Path-gated to /old-home.
 * - Pure white nav link color
 * - Fix Pricing (missing class + dead href)
 * - Order matches page story: About → How We Do It → Benefits → Pricing → FAQ → Insights
 */
(function () {
  try {
    var path = (window.location && window.location.pathname) || "";
    if (path !== "/old-home") return;
    if (window.__ohNavCleanup === 202607301) return;
    ...
    function injectStyles() {
      if (document.getElementById("oh-nav-cleanup-30a")) return;
      var style = document.createElement("style");
      style.id = "oh-nav-cleanup-30a";
      style.textContent = [
