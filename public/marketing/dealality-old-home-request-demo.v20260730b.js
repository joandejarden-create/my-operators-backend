/**
 * Old Home — Request a Demo (v20260730b)
...
(function () {
  ...
  try {
    ...
    if (window.__ohRequestDemo >= 202607302) return;
    window.__ohRequestDemo = 202607302;
    ...
    function injectCss() {
      ...
      css.textContent = [
        ...
        "#fsw-demo-wrap{margin-top:10px}",
        "#fsw-demo-link,#cta-band-demo{appearance:none;background:none;border:0;padding:0;margin:0;font:inherit;color:inherit;cursor:pointer;text-decoration:underline;text-underline-offset:3px}",
        "#cta-band-demo-wrap{margin-top:14px}",
        "#cta-band-demo{color:rgba(255,255,255,.88);font-size:14px;font-weight:600}",
    ...
    function enhanceCtas() {
      ...
      // Hero secondary row: add Request a Demo beside Watch Platform Overview
      var secondaryWrap = document.getElementById("fsw-secondary-wrap");
      if (secondaryWrap && !document.getElementById("fsw-demo-wrap")) {
        var demoWrap = document.createElement("p");
        demoWrap.id = "fsw-demo-wrap";
        demoWrap.className = secondaryWrap.className || "";
        var demoLink = document.createElement("button");
        demoLink.type = "button";
        demoLink.id = "fsw-demo-link";
        demoLink.className = "oh-fsw-secondary";
        demoLink.textContent = "Request a Demo";
        demoWrap.appendChild(demoLink);
        secondaryWrap.insertAdjacentElement("afterend", demoWrap);
        wireTrigger(demoLink, true);
      }
