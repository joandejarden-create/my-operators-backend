/**
 * Old Home nav cleanup (v20260730b)
 * Path-gated to /old-home.
 * - Pure white nav link color
 * - Fix Pricing (missing class + dead href)
 * - Order: About → How It Works → Benefits → Pricing → FAQ → Insights
...
(function () {
  try {
    ...
    if (window.__ohNavCleanup && window.__ohNavCleanup >= 202607302) return;
    ...
    function injectStyles() {
      if (document.getElementById("oh-nav-cleanup-30b")) return;
      var style = document.createElement("style");
      style.id = "oh-nav-cleanup-30b";
      style.textContent = [
