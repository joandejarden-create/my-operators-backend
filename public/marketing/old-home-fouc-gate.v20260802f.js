/**
 * Old Home FOUC gate (v20260802f)
 * Path-gated to /, /es, and /old-home (homepage + Spanish locale).
 * 02f: /es — Insights → Perspectivas in FOUC nav SoT.
 * 02d: /es — Title Case nav (Cómo Funciona, Preguntas Frecuentes) to match EN.
 * 02e: /es — Solicitar una Demo Title Case.
 * 02c: /es — Spanish nav labels + Solicitar una Demo (stop English overwrite).
...
(function () {
  try {
    ...
    if (window.__ohFoucGate && window.__ohFoucGate >= 2026080205) return;
    window.__ohFoucGate = 2026080205;
    ...
    function applyEsLocaleSwitcher() {
      if (!isEs) return;
      var links = d.querySelectorAll(".w-locales-list a[hreflang='es'], .w-locales-item a[href='/es']");
      for (var i = 0; i < links.length; i++) {
        var t = (links[i].textContent || "").replace(/\s+/g, " ").trim();
        if (/^Spanish$/i.test(t)) links[i].textContent = "Español";
      }
    }
    applyEsHeroStatic();
    applyEsLocaleSwitcher();
    ...
    var NAV_ORDER_ES = [
      { href: "#oh-how-we-do-it", label: "Cómo Funciona" },
      { href: "#modules", label: "Beneficios" },
      { href: "#many-futures", label: "Plataforma" },
      { href: "#faq", label: "Preguntas Frecuentes" },
      { href: "#insights", label: "Perspectivas" },
    ];
    var NAV_ORDER = isEs ? NAV_ORDER_ES : NAV_ORDER_EN;
    ...
    function ensureNavOrder() {
      ...
      if (mnav) {
        ...
        for (var j = 0; j < kids.length; j++) {
          ...
          if (
            /^about$|^how we do it$|^how it works$|^benefits$|^platform$|^pricing$|^faq$|^faqs$|^insights$/i.test(
              text
            ) ||
            /^acerca de$|^cómo funciona$|^beneficios$|^plataforma$|^preguntas frecuentes$/i.test(text)
          )
            continue;
          extras.push({
            href: href,
    ...
    function ensureDemoShell() {
      ...
      if (!demoBtn) {
        ...
        demoBtn.textContent = isEs ? "Solicitar una Demo" : "Request a Demo";
        demoBtn.setAttribute("aria-haspopup", "dialog");
        demoBtn.setAttribute("data-oh-fouc-demo", "1");
      } else {
        demoBtn.className = "oh-fsw-demo-btn";
        var demoLabel = isEs ? "Solicitar una Demo" : "Request a Demo";
        var demoCur = (demoBtn.textContent || "").replace(/\s+/g, " ").trim();
        if (
          !demoCur ||
          /^Request a Demo$/i.test(demoCur) ||
          /^Solicitar una Demo$/i.test(demoCur)
        ) {
          demoBtn.textContent = demoLabel;
        }
        demoBtn.setAttribute("aria-haspopup", "dialog");
      }
