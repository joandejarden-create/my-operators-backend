/**
 * Old Home — Features image-overlay tiles (v20260730o)
 ...
 * Owner-Controlled Process, and Structured Deal Responses use cropped app screenshots.
 */
(function () {
  try {
    var path = (location.pathname || "").replace(/\/+$/, "").toLowerCase() || "/";
    if (path !== "/old-home") return;
    if (window.__ohPlatformFeaturesTiles >= 202607314) return;
    window.__ohPlatformFeaturesTiles = 202607314;
    ...
    function ensureCss() {
      ...
      if (document.querySelector('link[data-oh-pf="30o"]')) return;
    ...
    function apply() {
      ...
      if (!grid) return;
      if (grid.getAttribute("data-oh-pf-tiles") === "30o") return;
      grid.innerHTML = TILES.map(cardHtml).join("");
      grid.setAttribute("data-oh-pf-tiles", "30o");
    }
