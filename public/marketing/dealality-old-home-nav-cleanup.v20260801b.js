/**
 * Old Home nav cleanup (v20260801a)
 * Path-gated to / and /old-home (homepage cutover).
 * - Pure white nav link color
 * - Order: How It Works → Benefits → Platform → FAQs → Insights
 * - How It Works → #oh-how-we-do-it
 * - Benefits → #modules
 * - Platform → #many-futures (Capabilities)
 * - FAQs → #faq
 * - Insights → #insights
 */
(function () {
  try {
    var path = ((window.location && window.location.pathname) || "").replace(/\/+$/, "").toLowerCase() || "/";
    if (path !== "/" && path !== "/old-home") return;
    if (window.__ohNavCleanup && window.__ohNavCleanup >= 202608011) return;
    ...
    function injectStyles() {
      if (document.getElementById("oh-nav-cleanup-31a")) return;
      var style = document.createElement("style");
      style.id = "oh-nav-cleanup-31a";
      style.textContent = [
    ...
    function rebuildMobile() {
      ...
      for (var i = 0; i < kids.length; i++) {
        ...
        extras.push({
          href: href,
          label: text,
          className: kids[i].className || "oh-mnav-link",
        });
      }
      host.innerHTML = "";
