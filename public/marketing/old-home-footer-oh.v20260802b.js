/* Locale helpers must be file-scoped — footer-oh is multiple IIFEs. */
var pathNorm=(location.pathname||"").replace(/\/+$/,"").toLowerCase()||"/";
var isEs=pathNorm==="/es"||pathNorm.indexOf("/es/")===0;
function orPath(){return isEs?"/es/opportunity-review":"/opportunity-review";}
...
function go(e){
  ...
  if(typeof window.ohOpenOpportunityReview==="function"){
    window.ohOpenOpportunityReview(url,t("Explore Your Hotel Opportunity","Explora Tu Oportunidad Hotelera"));
    return;
  }
  window.location.href=url;
}
if(form){
  form.setAttribute("action",orPath());
...
function ensureModal(){
  ...
  el.innerHTML='<div id="oh-ar-dialog" role="dialog" aria-modal="true" aria-labelledby="oh-ar-title">'+
    '<div id="oh-ar-bar">'+
      '<p id="oh-ar-title">'+t("Article","Artículo")+'</p>'+
      '<div id="oh-ar-actions">'+
        '<a id="oh-ar-open" href="#" target="_blank" rel="noopener noreferrer">'+t("Open full page","Abrir página completa")+'</a>'+
        '<button type="button" id="oh-ar-close" aria-label="'+t("Close","Cerrar")+'">×</button>'+
      '</div>'+
    '</div>'+
    '<div id="oh-ar-body">'+
      '<div id="oh-ar-loading" class="is-on" role="status">'+t("Loading…","Cargando…")+'</div>'+
      '<div id="oh-ar-error" role="alert">'+t("This page could not be loaded in the reader. ","No se pudo cargar esta página en el lector. ")+'<a id="oh-ar-error-link" href="#" target="_blank" rel="noopener noreferrer">'+t("Open it in a new tab","Ábrela en una pestaña nueva")+'</a>.</div>'+
      '<iframe id="oh-ar-frame" title="'+t("Embedded page reader","Lector de página integrada")+'" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>'+
    '</div>'+
  '</div>';
...
function setLoading(on,label){
  if(loadingEl){
    ...
    if(on)loadingEl.textContent=label||t("Loading…","Cargando…");
  }
}
...
function openReader(url,label,opts){
  ...
  setError(false);
  setLoading(true,opts.loadingText||t("Loading…","Cargando…"));
...
window.ohOpenOpportunityReview=function(url,label){
  openReader(url,label||t("Explore Your Hotel Opportunity","Explora Tu Oportunidad Hotelera"),{loadingText:t("Loading opportunity review…","Cargando revisión de oportunidad…")});
};
if(ctaBtn){
  ctaBtn.addEventListener("click",function(e){
    var href=ctaBtn.getAttribute("href")||orPath();
    if(!isOpportunityUrl(href)&&!/opportunity-review/i.test(href))return;
    e.preventDefault();
    var label=(document.getElementById("cta-band-btn-text")&&document.getElementById("cta-band-btn-text").textContent||ctaBtn.textContent||t("Start an Opportunity Review","Iniciar una Revisión de Oportunidad")).trim();
...
if(pricingOwnersCta){
  pricingOwnersCta.addEventListener("click",function(e){
    var href=pricingOwnersCta.getAttribute("href")||orPath();
    if(!isOpportunityUrl(href)&&!/opportunity-review/i.test(href))return;
    e.preventDefault();
...
var io=new IntersectionObserver(function(entries){
  ...
  for(var i=0;i<entries.length;i++){
    if(entries[i].isIntersecting&&entries[i].intersectionRatio>0.12){on=true;break;}
  }
