/**
 * Old Home — Features image-overlay hover tiles (v20260730h)
 ...
 * Structured Deal Responses uses cropped Deal Compare (no empty top navy).
 */
(function () {
  try {
    var path = (location.pathname || "").replace(/\/+$/, "").toLowerCase() || "/";
    if (path !== "/old-home") return;
    if (window.__ohPlatformFeaturesTiles >= 202607308) return;
    window.__ohPlatformFeaturesTiles = 202607308;

    var CSS_HREF =
      "https://cdn.prod.website-files.com/68108c29063eeb5d1bd7ae4a/6a6b635cb65f8fd12fa6416f_dealality-old-home-platform-features.v20260730h.css";
    ...
    function ensureCss() {
      ...
      for (i = 0; i < links.length; i++) {
        ...
        links[i].setAttribute("data-oh-pf", "30h");
      }
      if (document.querySelector('link[data-oh-pf="30h"]')) return;
      var link = document.createElement("link");
      link.rel = "stylesheet";
      link.href = CSS_HREF;
      link.setAttribute("data-oh-pf", "30h");
    ...
    function apply() {
      ...
      if (!grid) return;
      if (grid.getAttribute("data-oh-pf-tiles") === "30h") return;
      grid.innerHTML = TILES.map(cardHtml).join("");
      grid.setAttribute("data-oh-pf-tiles", "30h");
    }
