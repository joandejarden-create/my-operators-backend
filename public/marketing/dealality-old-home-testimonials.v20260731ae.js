/**
 * Old Home testimonials carousel (v20260731ae)
 ...
 * - 31ae: Gustavo Sarago new padded avatar (full head visible in circle)
 */
(function () {
  "use strict";
  ...
  try {
    ...
    if (window.__ohTestimonials >= 202607330) return;
  } catch (ePath) {
    return;
  }
  ...
  function start(root, viewport, slides, dotsWrap) {
    if (started) return;
    started = true;
    window.__ohTestimonials = 202607330;
  ...
  function tryBoot() {
    if (started || window.__ohTestimonials >= 202607330) return;
