/**
 * Old Home — Request a Demo (v20260730c)
...
(function () {
  ...
  try {
    ...
    if (window.__ohRequestDemo >= 202607303) return;
    window.__ohRequestDemo = 202607303;
    ...
    function injectCss() {
      ...
      css.textContent = [
        ...
        /* Hero dual CTA: Explore form + Request a Demo side by side */
        "#form-subscribe-wrap.oh-fsw-wrap,#form-subscribe-wrap{align-items:stretch!important}",
        "#oh-hero-cta-row{display:flex!important;flex-wrap:wrap!important;align-items:center!important;gap:12px!important;width:100%!important;max-width:760px!important;margin:0!important}",
        "#oh-hero-cta-row #fsw-shell,#oh-hero-cta-row .oh-fsw-form{flex:1 1 280px!important;min-width:0!important;max-width:100%!important;margin:0!important}",
        "#fsw-demo-link.oh-fsw-demo-btn,#fsw-demo-link{appearance:none!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;flex:0 0 auto!important;height:60px!important;min-height:60px!important;padding:0 22px!important;margin:0!important;box-sizing:border-box!important;border-radius:12px!important;border:1px solid rgba(255,255,255,.3)!important;background:linear-gradient(180deg,rgba(255,255,255,.07),rgba(255,255,255,.02))!important;color:#fff!important;font-size:15px!important;font-weight:600!important;letter-spacing:-.01em!important;line-height:1!important;white-space:nowrap!important;text-decoration:none!important;cursor:pointer!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.1)!important;transition:background .2s ease,border-color .2s ease,transform .15s ease,box-shadow .2s ease!important}",
        "#fsw-demo-link.oh-fsw-demo-btn:hover,#fsw-demo-link:hover{background:rgba(183,162,252,.18)!important;border-color:rgba(183,162,252,.6)!important;color:#fff!important;box-shadow:0 10px 24px rgba(109,92,216,.2)!important;transform:translateY(-1px)!important}",
        ...
        "@media (max-width:720px){#oh-hero-cta-row{flex-direction:column!important;align-items:stretch!important;max-width:100%!important}#fsw-demo-link.oh-fsw-demo-btn,#fsw-demo-link{width:100%!important}.oh-demo-toast{right:12px;left:12px;min-width:0;transform:translateY(-24px)}.oh-demo-toast.show{transform:translateY(0)}}",
      ].join("");
    ...
    function enhanceCtas() {
      ...
      // Hero: Request a Demo button beside the Explore CTA (same row)
      var outer = document.getElementById("form-subscribe-wrap");
      var shell = document.getElementById("fsw-shell");
      if (outer && shell && !document.getElementById("fsw-demo-link")) {
        ...
        if (!row) {
          ...
          outer.insertBefore(row, shell);
          row.appendChild(shell);
        }
        // Drop legacy underline demo wrap if present from older script
        ...
        var existing = document.getElementById("fsw-demo-link");
        if (existing) {
          existing.className = "oh-fsw-demo-btn";
          existing.textContent = "Request a Demo";
          wireTrigger(existing, true);
        }
      }
