/**
 * Old Home — Pricing enhance (v20260801a)
 * Path-gated to /old-home and /who-its-for.
 * Concrete fee lines, "When you engage" instead of volume row,
 * Pricing nav → /who-its-for (not landing #pricing), mobile role picker.
 * 01a: lives on /who-its-for; landing no longer shows #pricing.
 * 30g: pin matrix header CTAs to one horizontal line.
 */
(function () {
  "use strict";
  ...
  try {
    ...
    if (path !== "/old-home" && path !== "/who-its-for") return;
    if (window.__ohPricingEnhance >= 202608011) return;
    window.__ohPricingEnhance = 202608011;

    function ensureCtaAlignCss() {
      if (document.getElementById("oh-pricing-cta-align")) return;
      var st = document.createElement("style");
      st.id = "oh-pricing-cta-align";
      st.textContent =
        "#pricing-matrix thead th.oh-px-head-cell{height:1px!important}" +
        "#pricing-matrix thead th{vertical-align:top!important}" +
        ".oh-px-head{display:flex!important;flex-direction:column!important;" +
        "align-items:flex-start!important;height:100%!important;min-height:100%!important;" +
        "box-sizing:border-box!important}" +
        ".oh-px-fee{min-height:0!important}" +
        ".oh-px-cta-slot{width:100%!important;margin-top:auto!important;padding-top:.55rem!important}";
    ...
    var SLIM = {
      owners: {
        ...
        model: "Success-Based",
        fee: "Fee due at the milestone you agree up front — scoped to the opportunity.",
      ...
      brands: {
        ...
        model: "Annual Access",
        fee: "Custom quote by markets, users, and participation needs.",
      ...
      operators: {
        ...
        model: "Annual Access",
        fee: "Custom quote by footprint, markets, and users.",
      ...
      advisors: {
        ...
        model: "Annual Access",
        fee: "Custom quote by advisory seats and client scope.",
    ...
    var MATRIX_ROWS = [
      ...
      {
        label: "When you engage",
        owners: "Start with a confidential opportunity review — no platform fee to begin",
        brands: "Request access; pricing follows your market and participation scope",
        operators: "Request access; pricing follows footprint, markets, and users",
        advisors: "Request access; pricing follows advisory seats and client scope",
      },
      {
        label: "Scoped by",
    ...
    function applyIcons() {
      Object.keys(ICONS).forEach(function (id) {
        ...
        if (!el) return;
        if (el.querySelector("svg") && el.getAttribute("data-oh-pricing-svg") === "30f") return;
        el.setAttribute("data-oh-pricing-svg", "30f");
    ...
    function slimCards() {
      ROLES.forEach(function (role) {
        ...
        if (price) {
          ...
          if (span) span.textContent = conf.fee;
        }
        if (cta) {
    ...
    function fixPricingNav() {
      var roots = [document.getElementById("nav-links"), document.getElementById("mnav"), document];
      var seen = [];
      ...
      roots.forEach(function (root) {
        ...
        for (var i = 0; i < links.length; i++) {
          ...
          if (text !== "pricing" && href !== "#pricing" && href.indexOf("/old-home#pricing") === -1) continue;
          a.setAttribute("href", dest);
          if (text === "pricing") a.textContent = "Who It's For";
          if (!a.classList.contains("oh-nav-link") && a.closest && a.closest("#nav-links")) {
            a.classList.add("oh-nav-link");
          }
          if (!a.classList.contains("oh-mnav-link") && a.closest && a.closest("#mnav")) {
            a.classList.add("oh-mnav-link");
          }
        }
      });
    }
    ...
    function headCellHtml(role) {
      ...
      return [
        '<th scope="col" class="oh-px-head-cell oh-px-col-' +
          role +
          ownersCls +
          '" id="pricing-head-' +
          role +
        ...
        '<div class="oh-px-fee">' + conf.fee + "</div>",
    ...
    function setActiveRole(role) {
      ...
      if (!wrap || ROLES.indexOf(role) === -1) return;
      wrap.setAttribute("data-oh-role", role);
      var tabs = wrap.querySelectorAll("#pricing-role-tabs [data-oh-role]");
      for (var i = 0; i < tabs.length; i++) {
        var btn = tabs[i];
        var on = btn.getAttribute("data-oh-role") === role;
    ...
    function wireRoleTabs(wrap) {
      var tabs = wrap.querySelector("#pricing-role-tabs");
      if (!tabs || tabs.getAttribute("data-oh-wired") === "1") return;
      tabs.setAttribute("data-oh-wired", "1");
      tabs.addEventListener("click", function (e) {
        var t = e.target;
        while (t && t !== tabs && !(t.getAttribute && t.getAttribute("data-oh-role"))) {
          t = t.parentNode;
        }
        if (!t || t === tabs) return;
        var role = t.getAttribute("data-oh-role");
    ...
    function ensureMatrix() {
      ...
      if (wrap.getAttribute("data-oh-matrix") === "30f") {
        grid.setAttribute("hidden", "");
        grid.setAttribute("data-oh-pricing-hidden", "1");
        wireRoleTabs(wrap);
        if (!wrap.getAttribute("data-oh-role")) setActiveRole("owners");
        return;
      }
      ...
      var tabsHtml = ROLES.map(function (role) {
        return (
          '<button type="button" role="tab" data-oh-role="' +
          role +
          '" aria-selected="false">' +
          SLIM[role].title +
          "</button>"
      ...
      wrap.innerHTML = [
        '<div id="pricing-role-tabs" role="tablist" aria-label="Choose a path">' +
          tabsHtml +
          "</div>",
        '<p id="pricing-matrix-hint">Compare all paths below</p>',
      ...
      wrap.setAttribute("data-oh-matrix", "30f");
      grid.setAttribute("hidden", "");
      grid.setAttribute("data-oh-pricing-hidden", "1");
      wireRoleTabs(wrap);
      setActiveRole("owners");
    }
    ...
    function normalizeTerms() {
      ...
      if (owners) {
        owners.textContent =
          "Owner pricing varies by opportunity size, complexity, and scope. Fee is due at the agreed milestone.";
      }
      ...
      sub.textContent =
        "Subscription access is quoted to scope and does not guarantee a minimum volume of opportunities.";
