/**
 * Old Home testimonials carousel (v20260731ad)
 ...
 * - 31z: Gustavo full-head clean crop (no 18% face bias) avatar shifted down with headroom
 * - 31ab: Gustavo tighter head-shoulders zoom (no letterbox padding)
 * - 31ac: Justin Boutwell quote — centralize/streamline opportunity evaluation
 * - 31ad: Justin Boutwell title — Lead Asset Manager
 */
(function () {
  "use strict";
  ...
  try {
    ...
    if (window.__ohTestimonials >= 202607329) return;
  } catch (ePath) {
    return;
  }
  ...
  var GUSTAVO =
    "https://cdn.prod.website-files.com/68108c29063eeb5d1bd7ae4a/6a6cbc3d9731a1ce87e2f85b_gustavo-sarago-avatar.v20260731i.jpg";
  ...
  var CARDS = [
    ...
    {
      quote:
        "Dealality is the perfect tool to centralize and streamline all the information, communication, tools and hotel deal participants I need for evaluating an opportunity. It creates better visibility into the alternatives, the trade-offs, and the issues that deserve closer attention.",
    ...
    articles.forEach(function (article, i) {
      ...
      if (img) {
        ...
        if (card.name === "Justin Boutwell") {
          ...
          // Zoomed head-shoulders asset; keep center (not Justin's 18% face bias).
  ...
  function start(root, viewport, slides, dotsWrap) {
    if (started) return;
    started = true;
    window.__ohTestimonials = 202607329;
  ...
  function tryBoot() {
    if (started || window.__ohTestimonials >= 202607329) return;
