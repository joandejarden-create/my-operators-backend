...
(function () {
  try {
    ...
    if (window.__ohFoucGate && window.__ohFoucGate >= 202607314) return;
    window.__ohFoucGate = 202607314;
    ...
    var NAV_ORDER = [
      ...
      { href: "#oh-how-we-do-it", label: "How We Do It" },
    ...
    function ensureNavOrder() {
      // Same contract as nav-cleanup 30a — mark done so footer script does not rebuild/flash.
      if (window.__ohNavCleanup && window.__ohNavCleanup >= 202607301) return true;
      ...
      if (mnav) {
        ...
        for (var j = 0; j < kids.length; j++) {
          ...
          if (/^about$|^how we do it$|^benefits$|^pricing$|^faq$|^insights$/i.test(text)) continue;
          extras.push({
            href: href,
            label: text,
      ...
      window.__ohNavCleanup = 202607301;
      return true;
    }
