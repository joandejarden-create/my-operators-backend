/**
 * Old Home — Pricing enhance (v20260730d)
 * Path-gated to /old-home.
 * One composition: role headers (icon/model/CTA) live in the comparison matrix.
 * Card grid is collapsed after its icons/CTAs are moved (IDs preserved for demo wiring).
 */
(function () {
  "use strict";
  ...
  try {
    ...
    if (window.__ohPricingEnhance >= 202607304) return;
    window.__ohPricingEnhance = 202607304;
    ...
    function applyIcons() {
      Object.keys(ICONS).forEach(function (id) {
        ...
        if (!el) return;
        if (el.querySelector("svg") && el.getAttribute("data-oh-pricing-svg") === "30d") return;
        el.setAttribute("data-oh-pricing-svg", "30d");
    ...
    function ensureMatrix() {
      ...
      if (wrap.getAttribute("data-oh-matrix") === "30d") {
      ...
      var rowsHtml = MATRIX_ROWS.map(function (row) {
        function cell(role, text) {
          var cls = role === "owners" ? ' class="oh-px-owners"' : "";
      ...
      wrap.setAttribute("data-oh-matrix", "30d");
      grid.setAttribute("hidden", "");
      grid.setAttribute("data-oh-pricing-hidden", "1");
    }
