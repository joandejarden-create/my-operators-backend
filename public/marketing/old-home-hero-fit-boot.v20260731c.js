/**
 * Old Home — laptop-tight hero fit + wide/tall breathing room (v20260731c).
 * Loads hero-fit CSS only. PVL is owned by boot-guard (singleton v20260731b).
 * v20260731c: FOUC-safe Watch Platform Overview visibility (data-oh-visible).
 */
(function () {
  try {
    var path = (location.pathname || "").replace(/\/+$/, "").toLowerCase() || "/";
    if (path !== "/old-home") return;
    if (window.__ohHeroFitBoot && window.__ohHeroFitBoot >= 202607312) return;
    window.__ohHeroFitBoot = 202607312;
