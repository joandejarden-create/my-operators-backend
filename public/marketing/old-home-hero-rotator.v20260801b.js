/**
 * Old Home — hero headline rotator (v20260801b)
...
(function () {
  try {
    ...
    if (window.__ohHeroRotator >= 202608012) return;
    window.__ohHeroRotator = 202608012;
    ...
    var STYLE_ID = "oh-hero-rotator-01b";
    ...
    function start() {
      ...
      // Off-DOM probe: active-word metrics without flashing live stack to `.on`.
      function makeProbe() {
      ...
      function measureLine() {
        var text =
          ((words[ri] || words[startIdx] || words[0]).textContent || "").trim() ||
          "Dealality";
        var p = getProbe();
        p.textContent = text;
        return Math.max(p.offsetHeight || 0, 40);
      }
