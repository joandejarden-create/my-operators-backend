/**
 * Old Home — Features 9 equal tiles + Radar / Fee visuals (v20260730d)
 * Path-gated to /old-home.
 * Radar screenshot → Market Intelligence; Fee Estimator → And More…
 */
(function () {
  try {
    var path = (location.pathname || "").replace(/\/+$/, "").toLowerCase() || "/";
    if (path !== "/old-home") return;
    if (window.__ohPlatformFeaturesTiles >= 202607304) return;
    window.__ohPlatformFeaturesTiles = 202607304;

    var CSS_HREF =
      "https://cdn.prod.website-files.com/68108c29063eeb5d1bd7ae4a/6a6b36fe54007c9701cd00d0_dealality-old-home-platform-features.v20260730d.css";
    ...
    function ensureCss() {
      ...
      for (i = 0; i < links.length; i++) {
        ...
        links[i].setAttribute("data-oh-pf", "30d");
      }
      if (document.querySelector('link[data-oh-pf="30d"]')) return;
      var link = document.createElement("link");
      link.rel = "stylesheet";
      link.href = CSS_HREF;
      link.setAttribute("data-oh-pf", "30d");
    ...
    function apply() {
      ...
      if (!grid) return;
      if (grid.getAttribute("data-oh-pf-tiles") === "30d") return;
      grid.innerHTML = TILES.map(cardHtml).join("");
      grid.setAttribute("data-oh-pf-tiles", "30d");
    }
