/**
 * Old Home Many Futures Spanish i18n (v20260802e)
 * Path-gated to /es.
 *
 ...
 * - illustrative opportunity card (outside platform mocks)
 * - Purpose / Benefit kickers and copy
 *
 * Leaves ALL platform UI mocks in English (.mf-ui*), including Deal Compare
 * tables, Brand Explorer, Opportunity Review panels, and in-UI callouts.
 * Also leaves platform feature names (.mf-feat-name) in English.
 *
 * 02e: Spanish Title Case — lowercase mid-phrase de/del/el/a/o/y.
 * 02d: never translate descendants of .mf-ui (platform product chrome)
...
(function () {
  ...
  try {
    ...
    if (window.__ohMfI18n >= 202608024) return;
  } catch (ePath) {
    return;
  }
  ...
  function isInsidePlatformUi(el) {
    return !!(el && el.closest && el.closest(".mf-ui"));
  }

  function translateText(el) {
    if (!el || isInsidePlatformUi(el)) return false;
  ...
  function translateDt(dt) {
    if (!dt || isInsidePlatformUi(dt)) return;
  ...
  function translateOpportunityCallouts(root) {
    /* Only owner chrome callouts — never translate callouts inside .mf-ui mocks. */
    var callouts = root.querySelectorAll(".mf-ui-callout");
    for (var i = 0; i < callouts.length; i++) {
      if (isInsidePlatformUi(callouts[i])) continue;
  ...
  function forceOutcomeLabels(root) {
    ...
    for (var i = 0; i < labels.length; i++) {
      if (isInsidePlatformUi(labels[i])) continue;
  ...
  function apply(root) {
    ...
    for (var s = 0; s < selectors.length; s++) {
      ...
      for (var i = 0; i < nodes.length; i++) {
        if (isInsidePlatformUi(nodes[i])) continue;
        translateText(nodes[i]);
      }
    }
    ...
    for (var d = 0; d < dts.length; d++) {
      if (isInsidePlatformUi(dts[d])) continue;
      translateDt(dts[d]);
    }
    ...
    /* Do not touch .mf-feat-name or .mf-ui* — platform product chrome stays English. */
