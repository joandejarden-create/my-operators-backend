/**
 * Old Home — Features image-overlay hover tiles (v20260730k)
 ...
 * Opportunity Review, Operator Explorer, and Structured Deal Responses use cropped app screenshots.
 */
(function () {
  try {
    var path = (location.pathname || "").replace(/\/+$/, "").toLowerCase() || "/";
    if (path !== "/old-home") return;
    if (window.__ohPlatformFeaturesTiles >= 202607310) return;
    window.__ohPlatformFeaturesTiles = 202607310;
    ...
    function ensureCss() {
      ...
      for (i = 0; i < links.length; i++) {
        ...
        links[i].setAttribute("data-oh-pf", "30k");
      }
      if (document.querySelector('link[data-oh-pf="30i"]')) return;
      var link = document.createElement("link");
      link.rel = "stylesheet";
      link.href = CSS_HREF;
      link.setAttribute("data-oh-pf", "30k");
    ...
    function apply() {
      ...
      if (!grid) return;
      if (grid.getAttribute("data-oh-pf-tiles") === "30k") return;
      grid.innerHTML = TILES.map(cardHtml).join("");
      grid.setAttribute("data-oh-pf-tiles", "30k");
    }
