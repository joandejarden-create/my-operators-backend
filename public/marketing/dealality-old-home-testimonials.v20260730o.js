/**
 * Old Home testimonials carousel (v20260730o)
 ...
 * - Labels top-left (top:24px), no punch-out background; second accent line under label
...
(function () {
  ...
  try {
    ...
    if (window.__ohTestimonials >= 202607314) return;
  } catch (ePath) {
    return;
  }
  ...
  function start(root, viewport, slides, dotsWrap) {
    if (started) return;
    started = true;
    window.__ohTestimonials = 202607314;
  ...
  function tryBoot() {
    if (started || window.__ohTestimonials >= 202607314) return;
