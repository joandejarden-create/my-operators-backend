/**
 * Old Home — Features image-overlay tiles (v20260730r)
...
(function () {
  try {
    ...
    if (window.__ohPlatformFeaturesTiles >= 202607317) return;
    window.__ohPlatformFeaturesTiles = 202607317;
    ...
    function ensureCss() {
      ...
      if (document.querySelector('link[data-oh-pf="30r"]')) return;
    ...
    function apply() {
      ...
      if (!grid) return;
      if (grid.getAttribute("data-oh-pf-tiles") === "30r") return;
      grid.innerHTML = TILES.map(cardHtml).join("");
      grid.setAttribute("data-oh-pf-tiles", "30r");
    }
