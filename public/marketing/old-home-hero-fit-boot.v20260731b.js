/**
 * Old Home — laptop-tight hero fit + wide/tall breathing room (v20260731b).
 * Loads hero-fit CSS only. PVL is owned by boot-guard (singleton v20260731a).
 */
(function () {
  try {
    var path = (location.pathname || "").replace(/\/+$/, "").toLowerCase() || "/";
    if (path !== "/old-home") return;
    if (window.__ohHeroFitBoot && window.__ohHeroFitBoot >= 202607311) return;
    window.__ohHeroFitBoot = 202607311;
    ...
    // Do not inject PVL — boot-guard loads singleton launcher with data-oh-pvl="i".
