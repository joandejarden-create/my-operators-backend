/**
 * Old Home — Features image-overlay hover tiles (v20260730i)
 ...
 * Operator Explorer + Structured Deal Responses use cropped app screenshots.
 */
(function () {
  try {
    var path = (location.pathname || "").replace(/\/+$/, "").toLowerCase() || "/";
    if (path !== "/old-home") return;
    if (window.__ohPlatformFeaturesTiles >= 202607309) return;
    window.__ohPlatformFeaturesTiles = 202607309;
    ...
    function ensureCss() {
      ...
      for (i = 0; i < links.length; i++) {
        ...
        links[i].setAttribute("data-oh-pf", "30i");
      }
      if (document.querySelector('link[data-oh-pf="30i"]')) return;
      var link = document.createElement("link");
      link.rel = "stylesheet";
      link.href = CSS_HREF;
      link.setAttribute("data-oh-pf", "30i");
    ...
    function apply() {
      ...
      if (!grid) return;
      if (grid.getAttribute("data-oh-pf-tiles") === "30i") return;
      grid.innerHTML = TILES.map(cardHtml).join("");
      grid.setAttribute("data-oh-pf-tiles", "30i");
    }
