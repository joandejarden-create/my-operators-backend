...
(async () => {
  // Prefer explicit head dump from MCP if present
  let head;
  if (fs.existsSync("tmp-old-home-head-live.txt")) {
    head = fs.readFileSync("tmp-old-home-head-live.txt", "utf8");
  } else {
    throw new Error("missing tmp-old-home-head-live.txt — save MCP head content first");
  }
  if (!head.includes(OLD)) {
    console.error("360 block missing");
    process.exit(1);
  }
  ...
  console.log(JSON.stringify({ len: head.length, hasCalc: head.includes("--ins-card-w"), has360: /flex:0 0 360px/.test(head) }));
})().catch((e) => {
  console.error(e);
  process.exit(1);
});
