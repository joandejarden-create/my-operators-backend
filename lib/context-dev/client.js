...
async function withContextDev(fn) {
  try {
    ...
    // Preserve provider credit metadata when present (never log API keys).
    const key_metadata =
      data?.key_metadata ||
      (data && typeof data === "object" && data.credits_consumed != null
        ? {
    ...
    const classified = classifyContextDevError(err);
    const key_metadata =
      err?.key_metadata ||
      err?.error?.key_metadata ||
      err?.response?.key_metadata ||
      null;
    return {
      ok: false,
      data: null,
      ...
      ...(key_metadata
        ? {
            key_metadata: {
              credits_consumed:
                key_metadata.credits_consumed != null
                  ? Number(key_metadata.credits_consumed)
                  : null,
              credits_remaining:
                key_metadata.credits_remaining != null
                  ? Number(key_metadata.credits_remaining)
                  : null,
            },
          }
        : {}),
