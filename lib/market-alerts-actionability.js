...
import { RISK_POLICY_ACT_NOW_BLOCK_RE } from "./market-alerts-category-infer.js";
...
export function isActionableForAudience(input = {}) {
  ...
  // Risk / policy stories are Watch (or All), never Act Now dealmaking.
  if (String(category || "").toLowerCase() === "risk") return false;
  if (RISK_POLICY_ACT_NOW_BLOCK_RE.test(`${title} ${summary}`)) return false;
...
export function computeActionableFlags(audienceBundle = {}) {
  const base = {
    ...
    category: audienceBundle.category || null,
    title: audienceBundle.title || "",
    summary: audienceBundle.summary || "",
  };
