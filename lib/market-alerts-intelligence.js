...
export function computeMarketAlertIntelligence(input = {}) {
  ...
  // Tax / zoning / regulation: Watch-worthy at most — never Act Now dealmaking.
  const riskPolicyText = `${input.title || ""} ${input.summary || ""}`;
  if (RISK_POLICY_ACT_NOW_BLOCK_RE.test(riskPolicyText)) {
    actionableFlags.owner = false;
    actionableFlags.brand = false;
    actionableFlags.operator = false;
  }
