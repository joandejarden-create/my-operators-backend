 ...
 * Request gate (P8.5 / P84-C): HPC when request opts in via censusSource=hpc /
 * product=hotel-explorer|he|radar (see brand-presence-hpc-request.js).
 * Scout and production Radar omit opt-in and stay on Legacy.
 *
...
export function formatHpcHotelRecord(hotel) {
  ...
  // P8.6 geo shim: expose admin State/Region as region for Radar cascade filters.
  // Not Legacy Market/Submarket. dealality_market/submarket stay nullable from HPC fields.
  const stateRegion = readText(fields, HPC_FIELDS.state);
  const dealalityMarket = readText(fields, HPC_FIELDS.market);
  const dealalitySubmarket = readText(fields, HPC_FIELDS.submarket);
  ...
  return applyProductCensusQuarantine({
    ...
    submarket: dealalitySubmarket,
    dealality_market: dealalityMarket,
    dealality_submarket: dealalitySubmarket,
    ...
    _provenanceFlags: {
      ...
      region_from_state_admin: Boolean(stateRegion),
      dealality_geo_shim: true,
    },
  });
}
...
export function getHpcTableSelectOptions({ limit, page, brand, status, region, search } = {}) {
  ...
  if (region) {
    ...
    // P8.6: match admin State/Region or country (Dealality geo shim — not Legacy Market)
    parts.push(
      `OR({${HPC_FIELDS.state}}='${r}',{${HPC_FIELDS.country}}='${r}')`
    );
  }
