...
import { aggregateHpcBrandPresenceSummary } from "./brand-explorer-hpc-metrics.js";
import { brandExplorerCensusReadCounters } from "./brand-explorer-hpc-metrics.js";
 ...
 * @param {{ useHpc?: boolean }} [options]
 * @returns {Promise<object>} censusSummary shape for brand.censusSummary
 */
export async function buildBrandCensusSummary(requestedBrand, parentCompany, options = {}) {
  const brand = (requestedBrand || "").trim();
  const parent = (parentCompany || "").trim() || null;
  const useHpc = options.useHpc === true || options.useHpc === "1";
  ...
  try {
    ...
    const aggregateFn = useHpc ? aggregateHpcBrandPresenceSummary : aggregateCensusPresenceSummary;
    if (!useHpc) {
      brandExplorerCensusReadCounters.legacy += 1;
    }

    let summary = await aggregateFn({
    ...
    if (
      ...
      const relaxed = await aggregateFn({
        affiliationMatchers: resolution.affiliationMatchers,
        parentCompany: null,
      });
      if (
    ...
    if (useHpc) {
      warnings.push(
        "BE_HPC: Chain Scale UNKNOWN (SAFE_DEGRADATION); rooms are product-governed partial coverage; brand stock = BRANDED_CONFIRMED only."
      );
    }
    ...
    return {
      ...
      match: {
        ...
        parentCompanyUsedForCensusFilter: useHpc ? null : censusParentFilter,
        matchPath: resolution.matchPath,
      },
      ...
      affiliationClassCounts: summary.affiliationClassCounts || null,
      ...
      census: {
        ...
        excludedNotBrandedConfirmed: summary.excludedNotBrandedConfirmed ?? 0,
      },
