 ...
 * Radar requires RADAR_HPC_V2=1. Scout product on brand-presence stays Legacy unless Scout gate used.
 */
export function shouldUseHpcBrandPresence(req, env = process.env) {
  ...
  // Scout must never flip Brand Presence via shared flag alone.
  if (product === "scout") return false;
  if (product === "hotel-explorer" || product === "he") return true;
  // Bare censusSource=hpc is HE-compatible (headers). Do not treat as Radar/Scout bypass.
 ...
 * Requires BRAND_PRESENCE_HPC_V2 + SCOUT_HPC_V2 + product=scout (or censusSource=hpc with product=scout).
 */
export function shouldUseHpcScoutCensus(req, env = process.env) {
  ...
  if (censusSource === "hpc" && product === "scout") return true;
  return false;
}
