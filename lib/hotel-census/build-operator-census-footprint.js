...
export function applyCensusFootprintToOperatorPrefill(prefill, censusFootprint) {
  if (!prefill || !censusFootprint) {
    return { source: "operator_setup", applied: false };
  }

  const avail =
    censusFootprint.footprintAvailabilityState ||
    censusFootprint.footprintPresentation?.footprintAvailabilityState ||
    "";

  // P8.12D — intentional unavailable / disabled (HPC path): never paint factual zeroes
  if (
    !censusFootprint.ok ||
    avail === "INSUFFICIENT_CONFIRMED_DATA" ||
    avail === "CENSUS_DISABLED" ||
    censusFootprint.reason === "no_confirmed_operator_relationships" ||
    censusFootprint.reason === "census_footprint_disabled_for_master"
  ) {
    if (censusFootprint.hpcPath || censusFootprint.adapter === "operator-explorer-hpc-v2") {
      prefill.footprintAvailabilityState =
        avail ||
        (censusFootprint.reason === "census_footprint_disabled_for_master"
          ? "CENSUS_DISABLED"
          : "INSUFFICIENT_CONFIRMED_DATA");
      prefill.footprintPresentation = censusFootprint.footprintPresentation || {
      ...
      return { source: "hotel_census_unavailable", applied: true };
    }
    return { source: "operator_setup", applied: false };
  }

  if (!(censusFootprint.totals?.totalHotels > 0)) {
    return { source: "operator_setup", applied: false };
  }
  ...
  if (censusFootprint.totals) {
    ...
    prefill.geo_total_existing_rooms = String(
      censusFootprint.totals.roomsSuppressed
        ? ""
        : (censusFootprint.totals.totalExistingRooms ?? "")
    );
    prefill.geo_total_pipeline_hotels = String(censusFootprint.totals.totalPipelineHotels ?? "");
    prefill.geo_total_pipeline_rooms = String(
      censusFootprint.totals.roomsSuppressed
        ? ""
        : (censusFootprint.totals.totalPipelineRooms ?? "")
    );
    const confirmed =
      censusFootprint.totals.confirmedProperties ?? censusFootprint.totals.totalHotels;
    ...
    prefill.confirmedOperatingHotels = String(
      censusFootprint.totals.confirmedOperatingHotels ??
        censusFootprint.totals.totalExistingHotels ??
        ""
    );
    prefill.confirmedPipelineHotels = String(
      censusFootprint.totals.confirmedPipelineHotels ??
        censusFootprint.totals.totalPipelineHotels ??
        ""
    );
    if (censusFootprint.totals.roomsSuppressed) {
      prefill.totalRooms = "";
    } else {
  ...
  if (censusFootprint.footprintPresentation) {
    prefill.footprintPresentation = censusFootprint.footprintPresentation;
    prefill.portfolioCoverage =
      censusFootprint.footprintPresentation.portfolioCoverage || "Partial";
    prefill.footprintPrimaryLabel =
      censusFootprint.footprintPresentation.primaryLabel || "Confirmed properties";
    prefill.footprintAvailabilityState =
      censusFootprint.footprintPresentation.footprintAvailabilityState ||
      censusFootprint.footprintAvailabilityState ||
      "PARTIAL_CONFIRMED";
    ...
    prefill.footprintAvailabilityState =
      censusFootprint.footprintAvailabilityState || "PARTIAL_CONFIRMED";
  }
