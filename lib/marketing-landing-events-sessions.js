...
/** Map live old-home section ids (and aliases) onto the canonical funnel journey. */
...
const HERO_CTA_LOCATIONS = new Set(["hero", "explore", "hero_video"]);
const BOTTOM_CTA_LOCATIONS = new Set([
  "cta_section",
  "cta_band",
  "pricing_owners",
  "pricing_brands",
...
const SECTION_LABELS = {
  ...
  audiences: "Audiences / Pricing",
  why: "Why Dealality / Social Proof",
  faq: "FAQ",
...
function canonicalizeSection(section) {
  ...
  return SECTION_CANONICAL[key] || key;
}
...
export function buildSessionsMap(events) {
  ...
  for (const e of events) {
    ...
    if (e.event === "section_view" && e.section) {
      const canon = canonicalizeSection(e.section);
      s.sections.add(canon);
      const rank = SECTION_ORDER.indexOf(canon) + 1;
      if (rank > s.maxSectionRank) {
        s.maxSectionRank = rank;
        s.maxSection = canon;
      }
      if (canon === "cta") s.reachedBottomCta = true;
    }
    if (e.event === "max_section_depth" && e.section) {
      ...
      const rank =
        typeof e.depth === "number"
          ? e.depth
          : SECTION_ORDER.indexOf(canon) + 1;
      if (rank > s.maxSectionRank) {
        s.maxSectionRank = rank;
        s.maxSection = canon;
      }
      if (canon === "cta" || rank >= 8) s.reachedBottomCta = true;
    }
    if (e.event === "cta_click") {
      s.cta = true;
      if (e.location) {
        s.ctaLocations.add(e.location);
        if (!s.scrolled && HERO_CTA_LOCATIONS.has(e.location)) {
    ...
    if (
      e.event === "video_complete" ||
      e.event === "platform_video_complete"
    ) {
...
export function funnelFromSessions(sessions) {
  ...
  const reachedCta = sessions.filter(
    (s) => s.sections.has("cta") || s.maxSectionRank >= 8 || s.reachedBottomCta
  ).length;
...
export function buildCtaPathAnalysis(events) {
  ...
  const clickedHero = sessions.filter((s) =>
    [...s.ctaLocations].some((loc) => HERO_CTA_LOCATIONS.has(loc))
  ).length;
  const clickedBottom = sessions.filter((s) =>
    [...s.ctaLocations].some((loc) => BOTTOM_CTA_LOCATIONS.has(loc))
  ).length;
  ...
  return {
    paths: [
      ...
      {
        key: "clicked_hero",
        label: "Clicked Hero / Explore CTA",
        count: clickedHero,
        rate: pct(clickedHero, total),
      },
      {
        key: "clicked_bottom",
        label: "Clicked Bottom / Pricing / Demo CTA",
        count: clickedBottom,
...
export function buildDailyUniqueUsers(events, opts = {}) {
  const windowDays =
  ...
  if (windowDays == null || windowDays >= 14) {
    if (days.length >= 7) {
