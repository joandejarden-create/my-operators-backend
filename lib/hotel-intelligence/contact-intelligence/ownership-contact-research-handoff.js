...
async function chargedContext(ledger, kind, cost, meta, fn) {
  ...
  if (
    !result.ok &&
    (result.error?.class === "VALIDATION_OR_CLIENT" ||
...
export async function researchHotelOwnershipContactPath(caseInput = {}, budgets = {}) {
  ...
  for (const rawUrl of inspectUrls.slice(0, 4)) {
    ...
    if (match && !ownership.owner_display_name) {
      ...
      ownership = {
        ...
        classification: "STAGED_FROM_LIVE_RESEARCH",
        confidence: "PROBABLE",
  ...
  if (!ownership.owner_entity_id || ownership.return_to_ownership_lane) {
    ...
    if (isContextDevConfigured() && ledger.canAfford(ledger.estimateSearchCost(10))) {
      const cq = `${hotel.hotel_name} hotel owner OR ownership OR acquired OR "economic owner"`;
      const search = await chargedContext(ledger, "search", ledger.estimateSearchCost(10), { query: cq }, () =>
        // Context.dev /web/search rejects maxAgeMs (INPUT_VALIDATION_ERROR).
        contextDevSearch({ query: cq, numResults: 10 })
      );
      calls.push({
        provider: "context_dev",
        kind: "search",
        ok: search.ok,
        budget_blocked: search.budget_blocked,
        error: search.error || null,
      });
      if (search.ok) {
        const mapped = (search.data?.results || []).map((r) => ({
          title: r.title,
          url: r.url,
          snippet: r.description || r.snippet,
          provider: "context_dev",
        }));
        newly.ownership_claims.push(...mapped.slice(0, 5));
        sources.push({ kind: "context_ownership_search", query: cq, results: mapped.slice(0, 8) });

        const urls = pickSearchUrls(mapped, {
          excludeHosts,
          preferTokens: [hotel.hotel_name?.split(/\s+/).slice(0, 2).join(" ")],
        }).slice(0, 2);
        for (const u of urls) {
          ...
          const scraped = await chargedContext(
            ...
            CONTEXT_DEV_CREDIT_COSTS.scrape_markdown,
            { url: u.url },
            () => contextDevScrapeMarkdown({ url: u.url })
          );
          calls.push({ provider: "context_dev", kind: "scrape", url: u.url, ok: scraped.ok });
          if (scraped.ok && scraped.data) {
            ...
            sources.push({
              ...
              url: u.url,
              excerpt: String(md).slice(0, 1200),
              observed_at: new Date().toISOString(),
            });
            const claimBlob = `${u.title || ""} ${u.snippet || ""} ${String(md).slice(0, 2000)}`;
            const ownedBy = claimBlob.match(
              /(?:owned by|adquirid[oa] por|propiedad de|purchased by|acquired by|sold to|owner[:\s]+)\s*([A-Z][^.\n|]{3,80})/i
            );
            if (ownedBy && !ownership.owner_display_name) {
              const stagedName = ownedBy[1].trim().replace(/\s+/g, " ").slice(0, 120);
              if (
                isImplausibleOwnerDisplayName(stagedName) ||
                /whoistheownerof|whoownsthebrand/i.test(u.url || "")
              ) {
                newly.notes.push(`rejected_implausible_owner_name:${stagedName}:${u.url}`);
              } else {
              ownership = {
                ...ownership,
                owner_display_name: stagedName,
                ...
                classification: "STAGED_FROM_LIVE_RESEARCH",
                confidence: "PROBABLE",
                evidence_note: `Staged from live research scrape ${u.url} — not canonical HOTEL_TO_OWNER write.`,
                return_to_ownership_lane: false,
                ownership_vs_operator: "OWNER_CLAIM_STAGED_OPERATOR_NOT_ASSUMED",
                newly_researched: true,
              };
              newly.notes.push(`staged_owner_from_scrape:${u.url}`);
              }
            }
          }
        }
      } else if (search.budget_blocked) {
        unresolved.push("CONTEXT_DEV_BUDGET_EXHAUSTED_OWNERSHIP_SEARCH");
      } else {
        unresolved.push(`CONTEXT_DEV_SEARCH_FAILED:${search.error?.class || search.error?.message || "unknown"}`);
      }
    }
    ...
    for (const hp of (caseInput.person_hypotheses || []).slice(0, 2)) {
      ...
      for (const hit of serp.organic || []) {
        ...
        const liGate = validateLinkedInIdentifier({
          ...
          evidenceNote: `${hit.title || ""} ${hit.snippet || ""} ${ownership.owner_display_name || ""}`,
        });
        if (liGate.decision === "ALLOW") {
          acceptedUrl = url;
          break;
...
export function buildEnrichmentSubjectsFromResearch(researchRows, { maxPeople = 10, maxPerOwner = 2 } = {}) {
  ...
  for (const row of researchRows) {
    ...
    for (const p of ordered) {
        ...
        identifiers: {
          domain:
            domainHost && domainStatus === "CONFIRMED"
              ? { value: domainHost, status: "CONFIRMED", independently_supported: true }
              : domainHost
      ...
      const gated = gateProviderCandidates([candidate], { provider: "fullenrich" });
