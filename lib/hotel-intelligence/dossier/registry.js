...
export function listDossiersForHotel({ hotelId, airtableRecordId } = {}) {
  ...
  const fixtureCards = all
    ...
    .map((d) => toLibraryCard(enrichReportForPublishing(d)));
  const addendumCards = listPersistedResearchAddendums({ hotelId: hid || rid }).map((d) =>
    toLibraryCard(enrichReportForPublishing(d))
  );
...
export function getDossierById(dossierId) {
  ...
  if (persisted) return enrichReportForPublishing(persisted);
  const all = loadAll();
  ...
  return enrichReportForPublishing(matches[0]);
}
