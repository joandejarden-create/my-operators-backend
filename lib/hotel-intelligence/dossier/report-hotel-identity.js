...
function isForbiddenLocationLabel(label) {
  ...
  if (!s) return true;
  return /^(location\s+pending|pending|unknown|n\/?a|—|-)$/i.test(s);
}
...
export function resolveReportHotelIdentity(input = {}) {
  ...
  const city = known?.city || existing?.city || input.city || null;
  const locality = known?.locality || existing?.locality || input.locality || null;
  const state_region =
    known?.state_region ||
    ...
    input.state_region ||
    null;
  const country = known?.country || existing?.country || input.country || null;
