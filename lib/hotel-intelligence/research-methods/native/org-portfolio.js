...
const ORG_PORTFOLIO_URLS = {
  // Moved behind enableOptionalOwnerAdapters — see reconstructOrgPortfolio opts
};

const OPTIONAL_ORG_PORTFOLIO_URLS = {
...
export async function reconstructOrgPortfolio(opts = {}) {
  ...
  const urlMap = opts.enableOptionalOwnerAdapters ? OPTIONAL_ORG_PORTFOLIO_URLS : ORG_PORTFOLIO_URLS;
  let urls = [];
  ...
  if (
    opts.enableOptionalOwnerAdapters &&
    !urls.length &&
    /^(grupo\s+hotelero\s+santa\s+fe|gsf(\s+hotels)?)$/i.test(key)
  ) {
    urls = OPTIONAL_ORG_PORTFOLIO_URLS["grupo hotelero santa fe"];
  }
  // Explicit optional adapters only — never match bare "hotel" substring to GSF.
