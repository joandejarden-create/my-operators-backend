/**
 * Golden-demo Radar fixture fallback for Hotel Explorer share packs
 * (Mexico Sheraton/voco/KGPV + Bermuda Cambridge).
...
export const KGPV_ID = "recUNycnMwOVFX0hc";
export const CAMBRIDGE_ID = "recIwaP1etgx2g9nA";
/** @deprecated alias — all four golden HE hotel ids */
export const MEXICO_RADAR_HOTEL_IDS = new Set([
  SHERATON_GDL_ID,
  VOCO_CANCUN_ID,
  KGPV_ID,
  CAMBRIDGE_ID,
]);
export const GOLDEN_DEMO_RADAR_HOTEL_IDS = MEXICO_RADAR_HOTEL_IDS;
...
const HOTEL_PRESENTATION = {
  ...
  [KGPV_ID]: {
    ...
    brand: "Krystal Grand",
    parentCompany: "Grupo Hotelero Santa Fe (GSF)",
    managementCompany: "Grupo Hotelero Santa Fe, S.A.B. de C.V.",
    chainScale: "Luxury Chain",
    market: "Puerto Vallarta / Riviera Nayarit",
    submarket: "Marina Vallarta / Zona Hotelera Norte",
    city: "Puerto Vallarta",
    country: "Mexico",
    website: "https://en.krystalgrand-puertovallarta.com/",
  },
  [CAMBRIDGE_ID]: {
...
function loadAreaHotelPack(fileName) {
  ...
  const hotels = (data && Array.isArray(data.hotels) ? data.hotels : []).map((h) => {
    ...
    if (GOLDEN_DEMO_RADAR_HOTEL_IDS.has(row.id)) {
      ...
      if (presented) {
        return {
          ...
          city: presented.city || row.city,
          country: presented.country || row.country,
        };
      }
    }
    return row;
...
export function searchFixtureHotels(search, { limit = 400 } = {}) {
  ...
  let packs = [];
  const wantsCancun = q.includes("cancun") || q.includes("cancún");
  const wantsGdl =
    q.includes("zapopan") || q.includes("guadalajara") || q.includes("gdl") || q.includes("expo");
  const wantsPv =
    q.includes("vallarta") ||
    q.includes("nayarit") ||
    q.includes("marina vallarta") ||
    q.includes("krystal grand");
  const wantsBermuda =
    q.includes("bermuda") ||
    q.includes("somerset") ||
    q.includes("sandys") ||
  ...
  // Legacy STR bucket — do NOT default Pacific Central to Guadalajara only.
  const wantsPacificCentral = q.includes("pacific central");

  if (wantsCancun || q === "mexico") {
    packs = packs.concat(loadAreaHotelPack("area-hotels-cancun.json"));
  }
  if (wantsGdl || q === "mexico") {
  ...
  if (wantsPv || wantsPacificCentral || q === "mexico") {
    packs = packs.concat(loadAreaHotelPack("area-hotels-puerto-vallarta.json"));
  }
  if (wantsBermuda || q === "bermuda") {
    packs = packs.concat(loadAreaHotelPack("area-hotels-bermuda.json"));
  }

  if (!packs.length) {
    // Soft match across golden-demo area packs for unexpected query shapes.
    packs = packs
      ...
      .concat(loadAreaHotelPack("area-hotels-puerto-vallarta.json"))
      .concat(loadAreaHotelPack("area-hotels-bermuda.json"))
      .filter((h) => {
...
export function loadMexicoDemandAnchorFixturePoints(query = {}) {
  const files = [
    ...
    "demand-anchors-puerto-vallarta-riviera-nayarit-real.json",
  ];
...
export function loadMexicoTravelInfrastructureFixturePoints(query = {}) {
  const files = [
    ...
    "travel-infrastructure-puerto-vallarta-riviera-nayarit-real.json",
  ];
...
export function buildFixtureMarketCoverage(query = {}) {
  ...
  const country = String(query.country || "").trim() || "Mexico";

  const searchKey = city || market || submarket || country || "Mexico";
  let hotels = searchFixtureHotels(searchKey, { limit: 500 });
  if (submarket) {
    const sm = normSearch(submarket);
    const filtered = hotels.filter((h) => {
      const blob = normSearch(`${h.submarket || ""} ${h.market || ""} ${h.city || ""}`);
      return blob.includes(sm) || sm.split(/[\/|,]/).some((part) => part && blob.includes(part.trim()));
    });
    if (filtered.length) hotels = filtered;
  }
  if (country && hotels.length) {
    const c = normSearch(country);
    const countryHits = hotels.filter((h) => normSearch(h.country || "").includes(c));
    if (countryHits.length) hotels = countryHits;
  }
...
export function shouldUseMexicoRadarFixtureFallback(query = {}) {
  ...
  const submarket = normSearch(query.submarket || query.strSubmarket);
  const blob = `${market} ${city} ${search} ${submarket}`;
  if (country.includes("mexico") || country === "mx") return true;
  if (country.includes("bermuda") || country === "bm") return true;
  if (
    /cancun|zapopan|guadalajara|gdl|pacific central|vallarta|nayarit|bermuda|somerset|sandys|cambridge|west end/.test(
      blob
    )
  ) {
    return true;
