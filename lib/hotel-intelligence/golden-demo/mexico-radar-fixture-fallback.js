...
const HOTEL_PRESENTATION = {
  ...
  [VOCO_CANCUN_ID]: {
    ...
    amenities:
      "Outdoor swimming pool, Fitness center, Free WiFi, Free on-site parking, Restaurant (Stock Cafe), Bar / lounge, Room service, 24-hour front desk, Meeting rooms, Business center, Terrace / garden, Laundry facilities, Air conditioning, Non-smoking rooms, Facilities for disabled guests",
  },
};
...
export function buildFixtureMarketCoverage(query = {}) {
  ...
  for (const h of hotels) {
    ...
    if (h.parentCompany) parents.add(h.parentCompany);
    if (h.chainScale) {
      byChainScale[h.chainScale] = (byChainScale[h.chainScale] || 0) + 1;
    }
