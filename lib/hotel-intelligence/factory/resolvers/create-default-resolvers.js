...
import { getDefaultContactIntelligenceService } from "../../contact-intelligence/index.js";
...
export function createDefaultResolvers(opts = {}) {
  ...
  return {
    ...
    resolveContacts({ hotel_id }) {
      try {
        const svc = opts.contactService || getDefaultContactIntelligenceService();
        const result = svc.hotelGet({ hotel_id, audience: "public" });
        const pkg = result.package || null;
        const gaps = result.unresolved_reasons || [];
        const hasAny =
          (pkg?.hotel_contact?.channels || []).length > 0 ||
          (pkg?.organization_contact_route?.channels || []).length > 0 ||
          (pkg?.people || []).length > 0;
        let status = "MISSING";
        if (hasAny && gaps.length === 0) status = "STRONG";
        else if (hasAny) status = "PARTIAL";
        return domainResult({
          data: pkg,
          status,
          provenance: {
            surface: "contact-intelligence-v1",
        ...
        return domainResult({
          ...
          status: "error",
          provenance: {},
          gaps: ["contact_resolver_error"],
          conflicts: [err?.message || String(err)],
        });
      }
    },
    ...
    resolveBrandOperator({ hotel_id }) {
      ...
      const brand =
        oac.brand?.name ||
        own.hotel?.brand ||
        own.deep_research?.brand_resolution?.current_brand ||
        seed.brand ||
        null;
      const operator =
        oac.operator?.name ||
        own.deep_research?.operator_resolution?.current_operator?.name ||
        null;
      let status = "MISSING";
    ...
    resolveCompleteness({ hotel_id }) {
      ...
      const domains = {
        ...
        CONTACTS: mapStatus(resolvers.resolveContacts({ hotel_id })),
