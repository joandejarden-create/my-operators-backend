...
function stripInternalCost(request, { includeAdmin = false } = {}) {
...
export function buildResearchCenterPayload(input = {}) {
  ...
  const includeAdmin = Boolean(input.includeAdmin);
  ...
  const latest = completedFull
    ? stripInternalCost(completedFull, { includeAdmin })
    : liveArchive[0]
      ? stripInternalCost(liveArchive[0], { includeAdmin })
      : null;
  ...
  const archive = liveArchive.map((r) => stripInternalCost(r, { includeAdmin }));
  ...
  return {
    ...
    active_requests: active.map((r) => ({
      ...stripInternalCost(r, { includeAdmin }),
    ...
    completed_reports: liveArchive.map((r) => stripInternalCost(r, { includeAdmin })),
