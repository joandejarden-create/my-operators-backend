...
/**
 * Preserve provider claim metadata. Never silently downgrade VERIFIED→CANDIDATE
 * or drop confidence. If Dealality must derive confidence later, label it explicitly.
 */
function normalizeClaimStatus(rawStatus) {
  const s = String(rawStatus || "").trim();
  if (!s) return { status: "CANDIDATE", status_source: "dealality_default_absent_provider_status" };
  ...
  if (allowed.has(upper)) {
    return { status: upper, status_source: "provider" };
  }
  return { status: s, status_source: "provider_passthrough" };
}
...
function extractClaims(raw) {
  ...
  for (const list of lists) {
    for (const c of asArray(list)) {
      ...
      if (typeof c === "string") {
        out.push({
          claim_id: `cl_${out.length + 1}`,
          text: c,
          value: null,
          evidence: null,
          status: "CANDIDATE",
          status_source: "dealality_default_string_claim",
          confidence: null,
      ...
      const { status, status_source } = normalizeClaimStatus(
        c.status || c.verification_status || c.claim_status
      );
      const providerConfidence =
        c.confidence != null
          ...
          : c.confidence_score != null
            ? c.confidence_score
            : c.confidence_label != null
              ? c.confidence_label
              : null;
      out.push({
        claim_id: c.id || c.claim_id || `cl_${out.length + 1}`,
        text: c.text || c.claim || c.statement || "",
        value: c.value != null ? c.value : c.normalized_value != null ? c.normalized_value : null,
        evidence:
          c.evidence ||
          c.evidence_text ||
          c.excerpt ||
          (Array.isArray(c.evidence_items) ? c.evidence_items : null),
        status,
        status_source,
        confidence: providerConfidence,
        confidence_source: providerConfidence != null ? "provider" : null,
        inference:
          typeof c.inference === "boolean"
            ? c.inference
            : typeof c.is_inference === "boolean"
              ? c.is_inference
              : null,
        contradictions: asArray(c.contradictions || c.contradicting_evidence),
        corroboration_count:
          c.corroboration_count != null
            ? c.corroboration_count
            : Array.isArray(c.sources || c.source_ids)
              ? (c.sources || c.source_ids).length
              : null,
        sources: c.sources || c.source_ids || [],
        date: c.date || c.as_of || null,
        unresolved: c.unresolved === true || status === "UNRESOLVED",
      });
    }
  }
    ...
    const evidenceText =
      typeof c.evidence === "string"
        ...
        : Array.isArray(c.evidence)
          ? c.evidence
              .map((e) => (typeof e === "string" ? e : e?.excerpt_or_fact || e?.excerpt || ""))
              .filter(Boolean)
              .join("\n")
          : "";
    const body =
      evidenceText && evidenceText.trim().length > 40
        ? `${full}\n\n${evidenceText.trim()}`
        : full;
    findings.push({
      ...
      body,
      // Preserve provider verification status; keep finding_type separate.
      status: c.status || "RESEARCH_FINDING",
