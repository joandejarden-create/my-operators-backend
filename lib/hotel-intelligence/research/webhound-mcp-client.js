...
/** Test/cutover hard kill — any truthy value blocks all Webhound network calls. */
export function isWebhoundDisabled(env = process.env) {
  const v = String(env.WEBHOUND_DISABLED || "").trim().toLowerCase();
  return v === "1" || v === "true" || v === "yes" || v === "on";
}

export function assertWebhoundNetworkAllowed(env = process.env) {
  if (!isWebhoundDisabled(env)) return;
  const err = new Error("webhound_disabled");
  err.code = "webhound_disabled";
  err.customer_safe = "Webhound is disabled for this environment.";
  throw err;
}
