...
export function compileWebhoundPrompt(input = {}) {
  ...
  const isDevelopmentProject =
    ...
    Boolean(seed.development_project_id || spec.subject?.development_project_id);
  ...
  lines.push(
    isDevelopmentProject
      ? "## Subject identity (authoritative)"
      : "## Hotel identity (authoritative)"
  );
  lines.push(`- Canonical hotel ID: ${seed.hotel_id || spec.subject?.hotel_id || ""}`);
  if (seed.development_project_id || spec.subject?.development_project_id) {
    lines.push(
      `- Development project ID: ${seed.development_project_id || spec.subject.development_project_id}`
    );
  }
  if (spec.subject?.subject_type || seed.subject_type) {
    lines.push(`- Subject type: ${spec.subject?.subject_type || seed.subject_type}`);
  }
  lines.push(`- Current name: ${seed.hotel_name || spec.subject?.hotel_name || ""}`);
  if (spec.subject?.project_name) lines.push(`- Project name: ${spec.subject.project_name}`);
  if (spec.subject?.site_name) lines.push(`- Site name: ${spec.subject.site_name}`);
  if (seed.address || spec.subject?.address) {
    lines.push(`- Address / location: ${seed.address || spec.subject.address}`);
  }
  if (spec.subject?.parcel_or_site_reference) {
    lines.push(`- Parcel / site reference: ${spec.subject.parcel_or_site_reference}`);
  }
  if (spec.subject?.coordinates) {
    lines.push(`- Coordinates: ${JSON.stringify(spec.subject.coordinates)}`);
  }
  ...
  if (spec.subject?.known_developer) {
    lines.push(`- Developer (known, verify): ${spec.subject.known_developer}`);
  }
  ...
  if (spec.subject?.aliases?.length) {
    lines.push(`- Aliases: ${spec.subject.aliases.join("; ")}`);
  }
  ...
  if (template.template_id === "DEVELOPMENT_PROJECT_INTELLIGENCE") {
    lines.push("");
    lines.push("## Development & Project Intelligence special requirements");
    lines.push("This subject may be a vacant site, proposed hotel, hotel+residences, mixed-use,");
    lines.push("conversion, redevelopment, stalled project, or under-construction asset.");
    lines.push("Do NOT assume an operating hotel. Do NOT invent a Census hotel record.");
    lines.push("Separate land owner / PropCo / economic sponsor / developer / JV / brand / operator.");
    lines.push("Never collapse ANNOUNCED→CURRENT, APPROVED→UNDER_CONSTRUCTION, BRAND DISCUSSED→SIGNED.");
    lines.push("Apply exact-site negative screens: WRONG_SITE, ADJACENT_PROJECT, SAME_DEVELOPER_DIFFERENT_PROJECT.");
    lines.push("Every material conclusion needs status (VERIFIED / PROBABLE / SIGNAL_ONLY / UNRESOLVED / etc.).");
    lines.push("Produce CURRENT PROJECT ASSESSMENT and a VERIFIED VS UNVERIFIED table.");
    lines.push("Commercial opportunity signals may be STRONG / MODERATE / LIMITED / UNKNOWN — no deal claims.");
    lines.push("Correct abstention is valid. Do not auto-run additional paid research.");
  }
