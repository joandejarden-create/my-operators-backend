...
import {
  ...
  loadMexicoExplorerDemoCohort,
  tryBuildMexicoExplorerNeighborsPayload,
  MEXICO_EXPLORER_DEMO_VERSION,
} from "./mexico-explorer-demo-cohort.js";
...
export function buildNeighborsPayload(opts = {}) {
  const mexicoNeighbors = tryBuildMexicoExplorerNeighborsPayload(opts);
  if (mexicoNeighbors) return mexicoNeighbors;
...
export function buildHotelOwnershipReport(airtableRecordId) {
  // Thin router for legacy callers. Packet 2.8B assembler uses OwnershipProvider registry instead.
  if (isCambridgeHotelId(airtableRecordId)) {
    return buildCambridgeHotelOwnershipReport(airtableRecordId);
  }
  if (isMexicoExplorerDemoHotelId(airtableRecordId)) {
    return buildMexicoExplorerDemoOwnershipReport(airtableRecordId);
  }
  return buildGsfCohortHotelOwnershipReportOnly(airtableRecordId);
}

/**
 * GSF cohort / Case C ownership builder — no Cambridge/Mexico routing.
 * Used by OwnershipProvider registry (Packet 2.8B).
 */
export function buildGsfCohortHotelOwnershipReportOnly(airtableRecordId) {
  const cohort = loadGsfGoldenDemoCohort();
  const org = orgOf(cohort);
...
export function buildOwnershipGroupProfile(slug) {
  ...
  const mexicoGroup = buildMexicoExplorerOwnershipGroupProfile(slug);
  if (mexicoGroup && mexicoGroup.ok) {
    return mexicoGroup;
  }
...
export function listGoldenDemoCohortIndex() {
  ...
  const mexico = loadMexicoExplorerDemoCohort();
  const mexicoHotels = (mexico.hotels || [])
    .filter((h) => h.airtable_record_id)
    .map((h) => ({
      airtable_record_id: h.airtable_record_id,
      hotel_id: h.hotel_id || null,
      ...
      intelligence_case: "B",
      review_url: `/hotel-intelligence-golden-demo.html?autopen=1&hotelExplorer=${encodeURIComponent(
        h.airtable_record_id
      )}&hexTab=ownership`,
    }));

  const allHotels = [...hotels, ...cambridgeHotels, ...mexicoHotels];

  return {
    ok: true,
    version: GOLDEN_DEMO_GSF_VERSION,
    mexico_explorer_demo_version: MEXICO_EXPLORER_DEMO_VERSION,
    data_status: cohort.data_status,
    groups: [org.slug, DOVETAIL_SLUG, "mexico-explorer-demo"],
    hotel_airtable_ids: allHotels.map((h) => h.airtable_record_id).filter(Boolean),
    hotels: allHotels,
    hotels_with_owner_information: allHotels.filter((h) => h.economic_owner_verified),
    relationship_summary: summary,
    default_demo: {
      country: "Mexico",
      ...
      sheraton_gdl_demo_url:
        "/hotel-intelligence-golden-demo.html?autopen=1&hotelExplorer=recsYJb2R1jarPpK3&hexTab=hotel",
      real_inn_cancun_demo_url:
        "/hotel-intelligence-golden-demo.html?autopen=1&hotelExplorer=recTYaiA4S6fR6ixx&hexTab=hotel",
