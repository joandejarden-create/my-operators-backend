...
export function auditMarketTravelInfrastructure(records, options = {}) {
  ...
  const isCountrywideMarket = marketNorm.includes("countrywide");

  let keywords;
  if (options.keywords) {
    keywords = options.keywords.map(norm);
  } else if (marketConfig?.keywords) {
    keywords = marketConfig.keywords.map(norm);
  } else if (country === "mexico") {
    keywords = DEFAULT_CANCUN_KEYWORDS.map(norm);
  } else {
    keywords = countryLabel
      .split(/\s+/)
      .map(norm)
      .filter((w) => w.length > 2);
  }
  ...
  const expectedPatterns =
    ...
    marketConfig?.expectedPatterns ||
    (country === "mexico" ? EXPECTED_CANCUN_TI_PATTERNS : []);

  const usedCancunFallback =
    !options.keywords &&
    !marketConfig &&
    country === "mexico" &&
    !options.expectedPatterns;
  ...
  const marketRecords = countryRecords.filter((r) => {
    ...
    if (isCountrywideMarket && marketConfig) {
      return true;
    }
    if (isCountrywideMarket && !marketConfig && keywords.length) {
      return matchesKeywords(r, keywords) || keywords.some((kw) => kw.length > 4 && country.includes(kw));
    }
  ...
  return {
    ...
    notes: [
      ...
      ...(usedCancunFallback ? ["Used Mexico/Cancún default expected-node patterns (no country-specific audit config)."] : []),
      ...(!marketConfig && country !== "mexico"
        ? [`No dedicated TI audit config for ${countryLabel}; keywords derived from country name.`]
        : []),
    ],
  };
}
