...
function isResearchStageSubmissionStatus(status) {
  const s = String(status || "").trim().toLowerCase();
  return s === "research stage" || s === "research-stage" || s === "research_stage";
}

/**
 * Load Research Stage Operator Setup masters (internal pilot — not production Active).
 */
export async function loadResearchStageOperatorCandidatesForAlignment() {
  ...
  for (const master of masterRecords || []) {
    ...
    if (!isResearchStageSubmissionStatus(status)) continue;
    if (hideTest && /test|demo|sandbox/i.test(String(mf.company_name || mf["Company Name"] || mf.Name || ""))) continue;
    ...
    const companyName =
      formatListValue(mf.company_name || mf["Company Name"] || mf.Name) ||
      formatListValue(profile?.fields?.company_name) ||
      master.id;
    candidates.push({
