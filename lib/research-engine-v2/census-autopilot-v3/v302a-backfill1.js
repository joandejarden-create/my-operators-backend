/**
 * V3.0.2A — Apply authorized blank-fill backfill from V3.0.2 dry-run manifest.
 * Pilot A = 15, then remainder if gates pass. Blank-fill only.
 */
 ...
 */
export async function runV302Backfill1(opts) {
  const root = opts.root;
  const log = opts.log || console.log;
  const enableWrites = opts.enableWrites === true;
  const manifestPath = path.join(
    root,
    "data/research-engine-v2/census-autopilot-v3-airtable-migration/33-golden-geography-contact-research/18-production-backfill-dry-run.json"
  );
  const dry = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
  if (dry.run_id !== AUTHORIZED_RUN) throw new Error(`run_id mismatch ${dry.run_id}`);
  const mutations = dry.mutations || [];
  if (!mutations.length) throw new Error("empty_manifest");
  ...
  const trip = (reason, detail = {}) => {
    ...
    log(`[v3.0.2a.bf1] CIRCUIT: ${reason}`);
  };
  ...
  for (const m of mutations) {
    ...
    preWrite.push({
      ...
      fields: Object.fromEntries(
        [...ALLOWED_FIELDS].map((f) => [f, rec.fields?.[f] ?? null])
      ),
    });
    await sleep(60);
  }
  ...
  async function processOne(m, pilot) {
    ...
    for (const [f, v] of Object.entries(m.fields || {})) {
      ...
      if (!isBlank(cur)) continue; // blank-fill only
      toWrite[f] = v;
      rollback[f] = cur ?? null;
    }
  ...
  async function runPilot(list, label) {
    const results = [];
    log(`[v3.0.2a.bf1] Pilot ${label} n=${list.length} writes=${enableWrites}`);
    for (let i = 0; i < list.length; i++) {
      ...
      if ((i + 1) % 5 === 0) log(`[v3.0.2a.bf1] Pilot ${label} ${i + 1}/${list.length}`);
      await sleep(80);
    }
    return results;
  }
  ...
  const aFail = aResults.some((r) =>
    ["mismatch", "blocked_identity", "blocked_cvent", "blocked_legacy", "blocked_rights", "blocked_unexpected_field", "error"].includes(
      r.status
    )
  );
  ...
  let bExecuted = false;
  if (aPass) {
    log("[v3.0.2a.bf1] Pilot A PASSED — applying remainder");
    bExecuted = true;
    bResults = await runPilot(rest, "B");
  } else {
    log("[v3.0.2a.bf1] Pilot A FAILED — remainder skipped");
  }
