...
export const COUNTRY_TO_CONTINENT_SUBCONTINENT = Object.freeze({
  ...
  // —— Remaining CALA 52 (Dealality commercial geography) ——
  ...
  anguilla: { continent: "North America", subContinent: "Caribbean" },
  "antigua and barbuda": { continent: "North America", subContinent: "Caribbean" },
  bonaire: { continent: "North America", subContinent: "Caribbean" },
  "sint eustatius": { continent: "North America", subContinent: "Caribbean" },
  saba: { continent: "North America", subContinent: "Caribbean" },
  "british virgin islands": { continent: "North America", subContinent: "Caribbean" },
  "cayman islands": { continent: "North America", subContinent: "Caribbean" },
  ...
  "saint barthelemy": { continent: "North America", subContinent: "Caribbean" },
  "saint barthélemy": { continent: "North America", subContinent: "Caribbean" },
  "saint kitts and nevis": { continent: "North America", subContinent: "Caribbean" },
  "st kitts and nevis": { continent: "North America", subContinent: "Caribbean" },
  "saint lucia": { continent: "North America", subContinent: "Caribbean" },
  "st lucia": { continent: "North America", subContinent: "Caribbean" },
  "saint martin": { continent: "North America", subContinent: "Caribbean" },
  "st martin": { continent: "North America", subContinent: "Caribbean" },
  ...
  "u.s. virgin islands": { continent: "North America", subContinent: "Caribbean" },
  "us virgin islands": { continent: "North America", subContinent: "Caribbean" },
  "united states virgin islands": {
    continent: "North America",
    subContinent: "Caribbean",
  },
  bermuda: { continent: "North America", subContinent: "Caribbean" },
});
...
/**
 * Resolve Continent / Sub-Continent using Clean Core map, then Dealality CALA
 * geography region fallback so all 52 in-scope geos resolve deterministically.
 * @param {string} country
 * @param {{ resolveDealalityGeography?: Function }} [opts]
 */
export function resolveContinentSubContinentCanonical(country, opts = {}) {
  const direct = resolveContinentSubContinentFromCountry(country);
  if (direct?.continent && direct?.subContinent) return { ok: true, ...direct, method: "country_map" };

  const resolveGeo =
    opts.resolveDealalityGeography ||
    (() => {
      try {
        // Lazy import avoided — caller may pass resolver
        return null;
      } catch {
        return null;
      }
    });
  const geo = typeof resolveGeo === "function" ? resolveGeo(country) : null;
  const g = geo?.geography || geo;
  if (!g?.region) return { ok: false, reason: "unmapped_country" };

  if (g.region === "Central America") {
    if (/^mexico$/i.test(String(g.name || ""))) {
      return {
        ok: true,
        continent: "North America",
        subContinent: "Mexico",
        method: "dealality_cala_region",
      };
    }
    return {
      ok: true,
  ...
  return { ok: false, reason: "unmapped_dealality_region", region: g.region };
}
