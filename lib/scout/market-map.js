...
function mapCensusRow(rec) {
  ...
  let affiliation = exactMatchKey(f[CENSUS_FIELDS.affiliation]);
  ...
  } else if (affiliationStatus === "Unknown") {
    isIndependent = false;
    isBranded = false;
  ...
  const roomsRaw = f[CENSUS_FIELDS.rooms];
...
function rowInScope(row, filters) {
  ...
  if (filters.market) {
    const marketHit =
      textMatches(row.market, filters.market) ||
      textMatches(row.region, filters.market) ||
      textMatches(row.city, filters.market);
...
function buildHotelMarker(row, hotelSource = "Hotel Census") {
  if (!isValidCoordinate(row.latitude, row.longitude)) return null;
  const popupMetrics = {
    rooms: row.roomsKnown === false ? "Unknown" : row.rooms,
    status: row.status,
    chainScale: row.chainScale || "Unknown",
    locationType: row.locationType,
  };
  return {
    markerId: `hotel-${row.id}`,
    markerType: "hotel",
    airtableRecordId: row.hpcRecordId || row.id,
    dhl_: row.dhl_ || (String(row.id || "").startsWith("dhl_") ? row.id : null),
    hotelName: row.name,
    affiliation: row.affiliation,
    affiliationClass: row.affiliationClass || null,
    parentCompany: row.parentCompany,
    status: row.status,
    rooms: row.rooms,
    roomsKnown: row.roomsKnown !== false,
    country: row.country,
    city: row.city,
    market: row.market,
    ...
    popupTitle: row.name,
    popupSubtitle: [row.affiliation, row.market || row.city].filter(Boolean).join(" · "),
    popupMetrics,
    source: hotelSource,
  };
}
...
export async function buildMarketMapReport(query = {}) {
  ...
  const hotelSource = hpcEnabled ? "Hotel Property Census" : "Hotel Census";
  ...
  try {
    ...
    if (hpcEnabled) {
      warnings.push("SCOUT_HPC: Chain Scale UNKNOWN — scale filter/gap features SAFE_DEGRADATION.");
    }
  } catch (err) {
  ...
  let hotelMarkers = scopedRows
    .map((row) => buildHotelMarker(row, hotelSource))
    .filter(Boolean)
  ...
  const signalQuery = filters.includeSignals
    ? {
        ...
        _scoutHpc: hpcEnabled,
        useHpc: hpcEnabled ? "1" : "0",
