...
async function loadCensusRows(base, geoFilters = {}, options = {}) {
  const warnings = [];
  const hpcEnabled = options.useHpc === true;
  const cached = await fetchScoutCensusRecords({
    country: geoFilters.country,
    market: geoFilters.market,
    city: geoFilters.city,
    useHpc: hpcEnabled,
  });
...
export async function buildOpportunitySignalsReport(query = {}) {
  ...
  let hotelSource = "Hotel Census";
  const hpcEnabled =
    query._scoutHpc === true || query.useHpc === true || query.useHpc === "1";
  try {
    const loaded = await loadCensusRows(base, filters, { useHpc: hpcEnabled });
    rows = loaded.rows;
    fieldsLoaded = new Set(loaded.fieldsLoaded);
    censusRecordsLoaded = loaded.censusRecordsLoaded;
    hotelSource = loaded.hotelSource || hotelSource;
    warnings.push(...loaded.warnings);
    if (hpcEnabled) {
      warnings.push(
        "SCOUT_HPC: Chain Scale UNKNOWN — brand/parent gap signals that rely on comparable chain scale are SAFE_DEGRADATION."
      );
    }
  } catch (err) {
