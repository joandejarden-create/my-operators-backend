...
export function classifyEarlySignalCandidate(item = {}) {
  ...
  if (stale.stale) {
    ...
    if (
      relevance.reason === "CONSUMER_TRAVEL" ||
      relevance.reason === "LISTICLE" ||
      relevance.reason === "POINTS_HACKING" ||
      relevance.reason === "FNB_AMENITY" ||
      relevance.reason === "AWARD_FLUFF" ||
      relevance.reason === "GENERIC_AIRLINE" ||
      relevance.reason === "GENERIC_CRUISE" ||
      relevance.reason === "GENERIC_TRAVEL_APP"
    ) {
      rejection = relevance.reason;
    } else if (/industrial|commercial land|vacant land|warehouse for sale|real estate market/i.test(relevance.reason || "")) {
