 ...
 * Never sync local DEV_AUTH_BYPASS placeholder ids into Airtable.
 */
export function shouldSyncMemberstackIdToUsersRow(existingId, incomingId) {
  ...
  // Local auth bypass must never write into Users.Unique_Webflow_ID / Slug.
  if (incoming === "dev_bypass" || incoming.startsWith("dev_")) return false;
  if (!incoming.startsWith("mem_")) return false;
  if (!existing) return true;
  if (existing === incoming) return false;
  if (isSandboxMemberstackId(incoming) && isLiveMemberstackId(existing)) return false;
  // Prefer a real mem_ id over a polluted local bypass value.
  if (existing === "dev_bypass" || existing.startsWith("dev_")) return true;
  return true;
}
