 ...
 * - MEMBERSTACK_SIGNUP_PENDING_PLAN_ID — Basic/pending pln_… auto-assigned on public signup create
 * - MEMBERSTACK_ASSIGN_PLANS_ON_SIGNUP — set `false` to disable pending plan on signup
 * - Role plans (Hotel Owners, Brand, Operator, etc.) — assign manually after admin approval
 *
 * @returns {Promise<{ memberstackId: string | null, memberstackNote: string, created: boolean }>}
 */
function shouldAssignPlansOnCreate(assignPlansOnCreate) {
  if (typeof assignPlansOnCreate === "boolean") return assignPlansOnCreate;
  if (process.env.MEMBERSTACK_ASSIGN_PLANS_ON_SIGNUP === "false") return false;
  return Boolean((process.env.MEMBERSTACK_SIGNUP_PENDING_PLAN_ID || "").trim());
}
...
export async function provisionMemberstackForSignup({
  ...
  const assignPlans = shouldAssignPlansOnCreate(assignPlansOnCreate);
  const pendingPlanId = assignPlans
