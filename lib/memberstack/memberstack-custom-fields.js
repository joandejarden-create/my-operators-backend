...
export const MS_CF = {
  ...
  uniqueAirtableId: envKey("MEMBERSTACK_CF_UNIQUE_AIRTABLE_ID", "Unique AirTable ID"),
  uniqueWebflowId: envKey("MEMBERSTACK_CF_UNIQUE_WEBFLOW_ID", "Unique Webflow ID"),
...
const LEGACY_READ_ALIASES = {
  ...
  airtableUserId: ["airtableRecordId", "airtableUserId", MS_CF.airtableUserId, "air-table-user-id", "airtable-user-id"],
  uniqueAirtableId: ["uniqueAirtableId", MS_CF.uniqueAirtableId, "unique-air-table-id"],
  uniqueWebflowId: ["uniqueWebflowId", MS_CF.uniqueWebflowId, "unique-webflow-id"],
 ...
 * @param {{ airtableRecordId?: string, companyProfileId?: string, memberstackId?: string }} [extras]
 */
export function buildMemberstackCustomFields(body, extras = {}) {
  ...
  if (airtableRecordId) {
    out[MS_CF.airtableUserId] = airtableRecordId;
    // unique-airtable-id removed — duplicate of airtable-user-id (MS field deprecated).
  }

  const memberstackId =
    typeof extras.memberstackId === "string" ? extras.memberstackId.trim() : "";
  if (memberstackId) out[MS_CF.uniqueWebflowId] = memberstackId;
...
export function readLogicalCustomFields(cf) {
  return {
    ...
    airtableRecordId:
      pickFirst(cf, LEGACY_READ_ALIASES.airtableUserId) ||
      pickFirst(cf, LEGACY_READ_ALIASES.uniqueAirtableId),
    uniqueWebflowId: pickFirst(cf, LEGACY_READ_ALIASES.uniqueWebflowId),
