...
import { sanitizeMarketAlertPlainText } from "./market-alerts-plain-text.js";
...
export function extractSummaryFromHtml(html) {
  ...
  for (const re of META_PATTERNS) {
    ...
    const raw = (m?.[1] || "").trim();
    if (raw.length > 20) {
      const cleaned = sanitizeMarketAlertPlainText(raw, { maxLen: 500 });
      if (!cleaned || REJECT_SUMMARY_RE.test(cleaned)) continue;
      return cleaned;
    }
  }
  return "";
...
export async function fetchArticleSummary(url, opts = {}) {
  ...
  if (!target) return "";
  // Google News wrapper pages only expose a generic site description.
  if (/news\.google\.com/i.test(target)) return "";
