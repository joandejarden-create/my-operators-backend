...
export async function fetchProductionEarlySignalCandidates(opts = {}) {
  ...
  const queryCatalog = summarizeEarlySignalQueryCatalog(queries);
  ...
  for (const q of queries) {
    ...
    familyDiag[q.family].queriesAttempted += 1;
    const url = buildGoogleNewsRssUrlForQuery(q, { when });
    ...
    try {
      ...
      for (const item of items.slice(0, perQuery)) {
        ...
        familyDiag[q.family].rawItems += 1;
        raw.push({
          ...item,
          family: q.family,
          queryId: q.id,
          queryLabel: q.label,
          lang: q.lang || "en",
          regionPack: q.regionPack || null,
          cala: !!q.cala,
          requireTitleHospitality: !!q.requireTitleHospitality,
        });
      }
    } catch (err) {
      familyDiag[q.family].fetchErrors += 1;
      fetchErrors.push({ id: q.id, family: q.family, error: err.message || String(err) });
    }
    await sleep(350);
  }
  ...
  const afterRelevance = [];
  const afterEventTiming = [];

  for (let i = 0; i < unique.length; i += 1) {
    const row = classified[i];
    if (!row.relevanceReason && row.rejection !== "CONSUMER_TRAVEL" && !String(row.rejection || "").match(/LISTICLE|POINTS|FNB|AWARD|GENERIC_/)) {
      // kept by relevance if no fluff rejection mapped from relevance
    }
    if (!row.rejection || !["CONSUMER_TRAVEL", "LISTICLE", "POINTS_HACKING", "FNB_AMENITY", "AWARD_FLUFF", "GENERIC_AIRLINE", "GENERIC_CRUISE", "GENERIC_TRAVEL_APP", "non-hotel", "off-topic"].includes(row.rejection)) {
      if (row.rejection !== "stale") afterRelevance.push(row);
    }
    if (row.eventType && row.signalTiming) afterEventTiming.push(row);

    const gate = assessEarlySignalProductionReady(row);
    if (gate.ok) {
      productionReady.push({ item: unique[i], classified: row });
    } else {
      const reason = gate.reason || row.rejection || "other";
      rejectByReason[reason] = (rejectByReason[reason] || 0) + 1;
      rejectedProduction.push({ title: row.title, reason, family: row.family, lang: unique[i].lang });
    }
  }
  ...
  return {
    ...
    diagnostics: {
      ...
      afterEventTiming: afterEventTiming.length,
      accepted: productionReady.length,
      rejectedByReason: rejectByReason,
    },
