...
import { enrichQualificationPrecision } from "./qualification-precision.js";
import {
  BOOKING_WINDOW,
  CLAIM_KIND,
  CLAIM_KIND_LABEL,
...
function buildKnownVsEstimated(raw, evidence) {
  ...
  return {
    ...
    inferred: [
      ...
      pick("DMV competitiveness", raw.demandTerritoryRationale, CLAIM_KIND.INFERENCE),
    ].filter((x) => x.value),
    evidenceFieldCounts: {
...
export function buildOpportunity(raw = {}) {
  ...
  // Re-score Hotel Fit after contactability / geography adjustments from precision pass
  ...
  const priorityResult = classifyPriority({
    ...
    highPriorityGate: precision.highPriorityGate || null,
    opportunity: { ...raw, whyNow: raw.whyNow, bookingWindowStatus },
  });
  ...
  return {
    ...
    highPriorityGate: precision.highPriorityGate || raw.highPriorityGate || null,
    ...
    evidence,
    sources: raw.sources || buildSourcesSummary(evidence),
    knownVsEstimated: buildKnownVsEstimated(raw, evidence),
