...
import { isRejectedDeskPersona } from "./contact-desk-persona-gate.js";
import {
  CONTACT_TIER,
  classifyContactTier,
...
export function applySourceContactExtractionToOpportunity(opportunity = {}, extraction = {}) {
  ...
  const deskReject = isRejectedDeskPersona(primary);
  if (deskReject.reject) {
    // Keep functional mailbox / org path — do not invent a named person from desk personas
    const functionalEmail = primary.email && /webmaster|postmaster|webadmin/i.test(primary.email)
      ? null
      : primary.email;
    const nextDesk = {
      ...opportunity,
      ...
      contactResearchAudit: {
        ...(opportunity.contactResearchAudit || {}),
        deskPersonaRejected: deskReject.reason,
      },
    };
    if (functionalEmail && !nextDesk.primaryContactEmail) {
      nextDesk.primaryContactEmail = functionalEmail;
    }
    nextDesk.contactTier = classifyContactTier(nextDesk);
    return {
      opportunity: nextDesk,
      beforeTier,
      afterTier: nextDesk.contactTier,
      improved: nextDesk.contactTier !== beforeTier,
      extraction,
      deskPersonaRejected: deskReject.reason,
    };
  }
