...
/** CSV export row — stable Dealality Opportunity ID required. */
export function toExportRow(opp = {}, hotel = {}) {
  const cq = applyLiveCommercialQuality(opp);
  const c = cq.primaryContact || {};
  return {
    "Dealality Opportunity ID": cq.id || cq.opportunityId || "",
    Hotel: hotel.name || cq.hotelId || "",
    "Opportunity / Event": cq.title || "",
    "Canonical Event Name": cq.canonicalEventName || "",
    Organization: cq.organizationName || "",
    "Event Series": cq.eventSeriesId || "",
    "Event Cycle": cq.eventCycleId || "",
    "Start Date": cq.eventStartDate || cq.eventYear || "",
    "End Date": cq.eventEndDate || "",
    "Date Status": cq.eventDateStatus || "",
    Location: cq.eventLocationSummary || cq.destinationStatus || "",
    Venue: cq.venueStatus || cq.venueSourcingStatus || "",
    "Opportunity Type": cq.opportunityType || "",
    Priority: cq.priority || "",
    "This Week State": cq.weeklyDeltaState || "",
    Attendance: cq.attendance ?? "",
    "Attendance Status": cq.attendanceStatus || "",
    "Peak Rooms": cq.peakRooms ?? "",
    "Peak Rooms Status": cq.peakRoomsStatus || "",
    "Room Demand Status": cq.roomDemandLive || cq.roomDemandStatus || "",
    "Room Demand Thesis": cq.hotelDemandThesis || "",
    "Sourcing Status": cq.venueSourcingStatus || cq.sourcingStatus || "",
    "WHO Name": c.name || "",
    "WHO Role": c.role || c.title || "",
    "Contact Type": cq.contactPathClass || "",
    Email: c.email || "",
    Phone: c.phone || "",
    "Suggested Action": cq.recommendedAction || "",
    "Action Basis": cq.actionBasis || "",
    "Primary Source URL":
      (cq.sources && cq.sources[0] && (cq.sources[0].url || cq.sources[0].sourceUrl)) ||
      cq.officialSource ||
      "",
    "Validation Status": cq.shareValidation?.familiarityStatus || "",
    "Action Status": cq.commercialProgression?.latestActionType || cq.actionStatus || "",
    "Outcome Status": cq.commercialProgression?.latestOutcomeType || "",
    "First Seen": cq.firstSeenAt || cq.createdAt || "",
    "Last Updated": cq.updatedAt || cq.lastMaterialChangeAt || "",
  };
}
...
export function rowsToCsv(rows = []) {
  ...
  const escCell = (v) => {
    ...
    // CSV formula injection: neutralize leading = + - @ | and tab/CR
    if (/^[=+\-@|\t\r]/.test(s)) {
      s = `'${s}`;
    }
...
export function buildExportCsv(opportunities, hotel, filterState = {}) {
  const rows = (opportunities || []).map((o) => toExportRow(o, hotel));
  const meta = [
    `# GDI Export`,
    ...
    `# schema=${LIVE_COMMERCIAL_QUALITY_V1}`,
  ].join("\n");
  return `${meta}\n${rowsToCsv(rows)}\n`;
}
