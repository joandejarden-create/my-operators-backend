...
import {
  ...
  isExpandableDemandSignalType,
  isLodgingPlausibleDemandSignal,
  DEMAND_SIGNAL_TYPE,
  LODGING_PLAUSIBLE_RE,
  LOCAL_NO_ROOM_RE,
...
import {
  ...
  PAGE_SIGNAL_CLASS,
} from "./discovery-routing-v1-2.js";
...
export function classifyDateConfidence(opp = {}, { nowDate } = {}) {
  ...
  if (opp.pastEventConfirmed === true || opp._yearPast === true) {
    return {
  ...
  // Year granularity with no exact day → not DATE_PAST unless year < current year
  const gran = String(opp.eventDateGranularity || "").toUpperCase();
  const startStr = String(opp.eventStartDate || "").trim().slice(0, 10);
  const yearFloor = /^20\d{2}-01-01$/.test(startStr);
  if (gran === "YEAR" || yearFloor || (!opp.eventStartDate && opp.eventYear)) {
    const y = Number(
      opp.eventYear ||
        (startStr.match(/^(20\d{2})/) || [])[1] ||
        0
    );
    ...
    return {
      ...
      eventStartDate: null,
      eventEndDate: null,
      reasons: ["year_only_not_jan1"],
      highSupport: false,
      eventYear: y || opp.eventYear || null,
    };
  }
...
export function classifyHotelDemandThesis(opp = {}) {
  ...
  // Local / no overnight patterns never create hotel room demand
  if (LOCAL_NO_ROOM_RE.test(`${title} ${blob} ${thesisBlob}`) && !LODGING_PLAUSIBLE_RE.test(blob)) {
    return { ok: false, strength: "NONE", reasons: ["local_no_room_signal"] };
  }
  ...
  // V1.2: first-party official housing/hotel page + named event → strong room thesis
  if (
    /official\s+housing\/hotel\s+page/i.test(String(opp.housingEvidence || "")) ||
    (opp.openSourcingEvidence &&
      /hotel|housing|accommodation|lodging|travel/i.test(String(opp.officialSource || "")))
  ) {
    if (EVENT_MARKER_RE.test(title) || multiDay || LODGING_PLAUSIBLE_RE.test(blob)) {
      return {
        ok: true,
        strength: "STRONG",
        reasons: ["official_housing_page_plus_event"],
      };
    }
  }
  ...
  // Expanded demand signals (training/project/relocation/etc.) with lodging language
  if (strength !== "STRONG" && LODGING_PLAUSIBLE_RE.test(blob)) {
    const signalType = classifyDemandSignalType(opp);
    if (isExpandableDemandSignalType(signalType)) {
      strength = multiDay || /multi[- ]?week|traveling|temporary\s+housing/i.test(blob)
        ? "STRONG"
        : "WEAK";
      reasons.push(`demand_signal_lodging:${signalType}`);
    }
  }
 ...
 * V1.2: official housing/hotel page URLs are first-party open-sourcing evidence.
 */
const OFFICIAL_HOUSING_PAGE_RE =
  /(?:\/(?:hotel|hotels|housing|accommodations?|lodging|travel(?:-info)?|venue|room-block|official-hotels?)(?:\/|$|\.|\?)|hotel[-_]?travel|travel[-_]?venue|housing[-_]?information|hotel[-_]?transport)/i;
...
export function classifyOpenSourcingEvidence(opp = {}) {
  ...
  // V1.2: commercial housing status from routing extract
  const housingStatus = String(opp.housingStatus || opp.commercialEvidenceStatus || "").toUpperCase();
  if (
    /HOUSING_OPEN|BLOCK_OPEN|OVERFLOW_POSSIBLE|HOUSING_FORTHCOMING|OPEN|TBD|ANNOUNCED/.test(
      housingStatus
    )
  ) {
    return {
      open: true,
      status:
        housingStatus === "OVERFLOW_POSSIBLE"
          ? "PRIMARY_VENUE_SELECTED_OVERFLOW_POSSIBLE"
          : housingStatus === "TBD"
            ? "HOTEL_VENUE_TBD"
            : "OPEN_UNRESOLVED",
      supported: true,
      reasons: [`routing_housing_status:${housingStatus}`],
    };
  }

  const officialUrl = String(opp.officialSource || opp.locationEvidenceUrl || "");
  const urlTitleBlob = `${officialUrl} ${opp.officialSourceTitle || ""} ${opp.commercialEvidenceText || ""}`;
  if (officialUrl && OFFICIAL_HOUSING_PAGE_RE.test(officialUrl)) {
    return {
      open: true,
      status: "OPEN_UNRESOLVED",
      supported: true,
      reasons: ["official_housing_page_url"],
    };
  }
  if (
    /\b(?:hotel|housing|accommodations?|lodging|room\s+block|official\s+hotels?)\b/i.test(
      urlTitleBlob
    ) &&
    (opp.isOfficialSource || /\.(gov|edu|org)\b/i.test(officialUrl))
  ) {
    return {
      open: true,
      status: "OPEN_UNRESOLVED",
      supported: true,
      reasons: ["official_source_housing_language"],
    };
  }
...
function pack(partial) {
  ...
  return {
    ...
    demandSignalType: partial.demandSignalType || null,
...
export function qualifyOpportunityV3(hotelId, opp = {}, opts = {}) {
  ...
  let working = mergeEvidenceOverlay(opp, evidenceOverlay);
  // V1.2 routing: year-floor scrub + source lineage + geo separation + noise class
  working = scrubYearFloorForHygiene(working, { nowDate });
  working = enrichCandidateRoutingV12(working, {
    nowDate,
    marketTokens:
      marketTokens ||
      [
  ...
  const notes = [];
  if (
    working.pageSignalClass === PAGE_SIGNAL_CLASS.SUPPLY_SIDE_PAGE ||
    working.pageSignalClass === PAGE_SIGNAL_CLASS.REFERENCE_ONLY
  ) {
    return pack({
      ...
      demandSignalType: working.demandSignalType || null,
      failures: [
        {
          class: FAILURE_CLASS_V3.NON_EVENT_CONTENT,
          detail: working.pageSignalClass,
        },
      ],
      notes: [`routing_reject:${working.pageSignalClass}`],
      working,
    });
  }
  if (
    working.geoConflict &&
    (working.geoConflictReason === "FAR_STATE_VS_MARKET" ||
      working.geoConflictReason === "ORG_VS_EVENT" ||
      working.geoClass === "GEO_CONFLICT")
  ) {
    return pack({
      actionability: ACTIONABILITY_V3.INVALID,
      failureClass: FAILURE_CLASS_V3.OTHER,
      semanticType: classifyEventSemanticType(working),
      demandSignalType: working.demandSignalType || classifyDemandSignalType(working),
      failures: [{ class: FAILURE_CLASS_V3.OTHER, detail: "GEO_CONFLICT" }],
      notes: ["geo:GEO_CONFLICT", working.geoConflictReason || working.geoClass],
      working,
    });
  }
  const semanticType = classifyEventSemanticType(working);
  const demandSignalType =
    working.demandSignalType || classifyDemandSignalType(working);
  working.demandSignalType = demandSignalType;
  notes.push(`semantic:${semanticType}`, `demandSignal:${demandSignalType}`);

  // Local day / one-hour programs without lodging → INVALID (not hotel demand)
  const textBlob = `${norm(working.title)} ${collectText(working)}`;
  if (LOCAL_NO_ROOM_RE.test(textBlob) && !LODGING_PLAUSIBLE_RE.test(textBlob)) {
    return pack({
      actionability: ACTIONABILITY_V3.INVALID,
      ...
      failures: [
        {
          ...
          detail: "local_no_room_signal",
        },
      ],
      notes: [...notes, "local_no_room_invalid"],
      working,
    });
  }

  // Expanded demand signals with lodging thesis may pass NON_EVENT gate.
  // PLAN/REPORT/ORGANIZATION/VENUE without lodging remain INVALID.
  const lodgingDemandSignal =
    isExpandableDemandSignalType(demandSignalType) &&
    isLodgingPlausibleDemandSignal(working, demandSignalType) &&
    demandSignalType !== DEMAND_SIGNAL_TYPE.OTHER;

  // Non-event content → INVALID (unless lodging-plausible expanded demand signal)
  if (
    !lodgingDemandSignal &&
    ((NON_EVENT_TYPES.has(semanticType) &&
  ...
  // Expanded demand signals (training/project/etc.) qualify like events when lodging-strong
  const realDemandSignal = lodgingDemandSignal && demandStrong;

  if (
    (realEvent || realDemandSignal) &&
    dateOk &&
    geoOk &&
    sourceOk &&
    demandStrong &&
    ...
    const asOverflow =
      meetingVenueRoomsOpen ||
      hostLock.locked ||
      demandSignalType === DEMAND_SIGNAL_TYPE.OVERFLOW_HOUSING ||
    ...
    return pack({
      ...
      notes: [
        ...notes,
        realDemandSignal && !realEvent
          ? "true_actionable_demand_signal"
          : "true_actionable",
      ],
...
export function applyDiscoveryHygieneV3(hotelId, opportunities = [], opts = {}) {
  ...
  for (const opp of ordered) {
    ...
    const result = qualifyOpportunityV3(hotelId, opp, {
      ...
      seenKeys,
      marketTokens: opts.marketTokens || null,
    });
    ...
    rows.push({
      ...enrichOpportunityDemandSignal(
        {
          ...opp,
          demandSignalType: result.demandSignalType || opp.demandSignalType,
        },
        { nowDate: opts.nowDate }
      ),
      hygieneV3: result,
