...
import { enrichCandidateRoutingV12 } from "./discovery-routing-v1-2.js";
...
export function mapWebhoundOpportunityRow(row, hotelId, index = 0) {
  ...
  const mapped = {
    ...
    // V1.2: preserve official source lineage (do not drop through Webhound map)
    officialSource:
      row.officialSource ||
      row.sourceUrl ||
      (row.evidenceSources || [])[0]?.url ||
      (row.sources || [])[0]?.url ||
      null,
    officialSourceTitle: row.officialSourceTitle || row.sourceTitle || null,
    evidenceSources: row.evidenceSources || row.sources || null,
    sourceAuthority: row.sourceAuthority || null,
    isOfficialSource: row.isOfficialSource ?? null,
    sourceDomain: row.sourceDomain || null,
    sourceObject: row.sourceObject || null,
    demandSignalType: row.demandSignalType || null,
    organizationLocation: row.organizationLocation || null,
    eventLocation: row.eventLocation || row.programLocation || null,
    locationEvidenceText: row.locationEvidenceText || row.geographyEvidence || null,
    locationEvidenceUrl: row.locationEvidenceUrl || null,
    locationEvidenceType: row.locationEvidenceType || null,
    housingStatus: row.housingStatus || null,
    commercialEvidenceText: row.commercialEvidenceText || null,
    pageSignalClass: row.pageSignalClass || null,
    discoveryLane: row.discoveryLane || row.vertical || null,
  ...
  return enrichCandidateRoutingV12(mapped, {
