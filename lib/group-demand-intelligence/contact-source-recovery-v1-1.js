...
export async function recoverOfficialContactSources(opportunity = {}, opts = {}) {
  ...
  const gdiRouteBase = computeGdiContactRouting(opportunity, domainState);
  // Completeness V1: optional Jev safe-apply / ladder route override (source/stop only).
  const gdiRoute = opts.routeOverride
    ? { ...gdiRouteBase, ...opts.routeOverride, orgFamily: gdiRouteBase.orgFamily, currentTier: gdiRouteBase.currentTier }
    : gdiRouteBase;
