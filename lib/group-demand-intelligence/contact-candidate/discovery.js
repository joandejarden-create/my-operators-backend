...
function collectRawCandidates(opportunity = {}, opts = {}) {
  ...
  if (opportunity.primaryContact) push(opportunity.primaryContact, "pack_primary");
  for (const c of opportunity.contacts || []) push(c, "pack_contacts");
  for (const b of opportunity.backupContacts || []) push(b, "pack_backup");
...
function dedupeRaw(candidates = []) {
  ...
  for (const c of candidates) {
    ...
    // Prefer FACT + source, then email, then phone
    const score = (x) =>
    ...
    if (score(c) > score(existing)) byKey.set(key, { ...existing, ...c });
  }
  return [...byKey.values()];
}
 ...
 */
function selectionRank(candidate, opportunity) {
  let rank = candidate.candidateScore || 0;
  const isOverflow = opportunity.opportunityType === "OVERFLOW_HOUSING";
  // Housing / overflow: housing owner must beat association / tournament executives
  ...
  // Prefer official enrichment primary on near-ties
...
export function selectRankedCandidates(annotatedList = [], opportunity = {}) {
  const usable = annotatedList
    .filter((c) => isProbableNamedOrEntity(c, opportunity))
    .map((c) => ({ c, rank: selectionRank(c, opportunity) }))
    .sort((a, b) => {
