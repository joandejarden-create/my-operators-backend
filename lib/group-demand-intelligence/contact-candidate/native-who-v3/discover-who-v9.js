/**
 * Native WHO V9 — V7 discovery + entity-type person gate + deterministic event ranking.
...
import { ENTITY_TYPE } from "./entity-type-gate-v9.js";
...
const SYSTEM_WHO_V9 = `You extract WHO candidates for hotel group sales from official page SECTIONS.
Return JSON:
{
  "candidates": [
    {
...
function scoreAndConfirm(raw, input, eventFamily, metrics, suppressionLog, aliases = {}) {
  ...
  for (const p of raw) {
    ...
    // V11: entity typing + person confirmation BEFORE role / event ranking
    const typeGate = personTypeGateV11(cand);
    if (typeGate.reject) {
      // Block-level recovery: expand fragment to full person when parse succeeds
      if (typeGate.recoveredPersonName) {
        cand.name = typeGate.recoveredPersonName;
        const retry = personTypeGateV11(cand);
        if (retry.reject) {
    ...
    if (typeGate.normalizedPersonName) {
      cand.name = typeGate.normalizedPersonName;
    }
    cand.entityType = ENTITY_TYPE.PERSON;

    const aggGate = aggregatorWhoGateV11(cand, {
      organization: input.organization,
      opportunityName: input.opportunityName,
      eventSourceUrls: input.eventSourceUrls || [],
    });
    if (aggGate.reject) {
      metrics.suppressed += 1;
      suppressionLog.push({
 ...
 */
export async function discoverWhoV9(input) {
  ...
  // V12 recall may inject gap-directed queries first (precision gates unchanged)
  const directed = Array.isArray(input.recallV12?.directedQueries)
    ? input.recallV12.directedQueries
    : [];
  const mergedQs = [...directed, ...discoveryQs];
  const seenQ = new Set();
  const queryList = [];
  for (const q of mergedQs) {
    const k = String(q || "").toLowerCase().trim();
    if (!k || seenQ.has(k)) continue;
    seenQ.add(k);
    queryList.push(q);
    if (queryList.length >= 16) break;
  }
  ...
  return {
    ...
    passVersion: "native_who_blind_v9",
  };
}
