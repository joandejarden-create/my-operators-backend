/**
 * Native WHO V7 — source-discovery cascade for unseen organizations.
 * Preserves V5/V6 precision gates. Blind-safe.
 *
...
const SYSTEM_WHO_V7 = `You extract WHO candidates for hotel group sales from official page SECTIONS.
Return JSON:
{
  "candidates": [
    {
  ...
  scored.sort((a, b) => {
    const score = (p) => {
      ...
      if (/education|forum|engagement|summit|cups|tournament|institute|gad|nice|show/i.test(oppBlob)) {
        if (/education|engagement|partnership|events|cups|tournament|summit|institute|director|vp|vice president|program/i.test(role)) {
          s += 4;
        }
      }
      // Boost when evidence mentions opportunity core tokens
 ...
 */
export async function discoverWhoV7(input) {
  ...
  const recoverCandidates = pages
    .filter(
      (p) =>
        ...
        /staff|contact|team|leadership|tournament|memorial|directory|about-us|cups|conference|events|board|meet-staff|officers/i.test(
          p.url || ""
        )
    )
    .slice(0, 6);
  ...
  if (sameEventLeaders.length >= 2 && (distributed || sameEventLeaders.length >= 2)) {
    people = uniquePeople(sameEventLeaders, 2).map((p, i) => ({
      ...p,
  ...
  return {
    ...
    passVersion: "native_who_blind_v7",
  };
}
