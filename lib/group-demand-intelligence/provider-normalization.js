...
function normalizeVenueStatus(raw) {
  ...
  if (/tbd|unresolved|open/i.test(s)) return CANONICAL_VENUE_SOURCING.HOTEL_VENUE_TBD;
  if (/overflow/i.test(s)) return CANONICAL_VENUE_SOURCING.PRIMARY_VENUE_SELECTED_OVERFLOW_POSSIBLE;
  if (/fully.?placed|already.?selected/i.test(s)) return CANONICAL_VENUE_SOURCING.FULLY_PLACED;
