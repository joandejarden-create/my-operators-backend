 ...
 * keep working. Explicit REVOKED rows are never resurrected.
 ...
 * Never overwrites an existing REVOKED row.
 */
 ...
 * Requires valid HMAC under the current GDI_SHARE_CAPABILITY_SECRET.
 */
...
export function verifyGdiShareCapability(
  ...
  if (!row) {
    ...
    if (!upsert.ok) {
      return {
        ok: false,
        error: upsert.error === "token_revoked" ? "share_capability_revoked" : "share_capability_unknown",
        code: upsert.error === "token_revoked" ? "SHARE_REVOKED" : "SHARE_UNKNOWN",
      };
    }
    row = upsert.meta;
