...
export function buildHotelFitExplanation({
  ...
  if (
    ...
    (components.geographyFit || 0) >= 75
  ) {
    parts.push("Bethesda is a credible location for this demand");
  } else if ((components.geographyFit || 0) >= 55) {
    parts.push("Bethesda is a possible but stretch alternative within the DMV");
  }
...
export function classifyPriority({
  ...
  if (demandTerritoryFit === DEMAND_TERRITORY_FIT.OUTSIDE_REALISTIC_TERRITORY) {
    return {
      priority: PRIORITY.DISQUALIFIED,
      reason: "Outside realistic demand territory for Bethesda Marriott",
    };
  }
  ...
  if (venueSourcingStatus === VENUE_SOURCING_STATUS.PRIMARY_VENUE_SELECTED_NO_OVERFLOW_EVIDENCE) {
    if (opportunityType === OPPORTUNITY_TYPE.FUTURE_CYCLE || opportunityType === OPPORTUNITY_TYPE.REACTIVATION) {
      return {
        ...
        reason: "Primary venue selected with no overflow evidence — retain as future cycle / reactivation watch only",
      };
    }
    return {
      priority: PRIORITY.DISQUALIFIED,
