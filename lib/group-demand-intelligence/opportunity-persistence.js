...
export async function loadOpportunitiesCanonical(hotelId) {
  ...
  if (mode === "filesystem") {
    return {
      ...fsRepo.loadOpportunities(hotelId),
      persistence: "filesystem",
    };
  }
  try {
    const opportunities = await atStore.listOpportunitiesForHotel(hotelId);
  ...
  return { hotelId, opportunities: [], updatedAt: null, persistence: "airtable" };
}
