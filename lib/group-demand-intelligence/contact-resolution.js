...
export function hasNamedPerson(contact = {}) {
  ...
  // "NIST NICE Program Office / FIU" style — not a person
  if (/\b(office|staff|organizers|program)\b/i.test(name) && !/[A-Z][a-z]+\s+[A-Z][a-z]+/.test(name)) {
    return false;
  }
...
export function classifyTargetRoleMatch(contact = {}, opportunity = {}) {
  ...
  if (/hbc|housing manager|housing partner|group housing|lodging coordinator|passtkey|onpeak|stay-to-play housing/i.test(blob)) {
    return TARGET_ROLE_MATCH.HOUSING_SOURCING_CONTACT;
  }
  if (
    /tournament director|director of meetings|vp.{0,24}meetings|vp.{0,24}events|conference director|meeting planner|hotel sourcing/i.test(
      blob
    )
  ) {
    return isOverflow && /hbc|housing/i.test(blob)
      ? TARGET_ROLE_MATCH.HOUSING_SOURCING_CONTACT
      : TARGET_ROLE_MATCH.DIRECT_DECISION_MAKER;
  }
...
export function isActionableEntityContact(contact = {}, opportunity = {}) {
  ...
  if (roleMatch === TARGET_ROLE_MATCH.HOUSING_SOURCING_CONTACT) {
    return /hbc|housing|passtkey|onpeak|event services/i.test(
