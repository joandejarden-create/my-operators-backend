 ...
 *
 * NEW = NEW_TO_GDI only (firstSeenRunId == current run). Never NEW_TO_HOTEL
...
export function classifyWeeklyDelta(baselineRows, currentOpp, opts = {}) {
  ...
  if (!match.matched) {
    ...
    return {
      ...
      isNewThisWeek: true,
      newnessStatus: NEWNESS_STATUS.NEW_TO_GDI,
      match,
      firstSeenAt: nowIso,
  ...
  if (reactivated) {
    return {
      ...
      isNewThisWeek: false,
      newnessStatus: NEWNESS_STATUS.EXISTING_IN_GDI,
      match,
      firstSeenAt: prior.firstSeenAt,
      firstSeenRunId: prior.firstSeenRunId,
      lastSeenAt: nowIso,
...
export function applyWeeklyDeltaToOpportunities({
  ...
  for (const [id, opp] of byId) {
    ...
    opp.newnessStatus = resolveNewnessStatus({
      weeklyDeltaState: opp.weeklyDeltaState,
      isNewThisWeek: isNew,
    });
