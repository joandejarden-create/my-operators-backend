...
export function classifyEventLocation(opportunity = {}) {
  ...
  const namedVenue =
    ...
    /hotel|marriott|hilton|hyatt|convention|center|arena|stadium|plex|campus|gaylord|ritz/i.test(
      venue
    );
  ...
  if (
    /montgomery county|soccerplex|maryland soccer|prince george|fairfax county|loudoun/i.test(
      `${dest} ${venue} ${t}`
    )
  ) {
    return {
  ...
  if (/washington,\s*dc area|dmv|metro area|national capital|to be announced|location tbd/i.test(dest || t)) {
    return {
      eventLocationStatus: EVENT_LOCATION_STATUS.REGION_ONLY,
      eventLocationStatusLabel: EVENT_LOCATION_STATUS_LABEL.REGION_ONLY,
      eventLocationSummary: dest || "DMV / regional only",
  ...
  if (/bethesda|rockville|montgomery/i.test(org) && !cityMatch) {
    return {
  ...
  return {
    eventLocationStatus: EVENT_LOCATION_STATUS.UNKNOWN,
    eventLocationStatusLabel: EVENT_LOCATION_STATUS_LABEL.UNKNOWN,
    eventLocationSummary: dest || venue || "Event location not confirmed",
...
export function classifyContactQuality(opportunity = {}) {
  ...
  } else if (/director of meetings|vp.{0,20}meetings|meeting planner|event director|tournament director/i.test(role)) {
    contactQuality = named
      ? CONTACT_QUALITY.NAMED_DECISION_MAKER
      : CONTACT_QUALITY.NAMED_EVENT_MEETINGS_CONTACT;
    relationshipToEvent = "Official planning / meetings lead";
    relationshipConfidence = "HIGH";
    contactabilityAdjustment = named ? 10 : 0;
  } else if (/housing|sourcing|hbc|room block|passtkey/i.test(role + name + email)) {
...
export function runHighPriorityQualityGate(opportunity = {}, ctx = {}) {
  const checklist = {
    ...
    locationCredible:
      ...
      ctx.demandTerritoryFit === "BETHESDA_MONTGOMERY_CORE" ||
      ctx.demandTerritoryFit === "NORTH_DC_MEDICAL_CORRIDOR",
    meaningfulDemand: [
      ROOM_DEMAND_STATUS.VERIFIED_ROOM_BLOCK,
      ROOM_DEMAND_STATUS.VERIFIED_HOUSING_PROGRAM,
      ROOM_DEMAND_STATUS.STRONG_ROOM_DEMAND_EVIDENCE,
    ...
    physicalFitOk: (Number(opportunity.hotelFitScore) || Number(ctx.hotelFitScore) || 0) >= 75,
    bookingActionable: [
...
export function enrichQualificationPrecision(opportunity = {}) {
  ...
  return {
    ...
    fitComponents: {
      ...
      historicalFit: opportunity.fitComponents?.historicalFit ?? opportunity.historicalFitScore,
