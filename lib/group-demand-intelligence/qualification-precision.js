...
export function buildHotelOpportunityThesis(opportunity = {}, ctx = {}) {
  ...
  const base =
    ...
    `${ctx.hotelDisplayName || opportunity.hotelContext?.displayName || "This hotel"}'s capacity and market positioning may be relevant.`;

  return `${parts.join("; ")}. Size signal: ${sizeBit}. ${base}`.slice(0, 600);
}

function recommendedActionForType(opportunity, type, hotelCtx = {}) {
  const contact = opportunity.primaryContact?.name || "the identified planner";
  const hotelName =
    hotelCtx.hotelDisplayName ||
    opportunity.hotelContext?.displayName ||
    opportunity.hotelName ||
    "this hotel";
  const advantages =
    hotelCtx.positioningAdvantages ||
    opportunity.hotelContext?.positioningAdvantages ||
    "location, product, and service advantages";
  switch (type) {
    case OPPORTUNITY_TYPE.PRIMARY_PURSUIT:
      return `Verify sourcing status is still open, then contact ${contact} to introduce ${hotelName}, request RFP/site-visit inclusion, and position ${advantages}.`;
    ...
    case OPPORTUNITY_TYPE.REACTIVATION:
      return `Reopen the historical relationship, confirm the current decision maker, and reference prior hotel/brand interaction where evidence supports it — do not treat as cold discovery only.`;
...
export function enrichQualificationPrecision(opportunity = {}) {
  ...
  const hotelContext = {
    ...(opportunity.hotelContext || {}),
  };
  // Prefer hotel demand config for display/positioning (portable — no hotel-name switches)
  try {
    const cfg = loadHotelDemandConfig(opportunity.hotelId);
    if (cfg?.displayName) hotelContext.displayName = cfg.displayName;
    if (cfg?.demandTerritory?.label) hotelContext.marketLabel = cfg.demandTerritory.label;
    if (cfg?.capabilityProfile?.softBrand) hotelContext.brand = cfg.capabilityProfile.softBrand;
    if (cfg?.capabilityProfile?.totalGuestrooms) {
      hotelContext.rooms = cfg.capabilityProfile.totalGuestrooms;
    }
    if (cfg?.capabilityProfile?.totalMeetingSpaceSqFt) {
      hotelContext.meetingSpaceSqFt = cfg.capabilityProfile.totalMeetingSpaceSqFt;
    }
  } catch {
    /* config optional */
  }
  const positioningAdvantages = (() => {
    const market = hotelContext?.marketLabel || hotelContext?.demandTerritoryLabel;
    const brand = hotelContext?.brand || hotelContext?.softBrand;
    const bits = [];
    if (market) bits.push(market);
    if (brand) bits.push(`${brand} brand`);
    if (hotelContext?.meetingSpaceSqFt) bits.push("meeting inventory");
    if (hotelContext?.rooms) bits.push("guestroom capacity");
    return bits.length ? bits.join(" / ") : "location, product, and service advantages";
  })();
  const rawPeak =
    opportunity.estimatedPeakRooms ??
  ...
  const recommendedAction = recommendedActionForType(
    ...
    {
      hotelDisplayName:
        hotelContext?.displayName || opportunity.hotelName || null,
