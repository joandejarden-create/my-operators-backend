...
import { normalizeDateModel } from "./live-commercial-quality-v1.js";
import { rankOrganicByOfficialSource } from "./official-source-ranking.js";
...
export function mapNativeCandidateRow(row, hotelId, index = 0) {
  ...
  const base = {
    ...
    discoveryMode: row.discoveryMode || row.vertical || null,
    ...
    organizationLocation: row.organizationLocation || null,
    eventLocation: row.eventLocation || row.programLocation || row.projectLocation || null,
    locationEvidenceText: row.geographyEvidence || row.locationEvidenceText || null,
    locationEvidenceUrl: row.officialSource || row.locationEvidenceUrl || null,
    locationEvidenceType: row.locationEvidenceType || null,
  };
  const dated = normalizeDateModel(base, {});
  const routed = enrichCandidateRoutingV12(
    {
      ...base,
      eventStartDate: dated.eventStartDate,
      eventEndDate: dated.eventEndDate,
      eventDateGranularity: dated.eventDateGranularity,
      eventDateStatus: dated.eventDateStatus,
      eventYear: dated.eventYear,
    },
    {}
  );
  return enrichCommercialEvidenceV4(routed);
}
...
Candidate fields (omit if unsupported): eventName, organization, eventType, opportunityType, startDate, endDate, eventDateGranularity (EXACT_DATE|DATE_RANGE|MONTH|YEAR|UNKNOWN), organizationLocation, eventLocation, programLocation, projectLocation, venueLocation, hotelDemandLocation, locationEvidenceText, venue, venueStatus, sourcingStatus, housingStatus, attendance, peakRoomEstimate, roomDemandEvidence, housingEvidence, officialSource, supportingSources (array of {url,title}), geographyEvidence, futureCycleEvidence, whoClues ({name,role,organization,email,phone,sourceUrl}), whyRelevantToHotel, whyNow, recommendedAction, evidenceConfidence (0-100), claimKinds, demandTerritoryFit, vertical, segment.
Geography rules:
- Never set eventLocation from organization HQ alone.
- Prefer event/program/project/venue location for hotelDemandLocation.`;
...
export async function runNativeBlindDiscovery({
  ...
  for (const task of tasks) {
    ...
    for (let qi = 0; qi < (task.queries || []).length; qi += 1) {
      ...
      queryPlan.push({
        ...
        note: task.note,
        discoveryMode: task.discoveryMode || task.mode || null,
        knownOfficialDomains: task.knownOfficialDomains || null,
        hl: meta?.hl || null,
        gl: meta?.gl || null,
        lang: meta?.lang || null,
      });
  ...
  for (const item of selectedQueries) {
    try {
      ...
      const screened = filterSerpOrganicPast(serp.organic || [], {
        nowDate: new Date().toISOString().slice(0, 10),
      });
      ledger.skipPastSerp = (ledger.skipPastSerp || 0) + screened.skipPastCount;
      // Official-source ranking before fetch — prefer first-party calendars
      const ranked = rankOrganicByOfficialSource(screened.kept || [], {
        knownOfficialDomains: item.knownOfficialDomains || [],
      });
      const organicKept = ranked.filter((o) => (o.officialRank?.score ?? 0) >= -10);
      const urlsKept = organicKept.map((o) => o.url || o.link).filter(Boolean);
      ledger.serpDigest.push({
        query: item.query,
        vertical: item.vertical,
        discoveryMode: item.discoveryMode || null,
        hl,
        gl,
        urlCount: urls.length,
        urlCountAfterPastScreen: screened.kept.length,
        urlCountAfterOfficialRank: urlsKept.length,
        skipPastCount: screened.skipPastCount,
        topTitles: (organicKept || []).slice(0, 3).map((o) => o.title),
        topOfficialScores: (organicKept || [])
          .slice(0, 3)
          .map((o) => o.officialRank?.score ?? null),
        routingVersion: DISCOVERY_ROUTING_V1_2,
      });
      if (!urlsKept.length) {
        ledger.errors.push({
          stage: "serp_past_screen",
          query: item.query,
          error: "all_organic_skipped_past",
          skipPastCount: screened.skipPastCount,
        });
        continue;
      }
      const pages = await fetchPageSnippets(urlsKept, {
  ...
  for (let i = 0; i < corpora.length; i += batchSize) {
    ...
    try {
      ...
      ledger.extractBatches.push({
        ...
        discoveryMode: batch[0]?.discoveryMode || null,
      });
      const batchMode = batch[0]?.discoveryMode || null;
      for (const c of found) {
        if (!c.vertical && batch[0]?.vertical) c.vertical = batch[0].vertical;
        if (!c.discoveryMode && batchMode) c.discoveryMode = batchMode;
