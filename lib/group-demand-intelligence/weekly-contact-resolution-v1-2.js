...
export async function resolveOpportunityContact(opportunity = {}, opts = {}) {
  ...
  const urls = pickOfficialUrls(opportunity);
  let fetchesUsed = 0;
  let queriesUsed = 0;
  ...
  const discovery = discoverContactCandidates(opportunity, {
  ...
  const applied = applyWhoDiscoveryToOpportunity(opportunity, discovery);
  let next = applied.opportunity || opportunity;
