...
import { isPilotReferenceHotel } from "./pilot-path-policy.js";
import {
...
export function classifyDemandTerritoryFit(opportunity, hotelConfig = null) {
  // Hotel-config keyword maps (portable) take precedence when present.
  const fromConfig = classifyDemandTerritoryFitFromConfig(opportunity, hotelConfig);
  ...
  // Legacy Bethesda / DMV heuristic — only when hotel has no classificationKeywords.
  // Kept for Bethesda pilot regression; new hotels must supply config keywords.
...
export function classifyCompetitors(opportunity, config) {
  ...
  if (!likelyStrCompetitor) {
    // Prefer hotel STR set[0] when demand is core/nearby — never hardcode another hotel's comps.
    if (isCoreTerritory(opportunity.demandTerritoryFit) && strSet[0]?.name) {
      likelyStrCompetitor = strSet[0].name;
    }
  }
  if (!likelyGroupCompetitor && groupSet[0]?.name) {
    likelyGroupCompetitor = groupSet[0].name;
  }

  if (!competitorRationale) {
    if (isCompetitiveOrStretchTerritory(opportunity.demandTerritoryFit)) {
      competitorRationale = `Opportunity-market competitors matter more than local STR comps for this territory. STR context retained for hotel benchmarking: ${likelyStrCompetitor || "see hotel STR set"}. Group alternative: ${likelyGroupCompetitor || "see hotel config"}.`;
    } else {
      competitorRationale = `STR context: ${likelyStrCompetitor || "see hotel STR set"}. Broader group alternative: ${likelyGroupCompetitor || "see hotel config"}. Nearby properties may compete for the same block even when outside the formal STR set.`;
    }
  }
...
function inferPublicSourcingStatus(opportunity) {
  ...
  if (/rfp|sourcing|bid list|hotel selection underway/.test(t)) {
    return {
      sourcingStatus: SOURCING_STATUS.PUBLIC_EVIDENCE_OF_BROADER_RFP_HOUSING,
      alreadySourcedToHotelDisplay:
        "Public evidence of an RFP / selection process — whether this hotel was included is unknown",
    };
  }
...
function sharpenWhyMatters(o, hotelDisplayName = "this hotel") {
  ...
  return `${size} ${o.segment || "group"} opportunity${
    ...
    o.demandTerritoryFitLabel || "market"
  } positioning and hotel capacity make ${hotelDisplayName} a credible pursuable option — validate against the hotel's own lead systems before outbound.`;
}

function sharpenRecommendedAction(o, hotelDisplayName = "the hotel") {
  ...
  if (o.bookingWindowStatus === BOOKING_WINDOW.CONTACT_NOW && o.primaryContact?.email) {
    return `Confirm whether the event has entered hotel sourcing, then contact ${o.primaryContact.name || "the identified planner"} to request inclusion and position ${hotelDisplayName}.`;
  }
  if (
    o.bookingWindowStatus === BOOKING_WINDOW.QUALIFY_NOW ||
    o.bookingWindowStatus === BOOKING_WINDOW.RESEARCH_FURTHER
  ) {
    return `Qualify whether ${hotelDisplayName} is being considered and whether a housing list / RFP has opened; do not assume the lead is new to the hotel.`;
  }
...
export function enrichOpportunityForRadFeedback(opportunity, config) {
  const hotelDisplayName = config?.displayName || "this hotel";
  const territory = classifyDemandTerritoryFit(opportunity, config);
  const territoryLabel =
    resolveTerritoryFitLabel(territory.demandTerritoryFit, config) ||
    opportunity.demandTerritoryFitLabel;
  const withTerritory = {
    ...opportunity,
    ...territory,
    demandTerritoryFitLabel: territoryLabel,
  };
  ...
  const winThesis =
    ...
    (isPilotReferenceHotel(config?.hotelId)
      ? opportunity.bethesdaWinThesis
      : null) ||
    (isCompetitiveOrStretchTerritory(territory.demandTerritoryFit)
      ? opportunity.summaryWhyHotel || opportunity.fitExplanation || null
      : opportunity.hotelOpportunityThesis || null);
  ...
  const merged = {
    ...
    // Legacy alias only for pilot hotel rows — do not stamp Bethesda field on other hotels
    bethesdaWinThesis: isPilotReferenceHotel(config?.hotelId) ? winThesis : undefined,
