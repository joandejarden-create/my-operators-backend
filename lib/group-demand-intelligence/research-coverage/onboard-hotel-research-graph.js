...
export function buildGenericHotelSeedInput(hotelId, opts = {}) {
  const id = clean(hotelId);
  if (!id) {
    const err = new Error("hotelId_required");
    err.code = "hotelId_required";
    throw err;
  }
  if (!isHotelOnboardedForGdi(id) && !opts.allowMissingConfig) {
    const err = new Error("hotel_not_onboarded_for_gdi");
    err.code = "hotel_not_onboarded_for_gdi";
    throw err;
  }

  const config = loadHotelDemandConfig(id) || {};
  const profile = opts.profile || buildHotelGroupDemandProfile(id);
  ...
  return {
    ...
    city: clean(profile.city) || null,
    stateRegion: clean(profile.state) || clean(profile.stateRegion) || null,
    country: clean(profile.country) || "United States",
...
export function proposeHotelOnboardSeed(hotelId, opts = {}) {
  ...
  const { archetype, templates } = selectPortableTemplatesForHotel({
