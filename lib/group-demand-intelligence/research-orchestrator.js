...
export async function runGroupDemandResearch(opts = {}) {
  ...
  const isBethesdaPilot = hotelId === PILOT_HOTEL_ID;
  const demandConfig = loadHotelDemandConfig(hotelId);

  if (!isBethesdaPilot && !isHotelOnboardedForGdi(hotelId)) {
    const err = new Error("hotel_not_onboarded_for_gdi");
    err.code = "hotel_not_onboarded_for_gdi";
    err.message = `Hotel ${hotelId} is not onboarded. Add config/group-demand-intelligence/hotels/${hotelId}.json (or use pilot ${PILOT_HOTEL_ID}).`;
    throw err;
  }
  ...
  if (isBethesdaPilot && discoveryMode === "BETHESDA_LEGACY_SEEDS") {
