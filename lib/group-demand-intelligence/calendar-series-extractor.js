...
function candidateLines(text) {
  ...
  for (const line of lines) {
    const hasYear = /20(?:2[6-9]|3[0-9])/.test(line);
    const hasEventWord =
      /\b(?:meeting|symposium|summit|conference|forum|workshop|tournament|championship|annual|fly-?in|congress)\b/i.test(
        line
      );
    if (hasYear && hasEventWord) out.push(line);
    else if (hasEventWord && ANNUAL_RE.test(line) && /20\d{2}|tbd|coming soon/i.test(line)) {
      out.push(line);
    }
  }
...
export function cyclesToDiscoveryCandidates(cycles = [], { hotelId = null } = {}) {
  return (cycles || [])
    .filter((c) => c.title && (c.year == null || c.year >= 2026))
    .map((c, i) => ({
