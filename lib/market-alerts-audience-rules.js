...
function isSoftMarketNoise(text) {
  // Do not use bare "point(s)" — it matches place names like "Pearns Point".
  return /\b(revpar|occupancy|adr\b|loyalty|rewards?|member(?:ship)?|workforce|staffing|wage|labor|travel trends?|tourism board|visitor arrivals|STR report|pipeline report|outlook|forecast|survey|study finds|ranking|best hotels? to stay|hotel points?|points? (?:redemption|hack|program|maxim))\b/i.test(
    text
  );
}
...
function isBrandedOpeningSoft(text, eventType) {
  ...
  if (/\b(breaks? ground|groundbreaking|under construction|construction start|tops? out)\b/i.test(text)) {
    return false;
  }
...
export function resolveAudienceIntelligence(input = {}) {
  ...
  if (
    isSoftMarketNoise(text) &&
    !["Hotel For Sale", "Acquisition", "Sale", "Distress", ...EARLY_DEV_EVENTS].includes(eventType)
  ) {
    return { owner, brand, operator, treatment: "STANDARD" };
  }
  ...
  if (eventType === "Hotel For Sale") {
    ...
    owner.decisionStage =
      eventType === "Construction Start" || eventType === "Construction Progress"
        ? "Forming"
        : eventType === "Planning Approval"
          ? "Early"
          : "Early";
  } else if (eventType === "Pre-Opening Leadership") {
    owner.worthReviewing = true;
    owner.signalType = "New Competitive Supply";
    owner.decisionStage = "Forming";
  } else if (eventType === "Commercialization") {
    owner.worthReviewing = true;
    owner.signalType = "New Competitive Supply";
    owner.decisionStage = "Likely Decided";
  ...
  if (["Brand Signing", "Reflag", "Conversion"].includes(eventType)) {
    ...
    brand.worthReviewing = true;
    if (hasConfirmedPropertyBrand(text, entities)) {
      brand.signalType = "Competitive Brand Move";
      brand.decisionStage = "Likely Decided";
    } else {
      brand.signalType = "Potential Development Opportunity";
    ...
    brand.worthReviewing = true;
    brand.signalType = hasConfirmedPropertyBrand(text, entities)
  ...
  if (eventType === "Management Agreement") {
    ...
    operator.worthReviewing = true;
    if (hasEvidentOperatorStructure(text, entities)) {
      operator.signalType = "Competitive Operator Move";
      operator.decisionStage = "Likely Decided";
    } else {
    ...
    operator.worthReviewing = true;
    operator.signalType = hasEvidentOperatorStructure(text, entities)
