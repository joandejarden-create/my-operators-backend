...
/**
 * Deterministic event id for idempotent customer-reported events.
 * Same hotel + decision + kind + subtype + date + evidence key → same id.
 */
...
/** Append-only event write. Never mutates prior lines. Idempotent by event id. */
export function appendEvent(hotelId, decisionId, event) {
  const id = eventIdentity(event);
  if (id) {
    const existing = loadEvents(hotelId, decisionId).find(
      (e) => eventIdentity(e) === id
    );
