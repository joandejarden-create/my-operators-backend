...
async function hotelMetrics(hotelId, productModule) {
  ...
  for (const s of summaries) {
    ...
    funnel.opportunitiesSurfaced += 1;
    ...
    if (
      stage === GDI_COMMERCIAL_FUNNEL_STAGE.CONTACTED ||
      stage === GDI_COMMERCIAL_FUNNEL_STAGE.ADDED_TO_SOURCING ||
      stage === GDI_COMMERCIAL_FUNNEL_STAGE.RFP_RECEIVED ||
      stage === GDI_COMMERCIAL_FUNNEL_STAGE.PROPOSAL_SUBMITTED ||
      stage === GDI_COMMERCIAL_FUNNEL_STAGE.SHORTLISTED ||
      stage === GDI_COMMERCIAL_FUNNEL_STAGE.WON
    ) {
      funnel.contacted += 1;
    }
    if (
    ...
    if (
      ...
      funnel.proposalSubmitted += 1;
    }
    if (stage === GDI_COMMERCIAL_FUNNEL_STAGE.WON) funnel.won += 1;
    if (stage === GDI_COMMERCIAL_FUNNEL_STAGE.LOST) funnel.lost += 1;
  ...
  return {
    ...
    note: "Foundation metrics only — counts from canonical events; not statistical significance.",
  };
}
