...
const POST_DECISION_EVENTS = new Set([
  "Hotel For Sale",
  "Refinancing",
  "Major Renovation",
  "Commercialization",
...
const APPROVED_RE =
  /\b(approved|approval|permission granted|entitlement(?:s)? (?:approved|secured)|receives? approval|aprueban|aprobaci[oó]n|aprovação)\b/i;
...
export function inferSignalTiming(input = {}) {
  ...
  if (eventType === "Commercialization") return "Post-Decision";
  if (eventType === "Pre-Opening Leadership") return "Decision Forming";
  if (eventType === "Construction Progress") return "Decision Forming";

  if (OPENING_RE.test(text) && eventType !== "Construction Start" && eventType !== "Construction Progress") {
