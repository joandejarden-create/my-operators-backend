...
export const HDV_DEFINITIONS = Object.freeze({
  ...
  TOP_DECISION_TERRITORY:
    "Intent Territory with the highest presence rate (portfolio coverage or brand presence within that intent). Ties broken by Questions Won, then Recommendation Share, then name.",
  REGIONAL_LEADER:
...
function topIntentForBrand(observations, brandId) {
  ...
  return scored[0]?.presence >= 0 ? scored[0].intent : null;
}
...
function buildAiVsDealalityRows(evidenceRows, brandFactsById, entitledIds, limit = 6) {
  ...
  for (const ev of evidenceRows || []) {
    ...
    rows.push({
      question: ev.promptText || ev.promptId,
