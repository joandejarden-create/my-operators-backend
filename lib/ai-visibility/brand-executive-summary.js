...
import { redactCustomerOverviewPayload } from "./customer-prompt-disclosure.js";
...
import { buildLibraryPromptOriginSummary } from "./prompt-provenance.js";
...
export async function getBrandExecutiveSummaryPayload(args = {}) {
  ...
  return redactCustomerOverviewPayload(
    ...
    narrativeSummary: buildNarrativeSummaryForExecutive(executiveFindings),
    ...
    portfolioCompositeScore: null,
    promptOriginSummary: buildLibraryPromptOriginSummary(effectiveGeo),
    opportunityQueue: {
      status: AVAILABILITY.FUTURE_READY,
      items: [],
...
function buildNarrativeSummaryForExecutive(executiveFindings) {
  if (!executiveFindings || !Array.isArray(executiveFindings.findings)) {
    return { productionNarratives: [], sourceRelationships: [] };
  }
  const narrativeFindings = executiveFindings.findings.filter(
    (f) => f.findingType === "NARRATIVE_PATTERN" || f.findingType === "SOURCE_PATTERN"
  );
  return {
    productionNarratives: narrativeFindings.map((f) => ({
      brandId: f.brandId || null,
      brandName: f.brandName || null,
      narrativeFamily: f.narrativeFamily || null,
      headline: f.headline || null,
      evidence: f.evidenceSummary || null,
      disposition: f.actionDisposition || f.reviewDisposition || null,
      providerCount: f.providerCount || null,
