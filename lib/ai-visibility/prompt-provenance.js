...
export const OBSERVED_PROMPT_MIX_MIN_THEMES = 8;
...
export const CLIENT_PROMPT_ORIGIN_COPY = Object.freeze({
  ...
  lenses:
    "We use both observed demand and expert scenario intelligence. Observed demand reflects externally measured query themes. Scenario intelligence tests commercially important owner and developer decisions that may not appear as literal high-volume searches.",
  complementary: "Observed demand and scenario intelligence are complementary.",
  methodology:
  ...
  demandTierHelper:
    "Demand tier is relative to comparable observed queries within the source country and language cohort.",
  observedBadge: "Observed",
  derivedBadge: "Derived",
  scenarioBadge: "Scenario",
  derivedFromPrefix: "Derived from observed demand",
  expertScenario: "Expert scenario",
  promptCoverageLabel: "Prompt Intelligence",
  promptCoverageCompact: "Observed demand · Expert scenarios",
});

export const MONITORING_COST_RATES_USD = Object.freeze({
  ENV_DEFAULT: 0.25,
  HISTORIC_EFFECTIVE: 0.677,
  CONSERVATIVE: 1.33,
  historicSource: "WAVE1_COST_EVIDENCE OpenAI Phase 3A.9 expected/high per call",
});
...
export function toClientPromptOrigin(record = {}) {
  ...
  if (origin === "OBSERVED") {
    ...
    const bits = [
      record.originSourceName || record.originSourceType || null,
      record.demandGeography || null,
      record.demandTier && record.demandTier !== "UNKNOWN"
        ? `Demand tier ${record.demandTier}`
        : null,
      record.dateObserved || null,
    ].filter(Boolean);
    originDetail = bits.join(" · ") || CLIENT_PROMPT_ORIGIN_COPY.demandTierHelper;
  } else if (origin === "DERIVED") {
    originBadge = CLIENT_PROMPT_ORIGIN_COPY.derivedBadge;
    originDetail = CLIENT_PROMPT_ORIGIN_COPY.derivedFromPrefix;
  }
  return {
    promptOrigin: origin,
...
export function shouldShowExecutivePromptMix(summary = {}, options = {}) {
  const seed = options.seed || loadObservedDemandSeed();
  if (seed?.seedStatus === "OBSERVED_DEMAND_SEED_V1_VALIDATED") return true;
  const themes = Array.isArray(seed?.includedThemes) ? seed.includedThemes.length : 0;
  if (themes >= OBSERVED_PROMPT_MIX_MIN_THEMES) return true;
...
export function buildPromptOriginSummary(prompts = [], options = {}) {
  ...
  const summary = {
    ...
    ...counts,
    showPromptMix: false,
    promptCoverageLabel: CLIENT_PROMPT_ORIGIN_COPY.promptCoverageLabel,
    promptCoverageCompact: CLIENT_PROMPT_ORIGIN_COPY.promptCoverageCompact,
...
export function estimateProvenanceMonitoringCost(input = {}) {
  ...
  const historic = Number(input.historicUsdPerCall) || MONITORING_COST_RATES_USD.HISTORIC_EFFECTIVE;
  const conservative =
    Number(input.conservativeUsdPerCall) || MONITORING_COST_RATES_USD.CONSERVATIVE;
  ...
  return {
    ...
    HISTORIC_EFFECTIVE_USD_PER_CALL: historic,
    CONSERVATIVE_USD_PER_CALL: conservative,
    CALLS_PER_FULL_RUN: callsPerRun,
    COST_PER_FULL_RUN: costPerRun,
    HISTORIC_EFFECTIVE_COST_PER_RUN: Number((callsPerRun * historic).toFixed(4)),
    CONSERVATIVE_COST_PER_RUN: Number((callsPerRun * conservative).toFixed(4)),
...
export const EXISTING_OBSERVED_SIGNAL_SOURCES = Object.freeze([
  ...
  },
  {
    ...
    LIVE_MONITORING: "NO",
    BLOCKER: null,
    NOTE: "phase spent 0.474. V1 seed activated in file store. Remaining DataForSEO phase budget unused. Account funding is not a project budget. New observed prompts are monitoringEligible=false.",
  },
  {
