...
export const EXECUTIVE_FINDING_ENGINE_VERSION =
  "ai_visibility_executive_finding_engine_p0e_v1_2_theme_diversity";
...
const FINDING_TYPE_ORDER = Object.freeze([
  ...
  FINDING_TYPES.COMPETITIVE_POSITION,
  FINDING_TYPES.LARGEST_COMPETITIVE_STRENGTH,
...
/** One card per theme on Executive Summary — never two "Largest Competitive Gap" tiles. */
const MAX_GAP_TILES_PER_SUMMARY = 1;
...
function formatProvider(id) {
  ...
  if (key === "openai" || key === "chatgpt" || key === "gpt" || key === "gpt-4" || key === "gpt-4o") {
    return "ChatGPT";
  }
...
function buildMovementTile(presenceChange, geographyKey) {
  ...
  return {
    ...
    context: geographyKey,
    brandName: brand,
    presenceDeltaPp: Math.abs(delta),
    presenceDirection: direction.toLowerCase(),
    geographyKey,
...
function normalizeThemeTitle(title) {
  return String(title || "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, " ");
...
/**
 * Keep 3–5 tiles with unique finding types and unique category titles.
 * Ranked leftovers fill remaining slots instead of repeating a theme.
 */
function selectDiverseFindings(ranked = [], max = MAX_FINDINGS) {
  const selected = [];
  const seenTypes = new Set();
  const seenTitles = new Set();
  for (const f of ranked) {
    if (!f) continue;
    const type = String(f.findingType || "").trim();
    const title = normalizeThemeTitle(f.title);
...
function buildCertifiedScenarioBenchmarkTile(ctx, brandNamesById = {}, geographyKey = "CALA") {
  if (!ctx || ctx.indexValue == null) return null;
  const brand = brandNamesById[ctx.brandId] || ctx.brandId;
  if (!brand) return null;
  const scenario = ctx.scenarioName || "this owner-decision context";
  const gap = Math.round(Math.abs(ctx.relativeGapPct || 0));
  let headline;
  if (ctx.indexValue > 100) {
    headline = `${brand} is broadly at competitive parity in ${scenario.toLowerCase()} scenarios, with representation across the comparable owner-decision cohort.`;
    if (ctx.indexValue >= 110) {
      headline = `${brand} is represented above the competitive benchmark in ${scenario.toLowerCase()} scenarios across the comparable owner-decision cohort.`;
    }
  } else if (ctx.indexValue < 100) {
    headline = `${brand} is represented in ${scenario.toLowerCase()} scenarios but remains below the competitive benchmark for this owner-decision context.`;
  } else {
    headline = `${brand} is at competitive parity in ${scenario.toLowerCase()} scenarios across the comparable owner-decision cohort.`;
  }
  const evidenceSummary = `AI Presence Index ${ctx.indexValue} · ${ctx.positionCopy || `${gap}% vs benchmark`}`;
  return {
    findingType: FINDING_TYPES.COMPETITIVE_POSITION,
    title: "Competitive Position",
    headline,
    context: `${scenario} · ${geographyKey}`,
    evidenceSummary,
    reviewAction: "Monitor scenario-level benchmark movement in the next comparable period.",
    actionCategory: ACTION_CATEGORIES.COMPETITIVE_COMPARISON,
    ...
    peerBrandIds: [],
    leadPeerName: null,
    peerBrandNames: ctx.corePeerNames || [],
    classification: null,
    persistence: null,
    providerCount: null,
    observationCount: null,
    indexValue: ctx.indexValue,
    relativeGapPct: ctx.relativeGapPct,
    narrowCertification: ctx.narrow === true,
    dedupeKey: `competitive_position:${ctx.brandId}:${ctx.scenarioId}`,
    priorityRank: ctx.indexValue < 100 ? 88 : ctx.indexValue <= 105 ? 72 : 68,
    WHY_INCLUDED: "Customer-certified scenario AI Presence Index (All Providers).",
    executiveEligible: true,
  };
}

function buildCompetitiveStrengthTile(topByPresence, geographyKey, opts = {}) {
  if (!topByPresence) return null;
  const presence = topByPresence.presence;
  if (typeof presence !== "number" || !Number.isFinite(presence) || presence <= 0) {
    return null;
  }
  const brandId = topByPresence.brandId || null;
  if (opts.gapSubjectBrandId && brandId && opts.gapSubjectBrandId === brandId) {
    return null;
  }
  const brand = topByPresence.brandName || brandId;
  if (!brand) return null;
  const display = topByPresence.display || pct(presence);
  if (!display) return null;
  const geo = geographyKey || topByPresence.geography || "this region";

  return {
    findingType: FINDING_TYPES.LARGEST_COMPETITIVE_STRENGTH,
    title: "Largest Competitive Strength",
    headline: `${brand} leads this portfolio in AI Presence in ${geo}.`,
    context: geo,
    evidenceSummary: `Highest portfolio Presence · ${geo} · ${display}`,
    reviewAction: "Confirm the lead holds in the next comparable monitoring period.",
    actionCategory: ACTION_CATEGORIES.COMPETITIVE_COMPARISON,
    scenarioId: null,
    scenarioName: null,
    brandName: brand,
    ...
    associationAttributeId: null,
    gapId: null,
    dedupeKey: `strength:${brandId || brand}:${geo}`,
    priorityRank: 92,
    WHY_INCLUDED: "Highest entitled-brand AI Presence in the selected geography.",
    executiveEligible: true,
  };
}
...
export async function buildExecutiveFindings(input = {}) {
  const {
    ...
    scenarioBenchmarkContext = null,
    questionsMissingCorePeerSummary = null,
  } = input;
  ...
  for (const gap of executiveEligibleGaps) {
    ...
    if (gapTiles.length >= MAX_GAP_TILES_PER_SUMMARY) break;
    const subjectKey = gap.subjectBrandId || "unknown";
    if (scope === "portfolio" && seenGapSubjects.has(subjectKey)) continue;
  ...
  const strengthTile = buildCompetitiveStrengthTile(topByPresence, geographyKey, {
    gapSubjectBrandId: gapTiles[0]?.brandId || null,
  });
  if (strengthTile) candidates.push(strengthTile);

  const benchmarkTile = buildCertifiedScenarioBenchmarkTile(
    scenarioBenchmarkContext,
    brandNamesById,
    geographyKey
  );
  if (benchmarkTile) candidates.push(benchmarkTile);
  ...
  const selectedFindings = selectDiverseFindings(ranked, MAX_FINDINGS).map((f) =>
  ...
  const summary = {
    ...
    LARGEST_COMPETITIVE_STRENGTH: findings.find(
  ...
  return {
    ...
    themeDiversity: {
      UNIQUE_FINDING_TYPES:
        new Set(findings.map((f) => f.findingType).filter(Boolean)).size === findings.length,
      UNIQUE_CATEGORY_TITLES:
        new Set(findings.map((f) => normalizeThemeTitle(f.title)).filter(Boolean)).size ===
        findings.length,
      MAX_GAP_TILES: MAX_GAP_TILES_PER_SUMMARY,
      GAP_TILE_COUNT: findings.filter(
        (f) => f.findingType === FINDING_TYPES.LARGEST_COMPETITIVE_GAP
      ).length,
    },
    safety: {
...
export function executiveFindingsToInsightBoxes(executiveFindings) {
  ...
  const boxes = findings.map((f) => ({
    ...
    executiveFindingText: f.governedExecutiveFindingText || f.governedBody || f.headline,
    finding: f.governedExecutiveFindingText || f.governedBody || f.headline,
    detailBody: f.governedDetailBody || f.governedExecutiveFindingText || f.governedBody || f.headline,
