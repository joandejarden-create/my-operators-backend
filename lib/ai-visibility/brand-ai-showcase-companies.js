...
export const SHOWCASE_COMPANIES_VERSION = "1.2";
...
/** All showcase portfolio brand IDs (19) — monitoring universe, not competitive peer v2. */
export const SHOWCASE_MONITORING_BRAND_COUNT = 19;

export function listShowcaseMonitoringBrandIds(config) {
  const cfg = config || loadShowcaseCompaniesConfig();
  const ids = new Set();
...
export function isShowcaseMonitoringBrand(brandId, config) {
  const id = String(brandId || "").trim();
  if (!id) return false;
  return listShowcaseMonitoringBrandIds(config).includes(id);
}

/**
 * Brand display record for share-capability resolve responses.
 * Returns { brandId, brandName, companyKey } or null when brandId is unknown.
 */
export function getShowcaseBrandDisplay(brandId, config) {
  const cfg = config || loadShowcaseCompaniesConfig();
  const id = String(brandId || "").trim();
  if (!id) return null;
  for (const company of cfg.companies || []) {
    const brand = (company.brands || []).find((b) => b.brandId === id);
    if (brand) {
      return {
