...
} from "./competitive-moat/scenario-benchmark-customer-service.js";
import { buildUnifiedOwnerIntentCoverage } from "./competitive-moat/unified-owner-intent-coverage.js";
...
import { preferBaiCustomerCanonicalTrendPoints } from "./brand-longitudinal/resolve-bai-customer-longitudinal-history-v1.js";
import {
  BAI_CUSTOMER_PUBLISHED_PERIOD_ID,
  BAI_PERIOD_2_CANDIDATE_ID,
  BAI_PERIOD_2_CUSTOMER_CURRENT_DATE,
  BAI_PROVIDER_NONCOMPARABILITY_CUSTOMER_DISCLOSURE,
} from "./brand-longitudinal/resolve-bai-prior-comparable-period-v1.js";
import { buildBaiWave4ProviderMovementV1 } from "./brand-longitudinal/bai-wave4-longitudinal-presentation-v1.js";
import {
...
import { enrichRowWithObservationSummary } from "./stability-client.js";
import {
  redactCustomerEvidence,
  redactCustomerOverviewPayload,
  redactCustomerQuestionsPayload,
} from "./customer-prompt-disclosure.js";
...
export const BRAND_READ_SERVICE_VERSION = "ai_visibility_brand_read_v2_coverage_unification";

/**
 * When Period 2 is customer-published, bind Provider Presence / Coverage Diagnostics
 * "Best Provider" to P2 period observations (not federated Aug 14 baseline).
 * Provider Prior/Δ remain noncomparable for this monitoring pair.
 */
function overlayPublishedPeriodProviderPresencePanel(panel, brandId) {
  if (
    !panel ||
  ...
  if (!movement?.ok || !Array.isArray(movement.rows) || !movement.rows.length) {
    return {
      ...panel,
      CURRENT_PERIOD_BINDING: "STALE_OR_UNAVAILABLE",
      BAI_COVERAGE_DIAGNOSTICS_CURRENT_PERIOD_BINDING: false,
    };
  }
  const byProvider = new Map(
    movement.rows.map((r) => [String(r.provider || "").toLowerCase(), r])
  );
  const rows = (panel.rows || []).map((row) => {
    const key = String(row.PROVIDER || "").toLowerCase();
    const m = byProvider.get(key);
    if (!m || typeof m.currentPresence !== "number" || !Number.isFinite(m.currentPresence)) {
      return row;
    }
    const rate = m.currentPresence / 100;
    return {
      ...row,
      PRESENCE_RATE: rate,
      PRESENCE_RATE_DISPLAY: `${Number(m.currentPresence).toFixed(1)}%`,
      MONITORING_STATUS: "MONITORED",
      MONITORING_STATUS_DISPLAY: "Monitored",
      CURRENT_PERIOD: BAI_PERIOD_2_CUSTOMER_CURRENT_DATE,
      PRIOR_COMPARABLE_PERIOD: null,
      DELTA_PP: null,
      DELTA_STATUS: "NOT_COMPARABLE",
      DELTA_NOTE: BAI_PROVIDER_NONCOMPARABILITY_CUSTOMER_DISCLOSURE,
      PUBLISHED_PERIOD_BOUND: true,
    };
  });
  return {
    ...panel,
    rows,
    CURRENT_PERIOD: BAI_PERIOD_2_CUSTOMER_CURRENT_DATE,
    CURRENT_PERIOD_BINDING: "P2_PUBLISHED",
    BAI_COVERAGE_DIAGNOSTICS_CURRENT_PERIOD_BINDING: true,
 ...
 * Showcase portfolio brands (monitoring universe) are in-scope when a batch exists.
 */
export function entityInMonitoredUniverse(
  summary,
  entityId,
  peerSetEntityIds = [],
  monitoringUniverseIds = []
) {
  if (!entityId || !summary) return false;
  const monitoring = Array.isArray(monitoringUniverseIds) ? monitoringUniverseIds : [];
  if (monitoring.includes(entityId)) return true;
...
export async function resolveBrandGeographyMonitoringState(args = {}) {
  ...
  const inScope = entityInMonitoredUniverse(
    latestSummary,
    brandId,
    peerSetEntityIds,
...
export async function getBrandOverviewPayload(args = {}) {
  ...
  if (latestSummary && monitored) {
    try {
      const loaded = await loadObservationsFromBatchSummary(store, latestSummary, {
        language: language || undefined,
      });
      let scenarioObservations = loaded.observations || [];
      if (!allProvidersMode && dataProvider) {
        scenarioObservations = scenarioObservations.filter((o) =>
          providersMatch(o.provider, dataProvider)
        );
      }
      const unifiedOwnerIntentCoverage = buildUnifiedOwnerIntentCoverage(brandId, {
        observations: scenarioObservations,
        allProvidersMode,
        provider,
        brandNamesById,
        geography: effectiveGeo.commercialRegion || effectiveGeo.key || "CALA",
        language: language || "en",
      });
      if (unifiedOwnerIntentCoverage?.rows?.length) {
        decisionPatterns = decisionPatterns || {};
        decisionPatterns.unifiedOwnerIntentCoverage = unifiedOwnerIntentCoverage;
        decisionPatterns.ownerIntentCoverage = {
          availability: unifiedOwnerIntentCoverage.availability,
          message: null,
          rows: unifiedOwnerIntentCoverage.rows,
          unified: true,
        };
        const strongestUnified = [...unifiedOwnerIntentCoverage.rows]
          .filter((r) => typeof r.subjectPresence === "number")
          .sort((a, b) => b.subjectPresence - a.subjectPresence)[0];
        if (strongestUnified) {
          decisionPatterns.topDecisionTerritory = {
            intentTerritory: strongestUnified.intentLabel,
            intentLabel: strongestUnified.intentLabel,
            scenarioId: strongestUnified.scenarioId,
            display: strongestUnified.subjectPresenceDisplay,
            availability: AVAILABILITY.OBSERVED,
          };
        }
      }
      ...
      if (typeof console !== "undefined" && console.warn) {
        console.warn("[brand-read] unified owner-intent coverage skipped:", err.message);
      }
    }
  }
  ...
  const providerPresencePanelRaw = buildProviderPresencePanel({
    brandId,
    providers: providerRows,
  ...
  const providerPresencePanel = overlayPublishedPeriodProviderPresencePanel(
    providerPresencePanelRaw,
    brandId
  );
  ...
  if (allProvidersMode) {
    ...
    kpis.competitivePosition = gateClientMetric("competitivePosition", {
      ...
      display: "Use a specific provider for peer rank",
      delta: null,
      helper: "Peer rank is provider-specific. Select ChatGPT, Gemini, Perplexity, or Claude.",
  ...
  if (intentRows.length) {
    const unifiedMode = decisionPatterns?.ownerIntentCoverage?.unified === true;
    const weakest = unifiedMode
      ? [...intentRows]
          .filter((r) => typeof r.subjectPresence === "number")
          .sort((a, b) => a.subjectPresence - b.subjectPresence)[0]
      : [...intentRows]
    ...
    if (weakest) {
      if (unifiedMode) {
        topMissingPromptFamily = {
          promptFamily: weakest.intentLabel || null,
          intentTerritory: weakest.intentLabel || null,
          intentLabel: weakest.intentLabel || null,
          scenarioId: weakest.scenarioId || null,
          display: weakest.subjectPresenceDisplay || null,
          QUESTIONS_MISSING: weakest.missingCount ?? null,
          MONITORED_QUESTIONS: weakest.comparableObservationCount ?? null,
          missingN: weakest.missingCount ?? null,
          presentN: weakest.withPresenceCount ?? null,
        };
      } else {
        const monitored =
  ...
  try {
    ...
    brandExecutiveFindings = await buildExecutiveFindings({
      ...
      presenceChange: allProvidersMode ? null : presenceChange,
      peerSetId: PEER_SET_ID_V2,
      scenarioBenchmarkContext: allProvidersMode
        ? getCertifiedExecutiveBenchmarkContext(brandId)
        : null,
    });
  } catch (err) {
    if (typeof console !== "undefined" && console.warn) {
  ...
  return redactCustomerOverviewPayload(
    {
    ok: true,
    allowed: true,
    accessDepth: access.accessDepth,
...
export async function getBrandTrendPayload(args = {}) {
  ...
  // Published Period 2: prefer certified Aug14→Sep3 customer longitudinal history
  // (same source as Wave 3/4 Prior Run). Federated batch summaries alone are one-period.
  const canonicalTrend = preferBaiCustomerCanonicalTrendPoints({
    brandId,
    legacyPoints: points,
    geography: effectiveGeo.key,
    language: trendLanguage,
  });
  let usedCanonicalCustomerHistory = false;
  if (canonicalTrend.usedCanonical) {
    points = canonicalTrend.points;
    usedCanonicalCustomerHistory = true;
  }
  ...
  if (points.length === 1) {
    ...
    for (let i = 1; i < points.length; i++) {
      const cmp = compareTrendObservations(
        {
          ...points[i - 1],
          provider: usedCanonicalCustomerHistory ? "all" : provider,
          geographyKey: effectiveGeo.key,
          language: trendLanguage,
          metric: "aiPresenceRate",
        },
    ...
    if (comparablePairs === 0) {
      ...
      message = pairwiseBlocked
        ...
        : usedCanonicalCustomerHistory
          ? "Customer longitudinal history: prior monitoring run → current published run."
          : null;
    }
  }
  ...
  return {
    ...
    renderState,
    usedCanonicalCustomerHistory,
    customerLongitudinalHistorySource: usedCanonicalCustomerHistory
      ? canonicalTrend.canonical?.source || null
      : null,
    comparablePeriodCount: usedCanonicalCustomerHistory
      ? 2
      : pointCountComparable,
    availability:
      points.length === 0
...
export async function getBrandQuestionsPayload(args = {}) {
  ...
  allRows = allRows.map((q) => enrichRowWithObservationSummary(enrichRowWithPromptOriginFromLibrary(q)));
  ...
  try {
    if (latest) {
      ...
      qmObservations = loaded.observations || [];
    }
  } catch (err) {
    if (typeof console !== "undefined" && console.warn) {
      console.warn("[brand-read] QM competitive context observations skipped:", err.message);
    }
  }

  const enrichedWatchlistRows =
    allProvidersMode && qmObservations.length
      ? enrichQuestionsMissingWithCompetitiveContext(originWatchlistRows, {
          subjectBrandId: brandId,
          observations: qmObservations,
          brandNamesById: {
            ...peerSetBrandNamesById(PEER_SET_ID_V2),
            ...(brandNamesById || {}),
          },
  ...
  return redactCustomerQuestionsPayload(
    {
    ok: true,
    allowed: true,
    accessDepth: access.accessDepth,
...
export async function getBrandCompetitorsPayload(args = {}) {
  ...
  let scenarioObservations = [];
  try {
    if (latest) {
      const loaded = await loadObservationsFromBatchSummary(store, latest, {
        language: language || undefined,
      });
      scenarioObservations = loaded.observations || [];
    }
  } catch (err) {
    if (typeof console !== "undefined" && console.warn) {
      console.warn("[brand-read] owner-intent observations load failed:", err.message);
    }
  }

  const ownerIntentBenchmarkBlock = buildOwnerIntentBenchmarksForBrand(brandId, {
    allProvidersMode,
    provider,
    observations: scenarioObservations,
  });
  ...
  return {
    ...
    ownerIntentBenchmarks: ownerIntentBenchmarkBlock.ownerIntentBenchmarks || [],
    SCENARIO_BENCHMARK_UI: ownerIntentBenchmarkBlock.SCENARIO_BENCHMARK_UI || "OFF",
    CUSTOMER_INDEX_RENDERING: ownerIntentBenchmarkBlock.CUSTOMER_INDEX_RENDERING || "OFF",
    OWNER_INTENT_VISIBLE: ownerIntentBenchmarkBlock.OWNER_INTENT_VISIBLE === true,
    ALL_PROVIDERS_DERIVED: allProvidersMode,
    brandReadServiceVersion: BRAND_READ_SERVICE_VERSION,
  };
}
...
export async function getBrandEvidencePayload(args = {}) {
  ...
  const safeRecords = records.map((ev) =>
    redactCustomerEvidence(sanitizeEvidenceForClient(ev, brandId), { viewerContext: viewer })
  );
...
function sanitizeEvidenceForClient(ev, brandId) {
  ...
  return {
    ...
    intentTerritory: ev.intentTerritory || ev.payload?.intentTerritory || null,
    ...
    language: ev.language || ev.payload?.language || null,
