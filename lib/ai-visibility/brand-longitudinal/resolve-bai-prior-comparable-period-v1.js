...
export function dryRunBaiP2PromotionMechanicsV1() {
  ...
  return {
    ...
    /** Gates that must PASS before any future pointer flip / Railway publish. */
    requiredGatesBeforePromotionOrDeploy: [
      "BAI_P2_SINGLE_CUSTOMER_PRIOR_IDENTITY",
      "BAI_P2_NO_MIXED_PRIOR_DATE_IDENTITY",
      "BAI_PROMOTED_PERIOD_STORE_DEPLOYMENT_COMPLETENESS",
    ],
    storeDeploymentCompleteness: {
      gate: "BAI_PROMOTED_PERIOD_STORE_DEPLOYMENT_COMPLETENESS",
      requiredBeforePromotion: true,
      run: "npm run test:bai-promoted-period-store-deployment-completeness-v1",
      note: "Clean-deploy simulation of promoted period store; blocks pointer-without-artifacts failure class.",
    },
    wouldChange: {
      publishedPointer: {
