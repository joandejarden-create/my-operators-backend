...
import {
  ...
  BAI_VIEW_MODE,
  BAI_COHORT_CHANGE_CUSTOMER_DISCLOSURE,
  BAI_PROVIDER_NONCOMPARABILITY_CUSTOMER_DISCLOSURE,
  BAI_INTENT_NONCOMPARABILITY_CUSTOMER_DISCLOSURE,
...
export const BAI_PROVIDER_NONCOMPARABILITY_CUSTOMER_DISCLOSURE_GATE =
  "BAI_PROVIDER_NONCOMPARABILITY_CUSTOMER_DISCLOSURE";
export const BAI_COHORT_CHANGE_CUSTOMER_DISCLOSURE_GATE =
  "BAI_COHORT_CHANGE_CUSTOMER_DISCLOSURE";
export const BAI_COHORT_CHANGE_RANK_NARRATIVE_INTEGRITY =
  "BAI_COHORT_CHANGE_RANK_NARRATIVE_INTEGRITY";
export const BAI_ZERO_GAIN_NOT_CALLED_STRONGEST_MOVER =
...
export function buildBaiWave4ProviderMovementV1(opts = {}) {
  ...
  const rows = providers.map((provider) => {
    ...
    return {
      ...
      priorPresence: null,
      deltaPp: null,
      // Never use "—" (could read as zero / no change). Omit prior/Δ in customer UI.
      deltaDisplay: null,
  ...
  return {
    ...
    customerDisclosure: BAI_PROVIDER_NONCOMPARABILITY_CUSTOMER_DISCLOSURE,
    note: BAI_PROVIDER_NONCOMPARABILITY_CUSTOMER_DISCLOSURE,
    rows,
...
function buildCompetitiveNarrative(parentSummary, brands, periodResolve = null) {
  const parts = [];
  const cohortChanged =
    periodResolve?.cohortChanged === true ||
    periodResolve?.cohortChangeState === "YES";
  ...
  // BAI_COHORT_CHANGE_RANK_NARRATIVE_INTEGRITY — never claim "same peer set"
  if (up[0] && down[0]) {
    parts.push(
      cohortChanged
        ? `${up[0].brandName} improved ${Math.abs(up[0].rankDelta)} rank position${
            Math.abs(up[0].rankDelta) === 1 ? "" : "s"
          } while ${down[0].brandName} fell ${down[0].rankDelta} (directional vs changed peer universe).`
    ...
    parts.push(
      cohortChanged
        ? `${up[0].brandName} improved ${Math.abs(up[0].rankDelta)} rank position${
            Math.abs(up[0].rankDelta) === 1 ? "" : "s"
          } (directional vs changed peer universe).`
        : `${up[0].brandName} moved up ${Math.abs(up[0].rankDelta)} position${
            Math.abs(up[0].rankDelta) === 1 ? "" : "s"
          }.`
    );
    ...
    parts.push(
      cohortChanged
        ? `${down[0].brandName} fell ${down[0].rankDelta} rank position${
  ...
  if (parentSummary) {
    ...
    parts.push(
      `Portfolio visibility is ${fmtPct(parentSummary.currentPresence)} now (${
        parentSummary.portfolioDeltaDisplay || "n/a"
  ...
  if (cohortChanged) {
    parts.push(
      periodResolve?.cohortChangeDisclosure || BAI_COHORT_CHANGE_CUSTOMER_DISCLOSURE
    );
  }

  const narrative = parts.join(" ");
  const samePeerSetClaim = /same peer set/i.test(narrative);
  ...
  return {
    ...
    narrative,
    cohortChanged,
    cohortChangeDisclosure: cohortChanged
      ? periodResolve?.cohortChangeDisclosure || BAI_COHORT_CHANGE_CUSTOMER_DISCLOSURE
      : null,
    samePeerSetClaimForbidden: !samePeerSetClaim,
...
export function buildBaiWave4LongitudinalPresentationV1(opts = {}) {
    ...
    : [
        {
          ...
          mostStableBrand: wave3.portfolio?.mostStableBrand,
          noBrandsImproved: wave3.portfolio?.noBrandsImproved,
  ...
  const parentViews = (wantsFull ? cohort.parents : focusParents).map((pMeta) => {
    ...
    const competitive = buildCompetitiveNarrative(
      pSummary,
      brandRows,
      wave3.periodResolve
    );
    ...
    execParts.push(
      `Current portfolio presence ${fmtPct(pSummary.currentPresence)} (${
        pSummary.portfolioDeltaDisplay || "n/a"
      } vs prior).`
    );
    execParts.push(
      `Absolute ${absLabel(
    ...
    } else if (pSummary.noBrandsImproved) {
      execParts.push(
        pSummary.mostStableBrand
          ? `No brands improved. Most stable: ${pSummary.mostStableBrand.brandName} (${pSummary.mostStableBrand.deltaDisplay}).`
          : "No brands improved."
      );
    }
    ...
    if (competitive.cohortChanged) {
      execParts.push(competitive.cohortChangeDisclosure);
    }
    execParts.push(
    ...
    return {
      ...
      portfolio: {
        ...
        mostStableBrand: pSummary.mostStableBrand || null,
        noBrandsImproved: pSummary.noBrandsImproved === true,
      ...
      ownerIntent: {
        ...
        presentation: BAI_INTENT_NONCOMPARABILITY_CUSTOMER_DISCLOSURE,
        detail:
          "Current intent position remains available where certified, but change-over-time is not yet comparable for this monitoring pair.",
        dominatePage: false,
      },
      disclosures: {
        provider: provider.customerDisclosure,
        intent: BAI_INTENT_NONCOMPARABILITY_CUSTOMER_DISCLOSURE,
        cohortChange: competitive.cohortChangeDisclosure,
      },
      executiveRead: {
        available: true,
  ...
  const priorDatesOk = parentViews.every((p) => {
    const labels = (p.trend?.points || []).map((pt) => String(pt.label || ""));
    const hasAug18 = labels.some(
      (l) => l.includes(BAI_PERIOD_1_INTERNAL_HISTORY_DATE) || l.includes("2026-08-18")
    );
    const hasAug14 = labels.some((l) => l.includes("2026-08-14"));
    const hasSep3 = labels.some((l) => l.includes("2026-09-03"));
    return !hasAug18 && hasAug14 && hasSep3;
  });

  const zeroMoverOk = parentViews.every((p) => {
    const m = p.portfolio?.strongestPositiveMover;
    if (!m) return true;
    return isFiniteNumber(m.deltaPp) && Number(m.deltaPp) > 0.05;
  });
  ...
  const gates = {
    ...
    [BAI_PROVIDER_PRIOR_RUN_RECONCILIATION]: parentViews.every(
      (p) =>
        ...
        p.provider.comparabilityState ===
          INTENT_COMPARABILITY_STATE.NOT_COMPARABLE_FOR_THIS_PERIOD_PAIR &&
        p.provider.showPriorDeltaColumns === false &&
        p.provider.customerDisclosure ===
          BAI_PROVIDER_NONCOMPARABILITY_CUSTOMER_DISCLOSURE
    ),
    [BAI_PROVIDER_NONCOMPARABILITY_CUSTOMER_DISCLOSURE_GATE]: parentViews.every(
      (p) =>
        p.provider.showPriorDeltaColumns === false &&
        !p.provider.rows.some((r) => r.deltaDisplay === "—")
    ),
    ...
    [BAI_COHORT_CHANGE_CUSTOMER_DISCLOSURE_GATE]: parentViews.every(
      (p) =>
        p.competitive.story?.cohortChanged !== true ||
        Boolean(p.competitive.story?.cohortChangeDisclosure)
    ),
    [BAI_COHORT_CHANGE_RANK_NARRATIVE_INTEGRITY]: parentViews.every(
      (p) => p.competitive.story?.samePeerSetClaimForbidden !== false
    ),
    [BAI_ZERO_GAIN_NOT_CALLED_STRONGEST_MOVER]: zeroMoverOk,
    [BAI_P2_NO_MIXED_PRIOR_DATE_IDENTITY]: priorDatesOk,
    ...
    [BAI_WAVE4_UNPROMOTED_PERIOD_ISOLATION]:
      ...
      wave3.PERIOD_2_PUBLICATION_STATE === "PUBLISHED",
  ...
  return {
    ...
    PERIOD_2_PUBLICATION_STATE: wave3.PERIOD_2_PUBLICATION_STATE || "UNPROMOTED",
    LIVE_PROVIDER_CALLS: 0,
