...
export function buildOperatorCapabilitySnapshotV1(mergedFields, dealId, meta = {}) {
  ...
  if (access.status === SNAPSHOT_STATUS.BLOCKED) {
    ...
    reviewContextParts.push(
      "Snapshot is a limited internal draft. Clarifications may be resolved before external sharing."
    );
  } else {
    reviewContextParts.push(
      "Snapshot is ready for structured owner/advisor review based on current deal inputs."
    );
  }
  if (access.reasons?.length) {
    reviewContextParts.push(neutralizeSnapshotAccessReasons(access.reasons).slice(0, 3).join(" "));
  }
