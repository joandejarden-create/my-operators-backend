...
/** Bump when Match Score aggregation / factor set changes so list badges cannot stay on stale engine output. */
export const DEAL_BRAND_CACHE_SCORE_MODEL_VERSION = 12;
 ...
 * @property {number} [scoreModelVersion]
 */

/**
 * True only when cached breakdowns were produced by the exact current score model version.
 * Do not treat presence of v2 factor keys (geographyPriority, etc.) as current — those keys
 * also appear on older binary / room-gate caches and would block refresh.
 * @param {DealBrandCacheRow|null|undefined} cacheRow
 */
export function cacheHasCurrentScoreModel(cacheRow) {
  if (!cacheRow) return false;
  if (Number(cacheRow.scoreModelVersion) === DEAL_BRAND_CACHE_SCORE_MODEL_VERSION) return true;

  const bd = cacheRow.breakdownNewDetailsByBrand;
  if (!bd || typeof bd !== "object") return false;
  if (Number(bd._meta?.scoreModelVersion) === DEAL_BRAND_CACHE_SCORE_MODEL_VERSION) return true;
  return false;
}
...
/**
 * True when cache is missing, not on the current score model, or any required brand has no score.
 * @param {DealBrandCacheRow|null|undefined} cacheRow
 * @param {string[]} requiredBrandNames
 */
export function cacheNeedsRefresh(cacheRow, requiredBrandNames = []) {
  if (!cacheRow) return true;
  if (!cacheHasCurrentScoreModel(cacheRow)) return true;
