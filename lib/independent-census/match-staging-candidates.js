...
/**
 * Build a staging-shaped pool from a local OSM dry-run candidates array
 * (no Airtable). Used when the latest OSM batch is report-only.
 * @param {Array<object>} osmCandidates
 * @param {{ importBatchId?: string, countryFilter?: string }} [opts]
 */
export function loadOsmLocalAsStagingPool(osmCandidates, opts = {}) {
  const rows = [];
  for (const c of osmCandidates || []) {
    const coords = parseCoords(c.rawLatitude, c.rawLongitude);
    rows.push({
      airtableRecordId: "",
      sourceRecordId: c.sourceRecordId || "",
      sourceType: c.sourceType || SOURCE_TYPES.OSM,
      rawHotelName: normalizeText(c.rawHotelName),
      rawCity: normalizeText(c.rawCity),
      rawCountry: normalizeText(c.rawCountry),
      coords,
      websiteHost: websiteHost(c.rawWebsite),
      rawBrand: normalizeText(c.rawBrand),
  ...
  return {
    ...
    totalLoaded: rows.length,
    matchingPoolSize: filtered.length,
    importBatchId: opts.importBatchId || "local-osm",
