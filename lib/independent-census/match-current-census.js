...
} from "../research-engine-v2/census-postal-code-v1.js";
...
export function scoreCandidateAgainstCensus(candidate, census) {
  ...
  const postalSignal = comparePostalIdentitySignal(
    candidate.rawPostalCode || candidate.postal_code || candidate.postalCode,
    census.postalCode || census.postal_code,
    candidate.rawCountry || census.country
  );
  ...
  score = applyPostalToIdentityScore(score, postalSignal);
  ...
  return {
    ...
    websiteMatch,
    phoneMatch,
    postalIdentitySignal: postalSignal,
    score,
  };
}
