 ...
 * Mapping: Hotel Census Market ← Excel STR Market; Hotel Census Submarket ← Excel STR Submarket.
 * @param {{ force?: boolean, fillBlankOnly?: boolean, fieldsOnly?: string[] }} opts
 *   — force=true writes every non-empty Excel cell (full sync)
 *   — fillBlankOnly skips when census already has a value
 *   — fieldsOnly limits which Airtable columns may be written
 */
export function buildCensusUpdateFields(mapping, ex, censusEntry, opts = {}) {
  const force = opts.force === true;
  const fillBlankOnly = opts.fillBlankOnly === true;
  const fieldsOnly = opts.fieldsOnly?.length ? new Set(opts.fieldsOnly) : null;
  ...
  for (const [airtableField, excelVal, censusVal] of pairs) {
    ...
    if (fieldsOnly && !fieldsOnly.has(airtableField)) continue;
    const next = String(excelVal ?? "").trim();
    if (!next) continue;
    if (fillBlankOnly && String(censusVal ?? "").trim()) continue;
