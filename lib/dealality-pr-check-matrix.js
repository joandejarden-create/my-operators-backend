...
export const PR_CHECK_RULES = [
  ...
  },
  {
    ...
    {
    id: "brand-protected-baseline",
    label: "Brand Explorer protected 54 Active/Live public-full baseline",
    risk: "High",
    commands: [
    ...
    // Historical 46/45/39/27/24 baseline paths kept as match patterns so older
    // artifacts/docs still route through this High-risk rule.
    match: (p) =>
      p.includes("brand-explorer-54-active-public-full-baseline") ||
      p.includes("brand-explorer-46-active-public-full-baseline") ||
      p.includes("brand-explorer-45-active-public-full-baseline") ||
      p.includes("brand-explorer-39-active-public-full-baseline") ||
      p.includes("brand-explorer-27-active-public-full-baseline") ||
      p.includes("brand-explorer-24-active-public-full-baseline") ||
      p.includes("brand-explorer-protected-baseline") ||
      p.includes("brand-explorer-active-universe") ||
      p.includes("brand-explorer-public-visibility-quality-lock") ||
      p.includes("brand-explorer-recent-momentum-evidence") ||
      p.includes("brand-explorer-ai-assisted-footnote") ||
      p.includes("brand-explorer-24-tab-section-quality") ||
      p.includes("brand-explorer-24-pre-baseline") ||
      p.includes("brand-explorer-quiet-sequential") ||
      /^fixtures\/brand-explorer-presentation-/.test(p) ||
      /^scripts\/apply-.*explorer/.test(p) ||
      /^scripts\/apply-choice-/.test(p) ||
      /^scripts\/brand-explorer-/.test(p) ||
      /^lib\/partner-intelligence\/brand-explorer-/.test(p) ||
  ...
  {
    id: "brand-openings-ascend",
    label: "Brand Explorer Openings Ascend + CALA-first template",
    risk: "High",
    commands: [
      "npm run brand-explorer-openings-ascend-template-audit",
    ],
    match: (p) =>
      p.includes("brand-explorer-openings-property-card-contract") ||
      p.includes("brand-explorer-openings-ascend") ||
      p.includes("brand-explorer-cala-property-example-rules") ||
      p.includes("brand-explorer-openings-property-card-template") ||
      /^scripts\/brand-explorer-openings-/.test(p) ||
      (p.includes("brand-explorer-lane2") &&
        (p.includes("property-catalog") ||
          p.includes("founder-minor-cleanup") ||
          p.includes("image-asset-pack"))),
  },
  {
    id: "brand-api-ui",
  ...
  },
  {
    ...
    match: (p) =>
      ...
      /gtm|pilot-target|outreach-setup|owner-targets|decision-radar|decision-opportunit/.test(p),
  },
  {
    id: "gtm-decision-radar",
    label: "GTM Decision Radar schema/validation",
    risk: "High",
    commands: [
      "npm run test:gtm-decision-opportunity",
      "npm run ensure:gtm-decision-opportunities-schema",
    ],
    match: (p) =>
      /decision-opportunit|decision-radar|gtm-decision/.test(p) ||
      /^docs\/gtm-decision-radar\.md$/.test(p),
  },
  {
