...
import { attachExecutiveScanLayer } from "./customer/executive-scan-layer-v1.js";
...
function overlayGovernedCustomerIndex(propertyId, payload) {
  ...
  const hasBakedSubjectRates =
    bakedIndex &&
    typeof bakedIndex === "object" &&
    Object.values(bakedIndex).some(
      (row) =>
        row &&
        typeof row === "object" &&
        (row.subjectRatePct != null || row.myRate != null)
    );

  // ADP_TERRITORY_SUBJECT_RATE_INDEPENDENT_OF_BENCHMARK_STATUS
  // ADP_PUBLISHED_SUBJECT_RATE_IS_CUSTOMER_SOURCE_OF_TRUTH
  // Never overwrite certified published subject rates with a live rebuild.
  if (hasBakedSubjectRates) {
    return payload;
  }
...
export function enrichPayloadOptionalMetrics(propertyId, payload) {
  ...
  try {
    ...
    enriched = attachInactiveExecutiveReadV3PreviewIfMissing(propertyId, enriched, profile);
    // Hybrid scan layer after V3 stamp so left constraint can align to primaryIssueId
    if (enriched?.executiveRead) {
      enriched = {
        ...enriched,
        executiveRead: attachExecutiveScanLayer(enriched.executiveRead, enriched, profile),
      };
    }
...
function attachCompetitiveRankingIfMissing(propertyId, payload, period, scenarios, profile) {
  ...
  try {
    ...
    const needsEnrichment = rankingNeedsDisplacementEnrichment(ranking);
    if (!ranking?.byTerritory) {
      ...
      // Entity reconciliation / evidence-trace corrections require a full ranking rebuild,
      // not only re-attaching counts onto stale competitor rows.
      ranking = buildAllTerritoryCompetitiveRankings(observations, scenarios, profile);
      ranking = attachDisplacementToCompetitiveRanking(ranking, observations, scenarios, profile);
    }
...
/**
 * When V3 is activated: ensure published payloads expose customer-default compositionVersion + sections.
 * Never overwrites writeup/ux. Never writes disk.
 */
function ensureActivatedExecutiveReadV3(propertyId, payload, profile) {
  if (!payload?.executiveRead || COMPOSITION_V3_STATUS.activated !== true) return payload;
  const er = payload.executiveRead;
  if (er.compositionVersion === "ADP_EXECUTIVE_READ_COMPOSITION_V3" && er.sections) {
    return payload;
  }
  if (er.compositionV3?.ok && er.compositionV3?.sections) {
    return {
      ...payload,
      executiveRead: {
        ...er,
        compositionVersion: er.compositionV3.compositionVersion,
        sections: er.compositionV3.sections,
        numericAnchors: er.compositionV3.numericAnchors,
        primaryIssueId: er.compositionV3.primaryIssueId,
        compositionHash: er.compositionV3.compositionHash,
        sourceSnapshotHash: er.compositionV3.sourceSnapshotHash,
        evidenceTrace: er.compositionV3.evidenceTrace,
        qualityGates: er.compositionV3.qualityGates,
      },
    };
  }
  try {
    const v3 = composeExecutiveReadV3(payload, {
      propertyId: propertyId || profile?.propertyId,
      market: profile?.market,
      distributionStatus: "PUBLISHED_CURRENT",
      zeroCodePath: true,
      allowNewEdition: true,
    });
    if (!v3?.ok) return payload;
    return {
      ...payload,
      executiveRead: {
        ...er,
        ...
        compositionV3: { ...v3, activated: true, customerFacingDefault: true },
      },
    };
  } catch (err) {
    console.error("[ADP read] activated V3 ensure failed:", err.message);
    return payload;
  }
}

/**
 * In-memory inactive V3 attach for QA / future activation — never writes published snapshots.
 * Does not overwrite writeup/ux. Does not mutate issued share content on disk.
 */
function attachInactiveExecutiveReadV3PreviewIfMissing(propertyId, payload, profile) {
  if (!payload?.executiveRead) return payload;
  if (COMPOSITION_V3_STATUS.activated === true) {
    return ensureActivatedExecutiveReadV3(propertyId, payload, profile);
  }
  if (payload.executiveRead.compositionV3?.ok && payload.executiveRead.compositionV3?.sections) {
    return payload;
  }
  try {
    const v3 = composeExecutiveReadV3(payload, {
      ...
      return {
        ...
        executiveRead: {
          ...
          compositionV3: {
            ok: false,
            activated: false,
            reason: v3?.reason || "EXECUTIVE_READ_REVIEW_REQUIRED",
            detail: v3?.detail,
            compositionVersion: v3?.compositionVersion,
          },
        },
    ...
    return { ...payload, executiveRead: attached.executiveRead };
  } catch (err) {
    console.error("[ADP read] inactive V3 attach failed:", err.message);
    return payload;
  }
}
...
export async function getPublishedEvidenceResponse(propertyId, query) {
  ...
  // Displacement: prefer published canonical support sets (self-contained on lean deploys).
  // Fall back to live resolve only when runtime period observations are available.
  // Do not use the truncated published evidence-index competitor name cap.
  if (query.type === "displacement") {
    ...
    const publishedEvidence = loadPublishedEvidenceIndex(propertyId);
    const publishedSupport = competitorId
      ? getPublishedDisplacementSupportSet(publishedEvidence, competitorId, resolvedScope)
      : null;

    if (publishedSupport && Array.isArray(publishedSupport.evidence)) {
      const uniqueScenarioCount = publishedSupport.displacementScenarioCount ?? publishedSupport.uniqueScenarioIds?.length ?? 0;
      const evidence = publishedSupport.evidence;
      const evidenceAvailable = uniqueScenarioCount > 0 && evidence.length > 0;
      const failClosed = Boolean(competitorId || query.competitor) && !evidenceAvailable;
      return {
        ok: true,
        source: "published_displacement_support_set_v1",
        contract: ADP_DISPLACEMENT_EVIDENCE_PUBLISHED_SNAPSHOT_SELF_CONTAINED,
        supportSetVersion: publishedSupport.version || DISPLACEMENT_SUPPORT_SET_VERSION,
        propertyId,
        intent: publishedSupport.scope?.intent || query.intent || null,
        scope: publishedSupport.scope || { type: resolvedScope === "overall" ? "overall" : "demand_territory", key: resolvedScope },
        type: "displacement",
        competitor: publishedSupport.competitorName || query.competitor,
        competitorName: publishedSupport.competitorName || query.competitor,
        competitorId: publishedSupport.competitorEntityId || competitorId,
        evidence,
        total: evidence.length,
        count: uniqueScenarioCount,
        grain: "SCENARIO_ID",
        version: publishedSupport.resolverVersion || DISPLACEMENT_EVIDENCE_RESOLVER_VERSION,
        evidenceAvailable,
        supportingObservationCount: publishedSupport.supportingObservationIds?.length ?? evidence.length,
        uniqueScenarioCount,
        drawerItemCount: evidence.length,
        integrity: failClosed
          ? { failClosed: true, code: "ADP_COMPETITIVE_EVIDENCE_DRAWER_NONEMPTY" }
          : { failClosed: false },
      };
    }
    ...
    const failClosed = Boolean(competitorId || query.competitor) && !result.evidenceAvailable;
    ...
    return {
      ...
      competitorName: result.competitorName || query.competitor,
      ...
      supportingObservationCount: result.supportingObservationCount,
      uniqueScenarioCount: result.uniqueScenarioCount,
      drawerItemCount: result.drawerItemCount,
      integrity: failClosed
        ? { failClosed: true, code: "ADP_COMPETITIVE_EVIDENCE_DRAWER_NONEMPTY" }
        : { failClosed: false },
    };
