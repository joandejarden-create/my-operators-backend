...
function attachTrendsIfMissing(propertyId, payload, allPeriods) {
  ...
  const missingPrcOnly =
    hasTrends &&
    !ineligible &&
    payload.trends.some(
      (t) =>
        t.propertyRealityCoverage == null &&
        (t.considerationRate != null || t.scenarioPresenceRate != null || t.demandCaptureRate != null)
    );

  // Prefer in-place PRC patch when primary trend rates are already baked — full rebuild
  // can drop consideration/scenario rates when optional metrics fail for a property.
  if (missingPrcOnly) {
    try {
      const profile = loadPropertyProfile(propertyId);
      const patched = payload.trends.map((t) => {
        if (t.propertyRealityCoverage != null) return t;
        const period = (allPeriods || []).find((p) => p.periodId === t.periodId);
        const prc = period ? computePropertyRealityCoverage(period, profile) : null;
        return { ...t, propertyRealityCoverage: prc };
      });
      return { ...payload, trends: patched };
    } catch (err) {
      console.error("[ADP read] trends PRC patch failed:", err.message);
    }
  }
  ...
  try {
    ...
    const merged = (trends || []).map((t) => {
      const prior = (payload.trends || []).find((x) => x.periodId === t.periodId);
      if (!prior) return t;
      return {
        ...t,
        considerationRate: t.considerationRate ?? prior.considerationRate ?? null,
        scenarioPresenceRate: t.scenarioPresenceRate ?? prior.scenarioPresenceRate ?? null,
      };
    });
...
export function enrichPayloadOptionalMetrics(propertyId, payload) {
  ...
  try {
    ...
    return attachTrendsIfMissing(propertyId, enriched, allPeriods);
  } catch (err) {
