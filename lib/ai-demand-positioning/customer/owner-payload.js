...
import {
  ...
  isComparableObservation,
} from "../metrics/grain-governance.js";
...
/** Provider presence denominator = comparable observations only (not scheduled/failed). */
...
function computeEvidence(observations, scenarios, propertyProfile = null) {
  ...
  // Provider visibility — grain: comparable observations included in the metric.
  // scheduled = scenarios asked on this provider; comparable/total = successfully captured & included.
  const providerStats = {};
  for (const obs of observations) {
    if (!providerStats[obs.provider]) {
      providerStats[obs.provider] = {
        scheduled: 0,
        comparable: 0,
        mentioned: 0,
        withCitations: 0,
    ...
    providerStats[obs.provider].scheduled++;
    if (!isComparableObservation(obs)) continue;
    providerStats[obs.provider].comparable++;
  ...
  const providers = Object.entries(providerStats)
    .map(([provider, s]) => ({
      ...
      scenariosScheduled: s.scheduled,
      comparable: s.comparable,
      captured: s.comparable,
      // `total` is the metric denominator (comparable), not theoretical scheduled N.
      total: s.comparable,
      mentioned: s.mentioned,
      excludedFromMetric: Math.max(0, s.scheduled - s.comparable),
      denominatorGrain: ADP_PROVIDER_DENOMINATOR_GRAIN,
      presence: s.comparable > 0 ? roundAdpPercent((s.mentioned / s.comparable) * 100) : 0,
    }))
    .sort((a, b) => b.presence - a.presence);
  const providerCitations = {};
  for (const [provider, s] of Object.entries(providerStats)) {
    providerCitations[provider] =
      s.comparable > 0 ? roundAdpPercent((s.withCitations / s.comparable) * 100) : 0;
  }
