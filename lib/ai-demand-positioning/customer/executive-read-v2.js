...
import { attachExecutiveScanLayer } from "./executive-scan-layer-v1.js";
...
function buildWriteUp(inputs, strengthBox, constraintBox, changeBox, benchmarkFinding, weakBenchmark) {
  ...
  // LEGACY V1/V2 compatibility only — V3 must never use PRODUCTION_BUILDER_180_WORD_TRUNCATE.
...
export function attachExecutiveReadUxLayer(read, payload, period, scenarios, propertyProfile) {
  ...
  // Hybrid V3: left scan cards + right V3 synthesis. Attach scanLayer after UX.
  return attachExecutiveScanLayer(enriched, payload, propertyProfile);
}
...
export function executiveReadNeedsUxEnrichment(er) {
  ...
  if (!er.scanLayer?.biggestStrength || !er.scanLayer?.biggestConstraint || !er.scanLayer?.changeSincePrior) {
    return true;
  }
  const ux = er.ux;
  if (!ux) return true;
