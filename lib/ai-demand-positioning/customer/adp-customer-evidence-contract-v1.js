...
import { loadPublishedReport, loadPublishedEvidenceIndex } from "../published-snapshot.js";
...
export const ADP_EVIDENCE_PRODUCTION_PARITY = "ADP_EVIDENCE_PRODUCTION_PARITY";
export const ADP_AGGREGATE_CLAIM_SUPPORT_PARITY_V1 = "ADP_AGGREGATE_CLAIM_SUPPORT_PARITY_V1";
export const LINK_EXISTS = "LINK_EXISTS";
export const DRAWER_OPENS = "DRAWER_OPENS";
export const SUPPORT_NONEMPTY = "SUPPORT_NONEMPTY";
export const SEMANTIC_PARITY = "SEMANTIC_PARITY";
export const COUNT_PARITY = "COUNT_PARITY";
export const PUBLISHED_SNAPSHOT_PARITY = "PUBLISHED_SNAPSHOT_PARITY";
export const PRODUCTION_PARITY = "PRODUCTION_PARITY";
...
export async function auditPropertyEvidenceLinks(propertyId) {
  ...
  for (const link of enumerated.links || []) {
    ...
    if (link.bppItems) {
      ...
      // Exact count parity for Competitive Displacement (scenario grain)
      if (
        link.evidenceType === EVIDENCE_TYPE.COMPETITIVE_DISPLACEMENT &&
        link.expectedEvidenceCount != null
      ) {
        const uniqueScenarioIds = new Set(
          (evidence || []).map((ev) => ev.scenarioId).filter(Boolean)
        );
        const drawerCount = uniqueScenarioIds.size;
        const displayed = Number(link.expectedEvidenceCount);
        if (drawerCount !== displayed) {
          row.defects.push({
            gate: ADP_DISPLACEMENT_COUNT_EVIDENCE_EXACT_PARITY,
            code: "COUNT_PARITY_FAIL",
            detail: `displayed=${displayed} drawerUniqueScenarios=${drawerCount} apiSource=${api?.source || "none"}`,
          });
        }
        if (displayed > 0 && drawerCount === 0) {
          row.defects.push({
            gate: ADP_EVIDENCE_LINK_NONEMPTY,
            code: "EMPTY_DRAWER_WITH_NONEMPTY_COUNT",
            detail: `displayed=${displayed}`,
          });
          emptyLinks.push(row);
        }
        // Production self-containment: must resolve from published support set when available
        if (
          displayed > 0 &&
          api?.source &&
          api.source !== "published_displacement_support_set_v1" &&
          api.source !== "displacement_resolver_v1"
        ) {
          row.defects.push({
            gate: ADP_DISPLACEMENT_EVIDENCE_PUBLISHED_SNAPSHOT_SELF_CONTAINED,
            code: "UNEXPECTED_DISPLACEMENT_SOURCE",
            detail: api.source,
          });
        }
        // Prefer published snapshot path — flag when falling back despite pack missing support sets
        const publishedEv = loadPublishedEvidenceIndex(propertyId);
        if (displayed > 0 && !publishedEv?.displacementSupportSets) {
          row.defects.push({
            gate: ADP_DISPLACEMENT_EVIDENCE_PUBLISHED_SNAPSHOT_SELF_CONTAINED,
            code: "PUBLISHED_SUPPORT_SET_MISSING",
            detail: "displacementSupportSets not baked into published evidence index",
          });
        }
        if (
          displayed > 0 &&
          publishedEv?.displacementSupportSets &&
          api?.source !== "published_displacement_support_set_v1"
        ) {
          row.defects.push({
            gate: ADP_EVIDENCE_PRODUCTION_PARITY,
            code: "NOT_USING_PUBLISHED_SUPPORT_SET",
            detail: api?.source || "none",
          });
        }
      }
