 ...
 * Displacement counts are delegated to resolve-displacement-evidence-v1
 * (same source as competitive Overview + evidence modal). This module retains
 * lost-scenario narrative (reasons / relevance) only.
 */
...
import { registryHotelById } from "../metrics/adp-property-entity-registries.js";

export const SINGLE_CANONICAL_DISPLACEMENT_AGGREGATOR =
  "SINGLE_CANONICAL_DISPLACEMENT_AGGREGATOR";
export const ADP_LOST_DEMAND_DISPLACEMENT_DELEGATE_VERSION =
  "adp_lost_demand_displacement_delegate_v1";
...
export function computeLostDemand(observations, scenarios, propertyProfile) {
  ...
  // Canonical displacement counts — same resolver as Overview + evidence modal
  ...
  return {
    ...
    displacementSource: SINGLE_CANONICAL_DISPLACEMENT_AGGREGATOR,
...
/**
 * Thin delegate to canonical displacement aggregator.
 * Legacy local aggregation removed (was a parallel count path).
 */
function aggregateDisplacementFromCanonical(observations, scenarios, propertyProfile) {
  ...
  return Object.entries(counts || {})
    .map(([entityId, displacementCount]) => {
      const hotel = propertyId ? registryHotelById(propertyId, entityId) : null;
      return {
        entityId,
        name: hotel?.canonical || hotel?.displayName || entityId,
        displacementCount,
      };
    })
