...
export function computeCompetitiveSet(observations, propertyProfile) {
  ...
  const observed = merged
    .map((row) => {
      // Prefer merge entityId from aggregation — do not re-resolve display names
      // (canonical labels with punctuation can fail alias exact-match and zero scenarioCount).
      const key = row.entityId || null;
      return {
        name: row.name,
        mentions: row.count,
        scenarioCount: (key && competitorScenarios[key]?.size) || 0,
