...
/**
 * Extract competitor hotel names from response.
 */
export function extractCompetitors(response, propertyProfile) {
  ...
  const knownCompetitors = [
    ...
    "The Boca Raton", "Waldorf Astoria", "Mandarin Oriental",
    "Renaissance Boca Raton", "Wyndham Boca Raton", "Marriott Boca Raton",
    "Hilton Boca Raton Suites", "Boca Beach Club", "Royal Palm",
    "Addison", "Delray Beach Marriott", "Seagate Hotel",
    "Opal Grand", "Crane's Beach House", "Ocean Lodge",
  ];

  for (const comp of knownCompetitors) {
    if (response.toLowerCase().includes(comp.toLowerCase())) {
      if (!competitors.find((c) => normalizeHotelName(c) === normalizeHotelName(comp))) {
        competitors.push(comp);
      }
    }
  }

  const hotelPattern = /(?:the\s+)?[A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,4}\s+(?:Hotel|Resort|Inn|Suites?|Lodge|Club)/g;
  const matches = response.match(hotelPattern) || [];
  for (const match of matches) {
    const cleaned = match.trim();
    if (cleaned.toLowerCase().includes(propertyProfile.name.toLowerCase().split(" ")[0])) continue;
    if (!competitors.find((c) => normalizeHotelName(c) === normalizeHotelName(cleaned)) && competitors.length < 15) {
