/**
 * CALA six-hotel cohort — first official ADP property baselines.
 * Pattern: Monterrey Valle / Hotel Phillips first-official-property baseline.
...
import { CALA_SIX_ENTITY_VERSION_BY_PROPERTY } from "../metrics/cala-six-entity-registries.js";
...
export { CALA_SIX_PROPERTY_IDS };

export const CALA_SIX_BASELINE_MARKERS = Object.freeze({
  adp_st_regis_mexico_city: "ADP_ST_REGIS_MEXICO_CITY_BASELINE_PERIOD_001",
  adp_st_regis_cap_cana: "ADP_ST_REGIS_CAP_CANA_BASELINE_PERIOD_001",
  adp_jw_marriott_santo_domingo: "ADP_JW_MARRIOTT_SANTO_DOMINGO_BASELINE_PERIOD_001",
  adp_radisson_santo_domingo: "ADP_RADISSON_SANTO_DOMINGO_BASELINE_PERIOD_001",
  adp_casas_del_xvi: "ADP_CASAS_DEL_XVI_BASELINE_PERIOD_001",
  adp_hotel_caribe_faranda_grand: "ADP_HOTEL_CARIBE_FARANDA_GRAND_BASELINE_PERIOD_001",
  adp_faranda_collection_bogota: "ADP_FARANDA_COLLECTION_BOGOTA_BASELINE_PERIOD_001",
});

export const CALA_SIX_COST_CAP_USD = 12;
...
export function buildCalaSixPropertyPreflight(propertyId) {
  ...
  if (!CALA_SIX_PROPERTY_IDS.includes(propertyId)) {
    blockers.push("UNKNOWN_CALA_SIX_PROPERTY");
  }
  ...
  if (roundedTotal > CALA_SIX_COST_CAP_USD) blockers.push("COST_CAP_EXCEEDED");
  ...
  const PREFLIGHT =
    blockers.length === 0 && roundedTotal <= CALA_SIX_COST_CAP_USD ? "PASS" : "FAIL";
  ...
  return {
    ...
    COST_CAP_USD: CALA_SIX_COST_CAP_USD,
    CORE: coreRows,
    blockers,
    PREFLIGHT,
  };
...
export async function executeCalaSixBaselinePeriod001({
  propertyId,
  dryRun = true,
  onProgress = null,
  certify = false,
  ...
  const preflight = buildCalaSixPropertyPreflight(propertyId);
  ...
  finalPeriod = attachFirstOfficialPropertyPeriodMetadata(finalPeriod, {
    ...
    entityResolutionVersion: CALA_SIX_ENTITY_VERSION_BY_PROPERTY[propertyId] || null,
