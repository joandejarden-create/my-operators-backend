...
import {
  applyGovernedInterpretation,
  computeCanonicalSubjectPresence,
  projectGovernedObservations,
} from "../subject-presence/canonical-subject-presence-v1.js";
import { attachCanonicalInterpretation } from "../customer/adp-canonical-observation-interpretation-v1.js";

export const EXECUTIVE_METRICS_VERSION = "adp_executive_metrics_foundation_v1";

/**
 * Project observations onto the single canonical subject-presence path.
 * Mention = Path A (`detectPropertyMention` via computeCanonicalSubjectPresence).
 * Always recomputes from rawResponse — never trusts stale governedInterpretation.
 * Rank = Path A-gated rank fields only. No provider calls.
 */
export function enrichObservationsWithRank(observations, propertyProfile) {
  const withGoverned = (observations || []).map((obs) =>
    applyGovernedInterpretation(obs, propertyProfile, {
      correctionReason: "CANONICAL_OPTION_A_ENRICH_PROJECTION",
    })
  );
  const projected = projectGovernedObservations(withGoverned);
  return projected.map((obs) => {
    const governed =
      obs.governedInterpretation ||
      computeCanonicalSubjectPresence(obs.rawResponse || "", propertyProfile);
    const enriched = {
      ...obs,
      mentioned: Boolean(governed.subjectMentioned),
      position: governed.subjectRank,
      rankEligible: Boolean(governed.rankEligible),
      rankSource: governed.rankSource,
      positionConfidence: governed.positionConfidence,
      context: governed.matchProvenance?.context ?? obs.context,
      parsed: true,
      _subjectPresenceSource: "governedInterpretation",
    };
    // SINGLE_CANONICAL_OBSERVATION_INTERPRETATION_PATH — one object for metrics/UI/evidence
    return attachCanonicalInterpretation(enriched, propertyProfile);
  });
}
