...
const ARTIFACT_PATTERNS = [
  ...
  // Clause / sentence fragments mistaken for hotel names (Cambridge + peers).
  /^(while|although|because|since|when|where|if|as|and|but|or)\b/i,
  /^(its|their|our)\b/i,
  /^(one|many|some|all|several|most)\s+of\b/i,
  /\b(is presented as|is known as|is considered|is described as|offers a|features a)\b/i,
];
...
export function isLikelyArtifactEntity(name) {
  ...
  // Leading lowercase almost never a canonical hotel display name.
  if (/^[a-z]/.test(trimmed)) return true;
  const words = trimmed.split(/\s+/).filter(Boolean);
  // Long sentence-like strings are not hotel entities (even if they contain "resort").
  if (words.length >= 8) return true;
  // Short clause with lodging noun but no proper multi-word Title Case brand.
  if (words.length <= 4 && HOTEL_SIGNAL.test(trimmed) && !/[A-Z][a-z]+\s+[A-Z]/.test(trimmed)) {
    if (/^(the|a|an)\s+(hotel|resort|inn|suites?)\b/i.test(trimmed)) return true;
    if (/^(while|although)\s+the\s+(hotel|resort)\b/i.test(trimmed)) return true;
  }
