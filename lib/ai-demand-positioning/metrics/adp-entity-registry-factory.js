...
export function createEntityRegistry({ version, hotels, genericExact = [], venueOrClub = [], brandOnly = [], ambiguous = [] }) {
  ...
  // Normalize aliases once — raw alias punctuation (commas etc.) must match `norm(rawName)`.
  const hotelsNorm = hotels.map((h) => ({
    hotel: h,
    canonNorm: norm(h.canonical),
    aliasesNorm: [...new Set([norm(h.canonical), ...(h.aliases || []).map(norm)].filter(Boolean))],
  }));
  ...
  function classifyObservedEntity(rawName) {
    ...
    let best = null;
    for (const { hotel, canonNorm, aliasesNorm } of hotelsNorm) {
      const exact = aliasesNorm.includes(n) || n === canonNorm;
      const containedAlias = aliasesNorm
        .filter((a) => a.length >= 12 && n.includes(a))
        .sort((a, b) => b.length - a.length)[0];
      const shortFormAlias = aliasesNorm
        .filter((a) => a.length >= 12 && n.length >= 12 && a.includes(n))
        .sort((a, b) => b.length - a.length)[0];
      const canonContained =
        canonNorm.length >= 12 && (n.includes(canonNorm) || canonNorm.includes(n)) ? canonNorm : null;
