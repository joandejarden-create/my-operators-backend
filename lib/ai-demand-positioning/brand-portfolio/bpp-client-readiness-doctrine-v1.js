...
export function resolveBppPopulationAttempt({
  ...
  if (ready || (bppPayload?.status === BRAND_PORTFOLIO_STATUS.READY && bppPayload?.customerPublished)) {
    const adequacy = bppPayload?.peerSet?.adequacy || peerSet?.adequacy?.status || null;
    const rankOnly =
      adequacy === "RANK_ONLY_SUPPRESS_BENCHMARK_INDEX" ||
      bppPayload?.bppCustomerState === "BPP_READY_POPULATED_RANK_ONLY" ||
      bppPayload?.customerReadyClass === "BPP_READY_POPULATED_RANK_ONLY";
    const populatedClass = rankOnly
      ? BPP_CLIENT_READY_CLASS.BPP_READY_POPULATED_RANK_ONLY
      : BPP_CLIENT_READY_CLASS.BPP_READY_POPULATED_FULL;
    return {
      propertyId,
      clientReadyClass: populatedClass,
      // Legacy alias — still treated as populated
      clientReadyClassLegacy: BPP_CLIENT_READY_CLASS.BPP_READY_POPULATED,
      clientReady: true,
      eligibility,
      peerSetPresent: Boolean(peerSet),
      peerCount: peerSet?.peerCountExcludingSubject ?? bppPayload?.peerSet?.peerCount ?? 0,
      ...
      gates: {
        ...
        [ADP_BPP_READY_REQUIRES_CERTIFIED_PEER_SET]: Boolean(peerSet) || Boolean(bppPayload?.peerSet),
...
export function evaluateBppClientReadinessGate(resolution) {
  const ok =
    ...
    (resolution.clientReadyClass === BPP_CLIENT_READY_CLASS.BPP_READY_POPULATED ||
      resolution.clientReadyClass === BPP_CLIENT_READY_CLASS.BPP_READY_POPULATED_FULL ||
      resolution.clientReadyClass === BPP_CLIENT_READY_CLASS.BPP_READY_POPULATED_RANK_ONLY ||
