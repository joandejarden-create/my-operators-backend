...
function providerSpreadLine(byProvider) {
  ...
  const labels = { openai: "ChatGPT", gemini: "Gemini", perplexity: "Perplexity", claude: "Claude" };
  return order
    .map((p) => {
...
export function buildBrandPortfolioNarrativesV1({ profile, metrics, lens }) {
  ...
  if (propertyId === "adp_waterstone_boca_raton") {
    body =
      ...
      `Provider inconsistency remains material: Gemini and Perplexity are stronger; ChatGPT and Claude are weaker (${spread}). ` +
      `Index and rank do not imply uniformly strong AI visibility across every provider.`;
    actions = [
      {
        priority: "medium",
        title: "Close ChatGPT and Claude gaps",
        description:
          "Relative #2 standing and Index 137 sit atop 50% absolute presence. Strengthen signals that ChatGPT and Claude cite so cross-model coverage catches up to Gemini/Perplexity.",
      },
      {
        priority: "medium",
    ...
    body =
      ...
      `Scenario-level Top-3 Appearance is strong (${pct(top3)}), but #2 ranking must not obscure weak ChatGPT presence. ` +
      `Substantial provider inconsistency remains (${spread}).`;
    actions = [
      {
        priority: "high",
        title: "Address ChatGPT absence",
        description:
          "ChatGPT presence is near zero while Gemini/Perplexity carry most visibility. Ranking #2 does not mean consistent cross-model discovery.",
      },
      {
        priority: "medium",
