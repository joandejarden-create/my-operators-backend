...
function buildTrends(propertyId, scenarios, periods) {
  const customerPeriods = filterCustomerTrendPeriods(periods);
  if (!customerPeriods || customerPeriods.length < 1) return undefined;
  return customerPeriods.map((p) => {
    const profile = loadPropertyProfile(propertyId);
    const pPayload = buildOwnerPayload(p, scenarios, profile);
    const prc =
      pPayload.executiveMetrics?.propertyRealityCoverage ??
      pPayload.realityGap?.coverageRate ??
      computePropertyRealityCoverage(p, profile) ??
      null;
    return {
      periodId: p.periodId,
      date: p.executionDate,
      // Customer Trend shows Consideration + Scenario Presence only (not Demand Capture).
      propertyRealityCoverage: prc,
      scenarioPresenceRate: pPayload.executiveMetrics?.scenarioPresence?.rate ?? null,
      considerationRate: pPayload.executiveMetrics?.considerationRate?.rate ?? null,
