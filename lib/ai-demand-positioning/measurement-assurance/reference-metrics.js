...
} from "../metrics/scenario-eligibility.js";
...
/**
 * Consideration + scenario presence — same eligibility filter as production
 * (CORE + CONDITIONAL + PROPERTY_SPECIFIC). Demand Capture stays full-universe.
 */
export function refConsiderationAndScenario(observations, scenarios, propertyProfile = null) {
  const eligibility = propertyProfile
    ? buildScenarioEligibilityMap(scenarios, propertyProfile)
    : null;
  const eligibleScenarios = eligibility
    ? scenariosForEligibility(scenarios, eligibility, CONSIDERATION_ELIGIBLE_STATES)
    : scenarios || [];
  const scenarioIds = new Set(eligibleScenarios.map((s) => s.scenarioId));
  ...
  for (const s of eligibleScenarios) byScenario.set(s.scenarioId, false);
  ...
  const scenarioPresence =
    eligibleScenarios.length > 0
      ? roundAdpPercent((captured / eligibleScenarios.length) * 100)
      : null;
...
export function buildReferenceMetricPack(period, scenarios, propertyProfile) {
  ...
  const cons = refConsiderationAndScenario(interpreted, scenarios, propertyProfile);
