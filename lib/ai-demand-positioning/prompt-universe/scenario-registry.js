...
import { generateGenericProfileScenarios } from "./generic-profile-scenarios.js";
...
export function buildScenarioUniverse(propertyProfile) {
  ...
  // Generic fallback when no market pack / property pack — synthesizes from profile fields only.
  if (!combined.length && propertyProfile?.propertyId) {
    return generateGenericProfileScenarios(propertyProfile).map((s) => ({
      ...s,
