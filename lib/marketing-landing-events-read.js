...
import { buildLandingDashboard, buildPopularLandingPages } from "./marketing-landing-events-dashboard.js";
import { buildInsightsHubReport } from "./marketing-landing-events-insights-hub.js";
import {
  filterEventsBySurface,
  isInsightsEvent,
  SURFACE_LANDING,
} from "./marketing-landing-events-surface.js";
import { applyLandingReportFilters } from "./marketing-landing-events-version.js";
import { buildVideoEngagement } from "./marketing-landing-events-video.js";
import { buildLandingRecommendations } from "./marketing-landing-events-recommendations.js";

const INTERNAL_GEO_LABEL = "Barcelona, CT, Spain";
...
function isInternalGeoEvent(event) {
  if (!event || event.event !== "page_land") return false;
  const geoLabel = normalizeGeoValue(event.geoLabel);
  if (geoLabel === normalizeGeoValue(INTERNAL_GEO_LABEL)) return true;

  const city = normalizeGeoValue(event.geoCity);
  const region = normalizeGeoValue(event.geoRegion);
  const country = normalizeGeoValue(event.geoCountryName || event.geoCountry);
  return city === "barcelona" && region === "ct" && country === "spain";
}

function buildInternalTrafficMeta(events) {
  ...
  for (const event of events) {
    ...
    if (isInternalGeoEvent(event)) {
      internalSessionIds.add(event.sessionId);
    }
  }
  ...
  for (const event of events) {
    if (!event?.sessionId) continue;
    if (internalSessionIds.has(event.sessionId)) {
  ...
  return {
    internalLocationLabel: INTERNAL_GEO_LABEL,
    internalSessionIds,
    internalSessions: internalSessionIds.size,
    externalSessions: Math.max(0, allSessionIds.size - internalSessionIds.size),
...
export function loadLandingEvents(opts = {}) {
  ...
  if (opts.excludeInternal) {
    const internalTraffic = buildInternalTrafficMeta(events);
    events = events.filter(
      (event) =>
        !event?.sessionId ||
        !internalTraffic.internalSessionIds.has(event.sessionId)
    );
  }
  return events;
}
...
const SECTION_LABELS = {
  ...
  // New Webflow homepage (old-home)
  "oh-how-we-do-it": "How We Do It",
  "how-we-do-it": "How We Do It",
  modules: "Platform Modules",
  pricing: "Pricing",
  ecosystem: "Ecosystem / Globe",
...
const EVENT_LABELS = {
  ...
  platform_video_launcher_impression: "Video Launcher Seen",
  platform_video_launcher_open: "Platform Video Opened",
  platform_video_start: "Platform Video Play",
  platform_video_25: "Platform Video 25%",
  platform_video_50: "Platform Video 50%",
  platform_video_75: "Platform Video 75%",
  platform_video_complete: "Platform Video Finished",
  platform_video_close: "Platform Video Closed",
  platform_video_minimize: "Platform Video Minimized",
  platform_video_dismiss: "Video Launcher Dismissed",
  ...
  insights_article_click: "Insights Article Click",
};
...
const CTA_LABELS = {
  ...
  demo: "Request Demo",
  hero_video: "Watch Overview",
  pricing_owners: "Pricing — Owners",
  pricing_brands: "Pricing — Brands",
  pricing_operators: "Pricing — Operators",
  how_we_do_it: "How We Do It CTA",
  nav_signin: "Nav Sign In",
  nav_cta: "Nav Request Access",
};
...
function buildInteractionsSummary(byEvent, navClicks, personas) {
  const videoOpens =
    (byEvent.hero_video_open || 0) +
    (byEvent.platform_video_launcher_open || 0) +
    (byEvent.platform_video_start || 0);
  const videoCompletes =
    (byEvent.video_complete || 0) + (byEvent.platform_video_complete || 0);
  const items = [
    {
      key: "hero_video_open",
      label: "Video Opens",
...
function buildInsights(aggregate, funnel) {
  ...
  if (sessions === 0) {
    if (aggregate.totals?.insightsSessions > 0) {
      insights.push({
        tone: "info",
        title: "Insights Hub Traffic Only",
        body: `${aggregate.totals.insightsSessions} session${aggregate.totals.insightsSessions === 1 ? "" : "s"} on dealality.com/insights — homepage landing data will appear when embed traffic is recorded.`,
      });
      return insights;
    }
    insights.push({
      ...
      body: "Visit the homepage (Webflow + Railway embed) or Insights hub (/insights) to generate data. Events appear within seconds of a real visit.",
    });
    return insights;
  }
  ...
  if (aggregate.totals?.insightsSessions > 0) {
    insights.push({
      tone: "info",
      title: "Insights Hub Traffic",
      body: `${aggregate.totals.insightsSessions} session${aggregate.totals.insightsSessions === 1 ? "" : "s"} on dealality.com/insights in this window — see the Insights Hub section below.`,
    });
  }
  ...
  if (t.videoCompletes > 0) {
    const avg =
      t.videoAvgWatchSeconds != null ? ` Avg watch ${t.videoAvgWatchSeconds}s.` : "";
    insights.push({
      tone: "good",
      title: "Hero Video Resonance",
      body: `${t.videoCompletes} visitor${t.videoCompletes === 1 ? "" : "s"} watched the full overview video.${avg}`,
    });
  }
...
function humanizeRecentEvent(e) {
  ...
  if (e.language && e.event === "insights_article_click") {
    parts.push(e.language.toUpperCase());
  }
  if (e.destination && e.event === "insights_article_click") parts.push(e.destination);
...
export function aggregateLandingEvents(events, opts = {}) {
  const windowDays =
    typeof opts.windowDays === "number" && opts.windowDays > 0
      ? opts.windowDays
      : null;
  ...
  const funnel = buildFunnelFromEvents(filterEventsBySurface(events, SURFACE_LANDING));
  const geography = buildGeoAggregate(events, sessions.size);
  const utmRows = sortCountMap(utmSources);
  const landingEvents = filterEventsBySurface(events, SURFACE_LANDING);
  const video = buildVideoEngagement(landingEvents);
  const videoOpens = video.sessionsOpened || video.opens;
  const videoCompletes = video.sessionsCompleted || video.completes;
  ...
  const insightsSessionIds = new Set();
  for (const e of events) {
    if (isInsightsEvent(e) && e.sessionId) insightsSessionIds.add(e.sessionId);
  }

  const base = {
    totals: {
      events: events.length,
      sessions: funnel.sessionCount || sessions.size,
      rawSessions: sessions.size,
      insightsSessions: insightsSessionIds.size,
      embedSessions,
      standaloneSessions,
      videoCompletes,
      videoOpens,
      videoCompletionRate: video.completionRate,
      videoAvgWatchSeconds: video.avgWatchSeconds,
      videoMedianWatchSeconds: video.medianWatchSeconds,
    ...
    dailyUniqueUsers: buildDailyUniqueUsers(events, { windowDays }),
    // Audience/acquisition stay landing-scoped; Popular Pages spans all public surfaces.
    dashboard: {
      ...buildLandingDashboard(landingEvents),
      popularPages: buildPopularLandingPages(events),
    },
    insightsHub: buildInsightsHubReport(events),
...
export function buildLandingEventsReport(
  loadOpts = {},
  reportWindow = null,
  filterOpts = {}
) {
  const allEvents = loadLandingEvents({ ...loadOpts, excludeInternal: false });
  const internalTraffic = buildInternalTrafficMeta(allEvents);
  const excludeInternal = Boolean(loadOpts.excludeInternal);
  const loaded = excludeInternal
    ? allEvents.filter(
        (event) =>
          !event?.sessionId ||
          !internalTraffic.internalSessionIds.has(event.sessionId)
    ...
    : allEvents;
  const applied = applyLandingReportFilters(loaded, filterOpts);
  const events = applied.events;
  const windowDays = reportWindow?.days || null;
  const aggregate = aggregateLandingEvents(events, { windowDays });
  const recommendations = buildLandingRecommendations(aggregate, {
    localeCompare: applied.localeCompare,
    localeFilter: applied.filters?.locale || "all",
  });
  ...
  return {
    ...
    recommendations,
    window: {
      days: reportWindow?.days || null,
      label: reportWindow?.label || null,
      since: loadOpts.since || reportWindow?.since || null,
      until: loadOpts.until || reportWindow?.until || null,
      firstEventAt: firstTs,
      lastEventAt: lastTs,
      eventCount: events.length,
      excludedInternal: excludeInternal,
    },
    trafficSegments: {
      internalLocationLabel: internalTraffic.internalLocationLabel,
      internalSessions: internalTraffic.internalSessions,
      externalSessions: internalTraffic.externalSessions,
      internalEvents: internalTraffic.internalEvents,
      externalEvents: internalTraffic.externalEvents,
      excludedInternal: excludeInternal,
    },
    ...aggregate,
  };
