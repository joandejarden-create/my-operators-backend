...
/** "Daniel Shamoon partners with Orange Limited" — capture lead developer/principal. */
const PARTNER_OWNER_RE =
  /\b([A-Z][A-Za-z0-9&.'\-]+(?:\s+[A-Z][A-Za-z0-9&.'\-]+){0,4})\s+partners?\s+with\b/;
...
export function inferAssetProjectStage(eventType, text = "") {
  ...
  if (
    ...
    /\b(under construction|construction|breaks? ground|tops? out|topping out)\b/i.test(text)
  ) {
    return "Construction";
  }
  if (eventType === "Pre-Opening Leadership" || eventType === "Commercialization") {
    return "Construction";
  }
  if (
...
export function extractMarketAlertEntities(input = {}) {
  ...
  if (!ownerDeveloper) {
    const partnerMatch = text.match(PARTNER_OWNER_RE);
    if (partnerMatch) {
      // Prefer person-like "First Last" from end of capture (title/summary join can prepend place names).
      const parts = partnerMatch[1]
        .trim()
        .replace(/[.,;:]+$/g, "")
        .split(/\s+/)
        .filter(Boolean);
      const candidate = parts.slice(-2).join(" ");
      ownerDeveloper = candidate.length >= 3 && candidate.length <= 80 ? candidate : null;
      if (ownerDeveloper && /^(?:Hotel|Resort|New|Luxury|Boutique|Antigua|Caribbean)\b/i.test(ownerDeveloper)) {
        ownerDeveloper = null;
      }
    }
  }
