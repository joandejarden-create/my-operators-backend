/**
 * Wave 17 Batch A — Controlled tab build (three LOW-risk Hyatt brands).
 *
 * Allowed writes: target-brand Presentation rows only.
 * Forbidden: Brand Status, release, CV, Source Library, Registry, Census,
 * Recent Momentum, images, Batch B Hyatt brands, Dream Hotels, Active 65.
 */
...
import {
  ...
  WAVE17_BATCH_A_APPLY_FLAGS,
} from "./brand-explorer-wave17-batch-a-factory-plan.js";
import {
  WAVE17_SAFE_TGS,
  FORBIDDEN_SCENARIO_TITLES,
  WAVE17_BATCH_A_PORTFOLIO_MIX,
  getWave17BatchABrandContent,
} from "./brand-explorer-wave17-batch-a-tab-factory-content.js";
import { listActiveUniverseSlugs } from "./brand-explorer-active-universe.js";

const SAFE_TGS_OPTIONS = WAVE17_SAFE_TGS;

export const WAVE17_BATCH_A_BUILD_VERSION = "wave17-batch-a-controlled-tab-build-v1";

const PRESENTATION_TABLE = "Brand Setup - Brand Explorer Presentation";
const BASICS_TABLE = "Brand Setup - Brand Basics";
const FORBIDDEN_BATCH_A_BRANDS = new Set([
  "caption-by-hyatt",
  "destination-by-hyatt",
  "unbound-collection-by-hyatt",
  "dream-hotels",
]);
const URL_ALLOWED_SLOTS = new Set(["footprint.openings"]);
...
export function parseWave17BatchAFlags(argv = []) {
  const missing = WAVE17_BATCH_A_APPLY_FLAGS.filter((f) => !argv.includes(f));
  return {
...
export function generateWave17BatchAPresentationPack(slug, opts = {}) {
  const content = getWave17BatchABrandContent(slug);
  const pack = content.sourcePack;
  ...
  const parent = content.parent || pack.parentPlatform || "Hyatt Hotels Corporation";
  const loyalty = content.loyalty || "World of Hyatt";
  const pad = lifecyclePad(brandDisplayName, model);

  // Enforce Wave 17 Batch A scenario title rules
  ...
  const presentation = [
    row(
      "Brand Positioning",
      "",
      `${brandDisplayName} is positioned as a ${model}. Owner underwriting should prioritize brand-specific fit, demand alignment, and operating-model readiness over Hyatt Hotels Corporation corporate generalizations or sibling-brand assumptions.`,
      10
    ),
    row(
      "Guest Psychographics Description",
    ...
    row(
      ...
      "Owner Fit",
      `${brandDisplayName} should be chosen when the asset can deliver this positioning more credibly than ${peers}. Assess whether demand mix, product scope, and operating complexity favor ${brandDisplayName} over adjacent Hyatt alternatives in the same tier and geography.`,
      22
    ),
    ...
    row(
      ...
      "Proof Points",
      bullets([
        `Official brand positioning for ${brandDisplayName} is anchored in Hyatt brand and development materials specific to this brand—not Hyatt Hotels Corporation corporate copy`,
    ...
    row(
      ...
      44,
      {
        ...
        caseSummaryInterpretation: `Owner-facing synthesis grounded in official Hyatt brand sources.`,
    ...
    row(
      ...
      "Experience & Identity",
      bullets([
        `${brandDisplayName} guest promise is tied to a clear ${model} rather than a generic Hyatt corporate story`,
    ...
    row(
      ...
      `${brandDisplayName} is most useful when owners explicitly distinguish it from ${peers} instead of treating Hyatt brands as interchangeable lanes.`,
      49
    ),
    row(
    ...
    row(
      ...
      "Portfolio Mix",
      (WAVE17_BATCH_A_PORTFOLIO_MIX[slug] || []).join("\n") +
    ...
    row(
      ...
      ensureMinWords(
        `${removeRawUrls(pack.calaFirstPosture || "Use CALA-first references where available, otherwise label International Reference.")
          ...
          .replace(/\bsource-supported\b/gi, "verified")}. ${brandDisplayName} region cards below summarize owner-relevant geography with explicit CALA or International Reference labeling.`,
        40,
        pad
      ),
      470
    ...
    row(
      ...
      "Growth Editorial",
      ensureMinWords(
        `${brandDisplayName} growth should be evaluated through brand-specific positioning, not Hyatt Hotels Corporation expansion targets alone. ${ownerLens[0] || "Prioritize markets where demand, product, and operator capability align with the brand promise."} ${calaStrong ? `CALA markets (${calaMarkets}) already demonstrate operating proof for this positioning.` : "New CALA market claims should wait for verified property-level evidence with a named hotel and matching official property URL."}`,
        40,
        pad
      ),
      481
    ...
    row(
      ...
      "F&B Complexity",
      `F&B and public-space complexity varies by ${brandDisplayName} site type. Review concept, hours, and operator capability against local demand rather than copying another Hyatt brand's outlet assumptions.`,
      107
    ),
    row(
    ...
    row(
      ...
      ensureMinWords(
        `Standards should protect ${brandDisplayName} brand identity while remaining executable for the specific asset, market, and operator. ${distinctions[0] || "Design, service, and programming standards are brand-specific and should not be borrowed from adjacent Hyatt brands."} Align PIP and lifecycle capital from the asset review.`,
        35,
        pad
      ),
      112
    ...
    row(
      ...
      "Quality Review",
      `Quality review for ${brandDisplayName} should include defined inspection cadence, escalation paths, and remediation timelines. Standards compliance protects brand equity and owner investment through consistent guest-experience delivery.`,
      125
    ),
  ...
  // Batch A: Recent Momentum intentionally omitted (protected — zero writes).
  ...
  for (const p of openingsInputs) {
    ...
    presentation.push(
      row(
        ...
        490 + oi,
        {
          caseSummaryOverview: `${p.propertyName} is a ${geo} reference for ${brandDisplayName} diligence in ${market}.`,
  ...
  presentation.push(
    ...
    row(
      ...
      `Use concise chips and tags tied to owner decision points for ${brandDisplayName}. Keep copy free of raw source links and Hyatt corporate boilerplate. Every tag should help an owner evaluate brand fit for a specific asset and market.`,
      703,
      {
...
function resolveWave17BatchAIdentity(slug) {
  const id = WAVE17_BATCH_A_IDENTITIES[slug];
  if (!id?.recordId || !id?.exactBrandBasicsName) {
    throw new Error(`Missing Wave 16A identity for ${slug}`);
  }
  return {
    slug,
...
function getApprovedBatchAScope() {
  const wanted = [...WAVE17_BATCH_A_APPROVED_SLUGS];
  if (wanted.length !== 3) {
    throw new Error(`Wave 17 Batch A requires exactly three brands, got ${wanted.length}`);
  }
  if (wanted.some((s) => FORBIDDEN_BATCH_A_BRANDS.has(s))) {
    throw new Error("Forbidden brand detected in Batch A scope.");
  }
  return wanted;
}

export async function planWave17BatchABrand(slug, { tgsOptions = [] } = {}) {
  const identity = resolveWave17BatchAIdentity(slug);
  if (
  ...
  if (!WAVE17_BATCH_A_APPROVED_SLUGS.includes(slug)) {
    return {
      brandSlug: slug,
      blocked: true,
      blockers: ["not_wave17_batch_a_target"],
      patches: [],
      basicsPatches: [],
    };
  }
  if (FORBIDDEN_BATCH_A_BRANDS.has(slug)) {
    return {
      brandSlug: slug,
      blocked: true,
      blockers: ["forbidden_stage2a_brand"],
      patches: [],
      basicsPatches: [],
    };
  }

  const pack = generateWave17BatchAPresentationPack(slug, {
  ...
  for (const pRow of pack.presentation) {
    ...
    if (/^footprint\.momentum/i.test(slotKey)) continue; // Batch A protected
  ...
  // Batch A presentation-only — Brand Basics intentionally untouched.
  const basicsPatches = [];
...
const BATCH_A_PROHIBITED_PUBLIC = Object.freeze([
  { id: "census", re: /\bCensus\b/ },
  { id: "fdd", re: /\bFDD\b/ },
  { id: "item_19", re: /\bItem\s*19\b/i },
  { id: "loi", re: /\bLOI\b/ },
  { id: "webhound", re: /\bWebhound\b/i },
  { id: "cvent", re: /\bCvent\b/i },
  { id: "hbx", re: /\bHBX\b/ },
  { id: "hotelbeds", re: /\bHotelbeds\b/i },
  { id: "dataforseo", re: /\bDataForSEO\b/i },
  { id: "pvql", re: /\bPVQL\b/ },
  { id: "qa_token", re: /\bQA\b/ },
  { id: "gate", re: /\bgate\b/i },
  { id: "factory", re: /\bfactory\b/i },
  { id: "process", re: /\bprocess\b/i },
  { id: "validation_lane", re: /\bvalidation lane\b/i },
  { id: "source_supported", re: /\bsource-supported\b/i },
  { id: "source_pack", re: /\bsource pack\b/i },
  { id: "steward", re: /\bsteward\b/i },
  { id: "internal_review", re: /\binternal review\b/i },
  { id: "placeholder", re: /\bplaceholder\b/i },
  { id: "database", re: /\bdatabase\b/i },
  { id: "record_id", re: /\brecord ID\b/i },
  { id: "evidence_lane", re: /\bevidence lane\b/i },
  { id: "provenance", re: /\bprovenance\b/i },
  { id: "implementation_notes", re: /\bimplementation notes\b/i },
  { id: "scrape", re: /\bscrap(?:e|ed|ing)\b/i },
  { id: "raw_url", re: /https?:\/\/\S+/i },
]);

function sentenceIsContrastive(sentence) {
  return /\b(never|not|no|distinct|separate|reject|wrong|do not|don't|unlike|versus|vs\.?|rather than|instead of|without|than|from|against|alternatives?|peers?|compare|comparison|substitut(?:es|e|ing)?|import(?:s|ed|ing)?|weaker|interchangeable|as if|leans)\b/i.test(
    sentence
  );
}

function scanBatchAPublicCopy(text) {
  const hits = [];
  for (const rule of BATCH_A_PROHIBITED_PUBLIC) {
    if (rule.re.test(text)) hits.push(rule.id);
  }
  return hits;
}

function scanContamination(slug, text) {
  const hits = [];
  const sentences = String(text || "").split(/(?<=[.!?])\s+/);
  if (slug === "hyatt-regency") {
    for (const sentence of sentences) {
      // Peer chips like "Grand Hyatt: prestige..." are intentional differentiation.
      if (/^\s*(Similar Brands\n)?Grand Hyatt:/i.test(sentence)) continue;
      if (!/\b(Hyatt Regency|Regency)\b/i.test(sentence)) continue;
      // Fail only when Regency itself is framed as Grand Hyatt prestige/ritual.
      if (
        /\b(Hyatt Regency|Regency)\b/i.test(sentence) &&
        /\bGrand Hyatt\b/i.test(sentence) &&
        /\b(prestige|ritual|luxury flagship)\b/i.test(sentence) &&
        /\b(is|as|delivers|creates|equals|means)\b/i.test(sentence) &&
        !sentenceIsContrastive(sentence) &&
        !/\b(never|without|below|above|than|versus|not)\b/i.test(sentence)
      ) {
        hits.push({ id: "grand_hyatt_as_self", hard: true, sample: sentence.slice(0, 160) });
      }
      if (
        /\b(Marriott Hotels|Sheraton|Westin)\b/i.test(sentence) &&
        /\bHyatt Regency is\b/i.test(sentence) &&
        !sentenceIsContrastive(sentence)
      ) {
        hits.push({ id: "generic_full_service_peer_as_self", hard: true, sample: sentence.slice(0, 160) });
      }
    }
  }
  if (slug === "hyatt-centric") {
    for (const sentence of sentences) {
      if (!/\b(Hyatt Centric|Centric)\b/i.test(sentence)) continue;
      if (
        /\bCentric (is|delivers|creates|fits)\b/i.test(sentence) &&
        /\bmeetings-led\b/i.test(sentence) &&
        !sentenceIsContrastive(sentence)
      ) {
        hits.push({ id: "regency_meetings_as_self", hard: true, sample: sentence.slice(0, 160) });
      }
      if (/\bDream\b/i.test(sentence) && /\bnightlife\b/i.test(sentence) && !sentenceIsContrastive(sentence)) {
        hits.push({ id: "dream_nightlife_as_self", hard: true, sample: sentence.slice(0, 160) });
      }
    }
  }
  if (slug === "thompson-hotels") {
    for (const sentence of sentences) {
      if (!/\bThompson\b/i.test(sentence)) continue;
      if (
        /\bDream\b/i.test(sentence) &&
        /\bnightlife\b/i.test(sentence) &&
        /\bThompson (is|delivers|creates)\b/i.test(sentence) &&
        !sentenceIsContrastive(sentence)
      ) {
        hits.push({ id: "dream_as_self", hard: true, sample: sentence.slice(0, 160) });
      }
      if (
        /\b(EDITION|W Hotels)\b/i.test(sentence) &&
        /\bThompson (is|delivers|creates)\b/i.test(sentence) &&
        !sentenceIsContrastive(sentence)
      ) {
        hits.push({ id: "edition_w_as_self", hard: true, sample: sentence.slice(0, 160) });
      }
    }
  }
  return hits;
...
function countMomentumWrites(patches) {
  return (patches || []).filter((p) => /^footprint\.momentum/i.test(nz(p.slotKey))).length;
}

function countImageWrites(patches) {
  return (patches || []).filter((p) =>
    Object.keys(p.fields || {}).some((k) => /image/i.test(k))
  ).length;
}

function countBasicsWrites(patches) {
  return (patches || []).filter((p) => p.table === BASICS_TABLE).length;
}

async function runBatchAIdentityPreflight() {
  const activeSlugs = await listActiveUniverseSlugs();
  const activeCount = activeSlugs.length;
  const brands = {};
  const blockers = [];

  if (activeCount !== WAVE17_PROTECTED_ACTIVE_COUNT) {
    blockers.push(`active_universe_not_65:${activeCount}`);
  }
  ...
  for (const slug of WAVE17_BATCH_A_APPROVED_SLUGS) {
    ...
    const status = nz(fields["Brand Status"]);
    const name = nz(fields.Name || fields["Brand Name"] || identity.name);
    const parent = nz(fields["Parent Company"] || fields.Parent || identity.name);
    brands[slug] = {
      slug,
      recordId: identity.recordId,
      exactBrandBasicsName: name,
      brandStatus: status,
      parentCompany: parent || WAVE17_BATCH_A_IDENTITIES[slug].parentCompany,
      underReview: status === "Under Review",
    };
    if (status !== "Under Review") {
      blockers.push(`not_under_review:${slug}:${status || "missing"}`);
    }
    if (activeSlugs.includes(slug)) {
      blockers.push(`target_in_active_universe:${slug}`);
    }
    await sleep(120);
  }

  const outOfScope = {};
  for (const [slug, meta] of Object.entries(WAVE17_BATCH_A_OUT_OF_SCOPE)) {
    const basics = await fetchBasicsRecord(meta.recordId);
    const status = nz(basics?.fields?.["Brand Status"]);
    outOfScope[slug] = {
      recordId: meta.recordId,
      name: meta.name,
      brandStatus: status,
      inActiveUniverse: activeSlugs.includes(slug),
    };
    if (WAVE17_BATCH_A_APPROVED_SLUGS.includes(slug)) {
      blockers.push(`out_of_scope_in_approved:${slug}`);
    }
    await sleep(80);
  }
  ...
  return {
    ...
    blockers,
    activeUniverseBefore: activeCount,
    brands,
    outOfScope,
  };
}

function auditBrandSemantics(slug, presentationRows) {
  const corpus = (presentationRows || [])
    .map((r) => `${nz(r.title)}\n${nz(r.body)}`)
    .join("\n");
  const prohibited = scanBatchAPublicCopy(corpus);
  const contamination = scanContamination(slug, corpus);
  const hardContamination = contamination.filter((c) => c.hard);
  return {
    slug,
    prohibitedLanguageHits: prohibited,
    contaminationHits: contamination,
    pass: prohibited.length === 0 && hardContamination.length === 0,
  };
}

export async function planWave17BatchABuild({ preflight = null } = {}) {
  const scope = getApprovedBatchAScope();
  const tgsOptions = await listTargetGuestSegmentOptions();
  const brandResults = [];
  for (const slug of scope) {
    const brand = await planWave17BatchABrand(slug, { tgsOptions });
    const pack = generateWave17BatchAPresentationPack(slug, {
      airtableName: brand.brandName,
      recordId: brand.recordId,
    });
    const semantic = auditBrandSemantics(slug, pack.presentation);
    if (!semantic.pass) {
      brand.blockers = [
        ...(brand.blockers || []),
        ...semantic.prohibitedLanguageHits.map((h) => `prohibited:${h}`),
        ...semantic.contaminationHits.filter((c) => c.hard).map((c) => `contamination:${c.id}`),
      ];
      brand.blocked = true;
    }
    brand.semantic = semantic;
    brand.momentumWriteCount = countMomentumWrites(brand.patches);
    brand.imageWriteCount = countImageWrites(brand.patches);
    brand.basicsWriteCount = countBasicsWrites(brand.basicsPatches);

    // Lightweight brand-level audits (image/Momentum gates deferred)
    const openings = (pack.presentation || []).filter((r) => nz(r.slotKey) === "footprint.openings");
    const scenarios = (pack.presentation || []).filter((r) =>
      /^(overview\.scenario\.|valueOwners\.scenario\.)/i.test(nz(r.slotKey))
    );
    const emptyBodies = (pack.presentation || []).filter(
      (r) =>
        !nz(r.body) &&
        !/^footprint\.momentum/i.test(nz(r.slotKey)) &&
        !/^materials\.gallery/i.test(nz(r.slotKey))
    );
    const brandToken = slug.split("-")[0];
    const propertyIdentityFail = openings.some((r) => {
      const title = nz(r.title).toLowerCase();
      if (slug === "hyatt-regency") return !/hyatt regency/.test(title);
      if (slug === "hyatt-centric") return !/hyatt centric/.test(title);
      if (slug === "thompson-hotels") return !/thompson/.test(title);
      return !title.includes(brandToken);
    });
    brand.audits = {
      goldenContent: {
        pass: scenarios.length >= 7 && openings.length >= 3 && emptyBodies.length === 0,
        scenarioCount: scenarios.length,
        openingsCount: openings.length,
      },
      sourceProvenance: {
        pass:
          !!pack.sourcePackMeta?.officialBrandPage &&
          (pack.sourcePackMeta?.propertyExampleCount || 0) >= 3,
        officialBrandPage: pack.sourcePackMeta?.officialBrandPage || null,
        propertyExampleCount: pack.sourcePackMeta?.propertyExampleCount || 0,
      },
      propertyIdentity: {
        pass: openings.length >= 3 && !propertyIdentityFail,
        openingsTitles: openings.map((r) => r.title),
      },
      noEmptyExcludingDeferred: {
        pass: emptyBodies.length === 0,
        emptyCount: emptyBodies.length,
        deferredSlots: ["footprint.momentum*", "materials.gallery* (images deferred)"],
      },
      deferred: {
        images: "deferred",
        recentMomentum: "deferred_protected",
      },
    };
    if (!brand.audits.goldenContent.pass) {
      brand.blockers.push("golden_content_fail");
      brand.blocked = true;
    }
    if (!brand.audits.sourceProvenance.pass) {
      brand.blockers.push("source_provenance_fail");
      brand.blocked = true;
    }
    if (!brand.audits.propertyIdentity.pass) {
      brand.blockers.push("property_identity_fail");
      brand.blocked = true;
    }
    if (!brand.audits.noEmptyExcludingDeferred.pass) {
      brand.blockers.push("no_empty_fail");
      brand.blocked = true;
    }
    if (brand.momentumWriteCount > 0) {
      brand.blockers.push("recent_momentum_writes_proposed");
      brand.blocked = true;
    }
    if (brand.imageWriteCount > 0) {
      brand.blockers.push("image_writes_proposed");
      brand.blocked = true;
    }
    if (brand.basicsWriteCount > 0) {
      brand.blockers.push("basics_writes_proposed");
      brand.blocked = true;
    }
    for (const o of Object.values(WAVE17_BATCH_A_OUT_OF_SCOPE)) {
      if (brand.recordId === o.recordId) {
        brand.blockers.push(`out_of_scope_record_targeted:${o.slug}`);
        brand.blocked = true;
      }
    }
  ...
  const corpuses = Object.fromEntries(
    brandResults.map((b) => {
      const pack = generateWave17BatchAPresentationPack(b.brandSlug, {
        airtableName: b.brandName,
        recordId: b.recordId,
      });
      return [
        b.brandSlug,
        (pack.presentation || []).map((r) => `${nz(r.title)}\n${nz(r.body)}`).join("\n"),
      ];
    })
  );
  const crossBrandSemantic = {
    regencyVsCentric: {
      pass:
        /meetings|full-service|group/i.test(corpuses["hyatt-regency"] || "") &&
        /lifestyle|explorer|location|neighborhood/i.test(corpuses["hyatt-centric"] || "") &&
        !/(Hyatt Centric is a core upper-upscale full-service brand with meetings)/i.test(
          corpuses["hyatt-centric"] || ""
        ),
      note: "Regency meetings/full-service vs Centric location-led lifestyle",
    },
    centricVsThompson: {
      pass:
        /explorer|location|neighborhood/i.test(corpuses["hyatt-centric"] || "") &&
        /design-led|F&B|cultural|social energy/i.test(corpuses["thompson-hotels"] || "") &&
        !/Dream nightlife-led identity/i.test(
          (corpuses["thompson-hotels"] || "").replace(/never as Dream|not as Dream|distinct from Dream/gi, "")
        ),
      note: "Centric explorer lifestyle vs Thompson design-led F&B/cultural intensity",
    },
    regencyVsThompson: {
      pass:
        /meetings|group|convention/i.test(corpuses["hyatt-regency"] || "") &&
        /design-led|cultural|lifestyle/i.test(corpuses["thompson-hotels"] || ""),
      note: "Regency productive meetings vs Thompson design-led lifestyle",
    },
    contaminationWatch: [
      "Grand Hyatt",
      "Hyatt Place",
  ...
  crossBrandSemantic.pass = Object.values(crossBrandSemantic)
  ...
  for (const b of brandResults) {
    for (const p of b.patches || []) {
      writeDiff.push({
        ...
        table: p.table,
        action: p.action,
        presentationRecordId: p.recordId,
        slotKey: p.slotKey,
        fields: Object.keys(p.fields || {}),
        bodyPreview: String(p.fields?.Body || "").slice(0, 160),
        writeType: p.action,
        allowed: !b.blocked && !/^footprint\.momentum/i.test(nz(p.slotKey)),
        semanticRisk: (b.semantic?.contaminationHits || []).map((c) => c.id).join(",") || "low",
        sourceFoundationReference: `wave17-hyatt-foundation:${b.brandSlug}`,
      });
    }
  }

  return {
    version: WAVE17_BATCH_A_BUILD_VERSION,
    factoryVersion: WAVE17_BATCH_A_VERSION,
    stage: "batch-a-controlled-tab-build",
    ...
    approvedScope: scope,
    excluded: {
      batch_b_hyatt: "caption_destination_unbound_out_of_scope",
      dream_hotels: "identity_pending_out_of_scope",
      active_65: "read_only_regression_only",
      recent_momentum: "protected_zero_writes",
      images: "deferred",
    },
    brandResults,
    crossBrandSemantic,
    ...
    summary: {
      ...
      plannedMomentumWrites: brandResults.reduce((n, b) => n + (b.momentumWriteCount || 0), 0),
      plannedImageWrites: brandResults.reduce((n, b) => n + (b.imageWriteCount || 0), 0),
      blockedSlugs: brandResults.filter((b) => b.blocked).map((b) => b.brandSlug),
    },
    requiredApplyFlags: [...WAVE17_BATCH_A_APPLY_FLAGS],
  };
}

export async function applyWave17BatchABuild({ plan, apply = false, argv = [] } = {}) {
  const flagCheck = parseWave17BatchAFlags(argv);
  ...
  if (plan.preflight && plan.preflight.ok === false) {
    return { applied: false, reason: "preflight_failed", blockers: plan.preflight.blockers, flagCheck };
  }
  if ((plan.summary?.plannedMomentumWrites || 0) > 0) {
    return { applied: false, reason: "recent_momentum_writes_blocked", flagCheck };
  }
  if ((plan.summary?.plannedImageWrites || 0) > 0) {
    return { applied: false, reason: "image_writes_blocked", flagCheck };
  }
  if ((plan.summary?.plannedBasicsWrites || 0) > 0) {
    return { applied: false, reason: "basics_writes_blocked", flagCheck };
  }
  ...
  let stopShared = false;
  for (const brand of plan.brandResults || []) {
    if (stopShared) {
      resultsByBrand[brand.brandSlug] = { applied: false, reason: "stopped_after_shared_failure" };
      continue;
    }
    if (brand.blocked) {
    ...
    if (!WAVE17_BATCH_A_APPROVED_SLUGS.includes(brand.brandSlug)) {
      throw new Error(`Refusing non-target brand write: ${brand.brandSlug}`);
    }
    for (const o of Object.values(WAVE17_BATCH_A_OUT_OF_SCOPE)) {
      if (brand.recordId === o.recordId) throw new Error(`Refusing out-of-scope write: ${o.slug}`);
    }
    ...
    for (const patch of brand.patches || []) {
      if (patch.table === BASICS_TABLE) {
        throw new Error(`Presentation-only Batch A refused Basics write for ${brand.brandSlug}`);
      }
      if (/^footprint\.momentum/i.test(nz(patch.slotKey))) {
        throw new Error(`Recent Momentum write refused for ${brand.brandSlug}`);
      }
      for (const key of Object.keys(patch.fields || {})) {
        if (FORBIDDEN_WRITE_FIELDS.has(key)) throw new Error(`Forbidden field write: ${key}`);
        if (/image/i.test(key)) throw new Error(`Image field write refused: ${key}`);
      }
      try {
    ...
    resultsByBrand[brand.brandSlug] = {
      ...
      basicsUpdated: [],
      errors,
      writeCount: created.length + updated.length,
      releaseFieldsWritten: false,
      brandStatusUntouched: true,
      companyValidatedUntouched: true,
      imagesWritten: false,
      recentMomentumWritten: false,
    };
    if (!applied) {
      const sharedHint = errors.some((e) =>
        /forbidden|refused|Presentation-only|Recent Momentum|Image field/i.test(e.error || "")
      );
      if (sharedHint) stopShared = true;
    }
  }

  const activeAfter = (await listActiveUniverseSlugs()).length;
  return {
    applied: Object.values(resultsByBrand).some((r) => r.applied),
    allThreeApplied: WAVE17_BATCH_A_APPROVED_SLUGS.every((s) => resultsByBrand[s]?.applied),
    reason: "wave17_batch_a_controlled_tab_build_applied",
    flagCheck,
    resultsByBrand,
    activeUniverseAfter: activeAfter,
    companyValidatedUntouched: true,
    ...
    imagesWritten: false,
    recentMomentumWritten: false,
    protected65Untouched: activeAfter === WAVE17_PROTECTED_ACTIVE_COUNT,
    stopShared,
  };
}

function writeBrandMd(brand, reportsDir, applyResult = null) {
  const slug = brand.reportSlug || brand.brandSlug;
  const mdPath = path.join(reportsDir, `brand-explorer-wave17-batch-a-${slug}.md`);
  const applied = applyResult?.resultsByBrand?.[slug];
  const lines = [
    `# Wave 17 Batch A Controlled Tab Build — ${brand.brandName || slug}`,
    "",
    `- Slug: \`${slug}\``,
    ...
    `- Recent Momentum writes: ${brand.momentumWriteCount ?? 0}`,
    `- Image writes: ${brand.imageWriteCount ?? 0}`,
    `- Semantic pass: **${brand.semantic?.pass === true}**`,
    `- Golden content: **${brand.audits?.goldenContent?.pass === true}**`,
    `- Source provenance: **${brand.audits?.sourceProvenance?.pass === true}**`,
    `- Property identity: **${brand.audits?.propertyIdentity?.pass === true}**`,
    `- No-empty (excl. deferred image/Momentum): **${brand.audits?.noEmptyExcludingDeferred?.pass === true}**`,
    `- Blocked: **${brand.blocked === true}**`,
    `- Applied: **${applied?.applied === true}**`,
    `- Applied write count: ${applied?.writeCount ?? 0}`,
    "",
    "## Blockers",
    "",
    ...
    "## Semantic",
    "",
    `- Prohibited language: ${(brand.semantic?.prohibitedLanguageHits || []).join(", ") || "none"}`,
    `- Contamination: ${(brand.semantic?.contaminationHits || []).map((c) => c.id).join(", ") || "none"}`,
    "",
    "## Deferred",
    "",
    "- Recent Momentum (protected)",
...
export function writeWave17BatchAReports(plan, applyResult = null, meta = {}) {
  fs.mkdirSync(REPORTS_DIR, { recursive: true });
  fs.mkdirSync(DOCS_DIR, { recursive: true });

  const packPaths = [];
  for (const brand of plan.brandResults || []) {
    packPaths.push(writeBrandMd(brand, REPORTS_DIR, applyResult));
  }

  const writeDiffPath = path.join(REPORTS_DIR, "brand-explorer-wave17-batch-a-write-diff.json");
  fs.writeFileSync(writeDiffPath, `${JSON.stringify(plan.writeDiff || [], null, 2)}\n`, "utf8");

  const semanticPath = path.join(REPORTS_DIR, "brand-explorer-wave17-batch-a-semantic-audit.md");
  const semanticLines = [
    "# Wave 17 Batch A — Semantic Audit",
    "",
    `Generated: ${plan.generatedAt}`,
    "",
    "| Brand | Prohibited | Contamination | Pass |",
    "| --- | --- | --- | --- |",
    ...(plan.brandResults || []).map((b) => {
      const pro = (b.semantic?.prohibitedLanguageHits || []).join(", ") || "none";
      const con = (b.semantic?.contaminationHits || []).map((c) => c.id).join(", ") || "none";
      return `| \`${b.brandSlug}\` | ${pro} | ${con} | ${b.semantic?.pass === true} |`;
    }),
    "",
    "## Cross-brand differentiation",
    "",
    `- Overall pass: **${plan.crossBrandSemantic?.pass === true}**`,
    `- Regency vs Centric: **${plan.crossBrandSemantic?.regencyVsCentric?.pass === true}** — ${plan.crossBrandSemantic?.regencyVsCentric?.note || ""}`,
    `- Centric vs Thompson: **${plan.crossBrandSemantic?.centricVsThompson?.pass === true}** — ${plan.crossBrandSemantic?.centricVsThompson?.note || ""}`,
    `- Regency vs Thompson: **${plan.crossBrandSemantic?.regencyVsThompson?.pass === true}** — ${plan.crossBrandSemantic?.regencyVsThompson?.note || ""}`,
    `- Contamination watch: ${(plan.crossBrandSemantic?.contaminationWatch || []).join(", ")}`,
    "",
  ];
  fs.writeFileSync(semanticPath, semanticLines.join("\n"), "utf8");
  ...
  const jsonPath = path.join(REPORTS_DIR, "brand-explorer-wave17-batch-a-controlled-tab-build.json");
  fs.writeFileSync(jsonPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");

  const mdPath = path.join(REPORTS_DIR, "brand-explorer-wave17-batch-a-controlled-tab-build.md");
  const md = [
    "# Brand Explorer Wave 17 Batch A — Controlled Tab Build",
    "",
    `Generated: ${plan.generatedAt}`,
    `Ready: **${meta.readyStatement || "—"}**`,
    `Dry-run: **${!applyResult?.applied}** · Applied: **${applyResult?.applied === true}**`,
    `Active universe before/after: **${plan.preflight?.activeUniverseBefore ?? "—"}** / **${applyResult?.activeUniverseAfter ?? plan.preflight?.activeUniverseBefore ?? "—"}**`,
    `Scope: **${(plan.approvedScope || []).join(", ")}**`,
    `Presentation writes planned: **${plan.summary?.plannedPresentationWrites ?? 0}**`,
    `Basics / Momentum / Image writes planned: **${plan.summary?.plannedBasicsWrites ?? 0}** / **${plan.summary?.plannedMomentumWrites ?? 0}** / **${plan.summary?.plannedImageWrites ?? 0}**`,
    ...
    `| --- | --- | ---: | --- | ---: |`,
    ...(plan.brandResults || []).map((b) => {
      const st = plan.preflight?.brands?.[b.brandSlug]?.brandStatus || "—";
      const aw = applyResult?.resultsByBrand?.[b.brandSlug]?.writeCount ?? 0;
      return `| \`${b.brandSlug}\` | ${st} | ${b.patches?.length ?? 0} | ${b.semantic?.pass === true} | ${aw} |`;
    }),
    "",
    "## Deferred",
    "",
    "- Image materialization",
    "- Recent Momentum",
    "- Batch B (Caption / Destination / Unbound)",
    "- Dream Hotels readiness scoring / build",
    "",
  ].join("\n");
  fs.writeFileSync(mdPath, md, "utf8");

  const docPath = path.join(DOCS_DIR, "brand-explorer-wave17-batch-a-controlled-tab-build.md");
  fs.writeFileSync(
    docPath,
    [
      "# Wave 17 Batch A Controlled Tab Build",
      "",
      "LOW-risk Hyatt cohort only: Hyatt Regency, Hyatt Centric, Thompson Hotels.",
      "",
      "## Scope",
      "",
      ...WAVE17_BATCH_A_APPROVED_SLUGS.map((s) => `- \`${s}\``),
      "",
      "## Guards",
      "",
      "- Presentation-only writes",
      "- Zero Recent Momentum / image / Brand Status / release / CV / Census / Registry writes",
      "- Batch B and Dream Hotels out of scope",
      "- Active 65 protected",
      "",
      "## Ready statement (success)",
      "",
      "`wave17_batch_a_low_risk_tab_build_ready_for_image_stage`",
      "",
    ].join("\n"),
    "utf8"
  );

  return { jsonPath, mdPath, writeDiffPath, semanticPath, docPath, packPaths };
}

export async function runWave17BatchAControlledTabBuild({ dryRun = true, argv = [] } = {}) {
  const preflight = await runBatchAIdentityPreflight();
  if (!preflight.ok) {
    const plan = {
      version: WAVE17_BATCH_A_BUILD_VERSION,
      stage: "batch-a-controlled-tab-build",
      generatedAt: new Date().toISOString(),
      preflight,
      approvedScope: [...WAVE17_BATCH_A_APPROVED_SLUGS],
      brandResults: [],
      writeDiff: [],
      summary: { brandCount: 0, blockedSlugs: [...WAVE17_BATCH_A_APPROVED_SLUGS], plannedPresentationWrites: 0, plannedBasicsWrites: 0, plannedMomentumWrites: 0, plannedImageWrites: 0 },
      tgsValidated: false,
    };
    const readyStatement =
      preflight.activeUniverseBefore !== WAVE17_PROTECTED_ACTIVE_COUNT
        ? "wave17_batch_a_blocked_active_baseline_regression"
        : "wave17_batch_a_blocked_shared_architecture_issue";
    const paths = writeWave17BatchAReports(plan, null, { pass: false, readyStatement });
    return {
      ...plan,
      pass: false,
  ...
  const plan = await planWave17BatchABuild({ preflight });
  let applyResult = null;
  if (!dryRun && argv.includes("--apply")) {
    applyResult = await applyWave17BatchABuild({ plan, apply: true, argv });
  }

  const activeAfter =
    applyResult?.activeUniverseAfter ??
    (await listActiveUniverseSlugs()).length;
  const blocked = (plan.summary?.blockedSlugs || []).length > 0;
  const allSemanticPass = (plan.brandResults || []).every((b) => b.semantic?.pass === true);
  const allAuditsPass = (plan.brandResults || []).every(
    (b) =>
      b.audits?.goldenContent?.pass === true &&
      b.audits?.sourceProvenance?.pass === true &&
      b.audits?.propertyIdentity?.pass === true &&
      b.audits?.noEmptyExcludingDeferred?.pass === true
  );
  const crossPass = plan.crossBrandSemantic?.pass === true;
  const appliedCount = WAVE17_BATCH_A_APPROVED_SLUGS.filter(
    (s) => applyResult?.resultsByBrand?.[s]?.applied
  ).length;
  const sharedIssue = applyResult?.stopShared === true;
  const activeRegression = activeAfter !== WAVE17_PROTECTED_ACTIVE_COUNT;

  let readyStatement = "wave17_batch_a_dry_run_ready";
  if (activeRegression) readyStatement = "wave17_batch_a_blocked_active_baseline_regression";
  else if (sharedIssue) readyStatement = "wave17_batch_a_blocked_shared_architecture_issue";
  else if (
    applyResult?.applied &&
    appliedCount === 3 &&
    !blocked &&
    ...
    readyStatement = "wave17_batch_a_low_risk_tab_build_ready_for_image_stage";
  } else if (applyResult?.applied && appliedCount > 0 && appliedCount < 3) {
    readyStatement = "wave17_batch_a_partial_tab_build_remediation_required";
  } else if (blocked) readyStatement = "wave17_batch_a_build_blocked";
  ...
  const pass =
    ...
    allAuditsPass &&
    crossPass &&
    plan.tgsValidated === true &&
    (plan.summary?.brandCount || 0) === 3 &&
    !activeRegression &&
    (dryRun || appliedCount === 3);

  const paths = writeWave17BatchAReports(plan, applyResult, {
    pass,
    readyStatement,
    activeUniverseAfter: activeAfter,
  });
  ...
  return {
    ...
    pass,
    readyStatement,
    stopRecommended: blocked || !allSemanticPass || activeRegression || sharedIssue,
    activeUniverseBefore: preflight.activeUniverseBefore,
    activeUniverseAfter: activeAfter,
  };
}
