/**
 * Brand Explorer — protected 39 Active/Live public-full baseline freeze.
 *
 * Supersedes protected 27 Active/Live public-full freeze after Wave 12
 * (12 new brands) + final freeze-blocker cleanup.
...
export const BASELINE_VERSION_39 = "39-active-public-full-baseline-v1";
export const EXPECTED_ACTIVE_COUNT_39 = 39;
export const EXPECTED_QUALITY_RECOMMENDATION = "approve_for_baseline_freeze";

export const REPORT_JSON_39 = "brand-explorer-39-active-public-full-baseline.json";
export const REPORT_MD_39 = "brand-explorer-39-active-public-full-baseline.md";
export const DOCS_MD_39 = "brand-explorer-39-active-public-full-baseline.md";
...
/** Wave 12 brands promoted into the 39 Active/Live public-full freeze. */
export const BASELINE_39_WAVE12 = Object.freeze(
  WAVE12_SLUGS.map((slug) =>
    Object.freeze({
      slug,
      recordId: null,
      name: WAVE12_BRAND_PLAN[slug]?.name || slug,
      wave: 12,
      parentPlatform: WAVE12_BRAND_PLAN[slug]?.parentPlatform || null,
    })
  )
);

const NEW_WAVE_SLUGS = new Set(BASELINE_39_WAVE12.map((b) => b.slug));
const STALE_UNIVERSE_COUNTS = Object.freeze([23, 24, 25, 27]);
 ...
 */
export async function build39ActivePublicFullBaseline({
  requireReports = true,
    ...
    brands.push({
      ...
      recentMomentumEvidenceQualityStatus: evidence
        ...
        : NEW_WAVE_SLUGS.has(u.slug)
          ? "n_a_wave12_covered_by_mandatory_gate"
          : MOMENTUM_EVIDENCE_TARGET_SLUGS.includes(u.slug)
            ? "not_evaluated"
            : "n_a_protected_prior",
      ...
      addedInWave12: NEW_WAVE_SLUGS.has(u.slug),
      notes: NEW_WAVE_SLUGS.has(u.slug)
        ? ["wave12_public_full_baseline_39"]
        : ["carried_from_protected_27_baseline"],
      gateHits: {
  ...
  const freezeDecision =
    ...
    brands.length === EXPECTED_ACTIVE_COUNT_39 &&
    ...
    excluded.every((e) => e.includedInBaseline === false && e.isActiveLive !== true)
      ? "frozen_39_active_public_full_baseline"
      : "freeze_incomplete";

  const newWave = BASELINE_39_WAVE12.map((meta) => {
  ...
  return {
    ...
    wave12: {
      protectedBaselineBefore: WAVE12_PROTECTED_BASELINE_COUNT,
      expectedFinalActiveCount: WAVE12_EXPECTED_FINAL_ACTIVE_COUNT,
      slugCount: WAVE12_SLUGS.length,
      slugs: [...WAVE12_SLUGS],
      brands: newWave,
      finalBlockerCleanup: cleanup39
        ? {
            file: CLEANUP_REPORT_JSON_39,
            readyStatement: cleanup39.readyStatement || null,
            generatedAt: cleanup39.generatedAt || null,
          }
        : null,
    },
    ...
    frozen: freezeDecision === "frozen_39_active_public_full_baseline",
    freezeDecision,
    ...
    primaryReleaseNote:
      "PRIMARY_RELEASE_SLUGS is an operational overlay, not the Active/Live universe (39).",
    brands,
    ...
    summary: {
      ...
      wave12Count: newWave.length,
      wave12IncludedCount: newWave.filter((b) => b.included).length,
    },
    regressionRules: [
...
export function evaluate39ActivePublicFullBaselineRegression({
  frozen,
  liveUniverse,
  livePvql,
  liveQuality,
  ...
  if (liveUniverse?.totalCount !== EXPECTED_ACTIVE_COUNT_39) {
    failures.push(
      `active_universe_count_changed:${liveUniverse?.totalCount}_expected_${EXPECTED_ACTIVE_COUNT_39}`
    );
  }
  ...
  if (STALE_UNIVERSE_COUNTS.includes(liveUniverse?.totalCount) && liveUniverse?.totalCount !== EXPECTED_ACTIVE_COUNT_39) {
    failures.push(`stale_${liveUniverse.totalCount}_brand_list_used_as_universe_expected_${EXPECTED_ACTIVE_COUNT_39}`);
  }
  ...
  if (liveQuality?.brandResults) {
    ...
    if (freezeCount !== EXPECTED_ACTIVE_COUNT_39) {
      failures.push(`quality_freeze_count:${freezeCount}_expected_${EXPECTED_ACTIVE_COUNT_39}`);
    }
  } else {
  ...
  if (!livePvql?.brands?.length) {
    ...
    if (publicFullCount !== EXPECTED_ACTIVE_COUNT_39) {
      failures.push(`pvql_public_full_count:${publicFullCount}_expected_${EXPECTED_ACTIVE_COUNT_39}`);
    }
    const publicFullPass = (livePvql.brands || []).filter(
      (b) => b.publicFullProfile && b.lockPass
    ).length;
    if (publicFullPass !== EXPECTED_ACTIVE_COUNT_39) {
      failures.push(`pvql_public_full_pass_count:${publicFullPass}_expected_${EXPECTED_ACTIVE_COUNT_39}`);
    }
...
export async function run39ActivePublicFullBaselineRegression({
  reassessPvql = true,
  ...
  const frozenPath = path.join(REPORTS_DIR, REPORT_JSON_39);
  if (!fs.existsSync(frozenPath)) {
    throw new Error(`Frozen baseline missing: ${REPORT_JSON_39}. Run freeze --dry-run first.`);
  }
  ...
  // Public-full lockPass is authoritative for the 39 baseline. Do not require
  ...
  const pvqlCoversFrozen =
    publicFullRows.length === EXPECTED_ACTIVE_COUNT_39 &&
    existingPvql?.brands?.length >= EXPECTED_ACTIVE_COUNT_39 &&
  ...
  // for all 39. Wave preflight must run a fresh PVQL command first and must
  ...
  const regression = evaluate39ActivePublicFullBaselineRegression({
    frozen,
    liveUniverse,
    livePvql,
    liveQuality,
...
export function write39ActivePublicFullBaselineReports(report) {
  fs.mkdirSync(REPORTS_DIR, { recursive: true });
  fs.mkdirSync(DOCS_DIR, { recursive: true });

  const jsonPath = path.join(REPORTS_DIR, REPORT_JSON_39);
  const mdPath = path.join(REPORTS_DIR, REPORT_MD_39);
  const docsPath = path.join(DOCS_DIR, DOCS_MD_39);

  fs.writeFileSync(jsonPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");
  const md = render39BaselineMarkdown(report);
  fs.writeFileSync(mdPath, md, "utf8");
  fs.writeFileSync(docsPath, md, "utf8");
  return { jsonPath, mdPath, docsPath };
}

function render39BaselineMarkdown(report) {
  const lines = [];
  lines.push(`# Brand Explorer — Protected 39 Active/Live Public-Full Baseline`);
  lines.push("");
  ...
  lines.push("");
  lines.push(
    `This freeze locks the **${report.activeCount}** Brand Basics Active/Live Brand Explorer profiles that are public-full, PVQL-clean, and quality \`approve_for_baseline_freeze\` after Wave 12 + final freeze-blocker cleanup.`
  );
  lines.push("");
  ...
  lines.push(`| Wave 12 included | ${report.summary.wave12IncludedCount}/${report.summary.wave12Count} |`);
  ...
  lines.push(`## 3. 39-brand baseline table`);
  lines.push("");
  lines.push(
  ...
  lines.push(`## 4. Wave 12 summary`);
  lines.push("");
  lines.push(
    `Wave 12 promoted **${report.wave12?.slugCount || 12}** brands from the protected **${report.wave12?.protectedBaselineBefore || 27}** baseline to the **${report.wave12?.expectedFinalActiveCount || 39}** Active/Live public-full universe.`
  );
  lines.push("");
  lines.push(`| Brand | Slug | Parent | Status | Full | PVQL | Quality | Uniq | Role |`);
  lines.push(`|-------|------|--------|--------|------|------|---------|------|------|`);
  for (const b of report.newWaveBrands || []) {
    lines.push(
      `| ${b.name} | \`${b.slug}\` | ${b.parentPlatform || "—"} | ${b.brandStatus || "—"} | ${b.shouldRenderFullProfile} | ${b.pvqlStatus || "—"} | ${b.qualityRecommendation || "—"} | ${b.imageUniquenessStatus || "—"} | ${b.imageRoleMatchStatus || "—"} |`
    );
  }
  lines.push("");

  lines.push(`## 5. Final blocker cleanup summary`);
  lines.push("");
  const cleanup = report.validationSources?.finalBlockerCleanup || report.wave12?.finalBlockerCleanup;
  lines.push(`- Artifact: \`${cleanup?.file || CLEANUP_REPORT_JSON_39}\``);
  lines.push(`- Ready statement: **${cleanup?.readyStatement || "—"}**`);
  ...
  lines.push(`- Protected fields remained untouched during cleanup.`);
  lines.push("");
  ...
  lines.push(`## 7. Evidence quality result`);
  lines.push("");
  ...
  lines.push(`## 8. Image uniqueness / role-match result`);
  lines.push("");
  ...
  lines.push(`## 9. Excluded non-active brands`);
  lines.push("");
  ...
  lines.push(`## 10. Protected fields`);
  lines.push("");
  ...
  lines.push(`## 11. Regression rules`);
  lines.push("");
  for (const r of report.regressionRules || []) lines.push(`- ${r}`);
  lines.push("");
  lines.push(`Test: \`npm run test:brand-explorer-39-active-public-full-baseline\``);
  lines.push("");

  lines.push(`## 12. Rollback notes`);
  lines.push("");
  for (const r of report.rollbackNotes || []) lines.push(`- ${r}`);
  lines.push("");

  lines.push(`## 13. Future factory rules`);
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("npm run brand-explorer-39-active-public-full-baseline -- --dry-run");
  lines.push("npm run test:brand-explorer-39-active-public-full-baseline");
...
export {
  BASELINE_VERSION_39 as BASELINE_VERSION,
  EXPECTED_ACTIVE_COUNT_39 as EXPECTED_ACTIVE_COUNT,
  PROTECTED_FIELDS,
  ROOT,
};
