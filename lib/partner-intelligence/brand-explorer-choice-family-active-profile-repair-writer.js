 ...
 *
 * Dry-run / gated apply for Ascend Hotel Collection and Radisson Blu by Choice:
 ...
 * - parent-aware carryover false-positive cleanup (design hotels phrase, consumer site)
 * - classify pending facts (no auto-approval unless separate gate)
 *
 * @see docs/data-intelligence/brand-explorer-choice-family-active-profile-repair-writer-v29A.md
 */
...
export const WRITER_VERSION = "29A";
export const REPORT_JSON_NAME = "brand-explorer-choice-family-active-profile-repair-writer.json";
export const REPORT_MD_NAME = "brand-explorer-choice-family-active-profile-repair-writer.md";
export const DOC_MD_NAME = "brand-explorer-choice-family-active-profile-repair-writer-v29A.md";
...
export const APPLY_FLAG_APPROVE = "--approve-brand-explorer-v29A-choice-family-active-profile-repair";
export const APPLY_FLAG_FOUNDER = "--founder-reviewed-choice-family-copy-repair";
...
const FILES_READ = [
  ...
  "Tribute + Radisson active-profile rows (quality reference only)",
];

const FILES_CHANGED = [
  "lib/partner-intelligence/brand-explorer-choice-family-active-profile-repair-writer.js",
  "scripts/brand-explorer-choice-family-active-profile-repair-writer.mjs",
...
const BRAND_REPAIR_CONFIG = {
  ascend: {
    ...
    galleryCaptions: {
      "materials.gallery.1":
        "Amberes 64 bar and social space—reference for Ascend collection character: local F&B identity with branded touchpoints, not full prototype homogenization.",
      "materials.gallery.2":
        "El Cid Marina Beach Hotel beach front—coastal resort reference for conversion planning: outdoor activation, arrival sequence, and guest circulation typical of Ascend collection resorts.",
      "materials.gallery.3":
        "El Cid La Ceiba Beach Hotel exterior—arrival and façade reference for owners scoping repositioning capex against collection standards for curb appeal and wayfinding.",
      "materials.gallery.4":
        "El Cid Castilla Beach Hotel lobby—public-space reference for casegoods, lighting, and social zones owners should align to Ascend collection expectations before underwriting conversion.",
      "materials.gallery.5":
        "Manta Lodge Resort exterior—boutique resort reference illustrating how Ascend collection properties retain local architectural character within branded QA bands.",
      "materials.gallery.6":
        "Emotions All Inclusive Puerto Plata double-queen bedroom—guest-room reference for sleep zone, casegoods, and soft-goods coordination expected in refreshed Ascend collection rooms.",
    },
    copyRepairTargets: [
      {
        slotKey: "overview.scenarios",
        match: /Historic or design hotels/gi,
        replace: "Historic or design-forward hotels",
        reason: "parent_aware_carryover_false_positive_design_hotels",
      },
      {
  ...
  "radisson-blu": {
    valueScenarioPackages: [
      {
        ...
        body:
          "Independent or soft-branded upper-upscale hotels that need corporate access, recognizable loyalty, and brand.com strength—Radisson Blu fits when the asset can deliver design-forward full-service expectations within Nordic Nouveau cues.",
      },
      {
        slotKey: "valueOwners.scenario.2",
        sort: 1,
        title: "Design-Forward Conversion",
        body:
          "Mature assets needing refresh capital and relevancy—repositioning under Radisson Blu pairs credible upper-upscale guest expectations with public spaces, casegoods, and F&B that can support the Blu design language.",
      },
      {
        slotKey: "valueOwners.scenario.3",
        sort: 2,
        title: "CALA Gateway or Resort",
        body:
          "Gateway and resort corridors where guests compare flags directly—Radisson Blu competes on execution, location, and design differentiation. Best when the comp set is branded and loyalty participation influences booking.",
      },
      {
        slotKey: "valueOwners.scenario.4",
        sort: 3,
        title: "Third-Party Operator–Led",
        body:
          "Assets run by experienced third-party operators with upper-upscale depth—Radisson Blu works when management can staff F&B, meetings, and front-of-house to Blu standards while the sponsor needs affiliation for financing, sales, and loyalty lift.",
      },
    ],
    galleryCaptions: {
      "materials.gallery.1":
        "Prototype entryway reference for conversion planning—arrival sequence, lighting, and material palette aligned to Radisson Blu prototype standards.",
      "materials.gallery.2":
        "King room reference for footprint planning—sleep zone, casegoods, and in-room work surface typical of core Radisson Blu guest-room expectations.",
      "materials.gallery.3":
        "Bathroom reference layout for PIP scoping—fixtures, finishes, and accessibility band owners should align to current Blu prototype guidance before underwriting conversion capex.",
      "materials.gallery.4":
        "Alternate king room layout illustrating casegoods and seating options within the same prototype family—useful when comparing room modules for a conversion floor plan.",
      "materials.gallery.5":
        "King room floor plan reference for owner and designer workshops—shows spatial coordination expected in refreshed Blu guest rooms.",
      "materials.gallery.6":
        "Double-queen floor plan reference for stakeholder reviews—helps sponsors and operators align on prototype scope before final PIP sign-off.",
    },
    copyRepairTargets: [
      {
        slotKey: "loyalty.proof",
    ...
    forbiddenInProposedCopy: [/\bradisson by choice\b/i, /\btribute portfolio\b/i],
  },
};
...
export function resolveTargetBrands(brandsArg) {
  ...
  for (const slug of slugs) {
    if (PROTECTED_BRAND_SLUGS.includes(slug)) {
      throw new Error(`Brand ${slug} is protected and cannot be modified by v29A`);
    }
    const meta =
    ...
    if (!meta || !TARGET_BRANDS.some((b) => b.slug === meta.slug)) {
      throw new Error(`v29A supports Ascend and Radisson Blu only; got: ${slug}`);
    }
...
export function scanCopySafety(text, brandSlug) {
  ...
  if (/\bHistoric or design hotels\b/i.test(nz(text))) issues.push("design_hotels_false_positive_phrase");
  return issues;
}

export function classifyChoiceFamilyPendingFact(fact, brandSlug) {
  ...
  if (field === "be.footprint.geoIntro") {
    return {
      ...
      rationale:
        "Geo intro extract is a thin regional fragment only—needs a fuller approved footprint narrative before Explorer surfacing.",
  ...
  if (field === "be.overview.typicalUseCase") {
    return {
      ...
      rationale: "Typical use case value is a generic fragment—not owner-facing copy.",
      shouldNotSurfaceExternally: true,
      approveReady: false,
  ...
  if (field === "be.overview.whyValue") {
    return {
      classification: "needs_founder_review",
      rationale: "Why-value extract is partial or placeholder—requires rewritten owner bullets.",
      shouldNotSurfaceExternally: true,
      approveReady: false,
      recommendedAction: "needs_founder_review",
    };
  }
  if (field === "be.loyalty.programName" && /choice privileges/i.test(value)) {
    return {
      classification: "approve-ready",
      rationale:
        "Program name matches approved Choice Privileges branding—approve-ready but gated outside this visual/copy writer.",
      shouldNotSurfaceExternally: false,
      approveReady: true,
      recommendedAction: "approve-ready",
    };
  }
  if (field === "be.footprint.americasHotels") {
    return {
      classification: "needs_source_confirmation",
      rationale: "Americas hotel count extract needs source confirmation and presentation-row alignment before approval.",
      shouldNotSurfaceExternally: true,
      approveReady: false,
  ...
  return {
    classification: "needs_source_confirmation",
    rationale: `Pending fact for ${brandSlug} needs source confirmation before approval.`,
    shouldNotSurfaceExternally: true,
    approveReady: false,
    recommendedAction: "needs_source_confirmation",
  };
...
export function buildApplyCommand({ brands = TARGET_BRANDS.map((b) => b.slug).join(",") } = {}) {
  return [
    "npm run brand-explorer-choice-family-active-profile-repair-writer --",
    `--brands ${brands}`,
    "--apply",
    APPLY_FLAG_APPROVE,
    APPLY_FLAG_FOUNDER,
...
async function buildPerBrandReport(target, options) {
  ...
  const pendingFactDiagnosis = pendingFacts.map((f) => ({
    ...
    ...classifyChoiceFamilyPendingFact(f, brandSlug),
  }));
  ...
  const copyRepairCount = copyRepairs.filter((r) =>
    String(r.fixReason).includes("carryover") || String(r.fixReason).includes("internal")
  ).length;
  ...
  return {
    ...
    diagnosis: {
      ...
      nextRepairsRequired: [
        ...
        "Rephrase carryover false positives and internal/source-capture UI language",
        "Normalize writer-batch Sort Order on editorial/standards rows",
        "Classify pending facts—no auto-approval in this visual/copy writer",
      ],
    },
    ...
    exactDryRunCommand: `npm run brand-explorer-choice-family-active-profile-repair-writer -- --brands ${brandSlug} --dry-run`,
  };
}

export async function buildBrandExplorerChoiceFamilyActiveProfileRepairWriterReport({
  ...
  const brandsWithWork = brandReports.filter((b) => !b.skipped && b.hasWork);
  ...
  const report = {
    ...
    halted,
    haltReason,
    exactDryRunCommand: `npm run brand-explorer-choice-family-active-profile-repair-writer -- --brands ${targets
      .map((b) => b.slug)
      .join(",")} --dry-run`,
    exactApplyCommand:
      brandsApplyReady.length > 0 ? exactApplyCommand : null,
...
function buildMarkdown(report) {
  const lines = [];
  lines.push(`# Brand Explorer Choice-Family Active Profile Repair Writer v${WRITER_VERSION}`);
  lines.push("");
  lines.push(`- Generated: ${report.generatedAt}`);
  lines.push(`- Brands: **${report.brandsRequested.join(", ")}**`);
  lines.push(`- v29A exists: **${report.v29AWriterExists ? "yes" : "no"}**`);
...
export function buildBrandExplorerChoiceFamilyActiveProfileRepairWriterMarkdown(report) {
