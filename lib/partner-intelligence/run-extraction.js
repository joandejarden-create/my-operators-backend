...
async function extractFromSourceDocument(source, registryFields) {
...
async function extractFromBrandSourceDocument(source, registryFields) {
  const classification = classifySourceDocument(source);
  const doc = await loadSourceDocumentText(source);
...
export async function runPartnerSourceExtraction(sourceId, opts = {}) {
  ...
  const { tagged, documentKind, usedLlm, extractor } = await extractFromSourceDocument(
    source,
    registryFields
  );
...
export async function runPartnerOperatorExtraction(operatorId, opts = {}) {
  ...
  for (const source of extractable) {
    ...
    try {
      const result = await extractFromSourceDocument(source, registryFields);
