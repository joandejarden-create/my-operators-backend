/**
 * Brand Explorer — protected 46 Active/Live public-full baseline freeze.
 *
 * Supersedes protected 45 Active/Live public-full freeze after SO/ status
 * promotion + public release + section-pattern cleanup + global AI-Assisted
 * Profile footnote standardization. Read-only. No Airtable / Presentation /
...
export const BASELINE_VERSION_46 = "46-active-public-full-baseline-v1";
export const EXPECTED_ACTIVE_COUNT_46 = 46;
export const EXPECTED_QUALITY_RECOMMENDATION = "approve_for_baseline_freeze";

export const REPORT_JSON_46 = "brand-explorer-46-active-public-full-baseline.json";
export const REPORT_MD_46 = "brand-explorer-46-active-public-full-baseline.md";
export const DOCS_MD_46 = "brand-explorer-46-active-public-full-baseline.md";
...
export const WAVE13_SO_SECTION_PATTERN_CLEANUP_JSON =
  "brand-explorer-wave13-so-section-pattern-cleanup.json";
export const FOOTNOTE_STANDARDIZATION_JSON =
  "brand-explorer-ai-assisted-footnote-standardization.json";

/** Wave 13 Accor brands in the 46 freeze: public six + SO/ (promoted). */
export const BASELINE_46_WAVE13_PUBLIC_SEVEN = Object.freeze(
  [...WAVE13_PARTIAL_PROMOTION_SLUGS, WAVE13_HELD_PROMOTION_SLUG].map((slug) => {
    ...
    return Object.freeze({
      ...
      promotedVia: slug === WAVE13_HELD_PROMOTION_SLUG ? "so_status_and_public_release" : "wave13_public_six",
    });
  })
);

/** Explicitly held / excluded at 46 freeze (not Active/Live public-full). SO/ is NOT held. */
export const BASELINE_46_HELD_EXCLUDED = Object.freeze([
  Object.freeze({
...
export const BASELINE_46_EVIDENCE_SLUGS = Object.freeze([
  ...new Set([
    ...MOMENTUM_EVIDENCE_TARGET_SLUGS,
    ...WAVE13_PARTIAL_PROMOTION_SLUGS,
    WAVE13_HELD_PROMOTION_SLUG,
...
export const BASELINE_46_MANDATORY_EVIDENCE_SLUGS = Object.freeze([...MOMENTUM_EVIDENCE_TARGET_SLUGS]);
 ...
 * report-only freeze.
 */
export const BASELINE_46_WAVE13_EVIDENCE_SOFT_FAIL_IDS = Object.freeze([
  "cala_not_prioritized_first",
]);

const NEW_WAVE_SLUGS = new Set(BASELINE_46_WAVE13_PUBLIC_SEVEN.map((b) => b.slug));
const STALE_UNIVERSE_COUNTS = Object.freeze([23, 24, 25, 27, 39, 45]);
...
function isHardEvidenceFailure(failure) {
  ...
  if (!id) return true;
  return !BASELINE_46_WAVE13_EVIDENCE_SOFT_FAIL_IDS.some(
...
function evidencePassForFreeze(slug, evidence) {
  ...
  if (BASELINE_46_MANDATORY_EVIDENCE_SLUGS.includes(slug)) return evidence.pass === true;
...
async function listPresentationRowsForEvidence(brand, slug, frozenBrand = null) {
  ...
  try {
    const { WAVE13_BRAND_PLAN } = await import("./brand-explorer-wave13-factory-plan.js");
    const plan = WAVE13_BRAND_PLAN?.[slug] || WAVE13_BRAND_PLAN?.[BASELINE_SLUG_ALIASES[slug]];
    push(plan?.name);
    for (const a of plan?.nameAliases || []) push(a);
  } catch {
    // Wave 13 plan optional for non-wave brands
  }
  if (slug === "so-hotels-and-resorts" || slug === "so") {
 ...
 */
export async function build46ActivePublicFullBaseline({
  requireReports = true,
  ...
  const soSectionCleanup = readJsonReport(WAVE13_SO_SECTION_PATTERN_CLEANUP_JSON);
  ...
  if (evaluateEvidence) {
    const freezeBySlug = new Map((readJsonReport(REPORT_JSON_46)?.brands || []).map((b) => [b.slug, b]));
    for (const slug of BASELINE_46_EVIDENCE_SLUGS) {
    ...
    brands.push({
      ...
      recentMomentumEvidenceQualityStatus: evidence
        ...
        : NEW_WAVE_SLUGS.has(u.slug)
          ...
          : BASELINE_46_EVIDENCE_SLUGS.includes(u.slug)
            ? "not_evaluated"
            : "n_a_protected_prior",
      ...
      notes: u.slug === WAVE13_HELD_PROMOTION_SLUG
        ? ["wave13_so_promoted_public_full_baseline_46"]
        : NEW_WAVE_SLUGS.has(u.slug)
          ? ["wave13_public_full_baseline_46"]
          : ["carried_from_protected_45_baseline"],
      gateHits: {
  ...
  const heldExcludedProbes = [
    ...NON_ACTIVE_STATUS_CONFLICT_PROBES,
    ...BASELINE_46_HELD_EXCLUDED.filter((h) => h.recordId).map((h) => ({
      slug: h.slug,
      recordId: h.recordId,
      name: h.name,
      reason: h.reason,
  ...
  for (const probe of heldExcludedProbes) {
    ...
    const meta = BASELINE_46_HELD_EXCLUDED.find((h) => h.slug === probe.slug);
    excluded.push({
  ...
  for (const meta of BASELINE_46_HELD_EXCLUDED.filter((h) => !h.recordId)) {
  ...
  const evidenceWavePass = BASELINE_46_MANDATORY_EVIDENCE_SLUGS.every((slug) => {
    const e = evidenceBySlug.get(slug);
    return e && e.pass === true;
  });
  const evidenceWave13Pass = [...WAVE13_PARTIAL_PROMOTION_SLUGS, WAVE13_HELD_PROMOTION_SLUG].every(
    (slug) => evidencePassForFreeze(slug, evidenceBySlug.get(slug))
  );
  const wave13EvidenceSoftNotes = [...WAVE13_PARTIAL_PROMOTION_SLUGS, WAVE13_HELD_PROMOTION_SLUG].flatMap(
    (slug) => {
      const e = evidenceBySlug.get(slug);
  ...
  const soRow = brands.find((b) => b.slug === WAVE13_HELD_PROMOTION_SLUG);
  const soOk =
    Boolean(soRow) &&
    isBrandStatusActive(soRow.brandStatus) &&
    soRow.shouldRenderFullProfile === true &&
    soRow.publicFullProfile === true &&
    soRow.pvqlStatus === "pass" &&
    soRow.qualityRecommendation === EXPECTED_QUALITY_RECOMMENDATION &&
    soRow.aiAssistedProfileFootnoteVisible === true;

  const newWave = BASELINE_46_WAVE13_PUBLIC_SEVEN.map((meta) => {
  ...
  const freezeDecision =
    universe.totalCount === EXPECTED_ACTIVE_COUNT_46 &&
    brands.length === EXPECTED_ACTIVE_COUNT_46 &&
    ...
    newWave.every((b) => b.included)
      ? "frozen_46_active_public_full_baseline"
      : "freeze_incomplete";
  ...
  return {
    ...
    wave13: {
      ...
      expectedSoActiveCount: WAVE13_EXPECTED_SO_ACTIVE_COUNT,
      publicSixSlugCount: WAVE13_PARTIAL_PROMOTION_SLUGS.length,
      publicSixSlugs: [...WAVE13_PARTIAL_PROMOTION_SLUGS],
      soSlug: WAVE13_HELD_PROMOTION_SLUG,
      soPromoted: soOk === true,
      brands: newWave,
      ...
      geoMomentumCleanup: geoMomentumCleanup
        ...
        : null,
      soSectionPatternCleanup: soSectionCleanup
        ? {
            file: WAVE13_SO_SECTION_PATTERN_CLEANUP_JSON,
            readyStatement: soSectionCleanup.readyStatement || soSectionCleanup.ready || null,
            generatedAt: soSectionCleanup.generatedAt || null,
          }
        : null,
    },
    ...
    frozen: freezeDecision === "frozen_46_active_public_full_baseline",
    freezeDecision,
    ...
    primaryReleaseNote:
      "PRIMARY_RELEASE_SLUGS is an operational overlay, not the Active/Live universe (46).",
    brands,
    ...
    evidenceQuality: {
      ...
      targetSlugs: [...BASELINE_46_EVIDENCE_SLUGS],
      mandatorySlugs: [...BASELINE_46_MANDATORY_EVIDENCE_SLUGS],
      wave13Slugs: [...WAVE13_PARTIAL_PROMOTION_SLUGS, WAVE13_HELD_PROMOTION_SLUG],
    ...
    validationSources: {
      ...
      wave13GeoMomentumCleanup: geoMomentumCleanup
        ...
        : null,
      wave13SoSectionPatternCleanup: soSectionCleanup
        ? {
            file: WAVE13_SO_SECTION_PATTERN_CLEANUP_JSON,
            readyStatement: soSectionCleanup.readyStatement || soSectionCleanup.ready || null,
          }
    ...
    summary: {
      ...
      soPromotedPublicFull: soOk === true,
    ...
    regressionRules: [
      "Active/Live universe count must remain 46 unless freeze is explicitly revised",
      ...
      "SO/ must remain Active/Live public-full (promoted) — 45 baseline is historical",
      ...
      "Stale 23/24/25/27/39/45-brand / PRIMARY_RELEASE lists must never replace the Active/Live universe SoT",
    ],
    rollbackNotes: [
      "This freeze is report-only — no Airtable writes occurred.",
      "Protected 45 public-full freeze preserved at reports/brand-explorer-45-active-public-full-baseline.json",
      "Protected 39 / 27 / interim 27 / 24 / 25 freezes remain predecessor artifacts.",
      "To unfreeze: revise docs/reports and EXPECTED_ACTIVE_COUNT_46 after an explicit founder decision.",
    ...
    futureWorkRules: [
      "New Active/Live brands require a new baseline revision (count will leave 46).",
      "Wave 14 factory work starts from this 46 freeze — do not silently absorb new Active brands.",
      "Do not patch baseline brands casually — use targeted cleanup + re-audit + re-freeze.",
      "Required gates: test:brand-explorer-46-active-public-full-baseline · test:brand-explorer-public-visibility-quality-lock --public-full-only · test:brand-explorer-recent-momentum-evidence-quality · brand-explorer-ai-assisted-footnote-standardization --audit",
    ...
    futureWave14StartingConditions:
      "Start Wave 14 from frozen_46_active_public_full_baseline. Keep House of Originals / Morgans Originals / Radisson Collection excluded unless separate promotion. Preserve AI-Assisted footnote always-on gate. Do not use 45/39/27 lists as universe SoT.",
  };
}

export function evaluate46ActivePublicFullBaselineRegression({
  frozen,
  liveUniverse,
  livePvql,
  liveQuality,
  ...
  if (liveUniverse?.totalCount !== EXPECTED_ACTIVE_COUNT_46) {
    failures.push(
      `active_universe_count_changed:${liveUniverse?.totalCount}_expected_${EXPECTED_ACTIVE_COUNT_46}`
    );
  }
  ...
  if (STALE_UNIVERSE_COUNTS.includes(liveUniverse?.totalCount) && liveUniverse?.totalCount !== EXPECTED_ACTIVE_COUNT_46) {
    failures.push(`stale_${liveUniverse.totalCount}_brand_list_used_as_universe_expected_${EXPECTED_ACTIVE_COUNT_46}`);
  }
  ...
  if (!liveCanonical.has(canonicalBaselineSlug(WAVE13_HELD_PROMOTION_SLUG))) {
    failures.push("so_missing_from_active_universe_after_promotion");
  }
  ...
  for (const frozenBrand of frozen.brands) {
    ...
    if (BASELINE_46_MANDATORY_EVIDENCE_SLUGS.includes(slug)) {
  ...
  if (liveQuality?.brandResults) {
    ...
    if (freezeCount !== EXPECTED_ACTIVE_COUNT_46) {
      failures.push(`quality_freeze_count:${freezeCount}_expected_${EXPECTED_ACTIVE_COUNT_46}`);
    }
  } else {
  ...
  if (!livePvql?.brands?.length) {
    ...
    if (publicFullCount !== EXPECTED_ACTIVE_COUNT_46) {
      failures.push(`pvql_public_full_count:${publicFullCount}_expected_${EXPECTED_ACTIVE_COUNT_46}`);
    }
    const publicFullPass = (livePvql.brands || []).filter(
      (b) => b.publicFullProfile && b.lockPass
    ).length;
    if (publicFullPass !== EXPECTED_ACTIVE_COUNT_46) {
      failures.push(`pvql_public_full_pass_count:${publicFullPass}_expected_${EXPECTED_ACTIVE_COUNT_46}`);
    }
...
export async function run46ActivePublicFullBaselineRegression({
  reassessPvql = true,
  ...
  const frozenPath = path.join(REPORTS_DIR, REPORT_JSON_46);
  if (!fs.existsSync(frozenPath)) {
    throw new Error(`Frozen baseline missing: ${REPORT_JSON_46}. Run freeze --dry-run first.`);
  }
  ...
  const pvqlCoversFrozen =
    publicFullRows.length === EXPECTED_ACTIVE_COUNT_46 &&
    existingPvql?.brands?.length >= EXPECTED_ACTIVE_COUNT_46 &&
  ...
  const regression = evaluate46ActivePublicFullBaselineRegression({
    frozen,
    liveUniverse,
    livePvql,
    liveQuality,
...
export function write46ActivePublicFullBaselineReports(report) {
  fs.mkdirSync(REPORTS_DIR, { recursive: true });
  fs.mkdirSync(DOCS_DIR, { recursive: true });

  const jsonPath = path.join(REPORTS_DIR, REPORT_JSON_46);
  const mdPath = path.join(REPORTS_DIR, REPORT_MD_46);
  const docsPath = path.join(DOCS_DIR, DOCS_MD_46);

  fs.writeFileSync(jsonPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");
  const md = render46BaselineMarkdown(report);
  fs.writeFileSync(mdPath, md, "utf8");
  fs.writeFileSync(docsPath, md, "utf8");
  return { jsonPath, mdPath, docsPath };
}

function render46BaselineMarkdown(report) {
  const lines = [];
  const w13 = report.wave13 || {};
  const vs = report.validationSources || {};
  const s = report.summary || {};

  lines.push(`# Brand Explorer — Protected 46 Active/Live Public-Full Baseline`);
  lines.push("");
  ...
  lines.push("");
  lines.push(
    `This freeze locks the **${report.activeCount}** Brand Basics Active/Live Brand Explorer profiles that are public-full, PVQL-clean, quality \`approve_for_baseline_freeze\`, and AI-Assisted footnote-complete after SO/ promotion + public release + section-pattern cleanup + global footnote standardization. Predecessor: protected **45** (SO/ held).`
  );
  lines.push("");
  ...
  lines.push(`| Wave 13 Accor seven included | ${s.wave13IncludedCount}/${s.wave13Count} |`);
  lines.push(`| SO/ promoted public-full | ${s.soPromotedPublicFull} |`);
  ...
  lines.push("");
  lines.push(`- 24 / 25 / interim 27 / protected 27 / protected 39 / **protected 45** — preserved as predecessor artifacts.`);
  lines.push(
    `- Protected 45: \`${report.predecessorBaselines?.baseline45?.report}\` (${report.predecessorBaselines?.baseline45?.version})`
  );
  lines.push("");

  lines.push(`## 3. 46-brand baseline table`);
  lines.push("");
  lines.push(
  ...
  lines.push(`## 4. SO/ release summary`);
  lines.push("");
  lines.push(
    `SO/ (\`so-hotels-and-resorts\`) was held at the 45 freeze, then promoted via founder acceptance → Brand Status Active → public release → section-pattern cleanup. It is now part of the **46** Active/Live public-full universe.`
  );
  lines.push(`- SO slug: \`${w13.soSlug || "so-hotels-and-resorts"}\``);
  lines.push(`- SO promoted: **${w13.soPromoted}**`);
  lines.push(`- Expected SO active count: ${w13.expectedSoActiveCount || 46}`);
  lines.push(`- Section-pattern cleanup: \`${vs.wave13SoSectionPatternCleanup?.file || WAVE13_SO_SECTION_PATTERN_CLEANUP_JSON}\``);
  lines.push("");
  ...
  lines.push("");
  lines.push(`Public-full PVQL must be **${EXPECTED_ACTIVE_COUNT_46}/${EXPECTED_ACTIVE_COUNT_46}** lockPass, including \`ai_assisted_profile_footnote_visible\`.`);
  lines.push(`Snapshot: publicFull=${vs.pvql?.publicFull ?? "—"} · overallPass=${vs.pvql?.overallPass ?? "—"}`);
  lines.push("");

  lines.push(`## 8. 24-tab quality result`);
  lines.push("");
  lines.push(`All Active/Live brands must be \`approve_for_baseline_freeze\` with blockerCount=0.`);
  ...
  lines.push(`## 11. Value scenario pattern result`);
  lines.push("");
  ...
  lines.push(
    `- No \`owner-fit diligence\`; no standalone Accor/Ennismore platform placeholders; no Property Fit / Support Across Lifecycle titles.`
  );
  lines.push("");
  ...
  lines.push("");
  lines.push(`These brands are **explicitly excluded** from the 46 Active/Live public-full freeze (SO/ is **included**):`);
  lines.push("");
  ...
  lines.push("");
  lines.push(`Test: \`npm run test:brand-explorer-46-active-public-full-baseline\``);
  lines.push("");
  ...
  lines.push(`## 17. Future Wave 14 starting conditions`);
  lines.push("");
  lines.push(
    report.futureWave14StartingConditions ||
      "Start Wave 14 from frozen_46_active_public_full_baseline."
  );
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("npm run brand-explorer-46-active-public-full-baseline -- --dry-run");
  lines.push("npm run test:brand-explorer-46-active-public-full-baseline");
...
export {
  BASELINE_VERSION_46 as BASELINE_VERSION,
  EXPECTED_ACTIVE_COUNT_46 as EXPECTED_ACTIVE_COUNT,
  PROTECTED_FIELDS,
  ROOT,
};
