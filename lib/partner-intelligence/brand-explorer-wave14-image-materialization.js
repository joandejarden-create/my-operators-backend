/**
 * Wave 14 Stage 5 — image asset pack + Presentation Image materialization.
 *
...
import {
  getWave14SupplementalOpenings,
  isWave14PropertyHoldSlug,
  getWave14CuratedPoolSeed,
} from "./brand-explorer-wave14-image-supplemental.js";
import {
...
export const WAVE14_IMAGE_MATERIALIZATION_VERSION = "wave14-image-materialization-v1";
...
const SIBLING_RE_BY_SLUG = Object.freeze({
  "marriott-hotels":
    /jw-marriott|sheraton|westin|renaissance|autograph|tribute|w-hotel|st-regis|residence-inn|springhill|towneplace|aloft|moxy|courtyard|fairfield|four-points|element|studiores|ac-hotel|city-express/i,
  sheraton:
    /marriott-hotels|jw-marriott|westin|four-points|flex|residence-inn|aloft|moxy|courtyard|renaissance|autograph|tribute|w-hotel/i,
  westin:
    /sheraton|marriott-hotels|jw-marriott|w-hotel|renaissance|autograph|tribute|aloft|moxy|residence-inn/i,
  ...
  "aloft-hotels":
    /moxy|ac-hotel|four-points|element|w-hotel|courtyard|autograph|tribute|residence-inn/i,
  "four-points-flex-by-sheraton":
    /four-points-by-sheraton|\/fp(?!x)[a-z]{2}-|sheraton(?!.*flex)|marriott-hotels|westin|aloft|moxy|courtyard/i,
  studiores:
    /residence-inn|towneplace|element|apartments-by-marriott|springhill|fairfield|courtyard|aloft/i,
});

const BRAND_HINT_RE_BY_SLUG = Object.freeze({
  "marriott-hotels": /marriott(?!-)|\/mc-|marquis|wailea|culiacan|barranquilla|reforma|cancun-resort/i,
  sheraton: /sheraton|\/si-|\/sr-|\/ps-|\/js-|\/tc-/i,
  westin: /westin|\/wi-|\/wn-|heavenly/i,
  "residence-inn-by-marriott": /residence-inn|\/ri-|residenceinn/i,
  "springhill-suites-by-marriott": /springhill|\/sh-|\/es-|\/so-|spring-hill/i,
  "towneplace-suites-by-marriott": /towneplace|\/tp-|\/pe-|towne-place/i,
  "aloft-hotels": /aloft|\/al-|alf-/i,
  "four-points-flex-by-sheraton":
    /four-points-flex|four-points-express|fourpointsexpress|flex-by-sheraton|fp-flex|\/fpx-|xf-|marriotts7prod\/xf-/i,
  studiores: /studiores|\/sr-|\/oe-|\/ni-|rswsr|tysoe|atlni/i,
});
...
export function parseWave14ImageMaterializationFlags(argv = []) {
  const missing = WAVE14_IMAGE_MATERIALIZATION_APPLY_FLAGS.filter((f) => !argv.includes(f));
  return {
...
export function resolveWave14ImageIdentity(slug) {
  const s = String(slug || "").trim().toLowerCase();
  if (!isWave14Stage5Slug(s)) return null;
  return FACTORY_PREVIEW_CANDIDATE_IDENTITIES[s] || null;
}

export function loadWave14GalleryPool(slug) {
  const s = String(slug || "").trim().toLowerCase();
  const p = path.join(FIXTURES, `wave14-${s}-gallery-pool.json`);
  let rows = [];
  if (fs.existsSync(p)) {
    try {
      ...
      if (process.env.NODE_ENV !== "production") {
        console.warn(`[wave14-image] failed to read ${p}: ${err.message}`);
      }
    }
  }
  const seed = getWave14CuratedPoolSeed(s);
...
export function isWave14RejectedImageUrl(url, { brandSlug = "" } = {}) {
  const u = nz(url);
  ...
  // Marriott brand microsite / hotel-development galleries (Wave 14 Stage 5).
  const isWave14BrandHostImage =
    /(?:marriott-hotels|sheraton|westin|springhillsuites|towneplacesuites|aloft-hotels)\.marriott\.com\/wp-content\/uploads\//i.test(
      lower
  ...
  if (!isOfficialLifestylePropertyImageUrl(u) && !isWave14BrandHostImage) {
...
function normalizeWave14Pool(rawPool, brandSlug) {
  const accepted = [];
  const rejections = [];
  for (const row of rawPool) {
    const imageUrl = nz(row.imageUrl);
    const gate = isWave14RejectedImageUrl(imageUrl, { brandSlug });
    if (gate.rejected) {
...
function propertyCatalogForSlug(slug) {
  ...
  const push = (r) => {
    ...
    if (!/\/hotels\/[a-z0-9-]+\/overview/i.test(r.url) && !isWave14PropertyHoldSlug(slug)) {
      // Prefer marriott.com property overview URLs; skip brand-site placeholders.
      if (/marriott\.com\/brands\/|hotel-development\.marriott|springhillsuites\.marriott|towneplacesuites\.marriott/i.test(r.url)) {
        return;
      }
    }
...
export function buildWave14ImageAssetPackForBrand(brandSlug) {
  const identity = resolveWave14ImageIdentity(brandSlug);
  if (!identity) {
    return {
      brandSlug,
      status: "refused",
  ...
  const rawPool = loadWave14GalleryPool(slug);
  const { accepted, rejections } = normalizeWave14Pool(rawPool, slug);
  const blockers = [];
  const propertyHeldSlug = isWave14PropertyHoldSlug(slug);
...
export async function planWave14BrandImageMaterialization(brandSlug, { assetPackBrand } = {}) {
  const identity = resolveWave14ImageIdentity(brandSlug);
  if (!identity) {
  ...
  const packRow = assetPackBrand || buildWave14ImageAssetPackForBrand(identity.slug);
  ...
  for (const asset of assets) {
    const gate = isWave14RejectedImageUrl(asset.imageUrl, { brandSlug: identity.slug });
    if (gate.rejected) {
  ...
  const propertyHeldOk = isWave14PropertyHoldSlug(identity.slug) && propertyCount === 0;
  const galleryShortfallOk =
    isWave14PropertyHoldSlug(identity.slug) &&
...
export async function applyWave14ImageMaterializationPlans({
  brandResults,
  apply = false,
  argv = [],
} = {}) {
  const flagCheck = parseWave14ImageMaterializationFlags(argv);
  if (!apply) {
  ...
  for (const brand of brandResults) {
    if (!isWave14Stage5Slug(brand.brandSlug)) {
    ...
    const identity = resolveWave14ImageIdentity(brand.brandSlug);
...
function brandMd(plan) {
  const lines = [
    `# Wave 14 image materialization — ${plan.brandName || plan.brandSlug}`,
    ``,
...
export async function runWave14ImageMaterialization({ dryRun = true, argv = [], brands = null } = {}) {
  const flagCheck = parseWave14ImageMaterializationFlags(argv);
  const apply = argv.includes("--apply") && !dryRun;
  const brandPlans = [];
  const targetSlugs = (brands?.length ? brands : [...WAVE14_STAGE4_APPROVED_SLUGS])
    .map((s) => String(s || "").trim().toLowerCase())
    .filter((s) => WAVE14_STAGE4_APPROVED_SLUGS.includes(s));

  for (const slug of targetSlugs) {
    const pack = buildWave14ImageAssetPackForBrand(slug);
    const plan = await planWave14BrandImageMaterialization(slug, { assetPackBrand: pack });
    brandPlans.push(plan);
    writeReports(`brand-explorer-wave14-image-materialization-${slug}`, plan, brandMd(plan));
  }

  const applyResult = await applyWave14ImageMaterializationPlans({
  ...
  const summary = {
    version: WAVE14_IMAGE_MATERIALIZATION_VERSION,
    ...
    flagCheck,
    protectedBaselineCount: EXPECTED_ACTIVE_COUNT_46,
    counts: {
  ...
  const md = [
    ...
    `## Apply flags`,
    ``,
    ...WAVE14_IMAGE_MATERIALIZATION_APPLY_FLAGS.map((f) => `- \`${f}\``),
    ``,
  ].join("\n");

  writeReports("brand-explorer-wave14-image-materialization", summary, md);

  fs.mkdirSync(DOCS_DIR, { recursive: true });
  const docsPath = path.join(DOCS_DIR, "brand-explorer-wave14-image-materialization.md");
  fs.writeFileSync(
    docsPath,
    [
      ...
      ...WAVE14_IMAGE_MATERIALIZATION_APPLY_FLAGS.map((f, i, arr) =>
        i === arr.length - 1 ? `  ${f}` : `  ${f} \\`
      ),
      "```",
      ``,
