/**
 * Wave 16A Stage 2A — Controlled tab build (three LOW-risk Marriott brands).
 *
 * Allowed writes: target-brand Presentation rows only.
 * Forbidden: Brand Status, release, CV, Source Library, Registry, Census,
 * Recent Momentum, images, Four Points Flex, Wave 16B, remaining Wave 16A, Active 62.
 */
...
import {
  ...
  WAVE16A_FLEX_HOLD,
  WAVE16A_IDENTITIES,
  WAVE16A_STAGE2A_APPROVED_SLUGS,
  WAVE16A_STAGE2A_APPLY_FLAGS,
} from "./brand-explorer-wave16a-factory-plan.js";
import {
  WAVE16A_SAFE_TGS,
  FORBIDDEN_SCENARIO_TITLES,
  WAVE16A_STAGE2A_PORTFOLIO_MIX,
  getWave16aStage2aBrandContent,
} from "./brand-explorer-wave16a-stage2a-tab-factory-content.js";
import { listActiveUniverseSlugs } from "./brand-explorer-active-universe.js";

const SAFE_TGS_OPTIONS = WAVE16A_SAFE_TGS;

export const WAVE16A_STAGE2A_BUILD_VERSION = "wave16a-stage2a-controlled-tab-build-v1";

const PRESENTATION_TABLE = "Brand Setup - Brand Explorer Presentation";
const BASICS_TABLE = "Brand Setup - Brand Basics";
const FORBIDDEN_STAGE2A_BRANDS = new Set([
...
export function parseWave16aStage2aFlags(argv = []) {
  const missing = WAVE16A_STAGE2A_APPLY_FLAGS.filter((f) => !argv.includes(f));
  return {
...
export function generateWave16aStage2aPresentationPack(slug, opts = {}) {
  const content = getWave16aStage2aBrandContent(slug);
  ...
  const parent = content.parent || pack.parentPlatform || "Marriott International";
  const loyalty = content.loyalty || "Marriott Bonvoy";
  const pad = lifecyclePad(brandDisplayName, model);

  // Enforce Wave 16A Stage 2A scenario title rules
  ...
  const presentation = [
    row(
      "Brand Positioning",
      "",
      `${brandDisplayName} is positioned as a ${model}. Owner underwriting should prioritize brand-specific fit, demand alignment, and operating-model readiness over Marriott International corporate generalizations or sibling-brand assumptions.`,
      10
    ),
    row(
      "Guest Psychographics Description",
    ...
    row(
      ...
      "Owner Fit",
      `${brandDisplayName} should be chosen when the asset can deliver this positioning more credibly than ${peers}. Assess whether demand mix, product scope, and operating complexity favor ${brandDisplayName} over adjacent Marriott alternatives in the same tier and geography.`,
      22
    ),
    ...
    row(
      ...
      "Proof Points",
      bullets([
        `Official brand positioning for ${brandDisplayName} is anchored in Marriott brand and development materials specific to this brand—not Marriott International corporate copy`,
    ...
    row(
      ...
      44,
      {
        ...
        caseSummaryInterpretation: `Owner-facing synthesis grounded in official Marriott brand sources.`,
    ...
    row(
      ...
      "Experience & Identity",
      bullets([
        `${brandDisplayName} guest promise is tied to a clear ${model} rather than a generic Marriott corporate story`,
    ...
    row(
      ...
      `${brandDisplayName} is most useful when owners explicitly distinguish it from ${peers} instead of treating Marriott brands as interchangeable lanes.`,
      49
    ),
    row(
    ...
    row(
      ...
      "Portfolio Mix",
      (WAVE16A_STAGE2A_PORTFOLIO_MIX[slug] || []).join("\n") +
    ...
    row(
      ...
      "Growth Editorial",
      ensureMinWords(
        `${brandDisplayName} growth should be evaluated through brand-specific positioning, not Marriott International expansion targets alone. ${ownerLens[0] || "Prioritize markets where demand, product, and operator capability align with the brand promise."} ${calaStrong ? `CALA markets (${calaMarkets}) already demonstrate operating proof for this positioning.` : "New CALA market claims should wait for verified property-level evidence with a named hotel and matching official property URL."}`,
        40,
        pad
      ),
      481
    ...
    row(
      ...
      "F&B Complexity",
      `F&B and public-space complexity varies by ${brandDisplayName} site type. Review concept, hours, and operator capability against local demand rather than copying another Marriott brand’s outlet assumptions.`,
      107
    ),
    row(
    ...
    row(
      ...
      ensureMinWords(
        `Standards should protect ${brandDisplayName} brand identity while remaining executable for the specific asset, market, and operator. ${distinctions[0] || "Design, service, and programming standards are brand-specific and should not be borrowed from adjacent Marriott brands."} Align PIP and lifecycle capital from the asset review.`,
        35,
        pad
      ),
      112
  ...
  // Stage 2A: Recent Momentum intentionally omitted (protected — zero writes).
  ...
  presentation.push(
    ...
    row(
      ...
      `Use concise chips and tags tied to owner decision points for ${brandDisplayName}. Keep copy free of raw source links and Marriott corporate boilerplate. Every tag should help an owner evaluate brand fit for a specific asset and market.`,
      703,
      {
...
function resolveWave16aStage2aIdentity(slug) {
...
function getApprovedStage2aScope() {
  const wanted = [...WAVE16A_STAGE2A_APPROVED_SLUGS];
  if (wanted.length !== 3) {
    throw new Error(`Wave 16A Stage 2A requires exactly three brands, got ${wanted.length}`);
  }
  if (wanted.some((s) => FORBIDDEN_STAGE2A_BRANDS.has(s))) {
    throw new Error("Forbidden brand detected in Stage 2A scope.");
  }
  return wanted;
}

export async function planWave16aStage2aBrand(slug, { tgsOptions = [] } = {}) {
  const identity = resolveWave16aStage2aIdentity(slug);
  if (
  ...
  if (!WAVE16A_STAGE2A_APPROVED_SLUGS.includes(slug)) {
    return {
      brandSlug: slug,
      blocked: true,
      blockers: ["not_wave16a_stage2a_target"],
      patches: [],
      basicsPatches: [],
    };
  }
  if (FORBIDDEN_STAGE2A_BRANDS.has(slug)) {
    return {
      brandSlug: slug,
      blocked: true,
  ...
  const pack = generateWave16aStage2aPresentationPack(slug, {
  ...
  for (const pRow of pack.presentation) {
    ...
    if (/^footprint\.momentum/i.test(slotKey)) continue; // Stage 2A protected
  ...
  // Stage 2A presentation-only — Brand Basics intentionally untouched.
  const basicsPatches = [];
...
const STAGE2A_PROHIBITED_PUBLIC = Object.freeze([
...
function flexContaminationHits(text) {
  const hits = [];
  const sentences = String(text || "").split(/(?<=[.!?])\s+/);
  for (const sentence of sentences) {
    const mentionsFlex =
      /\bFour Points Flex\b/i.test(sentence) ||
      /\bFlex[- ]light\b/i.test(sentence) ||
      /\bFlex (?:hotel|hotels|propert|image|PIP|econom)/i.test(sentence);
    if (!mentionsFlex) continue;
    // Explicit differentiation / peer contrast is required and allowed.
    if (sentenceIsContrastive(sentence)) continue;
    hits.push({ id: "flex_contamination", hard: true, sample: sentence.slice(0, 160) });
  }
  return hits;
}

function scanStage2aPublicCopy(text) {
  const hits = [];
  for (const rule of STAGE2A_PROHIBITED_PUBLIC) {
...
function scanContamination(slug, text) {
  const hits = [];
  if (slug === "four-points-by-sheraton") {
    hits.push(...flexContaminationHits(text));
  }
  if (slug === "fairfield-by-marriott") {
    if (/\bFour Points Flex\b/i.test(text) && !sentenceIsContrastive(text)) {
      hits.push({ id: "four_points_flex", hard: true });
    }
    // Fail only if Fairfield is framed as suite/extended-stay product itself.
    const sentences = String(text || "").split(/(?<=[.!?])\s+/);
    for (const sentence of sentences) {
      if (!/\bFairfield\b/i.test(sentence)) continue;
      if (/\ball-suite\b/i.test(sentence) && !sentenceIsContrastive(sentence)) {
        hits.push({ id: "springhill_as_self", hard: true });
      }
      if (/\bextended[- ]stay\b/i.test(sentence) && !sentenceIsContrastive(sentence)) {
        hits.push({ id: "residence_as_self", hard: true });
      }
    }
  }
  if (slug === "delta-hotels-by-marriott") {
    const sentences = String(text || "").split(/(?<=[.!?])\s+/);
    for (const sentence of sentences) {
      if (!/\bDelta\b/i.test(sentence) && !/\bdiscovery\b/i.test(sentence)) continue;
      if (
        /\bDelta\b/i.test(sentence) &&
        /\bflagship\b/i.test(sentence) &&
        !sentenceIsContrastive(sentence) &&
        !/Marriott Hotels/i.test(sentence)
      ) {
        hits.push({ id: "marriott_hotels_as_self", hard: true });
      }
      if (/\bdiscovery\b/i.test(sentence) && !sentenceIsContrastive(sentence)) {
        hits.push({ id: "renaissance_as_self", hard: true });
      }
    }
  }
  return hits;
...
async function runStage2aIdentityPreflight() {
  const activeSlugs = await listActiveUniverseSlugs();
  const activeCount = activeSlugs.length;
  const brands = {};
  const blockers = [];

  if (activeCount !== WAVE16A_PROTECTED_BASELINE_COUNT) {
    blockers.push(`active_universe_not_62:${activeCount}`);
  }

  for (const slug of WAVE16A_STAGE2A_APPROVED_SLUGS) {
    const identity = resolveWave16aStage2aIdentity(slug);
    const basics = await fetchBasicsRecord(identity.recordId);
    ...
    brands[slug] = {
      ...
      brandStatus: status,
      parentCompany: parent || WAVE16A_IDENTITIES[slug].parentCompany,
  ...
  const flexBasics = await fetchBasicsRecord(WAVE16A_FLEX_HOLD.recordId);
  const flexStatus = nz(flexBasics?.fields?.["Brand Status"]);
  const flexOk =
    WAVE16A_FLEX_HOLD.recordId !== WAVE16A_IDENTITIES["four-points-by-sheraton"].recordId &&
    flexStatus === "Under Review" &&
    !activeSlugs.includes(WAVE16A_FLEX_HOLD.slug);

  if (!flexOk) {
    blockers.push(`four_points_flex_identity_separation_failed:${flexStatus || "missing"}`);
  }
  ...
  return {
    ...
    fourPointsFlex: {
      ...
      classification: WAVE16A_FLEX_HOLD.classification,
      inActiveUniverse: activeSlugs.includes(WAVE16A_FLEX_HOLD.slug),
      distinctFromFourPoints:
        WAVE16A_FLEX_HOLD.recordId !== WAVE16A_IDENTITIES["four-points-by-sheraton"].recordId,
    },
  };
}
...
function auditBrandSemantics(slug, presentationRows) {
  const corpus = (presentationRows || [])
    .map((r) => `${nz(r.title)}\n${nz(r.body)}`)
    .join("\n");
  const prohibited = scanStage2aPublicCopy(corpus);
...
export async function planWave16aStage2aBuild({ preflight = null } = {}) {
  const scope = getApprovedStage2aScope();
  const tgsOptions = await listTargetGuestSegmentOptions();
  const brandResults = [];
  for (const slug of scope) {
    const brand = await planWave16aStage2aBrand(slug, { tgsOptions });
    const pack = generateWave16aStage2aPresentationPack(slug, {
    ...
    if (brand.recordId === WAVE16A_FLEX_HOLD.recordId) {
      brand.blockers.push("four_points_flex_record_targeted");
      brand.blocked = true;
    }
  ...
  for (const b of brandResults) {
    for (const p of b.patches || []) {
      writeDiff.push({
        ...
        sourceFoundationReference: `wave16a-stage1:${b.brandSlug}`,
      });
    }
  }

  return {
    version: WAVE16A_STAGE2A_BUILD_VERSION,
    factoryVersion: WAVE16A_VERSION,
    stage: "stage2a-controlled-tab-build",
    generatedAt: new Date().toISOString(),
    dryRun: true,
    airtableWrites: false,
    protectedBaselineCount: WAVE16A_PROTECTED_BASELINE_COUNT,
    preflight,
    approvedScope: scope,
    excluded: {
      remaining_wave16a: "not_in_stage2a_scope",
      wave16b: "untouched",
      four_points_flex_by_sheraton: "PROTECTED_HOLD",
      marriott_conference_center: "untouched",
      active_62: "read_only_regression_only",
    ...
    requiredApplyFlags: [...WAVE16A_STAGE2A_APPLY_FLAGS],
  };
}

export async function applyWave16aStage2aBuild({ plan, apply = false, argv = [] } = {}) {
  const flagCheck = parseWave16aStage2aFlags(argv);
  ...
  for (const brand of plan.brandResults || []) {
    ...
    if (!WAVE16A_STAGE2A_APPROVED_SLUGS.includes(brand.brandSlug)) {
      throw new Error(`Refusing non-target brand write: ${brand.brandSlug}`);
    }
    if (brand.recordId === WAVE16A_FLEX_HOLD.recordId) {
      throw new Error("Refusing Four Points Flex write");
    }
    ...
    for (const patch of brand.patches || []) {
      if (patch.table === BASICS_TABLE) {
        throw new Error(`Presentation-only Stage 2A refused Basics write for ${brand.brandSlug}`);
      }
  ...
  return {
    ...
    allThreeApplied: WAVE16A_STAGE2A_APPROVED_SLUGS.every((s) => resultsByBrand[s]?.applied),
    reason: "wave16a_stage2a_controlled_tab_build_applied",
    flagCheck,
    resultsByBrand,
    ...
    imagesWritten: false,
    recentMomentumWritten: false,
    protected62Untouched: activeAfter === WAVE16A_PROTECTED_BASELINE_COUNT,
    stopShared,
  };
}
...
function writeBrandMd(brand, reportsDir, applyResult = null) {
  ...
  const mdPath = path.join(reportsDir, `brand-explorer-wave16a-stage2a-${slug}.md`);
  const applied = applyResult?.resultsByBrand?.[slug];
  const lines = [
    `# Wave 16A Stage 2A Controlled Tab Build — ${brand.brandName || slug}`,
    "",
    `- Slug: \`${slug}\``,
...
export function writeWave16aStage2aReports(plan, applyResult = null, meta = {}) {
  ...
  const writeDiffPath = path.join(REPORTS_DIR, "brand-explorer-wave16a-stage2a-write-diff.json");
  fs.writeFileSync(writeDiffPath, `${JSON.stringify(plan.writeDiff || [], null, 2)}\n`, "utf8");

  const semanticPath = path.join(REPORTS_DIR, "brand-explorer-wave16a-stage2a-semantic-audit.md");
  const semanticLines = [
    "# Wave 16A Stage 2A — Semantic Audit",
    "",
    ...
    "## Four Points vs Flex",
    "",
    `- Flex record: \`${plan.preflight?.fourPointsFlex?.recordId || WAVE16A_FLEX_HOLD.recordId}\``,
    `- Distinct from Four Points: **${plan.preflight?.fourPointsFlex?.distinctFromFourPoints === true}**`,
    `- Flex Brand Status: ${plan.preflight?.fourPointsFlex?.brandStatus || "—"}`,
    `- Flex in Active universe: **${plan.preflight?.fourPointsFlex?.inActiveUniverse === true}**`,
    "",
  ];
  ...
  const jsonPath = path.join(REPORTS_DIR, "brand-explorer-wave16a-stage2a-controlled-tab-build.json");
  fs.writeFileSync(jsonPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");

  const mdPath = path.join(REPORTS_DIR, "brand-explorer-wave16a-stage2a-controlled-tab-build.md");
  const md = [
  ...
  const docPath = path.join(DOCS_DIR, "brand-explorer-wave16a-stage2a-controlled-tab-build.md");
  fs.writeFileSync(
    docPath,
    [
      ...
      "## Scope",
      "",
      ...WAVE16A_STAGE2A_APPROVED_SLUGS.map((s) => `- \`${s}\``),
      "",
      "## Guards",
      "",
...
export async function runWave16aStage2aControlledTabBuild({ dryRun = true, argv = [] } = {}) {
  const preflight = await runStage2aIdentityPreflight();
  if (!preflight.ok) {
    const plan = {
      version: WAVE16A_STAGE2A_BUILD_VERSION,
      stage: "stage2a-controlled-tab-build",
      generatedAt: new Date().toISOString(),
      preflight,
      approvedScope: [...WAVE16A_STAGE2A_APPROVED_SLUGS],
      brandResults: [],
      writeDiff: [],
      summary: { brandCount: 0, blockedSlugs: [...WAVE16A_STAGE2A_APPROVED_SLUGS], plannedPresentationWrites: 0, plannedBasicsWrites: 0, plannedMomentumWrites: 0, plannedImageWrites: 0 },
      tgsValidated: false,
    };
    const readyStatement =
      preflight.activeUniverseBefore !== WAVE16A_PROTECTED_BASELINE_COUNT
        ? "wave16a_stage2a_blocked_active_baseline_regression"
        : "wave16a_stage2a_build_blocked_shared_architecture_issue";
    const paths = writeWave16aStage2aReports(plan, null, { pass: false, readyStatement });
    return {
      ...plan,
      pass: false,
  ...
  const plan = await planWave16aStage2aBuild({ preflight });
  let applyResult = null;
  if (!dryRun && argv.includes("--apply")) {
    applyResult = await applyWave16aStage2aBuild({ plan, apply: true, argv });
  }
  ...
  const appliedCount = WAVE16A_STAGE2A_APPROVED_SLUGS.filter(
    (s) => applyResult?.resultsByBrand?.[s]?.applied
  ).length;
  const sharedIssue = applyResult?.stopShared === true;
  const activeRegression = activeAfter !== WAVE16A_PROTECTED_BASELINE_COUNT;

  let readyStatement = "wave16a_stage2a_dry_run_ready";
  if (activeRegression) readyStatement = "wave16a_stage2a_blocked_active_baseline_regression";
  else if (sharedIssue) readyStatement = "wave16a_stage2a_build_blocked_shared_architecture_issue";
  else if (applyResult?.applied && appliedCount === 3 && !blocked && allSemanticPass) {
    readyStatement = "wave16a_stage2a_low_risk_tab_build_ready_for_image_stage";
  } else if (applyResult?.applied && appliedCount > 0 && appliedCount < 3) {
    readyStatement = "wave16a_stage2a_partial_build_complete_remediation_required";
  } else if (blocked) readyStatement = "wave16a_stage2a_build_blocked";
  ...
  const paths = writeWave16aStage2aReports(plan, applyResult, {
    pass,
    readyStatement,
    activeUniverseAfter: activeAfter,
  });
