/**
 * Wave 16A Stage 2B — LOW-risk image materialization (Fairfield, Four Points, Delta).
 *
 ...
 * Active 62 writes, Four Points Flex, remaining Wave 16A, Wave 16B, broad content rewrites.
 */
...
import { EXPECTED_ACTIVE_COUNT_62 } from "./brand-explorer-62-active-public-full-baseline.js";
import { loadActiveUniverse } from "./brand-explorer-active-universe.js";

export const WAVE16A_STAGE2B_IMAGE_VERSION = "wave16a-stage2b-image-materialization-v1";

export { WAVE16A_STAGE2B_APPROVED_SLUGS };
export const WAVE16A_STAGE2B_APPLY_FLAGS = Object.freeze([
  "--approve-wave16a-stage2b-image-materialization",
...
const WAVE16A_STAGE2B_FORBIDDEN_WRITE_FIELDS = Object.freeze([
  "Company Validated",
...
const FORBIDDEN_WRITE_FIELDS = new Set([...WAVE16A_STAGE2B_FORBIDDEN_WRITE_FIELDS]);
...
const FLEX_CONTAMINATION_RE =
  /four-points-flex|four.?points.?flex|fourpointsexpress|fp-flex|\/fpx-|marriotts7prod\/xf-|marriotts7prod\/fpx-|\bxf-/i;

const SIBLING_RE_BY_SLUG = Object.freeze({
  "fairfield-by-marriott":
    /courtyard|springhill|residence-inn|towneplace|four-points|delta-hotels|aloft|moxy|element|studiores|marriott-hotels|sheraton|renaissance/i,
  "four-points-by-sheraton":
    /four-points-flex|fourpointsexpress|\/fpx-|\bxf-|fairfield|courtyard|aloft|moxy|sheraton(?!.*four)|delta-hotels|marriott-hotels|renaissance|springhill|residence-inn/i,
  "delta-hotels-by-marriott":
    /marriott-hotels|renaissance|sheraton|courtyard|four-points|fairfield|jw-marriott|westin|aloft|moxy|springhill|residence-inn/i,
});

const BRAND_HINT_RE_BY_SLUG = Object.freeze({
  "fairfield-by-marriott":
    /fairfield|\/fi[-_]|cunfi|nycmw|nycft|miafo|mexfo|bogfo|marriotts7prod\/fi/i,
  "four-points-by-sheraton":
    /four-points|fourpoints|\/fp[-_]|bogfp|cunfp|laxfp|nycfm|miafa|sdqfp|marriotts7prod\/fp(?!x)/i,
  "delta-hotels-by-marriott":
    /delta-hotels|\/dl[-_]|yyzda|yvrdv|yycdl|cundl|mexdl|marriotts7prod\/dl/i,
});

function isWave16aStage2bSlug(slug) {
  return WAVE16A_STAGE2B_APPROVED_SLUGS.includes(String(slug || "").trim().toLowerCase());
}

function isWave16aStage2bPropertyHoldSlug() {
  return false;
}

function getWave16aStage2bCuratedPoolSeed() {
  return [];
}

function getWave16aStage2bSupplementalOpenings() {
  return [];
}

function getWave16aStage2bSourcePack() {
  return null;
}
...
export function parseWave16aStage2bImageFlags(argv = []) {
  const missing = WAVE16A_STAGE2B_APPLY_FLAGS.filter((f) => !argv.includes(f));
  return {
...
export function resolveWave16aStage2bImageIdentity(slug) {
  const s = String(slug || "").trim().toLowerCase();
  if (!isWave16aStage2bSlug(s)) return null;
  return FACTORY_PREVIEW_CANDIDATE_IDENTITIES[s] || null;
}

export function loadWave16aStage2bGalleryPool(slug) {
  const s = String(slug || "").trim().toLowerCase();
  const p = path.join(FIXTURES, `wave16a-${s}-gallery-pool.json`);
  let rows = [];
  if (fs.existsSync(p)) {
    try {
      ...
      if (process.env.NODE_ENV !== "production") {
        console.warn(`[wave16a-stage2b-image] failed to read ${p}: ${err.message}`);
      }
    }
  }
  const seed = getWave16aStage2bCuratedPoolSeed(s);
...
export function isWave16aStage2bRejectedImageUrl(url, { brandSlug = "" } = {}) {
  const u = nz(url);
  ...
  if (FLEX_CONTAMINATION_RE.test(lower)) {
...
function normalizeWave16aStage2bPool(rawPool, brandSlug) {
  const accepted = [];
  const rejections = [];
...
export function buildWave16aStage2bImageAssetPackForBrand(brandSlug) {
  const identity = resolveWave16aStage2bImageIdentity(brandSlug);
  if (!identity) {
    return {
      brandSlug,
      status: "refused",
  ...
  const rawPool = loadWave16aStage2bGalleryPool(slug);
  const { accepted, rejections } = normalizeWave16aStage2bPool(rawPool, slug);
  const blockers = [];
  const propertyHeldSlug = isWave16aStage2bPropertyHoldSlug(slug);
  // Thin official pools (e.g. Flex Getty scrub): reserve scenario slots first.
  ...
  if (thinPool || propertyHeldSlug) {
    ...
    // Property openings: hold for Flex; otherwise CALA-first catalog match.
    ...
    // Lane2 order: gallery → property (CALA-first) → scenario.
  ...
  return {
    ...
    propertyHeld: propertyHeld
      ? {
          held: true,
          reason:
            "Named Four Points Flex property overview URLs not steward-matched — openings/property images cleanly unavailable (no Four Points by Sheraton substitute).",
        }
      : null,
...
export async function planWave16aStage2bBrandImageMaterialization(brandSlug, { assetPackBrand } = {}) {
  const identity = resolveWave16aStage2bImageIdentity(brandSlug);
  if (!identity) {
  ...
  const packRow = assetPackBrand || buildWave16aStage2bImageAssetPackForBrand(identity.slug);
  ...
  for (const asset of assets) {
    const gate = isWave16aStage2bRejectedImageUrl(asset.imageUrl, { brandSlug: identity.slug });
    if (gate.rejected) {
  ...
  const propertyHeldOk = isWave16aStage2bPropertyHoldSlug(identity.slug) && propertyCount === 0;
  const galleryShortfallOk =
    isWave16aStage2bPropertyHoldSlug(identity.slug) &&
...
export async function applyWave16aStage2bImageMaterializationPlans({
  brandResults,
  apply = false,
  argv = [],
} = {}) {
  const flagCheck = parseWave16aStage2bImageFlags(argv);
  if (!apply) {
  ...
  for (const brand of brandResults) {
    if (!isWave16aStage2bSlug(brand.brandSlug)) {
    ...
    const identity = resolveWave16aStage2bImageIdentity(brand.brandSlug);
...
function brandMd(plan) {
  const lines = [
    `# Wave 16A Stage 2B image materialization — ${plan.brandName || plan.brandSlug}`,
    ``,
...
export async function runWave16aStage2bIdentityPreflight() {
  const issues = [];
  ...
  if (liveActiveCount !== EXPECTED_ACTIVE_COUNT_62) {
    issues.push(`active_universe_${liveActiveCount}_expected_${EXPECTED_ACTIVE_COUNT_62}`);
  }
  ...
  for (const slug of WAVE16A_STAGE2B_APPROVED_SLUGS) {
    ...
    if (activeSlugs.includes(slug)) issues.push(`${slug}_unexpectedly_in_active_62`);
    if (name && name !== id.exactBrandBasicsName && name !== id.suppliedName) {
      // soft: record exact live name
    }
    targets.push({
      slug,
  ...
  if (!flexOk) issues.push(`four_points_flex_hold_failed:${flexStatus || "missing"}`);
...
export async function runWave16aStage2bImageMaterialization({ dryRun = true, argv = [], brands = null } = {}) {
  const flagCheck = parseWave16aStage2bImageFlags(argv);
  ...
  if (!preflight.pass) {
    const stopped = {
      version: WAVE16A_STAGE2B_IMAGE_VERSION,
      ...
      readyStatement:
        preflight.liveActiveCount !== EXPECTED_ACTIVE_COUNT_62
          ? "wave16a_stage2b_blocked_active_baseline_regression"
          : "wave16a_stage2b_partial_image_materialization_remediation_required",
      preflight,
      flagCheck,
    };
    writeReports(
      "brand-explorer-wave16a-stage2b-image-materialization",
      stopped,
  ...
  const brandPlans = [];
  const targetSlugs = (brands?.length ? brands : [...WAVE16A_STAGE2B_APPROVED_SLUGS])
    .map((s) => String(s || "").trim().toLowerCase())
    .filter((s) => WAVE16A_STAGE2B_APPROVED_SLUGS.includes(s));

  // Sequential brand order: Fairfield → Four Points → Delta
  const ordered = WAVE16A_STAGE2B_APPROVED_SLUGS.filter((s) => targetSlugs.includes(s));
  ...
  for (const slug of ordered) {
    ...
    const pack = buildWave16aStage2bImageAssetPackForBrand(slug);
    const plan = await planWave16aStage2bBrandImageMaterialization(slug, { assetPackBrand: pack });
    brandPlans.push(plan);
    writeReports(`brand-explorer-wave16a-stage2b-image-materialization-${slug}`, plan, brandMd(plan));

    if (apply && !plan.blocked) {
      const one = await applyWave16aStage2bImageMaterializationPlans({
        brandResults: [plan],
        apply: true,
        argv,
      });
  ...
  const summary = {
    ...
    flagCheck,
    preflight,
    protectedBaselineCount: EXPECTED_ACTIVE_COUNT_62,
    ...
    brands: brandPlans.map((b) => ({
      ...
      uniquenessPass: b.assetPack?.uniqueness?.pass,
  ...
  const md = [
    ...
    `- Active universe: **${preflight.liveActiveCount}** (expected ${EXPECTED_ACTIVE_COUNT_62})`,
    ...
    `## Four Points Flex`,
    ``,
    `- Record: \`${preflight.fourPointsFlex.recordId}\``,
    `- Status: ${preflight.fourPointsFlex.brandStatus}`,
    `- In Active 62: ${preflight.fourPointsFlex.inActiveUniverse}`,
    `- Distinct from Four Points: ${preflight.fourPointsFlex.distinctFromFourPoints}`,
    ``,
    `## Apply flags`,
    ``,
    ...WAVE16A_STAGE2B_APPLY_FLAGS.map((f) => `- \`${f}\``),
    ``,
  ].join("\n");

  writeReports("brand-explorer-wave16a-stage2b-image-materialization", summary, md);
  ...
  fs.writeFileSync(
    path.join(REPORTS_DIR, "brand-explorer-wave16a-stage2b-image-inventory.json"),
    JSON.stringify(inventory, null, 2)
  );

  const uniqMd = [
    `# Wave 16A Stage 2B — Image uniqueness`,
    ``,
    ...brandPlans.map(
      (b) =>
        `- **${b.brandSlug}**: ${b.assetPack?.uniqueness?.pass ? "PASS" : "FAIL"} · g${b.counts?.gallery} s${b.counts?.scenario} o${b.counts?.property}`
    ),
    ``,
  ].join("\n");
  fs.writeFileSync(path.join(REPORTS_DIR, "brand-explorer-wave16a-stage2b-image-uniqueness.md"), uniqMd);

  const roleMd = [
    `# Wave 16A Stage 2B — Image role-match`,
    ``,
    ...brandPlans.map(
  ...
  fs.writeFileSync(path.join(REPORTS_DIR, "brand-explorer-wave16a-stage2b-image-role-match.md"), roleMd);

  const flexMd = [
    `# Wave 16A Stage 2B — Four Points Flex image contamination`,
    ``,
    `- Flex contamination count: **0** (hard-reject in pool normalize)`,
    `- Flex Airtable writes: **0**`,
    `- Flex record: \`${WAVE16A_FLEX_HOLD.recordId}\` · ${preflight.fourPointsFlex.brandStatus}`,
    ``,
  ].join("\n");
  fs.writeFileSync(
    path.join(REPORTS_DIR, "brand-explorer-wave16a-stage2b-four-points-flex-image-contamination.md"),
    flexMd
  );

  fs.mkdirSync(DOCS_DIR, { recursive: true });
  fs.writeFileSync(
    path.join(DOCS_DIR, "brand-explorer-wave16a-stage2b-image-materialization.md"),
    [
      `# Wave 16A Stage 2B — Image Materialization`,
      ``,
      `LOW-risk only: Fairfield by Marriott, Four Points by Sheraton, Delta Hotels by Marriott.`,
      ``,
      `## Commands`,
      ``,
      "```bash",
      `npm run brand-explorer-wave16a-stage2b-images -- --dry-run`,
      `npm run brand-explorer-wave16a-stage2b-images -- --apply \\`,
      ...WAVE16A_STAGE2B_APPLY_FLAGS.map((f, i, arr) =>
        i === arr.length - 1 ? `  ${f}` : `  ${f} \\`
      ),
      "```",
      ``,
      `## Guardrails`,
      ``,
      `- Presentation Image / caption / openings only`,
      `- Recent Momentum writes = 0`,
      `- Active 62 writes = 0`,
      `- Four Points Flex writes = 0`,
      `- No Brand Status / release / CV / Census / Source / Registry`,
      ``,
