/**
 * Wave 14 Stage 4 — Tab Factory content build (nine Marriott brands).
 *
 ...
 * protected 46, Radisson Collection, images, House of Originals, Morgans Originals,
...
import {
  ...
  WAVE14_STAGE4_APPROVED_SLUGS,
  WAVE14_TAB_FACTORY_BUILD_APPLY_FLAGS,
  WAVE14_STAGE4_CONTENT_CLEANUP_APPLY_FLAGS,
...
import {
  ...
  WAVE14_PORTFOLIO_MIX,
  getWave14BrandContent,
} from "./brand-explorer-wave14-tab-factory-content.js";

const SAFE_TGS_OPTIONS = WAVE14_SAFE_TGS;

export const WAVE14_TAB_FACTORY_BUILD_VERSION = "wave14-tab-factory-build-v1";
...
export function parseWave14TabFactoryBuildFlags(argv = []) {
  const missing = WAVE14_TAB_FACTORY_BUILD_APPLY_FLAGS.filter((f) => !argv.includes(f));
  return {
...
export function generateWave14PresentationPack(slug, opts = {}) {
  const content = getWave14BrandContent(slug);
  const pack = content.sourcePack || getWave14SourcePack(slug);
  ...
  // Enforce Wave 14 scenario title rules
  ...
  const presentation = [
    ...
    row(
      ...
      "Portfolio Mix",
      (WAVE14_PORTFOLIO_MIX[slug] || []).join("\n") +
...
function resolveWave14Identity(slug) {
...
function getApprovedStage4Scope() {
  const wanted = [...WAVE14_STAGE4_APPROVED_SLUGS];
  if (wanted.length !== 9) {
    throw new Error(`Wave 14 Stage 4 requires exactly nine brands, got ${wanted.length}`);
  }
...
export async function planWave14TabFactoryBrand(slug, { tgsOptions = [] } = {}) {
  const identity = resolveWave14Identity(slug);
  if (
  ...
  if (!WAVE14_STAGE4_APPROVED_SLUGS.includes(slug)) {
    return {
      brandSlug: slug,
      blocked: true,
      blockers: ["not_wave14_stage4_target"],
      patches: [],
      basicsPatches: [],
    };
  }
...
export async function planWave14TabFactoryBuild() {
  const scope = getApprovedStage4Scope();
  const tgsOptions = await listTargetGuestSegmentOptions();
  const brandResults = [];
  for (const slug of scope) {
    brandResults.push(await planWave14TabFactoryBrand(slug, { tgsOptions }));
  }
  ...
  return {
    version: WAVE14_TAB_FACTORY_BUILD_VERSION,
    ...
    requiredApplyFlags: [...WAVE14_TAB_FACTORY_BUILD_APPLY_FLAGS],
  };
}

export async function applyWave14TabFactoryBuild({ plan, apply = false, argv = [] } = {}) {
  const flagCheck = parseWave14TabFactoryBuildFlags(argv);
  ...
  for (const brand of plan.brandResults || []) {
    ...
    if (!WAVE14_STAGE4_APPROVED_SLUGS.includes(brand.brandSlug)) {
  ...
  return {
    ...
    reason: "wave14_tab_factory_build_applied",
    flagCheck,
    resultsByBrand,
    companyValidatedUntouched: true,
...
function writeBrandMd(brand, reportsDir) {
  const mdPath = path.join(reportsDir, `brand-explorer-wave14-tab-factory-build-${brand.reportSlug || brand.brandSlug}.md`);
  const lines = [
    `# Wave 14 Tab Factory Build — ${brand.brandName || brand.brandSlug}`,
    "",
...
export function writeWave14TabFactoryBuildReports(plan, applyResult = null) {
  ...
  const tgsMdPath = path.join(REPORTS_DIR, "brand-explorer-wave14-target-guest-segments.md");
  const tgsLines = [
    "# Wave 14 — Target Guest Segments",
    "",
  ...
  const jsonPath = path.join(REPORTS_DIR, "brand-explorer-wave14-tab-factory-build.json");
  fs.writeFileSync(jsonPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");

  const mdPath = path.join(REPORTS_DIR, "brand-explorer-wave14-tab-factory-build.md");
  const md = [
  ...
  const docPath = path.join(DOCS_DIR, "brand-explorer-wave14-tab-factory-build.md");
  fs.writeFileSync(
    docPath,
    [
      ...
      "",
      ...WAVE14_STAGE4_APPROVED_SLUGS.map((s) => `- \`${s}\``),
      "",
      "## Exclusions",
      "",
      "- `the-house-of-originals`",
...
export async function runWave14TabFactoryBuild({ dryRun = true, argv = [] } = {}) {
  const plan = await planWave14TabFactoryBuild();
  let applyResult = null;
  if (!dryRun && argv.includes("--apply")) {
    applyResult = await applyWave14TabFactoryBuild({ plan, apply: true, argv });
    if (!applyResult.applied) {
      const paths = writeWave14TabFactoryBuildReports(plan, applyResult);
      return {
        ...plan,
        pass: false,
  ...
  return {
    ...
    applyResult,
    paths,
    pass: !blocked && plan.tgsValidated === true && (plan.summary?.brandCount || 0) === 9,
    readyStatement:
      !blocked && plan.tgsValidated === true && (plan.summary?.brandCount || 0) === 9 && applyResult?.applied
        ? "wave14_stage4_content_clean_ready_for_image_materialization"
        : !blocked && plan.tgsValidated === true && (plan.summary?.brandCount || 0) === 9
          ? "wave14_stage4_tab_factory_build_dry_run_ready"
          : "wave14_stage4_tab_factory_build_blocked",
    stopRecommended: blocked || plan.tgsValidated === false,
  };
}

export const WAVE14_STAGE4_CONTENT_CLEANUP_VERSION = "wave13-stage4-content-cleanup-v1";

function parseWave14Stage4ContentCleanupFlags(argv = []) {
  const missing = WAVE14_STAGE4_CONTENT_CLEANUP_APPLY_FLAGS.filter((f) => !argv.includes(f));
  return {
    apply: argv.includes("--apply"),
    missing,
    ok: argv.includes("--apply") && missing.length === 0,
...
function writeCleanupBrandMd(brand, reportsDir) {
  const mdPath = path.join(reportsDir, `brand-explorer-wave14-stage4-content-cleanup-${brand.reportSlug || brand.brandSlug}.md`);
  const lines = [
    `# Wave 14 Stage 4.5 Content Cleanup — ${brand.brandName || brand.brandSlug}`,
    "",
    `- Slug: \`${brand.brandSlug}\``,
    `- Record: \`${brand.recordId || "—"}\``,
    `- Presentation pack rows: ${brand.presentationRowCount ?? 0}`,
...
function writeWave14Stage4ContentCleanupReports(plan, applyResult = null) {
  fs.mkdirSync(REPORTS_DIR, { recursive: true });
  fs.mkdirSync(DOCS_DIR, { recursive: true });
  ...
  const jsonPath = path.join(REPORTS_DIR, "brand-explorer-wave14-stage4-content-cleanup.json");
  fs.writeFileSync(jsonPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");

  const mdPath = path.join(REPORTS_DIR, "brand-explorer-wave14-stage4-content-cleanup.md");
  const md = [
    "# Brand Explorer Wave 14 — Stage 4.5 Content Cleanup",
    "",
    `Generated: ${plan.generatedAt}`,
    `Dry-run: **${!applyResult?.applied}** · Applied: **${applyResult?.applied === true}**`,
    `Scope: **${plan.approvedScope.join(", ")}**`,
  ...
  const docPath = path.join(DOCS_DIR, "brand-explorer-wave14-stage4-content-cleanup.md");
  fs.writeFileSync(
    docPath,
    [
      "# Wave 14 Stage 4.5 — Content Cleanup",
      "",
      "Stage 4.5 thickens thin/blank Presentation content for nine approved Wave 14 brands",
...
export async function runWave14Stage4ContentCleanup({ dryRun = true, argv = [] } = {}) {
  const plan = await planWave14TabFactoryBuild();
  plan.stage = "stage4-content-cleanup";
  plan.version = WAVE14_STAGE4_CONTENT_CLEANUP_VERSION;
  plan.requiredApplyFlags = [...WAVE14_STAGE4_CONTENT_CLEANUP_APPLY_FLAGS];

  let applyResult = null;
  if (!dryRun && argv.includes("--apply")) {
    const flagCheck = parseWave14Stage4ContentCleanupFlags(argv);
    if (!flagCheck.ok) {
      const paths = writeWave14Stage4ContentCleanupReports(plan, { applied: false, reason: "missing_apply_flags", missing: flagCheck.missing, flagCheck });
    ...
    applyResult = await applyWave14TabFactoryBuild({ plan, apply: true, argv: [...argv, ...WAVE14_TAB_FACTORY_BUILD_APPLY_FLAGS] });
    if (!applyResult.applied) {
      const paths = writeWave14Stage4ContentCleanupReports(plan, applyResult);
  ...
  return {
    ...
    applyResult,
    paths,
    pass: !blocked && (plan.summary?.brandCount || 0) === 9,
    stopRecommended: blocked,
    readyStatement: applyResult?.applied ? "wave14_stage4_content_clean_ready_for_image_materialization" : "dry_run_only",
  };
}
