...
export function buildNewBaseListRow({
    ...
    const serviceModelsSupported = formatMultiFieldForList(
        pf.serviceModelsSupported ||
            pf["Service Models Supported"] ||
            plf.serviceModelsSupported ||
            plf["Service Models Supported"]
    );
    const chainScalesSupported = formatMultiFieldForList(
        pf.chainScalesSupported || pf["Chain Scales Supported"]
    );
    const activeCountries = formatMultiFieldForList(plf.activeCountries || plf["Active Countries"]);
    const brandedResidentialCapable = deriveBrandedResidentialCapable({
        profile,
        platform,
        commercial,
        governance,
