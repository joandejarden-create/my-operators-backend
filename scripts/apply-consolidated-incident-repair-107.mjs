/**
 * Apply authorized consolidated incident manifest 107.
 * Pilot A (25 eligible APPLY) → full remainder if PASS.
 * V4 remains PAUSED. Joan authorized this run.
 *
...
import { isStreetLineAsCity, isPostalAsCity } from "../lib/research-engine-v2/census-autopilot-v3/geography/city-resolver-v4.js";
...
const MANIFEST_REL =
  "data/research-engine-v2/census-autopilot-v4-standing/geography-quality-incident-v1/107-consolidated-incident-manifest.json";
...
/**
 * Classify against live Airtable: APPLY | ALREADY_CORRECT | STALE | BLOCK
 */
function classifyLiveMutation(m, currentVal, liveFields = {}, pendingCityByKey = null) {
  ...
  if (beforeBlank) {
    ...
    // Clear of known-invalid: if current still invalid same class, allow clear of current
    if (
      ...
      if (!cls.ok) {
        // still invalid — clear current wrong value
      } else {
      ...
      // still invalid city — proceed to clear/replace path if after set, else clear
    } else {
  ...
  if (m.field === "Current Brand") {
    if (isParentCompanyAsCurrentBrand(m.after)) {
      return { action: "BLOCK", reason: "semantic_parent_as_brand", circuit: true };
    }
    if (!validateCurrentBrandSemantics(m.after).ok) {
      return { action: "BLOCK", reason: "semantic_brand_invalid", circuit: true };
    }
  }
  ...
  if (m.field === "Address" && m.after != null) {
    if (typeof m.after !== "string" || m.after === "[object Object]" || m.after.startsWith("{")) {
      return { action: "BLOCK", reason: "address_not_formatted_string", circuit: true };
    }
  }
  if (m.field === "Market" && m.after != null) {
    // Prefer pending City correction from same property when live City is still invalid
    const cityForGate =
  ...
  if (m.field === "Submarket" && m.after != null) {
    const sg = assertSubmarketWriteGate({
      country: liveFields.Country,
      market: liveFields.Market || m.market_parent,
      submarket: m.after,
      status: "MATCHED",
    });
    if (!sg.write_allowed) {
...
async function main() {
  ...
  manifest.authorized_at = new Date().toISOString();
  manifest.authorized_by = "Joan";
  manifest.apply_note = "Authorized consolidated incident repair; V4 remains PAUSED";
  ...
  wj("111-consolidated-repair-apply-manifest.json", {
    authorized: true,
    authorized_by: "Joan",
    v4_paused: true,
  ...
  const trip = (reason, detail = {}) => {
    ...
    log(`[107] CIRCUIT: ${reason}`, JSON.stringify(detail));
  };

  const txPath = path.join(OUT, "114-consolidated-repair-transactions.jsonl");
  ...
  // Build pending City corrections for Market gate coherence
  ...
  // Pre-classify all safe mutations
  log(`[107] Pre-read + classify n=${bound.length}`);
  const classified = [];
  for (let i = 0; i < bound.length; i++) {
    ...
    try {
      ...
      const d = classifyLiveMutation(m, currentVal, live.fields || {}, pendingCityByKey);
      classified.push({
        ...m,
        decision: d.action,
        reason: d.reason,
      ...
      if ((i + 1) % 40 === 0) log(`[107] pre-classify ${i + 1}/${bound.length}`);
    } catch (err) {
  ...
  wj("112-consolidated-repair-prewrite.json", {
    n: prewrite.length,
  ...
  if (circuit.tripped) {
    log("[107] Circuit during prewrite — abort");
    process.exit(3);
  }
  ...
  log(`[107] Eligible APPLY=${eligible.length} · Pilot A=${pilotA.length} · Remainder=${remainder.length}`);
  log(
    `[107] Already=${classified.filter((c) => c.decision === "ALREADY_CORRECT").length} Stale=${classified.filter((c) => c.decision === "STALE").length} Block=${classified.filter((c) => c.decision === "BLOCK").length}`
  );
  ...
  async function processMutation(m, phase) {
    ...
    try {
      ...
      const currentVal = live.fields?.[m.field] ?? null;
      const decision = classifyLiveMutation(m, currentVal, live.fields || {}, pendingCityByKey);
      ...
      if (decision.action === "ALREADY_CORRECT") {
        appendTx({ timestamp: ts, ...m, status: "ALREADY_CORRECT", expected: m.after, actual: currentVal });
  ...
  for (let i = 0; i < pilotA.length; i++) {
    ...
    if ((i + 1) % 5 === 0) log(`[107] Pilot A ${i + 1}/${pilotA.length}`);
  }
  ...
  wj("113-consolidated-repair-pilot-a.json", {
  ...
  if (pilotAPass) {
    log(`[107] Pilot A PASS — remainder n=${remainder.length}`);
    ...
    for (let i = 0; i < remainder.length; i++) {
      ...
      if ((i + 1) % 25 === 0) log(`[107] Remainder ${i + 1}/${remainder.length}`);
    }
  } else {
    log("[107] Pilot A FAIL — remainder skipped");
  }

  const allResults = [...pilotAResults, ...remainderResults];
  const updated = allResults.filter((r) => r.status === "updated");

  wj("115-consolidated-repair-postwrite.json", {
    ...
    counts: {
      ...
      stale: classified.filter((c) => c.decision === "STALE").length + allResults.filter((r) => r.status === "STALE").length,
