/**
 * Builds self-contained Old Home owner inject as a single .js file
 * (HTML + CSS embedded) for Webflow CDN hosting.
 */
import fs from 'fs';
...
const js = `/*! Dealality Old Home owner-first inject — self-contained (no Railway fetch) */
(function () {
  var API_BASE = window.DEALALITY_API_BASE || 'https://my-operators-backend-production.up.railway.app';
...
const outJs = path.join(outDir, 'dealality-old-home-owner-boot.bundle.js');
fs.writeFileSync(outJs, js);
const md5 = crypto.createHash('md5').update(js).digest('hex');
console.log(JSON.stringify({ outJs, bytes: Buffer.byteLength(js), md5 }));
