#!/usr/bin/env node
/**
 * Backfill Submarket field for Puerto Rico Demand Anchors + Travel Infrastructure.
 *
 *   node scripts/backfill-puerto-rico-submarkets.mjs
 *   node scripts/backfill-puerto-rico-submarkets.mjs --apply
 *   node scripts/backfill-puerto-rico-submarkets.mjs --table demand-anchors --apply
 *   node scripts/backfill-puerto-rico-submarkets.mjs --table travel-infrastructure --limit 10 --verbose
 *   node scripts/backfill-puerto-rico-submarkets.mjs --apply --force
 */
import "../load-env.js";
...
const TABLE_ARG = (() => {
  ...
  if (v === "demand-anchors" || v === "travel-infrastructure" || v === "both") return v;
  throw new Error('--table must be "demand-anchors", "travel-infrastructure", or "both"');
})();
...
function printSummary(label, summary) {
  ...
  console.log("Skipped (already populated):", summary.skippedAlreadyPopulated);
  ...
  if (summary.missingSubmarketAfter.length) {
    console.log("No Notes prefix (up to 20):");
    for (const m of summary.missingSubmarketAfter) {
      console.log(`  ${m.id} | ${m.name} | notes: ${m.notesPreview || "(empty)"}`);
    }
  }
}
...
async function backfillTable({ label, cfg, tableName, nameField, notesField, submarketField, countryField, cityField, selectFields, countryFilter }) {
  ...
  if (schema && !schema.has(submarketField)) {
    console.warn(`WARN: ${submarketField} not on ${tableName} — run ensure schema with --apply first`);
  }

  const requestedFields = schema
    ? selectFields.filter((name) => schema.has(name))
    : selectFields;
...
async function main() {
  ...
  if (TABLE_ARG === "demand-anchors" || TABLE_ARG === "both") {
    ...
    const r = await backfillTable({
      ...
      countryFilter: "Puerto Rico",
    });
    totals.demandUpdated = r.updated || 0;
    totals.demandFailed = r.failed || 0;
  }

  if (TABLE_ARG === "travel-infrastructure" || TABLE_ARG === "both") {
