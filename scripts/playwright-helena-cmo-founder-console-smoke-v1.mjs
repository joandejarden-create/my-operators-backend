...
function startHarness() {
  const server = http.createServer((req, res) => {
    ...
    if (p === "/api/admin/helena-cmo/attention-count") {
      return sendJson(res, 200, { ok: true, success: true, ...getHelenaAttentionCount() });
    }
...
`,
  ...
  try {
    ...
    await page.waitForSelector("#tab-brief .helena-prose", { timeout: 30000 });

    const probe = await page.evaluate(() => {
      const brief = document.getElementById("tab-brief");
      const text = brief ? brief.innerText : "";
      return {
        hasTop3: /Top 3 things that matter/i.test(text),
        hasDecide: /Joan needs to decide/i.test(text),
        hasUnknown: /UNKNOWN|DATA_GAP/i.test(text),
        hasExecuteOff: /EXECUTE OFF/i.test(document.body.innerText),
        hasPublishButton: !!document.querySelector("[data-action='PUBLISH'], [data-execute='1']"),
        decisionCards: document.querySelectorAll("#tab-brief [data-open-decision]").length,
      };
    });

    mark("page_loads", true);
    mark("js_parses_no_syntax_error", !pageErrors.some((e) => /SyntaxError/i.test(e)), pageErrors.join(" | "));
    mark("founder_brief_renders", !!probe.hasTop3 && !!probe.hasDecide);
    mark("top3_section", probe.hasTop3);
    mark("joan_decisions_section", probe.hasDecide && probe.decisionCards > 0, `cards=${probe.decisionCards}`);
