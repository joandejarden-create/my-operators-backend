#!/usr/bin/env node
/**
 * Freeze / report protected 39 Active/Live public-full Brand Explorer baseline (read-only).
 *
...
import {
  BASELINE_VERSION_39,
  build39ActivePublicFullBaseline,
  write39ActivePublicFullBaselineReports,
...
async function main() {
  ...
  console.log(`[${BASELINE_VERSION_39}] public-full freeze report (read-only)`);
  const report = await build39ActivePublicFullBaseline({
    requireReports: true,
    evaluateEvidence: !argv.includes("--skip-evidence"),
  });
  const paths = write39ActivePublicFullBaselineReports(report);
