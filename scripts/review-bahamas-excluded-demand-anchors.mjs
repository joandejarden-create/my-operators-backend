#!/usr/bin/env node
/**
 * Review excluded Bahamas demand anchors after Google verification.
 */
import "../load-env.js";
...
function recommendAction(row) {
  ...
  if (status === "No Match") {
    if (/beach|corridor|island|cay|marina|waterfront|growth|reserve|centre|center/i.test(name + pointType)) {
      return "manually verify broad island/resort zone using official/public source; add island, Bahamas to query";
    }
    return "fix official name; add island context; retry alternate query; or remove weak/minor record";
  }
  ...
  if (/business|restaurant|shop|store|hotel/i.test(row.googleName || "")) {
    return "island/resort incorrectly matched to specific business — fix name or use manual verification";
  }
  if (notes.includes("cityCountryMatch=no")) {
    return "add Nassau, Bahamas or island name to candidate city/query";
  }
  if (/beach|corridor|growth|waterfront|island|cay|marina/i.test(name + pointType)) {
    return "manually verify broad island/resort zone using official tourism source";
  }
...
const reportPath =
  getArg("--report") ||
  "fixtures/demand-anchors-bahamas-google-verification-report.json";
...
const outPath =
  getArg("--output") || "fixtures/demand-anchors-bahamas-google-review-pass-summary.json";
writeFileSync(join(root, outPath), JSON.stringify(out, null, 2) + "\n");

console.log("Bahamas excluded review:", out.summary);
