/**
 * Add optional single-line fields on "Operator Setup - Master" for Operator Explorer hero:
 *   - Explorer Hero Verification  (badge text)
 *   - Explorer Hero Data Source    (muted line after badge)
 * Mirrors Brand Setup - Brand Basics explorer hero labels.
 *
 * Env: AIRTABLE_API_KEY (schema.bases:write), AIRTABLE_BASE_ID
 *
 * Usage:
 *   node scripts/ensure-operator-master-explorer-hero-labels.mjs
 *   node scripts/ensure-operator-master-explorer-hero-labels.mjs --dry-run
 */
import "../load-env.js";
import { OPERATOR_EXPLORER_HERO_AIRTABLE } from "../lib/operator-explorer-hero-labels.js";

const TABLE_NAME = "Operator Setup - Master";

const FIELDS = [
  {
    name: OPERATOR_EXPLORER_HERO_AIRTABLE.verification,
    type: "singleLineText",
    description:
      "Operator Explorer hero badge text (e.g. Verified by operator, Demo — Not Brand-Verified). Empty = no badge unless UI default applies.",
  },
  {
    name: OPERATOR_EXPLORER_HERO_AIRTABLE.dataSource,
    type: "singleLineText",
    description:
      "Operator Explorer hero muted line after the badge (e.g. Live Airtable / Operator Setup data). Empty = UI default.",
  },
];
