/**
 * Attach materials.gallery images for Radisson RED (Choice) — press kit + hoteldam.
 *
 *   node scripts/restore-radisson-red-choice-gallery-images.mjs --dry-run
 *   node scripts/restore-radisson-red-choice-gallery-images.mjs
 *   node scripts/restore-radisson-red-choice-gallery-images.mjs --force
 */
import "../load-env.js";
...
import {
  RADISSON_RED_CHOICE_BRAND_AIRTABLE_NAME,
  RADISSON_RED_CHOICE_GALLERY_HOTELDAM_URL,
  RADISSON_RED_CHOICE_GALLERY_IMAGE_URL,
  RADISSON_RED_CHOICE_GALLERY_PROPERTY_URL,
  RADISSON_RED_CHOICE_GALLERY_TO_OPENING_KEY,
} from "../lib/radisson-red-choice-materials-restore.js";

const TABLE = "Brand Setup - Brand Explorer Presentation";
const IMAGE_FIELD = "Image";
const BRAND_NAME = RADISSON_RED_CHOICE_BRAND_AIRTABLE_NAME;
...
async function attachFromUrl(baseId, record, imageUrl, referer) {
  let downloaded = null;
...
async function main() {
  ...
  console.log(`${DRY ? "[dry-run] " : ""}Radisson RED gallery images: ${galleries.length} slot(s)`);
  ...
  for (const g of galleries) {
    ...
    const key = RADISSON_RED_CHOICE_GALLERY_TO_OPENING_KEY[slot];
    if (key) {
    ...
    const referer = RADISSON_RED_CHOICE_GALLERY_PROPERTY_URL[slot] || "";
    const candidates = [
      RADISSON_RED_CHOICE_GALLERY_IMAGE_URL[slot],
      RADISSON_RED_CHOICE_GALLERY_HOTELDAM_URL[slot],
    ].filter(Boolean);

    let ok = false;
    for (const imageUrl of candidates) {
      console.log(`  ${slot} "${title}": ${imageUrl.slice(0, 88)}…`);
      ok = await attachFromUrl(baseId, g, imageUrl, referer);
      if (ok) break;
      if (/media\.choicehotels\.com/i.test(imageUrl) && !DRY) {
        try {
          await base(TABLE).update(g.id, { [IMAGE_FIELD]: [{ url: imageUrl }] });
          console.log(`  ${slot} "${title}": Airtable URL attach fallback`);
          ok = true;
          break;
        } catch (err) {
          console.log(`  ${slot} URL attach failed: ${err.message}`);
        }
      }
    }
    if (ok) updated++;
