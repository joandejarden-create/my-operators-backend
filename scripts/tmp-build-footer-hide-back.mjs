  ...
  el.innerHTML='<div id="oh-ar-dialog" role="dialog" aria-modal="true" aria-labelledby="oh-ar-title">'+
    '<div id="oh-ar-bar">'+
      ...
      '<div id="oh-ar-actions">'+
        ...
        '<button type="button" id="oh-ar-close" aria-label="Close article">×</button>'+
      '</div>'+
    '</div>'+
    '<div id="oh-ar-body">'+
      '<div id="oh-ar-loading" class="is-on" role="status">Loading article…</div>'+
      '<div id="oh-ar-error" role="alert">This article could not be loaded in the reader. <a id="oh-ar-error-link" href="#" target="_blank" rel="noopener noreferrer">Open it in a new tab</a>.</div>'+
      '<iframe id="oh-ar-frame" title="Article reader" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>'+
    '</div>'+
  '</div>';
...
function setLoading(on){
  if(loadingEl)loadingEl.classList.toggle("is-on",!!on);
}
...
fs.writeFileSync("tmp-old-home-footer-hide-back.txt", footer);
fs.writeFileSync(
  "tmp-old-home-footer-hide-back.json",
  JSON.stringify({ content: footer })
);
console.log(JSON.stringify({
  len: footer.length,
  hasHide: footer.includes("hideEmbedChrome"),
  hasClass: footer.includes("insights-back-link"),
}));
