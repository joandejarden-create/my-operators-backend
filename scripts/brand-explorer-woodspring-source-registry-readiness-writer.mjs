...
import {
  ...
  REPORT_JSON_NAME,
  REPORT_MD_NAME,
  buildBrandExplorerWoodspringSourceRegistryReadinessWriterReport,
  v33aWriterExists,
} from "../lib/partner-intelligence/brand-explorer-woodspring-source-registry-readiness-writer.js";
...
async function main() {
  ...
  const report = await buildBrandExplorerWoodspringSourceRegistryReadinessWriterReport({
    brandArg: brand,
    apply,
    ...
    noPresentationCopy: hasFlag(APPLY_FLAG_NO_PRESENTATION_COPY),
  ...
  console.log(`v33A exists: ${v33aWriterExists() ? "yes" : "no"}`);
  console.log(`Mode: ${report.mode}`);
  console.log(`Dry-run clean: ${report.dryRunClean ? "yes" : "no"}`);
  console.log(`Airtable modified: ${report.airtableModified ? "yes" : "no"}`);
  console.log(`Company Validated untouched: ${report.companyValidatedUntouched ? "yes" : "no"}`);
  console.log(`Source updates: ${report.sourceRowsToUpdate.length}`);
  console.log(`Registry creates: ${report.registryProposedCreates.length}`);
  console.log(`Config recordId null: ${report.resolverConfigFindings.configRecordIdStillNull ? "yes" : "no"}`);
