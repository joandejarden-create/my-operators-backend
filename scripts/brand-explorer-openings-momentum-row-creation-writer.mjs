#!/usr/bin/env node
/**
 * Brand Explorer Openings + Momentum Row Creation Writer v25C-3C (Tribute Portfolio pilot).
 *
 *   npm run brand-explorer-openings-momentum-row-creation-writer -- --brand tribute-portfolio --dry-run
...
import {
  ...
  REPORT_JSON_NAME,
  REPORT_MD_NAME,
  buildBrandExplorerOpeningsMomentumRowCreationWriterMarkdown,
  buildBrandExplorerOpeningsMomentumRowCreationWriterReport,
} from "../lib/partner-intelligence/brand-explorer-openings-momentum-row-creation-writer.js";
...
async function main() {
  ...
  if (apply && (!approveBatch || !founderReviewed || !createApproved)) {
    console.error(
      `[brand-explorer-openings-momentum-row-creation-writer] Apply requires ${APPLY_FLAG_APPROVE}, ${APPLY_FLAG_FOUNDER}, and ${APPLY_FLAG_CREATE}`
    );
    process.exit(1);
  }
  ...
  const report = await buildBrandExplorerOpeningsMomentumRowCreationWriterReport({
  ...
  const markdown = buildBrandExplorerOpeningsMomentumRowCreationWriterMarkdown(report);
  ...
  console.log(`Openings planned: ${report.openingsRowsPlanned.length}`);
  console.log(`Momentum planned: ${report.momentumRowsPlanned.length}`);
  ...
  console.log(`Future-dated momentum excluded: ${report.futureDatedRowsExcludedFromMomentum ? "yes" : "no"}`);
