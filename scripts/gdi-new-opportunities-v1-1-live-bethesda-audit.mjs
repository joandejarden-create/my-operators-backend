...
const OUT_DIR = path.join(
  ROOT,
  "reports/group-demand-intelligence/gdi-new-opportunities-v1-1-live-bethesda-audit"
);
const CONCURRENCY = 4;
...
async function runLaneDiscovery(lane, ctx) {
  ...
  console.error(
    `[lane] done ${lane} serp=${native.ledger?.serpQueries || 0} urls=${urls} cand=${candidates.length} ${elapsedMs}ms`
  );
...
async function main() {
  ...
  console.error(
    `[v1.1] LIVE Bethesda audit archetype=${archetype} lanes=${AUDIT_LANES.length} concurrency=${CONCURRENCY}`
  );
  ...
  const manualAudit = liveNewActionable.map((o) => {
    ...
    const audit = {
      ...
      geographyOk: Boolean(o.demandTerritoryFit || o.destinationStatus || o.location),
      failureHint: failureReason(o, laneResults.find((r) => r.lane === o.discoveryLane)),
      // Offline auto-audit flags — human still reviews in founder report
      autoAuditResult:
  ...
  const report = {
    version: "gdi_new_opportunities_v1_1_live_bethesda",
    generatedAt: new Date().toISOString(),
    mode: "LIVE_SERP_DRY_RUN_QUALIFY",
    hotel: HOTEL,
    archetype,
    baselineCanonicalCount: baselineOpps.length,
    baselineIds,
    ...
    moreAtBats: {
      ...
      eventOnlyActionableNew: eventOnlyActionable.length,
      expandedActionableNew: expandedActionable.length,
      note: "Incremental = expanded-demand NEW that pass source+thesis auto-audit (fixtures excluded)",
    },
    quality: {
      liveNewActionable: liveNewActionable.length,
      trueAuto: passedAudit.length,
