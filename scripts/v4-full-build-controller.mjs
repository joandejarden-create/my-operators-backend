  ...
  for (let iter = 1; iter <= cfg.max_iterations_per_process; iter++) {
    ...
    // ========== LANE: MISSING FIELD SOURCE STRATEGY ==========
      ...
      if (!policyControllerEnabled) {
        ...
        let fieldReport;
        try {
          fieldReport = await runCensusMissingFieldSourceStrategyControllerV1Mission({
          ...
          console.error(
            "[controller] field_strategy_error",
            String(err?.stack || err?.message || err)
          );
          fieldReport = {
            status: "production_census_missing_field_source_strategy_controller_v1_blocked",
