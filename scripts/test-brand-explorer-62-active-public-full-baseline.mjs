#!/usr/bin/env node
/**
 * Regression lock for protected 62 Active/Live public-full baseline.
 *
 *   npm run test:brand-explorer-62-active-public-full-baseline
 *
 * Requires:
 * - reports/brand-explorer-62-active-public-full-baseline.json (freeze artifact)
 ...
 *   lockPass-clean for all 62). Wave preflight must run fresh PVQL first and
...
import {
  ...
  REPORT_JSON_62,
  ROOT,
  run62ActivePublicFullBaselineRegression,
...
async function main() {
  ...
  console.log(
    `[${BASELINE_VERSION_62}] regression test (expected Active/Live=${EXPECTED_ACTIVE_COUNT_62})`
  );
  ...
  const frozenPath = path.join(ROOT, "reports", REPORT_JSON_62);
  if (!fs.existsSync(frozenPath)) {
    console.error(`[FAIL] missing freeze artifact ${REPORT_JSON_62}`);
    console.error("  Run: npm run brand-explorer-62-active-public-full-baseline -- --dry-run");
    process.exit(1);
  }
  ...
  if (frozen.freezeDecision !== "frozen_62_active_public_full_baseline_quality_clean_flex_held") {
    console.error(
      `[FAIL] freezeDecision is not frozen_62_active_public_full_baseline_quality_clean_flex_held (${frozen.freezeDecision}). Re-run freeze.`
    );
    process.exit(1);
  }

  const result = await run62ActivePublicFullBaselineRegression({
  ...
  if (failed || !regression.pass) {
    console.error(`\n${BASELINE_VERSION_62} regression FAILED.`);
    process.exit(1);
  }

  console.log(
    `\nAll ${EXPECTED_ACTIVE_COUNT_62} Active/Live public-full baseline checks passed. writePerformed=false.`
  );
}
