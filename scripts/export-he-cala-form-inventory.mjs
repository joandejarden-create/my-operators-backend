/**
 * Map Operator Setup (new two) HTML fields to prefill values for a Master record.
 * Output: JSON lines for review (tab, field name, label, current value preview).
 */
import fs from "fs";
...
import { getSuggestionForRow } from "./he-cala-field-suggestions.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const HTML_PATH = path.join(__dirname, "..", "public", "third-party-operator-setup-new-two.html");
const MASTER_ID = process.argv[2] || "recWPKu5laVZxsvpn";
...
async function main() {
    ...
    const [bundle, ctx] = await Promise.all([loadNewBaseOperatorBundle(MASTER_ID), fetchThirdPartyOperatorPrefillContext()]);
    ...
    const outPath = path.join(__dirname, "..", "scripts", "he-cala-form-inventory.json");
    fs.writeFileSync(outPath, JSON.stringify(out, null, 2), "utf8");
    console.log("Wrote", outPath, "rows:", out.length);

    const csvPath = path.join(__dirname, "..", "scripts", "he-cala-form-inventory.csv");
    ...
    const empty = out.filter((r) => r.isEmpty).length;
    console.log("Empty-ish fields:", empty, "filled:", out.length - empty);
}
