#!/usr/bin/env node
/**
 * Brand Explorer Wave 15 factory CLI (Hilton Brand Family cohort).
 *
 *   npm run brand-explorer-wave15-factory -- --stage preflight --dry-run --reuse-fresh-reports
 *   npm run brand-explorer-wave15-factory -- --stage manifest --dry-run
 *   npm run brand-explorer-wave15-factory -- --stage factory-preview-cohort --dry-run
 *   npm run brand-explorer-wave15-factory -- --stage factory-preview-cohort --apply ...flags
...
import {
  ...
  WAVE15_STAGES,
  runWave15Factory,
} from "../lib/partner-intelligence/brand-explorer-wave15-factory.js";
...
async function main() {
  ...
  if (!stage) {
    console.error(
      `Usage: --stage <${WAVE15_STAGES.join("|")}> [--dry-run|--apply ...] [--reuse-fresh-reports]`
    );
    process.exit(2);
  }

  console.log(`[${WAVE15_VERSION}] stage=${stage} dryRun=${dryRun}`);
  const result = await runWave15Factory({ stage, dryRun, argv });
  ...
  if (result.deferred) {
    ...
    process.exitCode =
      result.reason === "protected_54_preflight_not_clean" ||
      result.reason === "wave15_manifest_not_ready"
        ? 1
        : 0;
    return;
  }
