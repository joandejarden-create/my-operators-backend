/**
 * Seed / update Company Profile records for real owner pilots.
 *
 *   node scripts/seed-pilot-company-profile.mjs
 *   node scripts/seed-pilot-company-profile.mjs --apply
 *   node scripts/seed-pilot-company-profile.mjs --apply --name Altiplano
 */
import "../load-env.js";
...
async function main() {
  ...
  const seedPath = join(__dirname, "../data/pilot-company-profiles-seed.json");
  ...
  const companyRecords = await fetchAll(TABLE);
  ...
  for (const profile of profiles) {
    ...
    const entry = {
      ...
      fieldsPreview: Object.fromEntries(
        Object.entries(fields).filter(
          ([k]) => !k.startsWith("Primary - ") && !k.startsWith("Addl - ")
        )
      ),
    };
    ...
    if (entry.fieldsPreview?.["Workspace Access"]) {
      console.log("  Workspace Access:", entry.fieldsPreview["Workspace Access"]);
    }
    if (entry.fieldsPreview?.["Company Platform Visibility"]) {
      console.log("  Platform Visibility:", entry.fieldsPreview["Company Platform Visibility"]);
    }
  }

  const reportPath = join(__dirname, "../reports/pilot-company-profiles-seed-log.json");
