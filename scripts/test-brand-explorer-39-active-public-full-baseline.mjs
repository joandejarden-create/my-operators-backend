#!/usr/bin/env node
/**
 * Regression lock for protected 39 Active/Live public-full baseline.
 *
 *   npm run test:brand-explorer-39-active-public-full-baseline
 *
 * Requires:
 * - reports/brand-explorer-39-active-public-full-baseline.json (freeze artifact)
 ...
 *   lockPass-clean for all 39). Wave preflight must run fresh PVQL first and
 *   must not rely on a stale cached pass alone — see
 *   reports/brand-explorer-39-live-pvql-regression-tightening.md
...
} from "../lib/partner-intelligence/brand-explorer-39-active-public-full-baseline.js";
...
async function main() {
  ...
  console.log(
    `[${BASELINE_VERSION_39}] regression test (expected Active/Live=${EXPECTED_ACTIVE_COUNT_39})`
  );
  ...
  const frozenPath = path.join(ROOT, "reports", REPORT_JSON_39);
  if (!fs.existsSync(frozenPath)) {
    console.error(`[FAIL] missing freeze artifact ${REPORT_JSON_39}`);
    console.error("  Run: npm run brand-explorer-39-active-public-full-baseline -- --dry-run");
    process.exit(1);
  }
  ...
  if (failed || !regression.pass) {
    console.error(`\n${BASELINE_VERSION_39} regression FAILED.`);
    process.exit(1);
  }

  console.log(
    `\nAll ${EXPECTED_ACTIVE_COUNT_39} Active/Live public-full baseline checks passed. writePerformed=false.`
  );
}
