#!/usr/bin/env node
/**
 * Freeze / report protected 54 Active/Live public-full Brand Explorer baseline (read-only).
 *
...
import {
  BASELINE_VERSION_54,
  build54ActivePublicFullBaseline,
  write54ActivePublicFullBaselineReports,
...
async function main() {
  ...
  console.log(`[${BASELINE_VERSION_54}] public-full freeze report (read-only)`);
  const report = await build54ActivePublicFullBaseline({
    requireReports: true,
    evaluateEvidence: !argv.includes("--skip-evidence"),
  });
  const paths = write54ActivePublicFullBaselineReports(report);
