...
// Fetch current footer from a saved snapshot pattern — we'll patch programmatically via a string the agent will set.
// This script builds the NEW article-reader script block that also opens opportunity-review for CTA.
...
if(ctaBtn){
  ctaBtn.addEventListener("click",function(e){
    ...
    e.preventDefault();
    var label=(document.getElementById("cta-band-btn-text")&&document.getElementById("cta-band-btn-text").textContent||ctaBtn.textContent||"Explore Your Hotel Opportunity").trim();
    openReader(ctaBtn.href||href,label,{loadingText:"Loading opportunity review…"});
  });
}
...
fs.writeFileSync("tmp-oh-reader-script.html", readerScript);
console.log("wrote reader script", readerScript.length);
