/**
 * Apply HE CALA Owner Engagement & Reporting content to:
 *   - Operator Setup - Commercial Fit & Terms (value cards + legacy JSON mirror)
 *   - Operator Setup - Engagement & Reporting (child table rows)
 *
 *   node scripts/restore-he-cala-engagement.mjs [masterRecordId] [path-to-fixture.json]
 *
 * Default master: recWPKu5laVZxsvpn
 * Default fixture: fixtures/operator-engagement-explorer-he-cala.json
 */
import fs from "fs";
...
const DEFAULT_FIXTURE = path.join(ROOT, "fixtures", "operator-engagement-explorer-he-cala.json");
...
function buildCommercialPatch(commercialRaw, engagementReporting) {
  ...
  for (const [key, value] of Object.entries(commercialRaw || {})) {
    ...
    if (JSON_COMMERCIAL_KEYS.includes(key)) continue;
    patch[key] = value;
  }
  return patch;
}
...
async function main() {
  ...
  const publicFixture = path.join(ROOT, "public", "fixtures", "operator-engagement-explorer-he-cala.json");
