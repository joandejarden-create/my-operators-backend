 ...
 *
 * Dry-run:  npm run census:core-geography-closeout -- --mode dry-run
 * Run:      …gates… npm run census:core-geography-closeout -- --mode run --enable-production-writes
 */
import "dotenv/config";
import { runCoreGeographyCloseoutV1 } from "../lib/research-engine-v2/core-geography-closeout-v1.js";
...
const report = await runCoreGeographyCloseoutV1({
  mode,
  enableProductionWrites,
  serpMax: Number(argValue("--serp-max") || 25),
  log: (m) => console.log(m),
});
