/**
 * Kimpton footprint fix: census alias + CALA footprint rows (replaces template global placeholders).
 *
 *   node scripts/apply-kimpton-census-footprint-fix.mjs --dry-run
 *   node scripts/apply-kimpton-census-footprint-fix.mjs
 */
import "../load-env.js";
...
const BASICS_ID = "recCKuXCmGvxHPfb3";
...
function footprintPatchFromCensus(summary) {
  ...
  const patch = {
    ...
    "Footprint Data Source": "Dealality Hotel Census — CALA STR import (Affiliation: Kimpton)",
    "Footprint Data Status": "Estimated",
    "Footprint Notes":
      "CALA footprint from Hotel Census (open + pipeline). Explorer Footprint tab uses census for country/archetype breakdowns when alias is active.",
...
async function main() {
  ...
  const aliasArgs = [
    ...
    "--file=fixtures/kimpton-brand-alias-reviewed.json",
  ];
  ...
  if (!footprintId) throw new Error("Kimpton Brand Footprint link missing on Basics");
  ...
  const { removed } = await writeWithFieldPruning(base, "Brand Setup - Brand Footprint", footprintId, patch);
  ...
  const cs = await buildBrandCensusSummary("Kimpton Hotels", "IHG Hotels & Resorts");
