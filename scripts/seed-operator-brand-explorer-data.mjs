#!/usr/bin/env node
/**
 * Seed Brand & Relationships Explorer fields on Profile rows linked to Master.
 *
...
} from "../lib/operator-brand-explorer-seed-data.js";
import {
...
function hasBrandSeed(fields) {
  return brandExplorerSeedMarkerKeys().some((k) => nz(fields[k]));
}

async function patchProfile(recordId, fields) {
...
async function main() {
  const [masters, profileRows] = await Promise.all([
  ...
  for (const row of profileRows) {
    ...
    if (hasBrandSeed(fields) && !FORCE) {
      plan.push({ recordId: row.id, companyName, action: "skip" });
      continue;
    }

    const seedFields = buildBrandExplorerSeedFields({
      index: plan.length,
    ...
    if (APPLY) {
      const { ok, status, json } = await patchProfile(row.id, seedFields);
      if (!ok) {
  ...
  const outPath = path.join(REPORTS, `operator-brand-explorer-seed-${stamp}.json`);
