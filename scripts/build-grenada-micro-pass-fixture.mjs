#!/usr/bin/env node
/**
 * Micro-pass fixture for Grenada DA batch-dedup skips (unique source refs).
 */
  ...
  const rules = [
    {
        ...
        "template": "https://www.puregrenada.com/places/{key}"
    }
];
...
const real = JSON.parse(
  readFileSync(join(root, "fixtures/demand-anchors-grenada-countrywide-real.json"), "utf8")
);
...
const points = (real.points || [])
  ...
  .map((p) => ({
    ...
    notes: `${p.notes || ""} Grenada micro-pass import with unique source reference.`.trim(),
  }));
...
for (const rel of [
  "fixtures/demand-anchors-grenada-countrywide-micro-pass.json",
  "public/fixtures/demand-anchors-grenada-countrywide-micro-pass.json",
]) {
