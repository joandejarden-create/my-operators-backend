/**
 * Kimpton brand pipeline: sync reference folder → extract facts → write fixtures → apply to Airtable.
 *
 *   node scripts/kimpton-brand-source-pipeline.mjs
 *   node scripts/kimpton-brand-source-pipeline.mjs --apply
 *   node scripts/kimpton-brand-source-pipeline.mjs --apply --skip-airtable
 */
import fs from "fs";
...
const TEMPLATE_FIXTURE = path.join(
  ROOT,
  "fixtures",
  "brand-explorer-presentation-kimpton-full.json"
);
...
async function main() {
  ...
  const artifacts = writeKimptonExtractionArtifacts(result.merged, PILOT, TEMPLATE_FIXTURE);
  console.log("\nWrote fixtures:");
  ...
  const reportPath = path.join(ROOT, "reports", "kimpton-brand-source-pipeline.json");
  ...
  if (seed.status !== 0) {
    console.error("Brand Setup seed failed — extend seed-kimpton-brand-setup.mjs --fixture support or apply manually.");
  }
  ...
  console.log("\nDone. Review extracted facts at /partner-intelligence-review (brand UI coming soon).");
}
