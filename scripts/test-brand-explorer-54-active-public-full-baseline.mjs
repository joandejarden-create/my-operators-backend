#!/usr/bin/env node
/**
 * Regression lock for protected 54 Active/Live public-full baseline.
 *
 *   npm run test:brand-explorer-54-active-public-full-baseline
 *
 * Requires:
 * - reports/brand-explorer-54-active-public-full-baseline.json (freeze artifact)
 * - reports/brand-explorer-24-tab-section-quality-audit.json (latest quality audit)
 *
 * Live-checks: Active/Live universe, held/excluded probes (House of Originals,
 * Morgans, Radisson, Four Points Flex by Sheraton), fresh public-full PVQL
 * (default), evidence quality for wave brands, protected Brand Status /
 * Company Validated vs freeze.
 *
 * Optional: --allow-cached-pvql-if-pass (only if on-disk PVQL is public-full
 *   lockPass-clean for all 54). Wave preflight must run fresh PVQL first and
...
import {
  ...
  REPORT_JSON_54,
  ROOT,
  run54ActivePublicFullBaselineRegression,
} from "../lib/partner-intelligence/brand-explorer-54-active-public-full-baseline.js";
...
async function main() {
  ...
  console.log(
    `[${BASELINE_VERSION_54}] regression test (expected Active/Live=${EXPECTED_ACTIVE_COUNT_54})`
  );
  ...
  const frozenPath = path.join(ROOT, "reports", REPORT_JSON_54);
  if (!fs.existsSync(frozenPath)) {
    console.error(`[FAIL] missing freeze artifact ${REPORT_JSON_54}`);
    console.error("  Run: npm run brand-explorer-54-active-public-full-baseline -- --dry-run");
    process.exit(1);
  }
  ...
  if (frozen.freezeDecision !== "frozen_54_active_public_full_baseline_semantic_clean_flex_held") {
    console.error(
      `[FAIL] freezeDecision is not frozen_54_active_public_full_baseline_semantic_clean_flex_held (${frozen.freezeDecision}). Re-run freeze.`
    );
    process.exit(1);
  }
  ...
  if (failed || !regression.pass) {
    console.error(`\n${BASELINE_VERSION_54} regression FAILED.`);
    process.exit(1);
  }

  console.log(
    `\nAll ${EXPECTED_ACTIVE_COUNT_54} Active/Live public-full baseline checks passed. writePerformed=false.`
  );
}
