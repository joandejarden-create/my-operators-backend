#!/usr/bin/env node
/**
 * Regression lock for protected 46 Active/Live public-full baseline.
 *
 *   npm run test:brand-explorer-46-active-public-full-baseline
 *
 * Requires:
 * - reports/brand-explorer-46-active-public-full-baseline.json (freeze artifact)
 ...
 *   lockPass-clean for all 46). Wave preflight must run fresh PVQL first and
...
async function main() {
  ...
  console.log(
    `[${BASELINE_VERSION_46}] regression test (expected Active/Live=${EXPECTED_ACTIVE_COUNT_46})`
  );
  ...
  const frozenPath = path.join(ROOT, "reports", REPORT_JSON_46);
  if (!fs.existsSync(frozenPath)) {
    console.error(`[FAIL] missing freeze artifact ${REPORT_JSON_46}`);
    console.error("  Run: npm run brand-explorer-46-active-public-full-baseline -- --dry-run");
    process.exit(1);
  }
  ...
  if (frozen.freezeDecision !== "frozen_46_active_public_full_baseline") {
    console.error(
      `[FAIL] freezeDecision is not frozen_46_active_public_full_baseline (${frozen.freezeDecision}). Re-run freeze.`
    );
    process.exit(1);
  }
  ...
  if (failed || !regression.pass) {
    console.error(`\n${BASELINE_VERSION_46} regression FAILED.`);
    process.exit(1);
  }

  console.log(
    `\nAll ${EXPECTED_ACTIVE_COUNT_46} Active/Live public-full baseline checks passed. writePerformed=false.`
  );
}
