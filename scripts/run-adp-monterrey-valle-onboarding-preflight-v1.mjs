#!/usr/bin/env node
/**
 * Monterrey Valle ADP onboarding — zero-cost preflight for JW + Westin.
 *
 * Usage:
 *   node scripts/run-adp-monterrey-valle-onboarding-preflight-v1.mjs
 *   node scripts/run-adp-monterrey-valle-onboarding-preflight-v1.mjs --property adp_jw_marriott_monterrey_valle
 *
...
const out = {
  gate: "ADP_MONTERREY_VALLE_ONBOARDING_PREFLIGHT_V1",
  baseCheckpoint: "98a4e32",
  PREFLIGHT: allPass ? "PASS" : "FAIL",
  PROPERTY_COUNT: rows.length,
  TOTAL_ESTIMATED_COST_BOTH: totalCost,
  COST_NOTE: "Estimate only — founder approval required before --apply provider runs",
  NEXT: allPass
...
const outPath = join(outDir, "adp-monterrey-valle-onboarding-preflight-v1.json");
