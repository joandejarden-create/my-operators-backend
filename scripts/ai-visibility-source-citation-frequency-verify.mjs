#!/usr/bin/env node
/**
 * Read-only: SOURCE_CITATION_FREQUENCY for Marriott / CALA / OpenAI / EN.
...
async function main() {
  ...
  const owned =
    portfolioOwned.ownedDomainEntries?.length
  ...
  const report = {
    ...
    CITATION_RATE: panel.CITATION_RATE,
    RESPONSES_WITH_CITATIONS: panel.RESPONSES_WITH_CITATIONS,
    OWNED_SOURCE_CITATION_RATE: panel.OWNED_SOURCE_CITATION_RATE,
    EXTERNAL_SOURCE_CITATION_RATE: panel.EXTERNAL_SOURCE_CITATION_RATE,
