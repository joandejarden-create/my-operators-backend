#!/usr/bin/env node
/**
 * Review excluded Curaçao demand anchors after Google verification.
 */
import "../load-env.js";
...
function recommendAction(row) {
  ...
  if (status === "No Match") {
    if (/beach|corridor|district|waterfront|growth|zone|lagoon|bay/i.test(name + pointType)) {
      return "manually verify broad beach/resort/port zone using official/public source; add Curaçao district to query";
    }
    return "fix official name; add Curaçao context; retry alternate query; or remove weak/minor record";
  }
  ...
  if (notes.includes("cityCountryMatch=no")) {
    return "add Willemstad, Curaçao or district to candidate city/query";
  }
  if (/beach|corridor|growth|waterfront|resort|port|lagoon/i.test(name + pointType)) {
    return "manually verify broad beach/resort/port zone using official tourism source";
  }
...
const reportPath =
  getArg("--report") ||
  "fixtures/demand-anchors-curacao-google-verification-report.json";
...
const outPath =
  getArg("--output") || "fixtures/demand-anchors-curacao-google-review-pass-summary.json";
writeFileSync(join(root, outPath), JSON.stringify(out, null, 2) + "\n");

console.log("Curaçao excluded review:", out.summary);
