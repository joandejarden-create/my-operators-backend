...
import { getBrandFieldHints } from "./brand-field-extraction-hints.js";
import {
  isIdentityFieldKey,
  isWrongBrandIdentityLeak,
  identityValueMatchesBrandContext,
} from "./brand-extraction-context.js";
...
function manualReviewFact(field, anchor, reason) {
  return factCandidate(field, {
    extractedValue: PARTNER_INTELLIGENCE_GAP_COPY,
    normalizedValue: PARTNER_INTELLIGENCE_GAP_COPY,
    evidenceText: `Skipped extraction — ${reason} (${anchor})`,
    extractionType: "Needs Confirmation",
    confidenceLevel: "Low",
    confidenceScore: 15,
    dataGap: "Yes",
    followUpQuestion: `Manual review required: ${reason}`,
    _extractionSkipped: reason,
  });
}

function pilotHintMatches(hints, brandContext) {
  if (!hints?.pilotKey) return false;
  return Boolean(brandContext?.pilotKey && hints.pilotKey === brandContext.pilotKey);
}

function tryExtractField(field, raw, anchor, brandContext) {
  const hints = getBrandFieldHints(field.fieldKey, brandContext);
  ...
  if (hints.fixedValue) {
    ...
    if (isWrongBrandIdentityLeak(field.fieldKey, val, brandContext)) {
      return manualReviewFact(field, anchor, "wrong_brand_identity_fallback_blocked");
    }

    const idx = raw.toLowerCase().indexOf(String(val).toLowerCase().slice(0, 12));
    if (idx >= 0) {
      if (!identityValueMatchesBrandContext(field.fieldKey, val, brandContext)) {
        return manualReviewFact(field, anchor, "identity_value_context_mismatch");
      }
      return factCandidate(field, {
        extractedValue: val,
        normalizedValue: val,
        evidenceText: excerptAround(raw, idx, 140),
    ...
    if (hints.identityFallback && pilotHintMatches(hints, brandContext)) {
      return factCandidate(field, {
        extractedValue: val,
        normalizedValue: val,
        evidenceText: `Fixed from brand registry (${anchor}) for ${brandContext.brandName || brandContext.pilotKey}`,
        pageSectionAnchor: anchor,
        extractionType: "Directly Stated",
    ...
    if (pilotHintMatches(hints, brandContext) && !isIdentityFieldKey(field.fieldKey)) {
      return factCandidate(field, {
        extractedValue: val,
        normalizedValue: val,
        evidenceText: `Fixed from brand registry (${anchor}) for ${brandContext.brandName || brandContext.pilotKey}`,
  ...
  for (const pattern of hints.patterns || []) {
    ...
    if (isWrongBrandIdentityLeak(field.fieldKey, value, brandContext)) {
      return manualReviewFact(field, anchor, "wrong_brand_identity_pattern_blocked");
    }
    if (isIdentityFieldKey(field.fieldKey) && !identityValueMatchesBrandContext(field.fieldKey, value, brandContext)) {
      return manualReviewFact(field, anchor, "identity_value_context_mismatch");
    }
 ...
 * @param {{ sourceTitle?: string, sourceRole?: string, localFilePath?: string, brandContext?: object }} meta
 */
export function extractBrandFactsFromText(sourceText, fields, meta = {}) {
  ...
  const brandContext = meta.brandContext || {};
  ...
  for (const field of applicable) {
    const hit = tryExtractField(field, raw, anchor, brandContext);
