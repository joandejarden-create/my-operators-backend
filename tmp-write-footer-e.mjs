...
const path = "public/marketing/old-home-footer-oh.v20260729e.js";
let js = fs.readFileSync(path, "utf8");
if (js.includes("data-oh-problem-v2")) {
  console.log("already present");
  process.exit(0);
}

// Prefer latest local d as base if e was reset
const d = fs.readFileSync("public/marketing/old-home-footer-oh.v20260729d.js", "utf8");
if (d.length >= js.length) js = d;
...
fs.writeFileSync(path, js.trimEnd() + "\n" + snippet);
console.log("wrote", path, "bytes", fs.statSync(path).size);
console.log("has problem", fs.readFileSync(path, "utf8").includes("data-oh-problem-v2"));
console.log("has opportunity", fs.readFileSync(path, "utf8").includes("ohOpenOpportunityReview"));
