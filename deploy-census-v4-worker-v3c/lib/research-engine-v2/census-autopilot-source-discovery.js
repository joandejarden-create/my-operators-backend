...
import { discoverCalaPropertiesRailwaySafe } from "./census-autopilot-v4/discover-cala-properties-railway-safe.js";
import {
...
export async function discoverCalaProperties(opts = {}) {
  ...
  const discoverySafeCfg = loadDiscoveryRailwaySafeConfig(opts.discoverySafeConfig || {});
  if (opts.railwaySafe === true || discoverySafeCfg.railway_safe_mode) {
    const safe = await discoverCalaPropertiesRailwaySafe({
      opts: {
        ...opts,
        discoverySafeConfig: discoverySafeCfg,
      },
      controlList,
      discoverAllOfficial,
      requireBrandMatch,
    ...
    if (opts.includeVicEvidence !== false) {
      ...
      for (const claim of vic.byId.values()) {
        ...
        if (controlList.brands.some((b) => b.brand_family === fam) || !opts.parentCompany) {
          vicEvidence.push(vicClaimToEvidenceRow(claim, fam));
        }
      }
      safe.sourceReport.vic_evidence_rows = vicEvidence.length;
      if (!safe.sourceReport.families_used.includes("VIC_evidence")) {
        safe.sourceReport.families_used.push("VIC_evidence");
      }
    }

    const annotated = (safe.discovered || []).map((row) => annotateDiscoveredGovernance(row, opts));
    const governanceCounts = {};
    for (const row of annotated) {
      const key = row.brand_governance_status || "unknown";
      governanceCounts[key] = (governanceCounts[key] || 0) + 1;
    }
    safe.sourceReport.cache_stats = getFamilyAdapterCacheStats();
    safe.sourceReport.discovered_count = annotated.length;
    safe.sourceReport.brand_governance_counts = governanceCounts;
    safe.sourceReport.owner_facing_eligible_count = annotated.filter(
      (r) => r.owner_facing_eligible
    ).length;
    safe.sourceReport.promotion_candidate_count = annotated.filter(
