...
export async function aggregateCensusPresenceSummary(params) {
  ...
  for (const rec of records) {
    ...
    const row = {
      ...
      region: exactMatchKey(f[CENSUS_FIELDS.region]),
  ...
  for (const row of openRows) {
    ...
    bumpBreakdown(regionMap, resolveDealalityRegion(row.region, row.country), STATUS_OPEN, 1, row.rooms);
  ...
  for (const row of pipelineRows) {
    ...
    bumpBreakdown(regionMap, resolveDealalityRegion(row.region, row.country), STATUS_PIPELINE, 1, row.rooms);
