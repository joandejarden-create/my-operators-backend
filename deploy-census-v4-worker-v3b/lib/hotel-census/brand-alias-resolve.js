...
import { normalizeBrandNameForCensusAliasLookup } from "./mvp-census-canonical-brands.js";
...
export function normalizeParentCompanyKey(value) {
  ...
  if (
    ...
    s === "ihg hotels" ||
    s === "intercontinental hotels group" ||
    s === "ihg"
  ) {
    return "ihg hotels";
  }
...
export async function resolveBrandAffiliationMatchers(requestedBrand, parentCompany, opts = {}) {
  ...
  const aliasLookupName = normalizeBrandNameForCensusAliasLookup(requested);
  const warnings = [];
  const matchPath = [];
  if (aliasLookupName !== requested) {
    matchPath.push("mvp_canonical_map");
  }
  ...
  for (const row of aliasRows) {
    if (row.canonicalBrandName === aliasLookupName) {
  ...
  if (!canonicalBrandName) {
    for (const row of aliasRows) {
      if (row.aliasSourceBrandName === aliasLookupName) {
        canonicalBrandName = row.canonicalBrandName;
  ...
  if (canonicalBrandName && scopedRows.length > 0) {
    ...
    if (!canonicalBrandName) {
      canonicalBrandName = aliasLookupName;
    ...
    affiliationMatchers = [aliasLookupName];
  }
