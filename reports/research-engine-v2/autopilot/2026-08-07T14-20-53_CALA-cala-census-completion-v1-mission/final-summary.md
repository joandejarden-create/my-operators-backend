# CALA Census Completion Mission — Final Summary

**Status:** `production_census_cala_completion_v1_partial_source_remaining`  
**Objective:** `cala-census-completion-v1`  
**Generated:** 2026-08-07T14:52:32.150Z  
**Airtable writes:** yes  
**Inserts:** 0  
**Updates:** 22  
**Run dir:** `C:\Dev\deal-capture-proxy\reports\research-engine-v2\autopilot\2026-08-07T14-20-53_CALA-cala-census-completion-v1-mission`

## Before → After

| Metric | Before | After |
|--------|-------:|------:|
| Total records | 1205 | 1224 |
| Clean Core | 1002 | 1022 |
| Below Clean Core | 203 | 202 |
| Continent complete | 1201 | 1220 |
| Sub-Continent complete | 1201 | 1220 |
| Market complete | 758 | 776 |
| Submarket complete | 161 | 200 |
| State / Region complete | 231 | 250 |
| Address complete | 379 | 403 |
| Address Confidence High | 303 | 327 |
| Address Source URL complete | 304 | 328 |
| Lat/Long complete | 366 | 374 |
| Mapbox eligible | 86 | 93 |
| Phone complete | 299 | 350 |
| Rooms complete | 116 | 116 |
| Complete Census v1 | 1 | 8 |
| Map Ready | 192 | 219 |
| Contact Ready | 190 | 237 |
| Size Ready | 79 | 86 |
| Blocked missing address | 724 | 710 |
| Blocked dirty identity | 181 | 199 |
| Blocked source access | 0 | 0 |
| Coord blocked dirty identity | 17 | 17 |
| Steward remaining | 28 | 27 |
| Source lookup remaining | 5 | 5 |
| Duplicate risk remaining | 170 | 170 |
| Est. Mapbox requests | 86 | 93 |
| Est. Mapbox cost USD | 0.43 | 0.465 |

## Soft targets met

- address_complete: yes
- lat_long_complete: yes
- phone_complete: yes
- rooms_complete: yes
- complete_census_v1: no

## Fields written

- Submarket
- Latitude
- Longitude
- Coordinate Source Type
- Coordinate Confidence
- Geocode Provider
- Geocode Method
- Geocode Reviewed Date
- Radar Geography Status
- Radar Display Status
- Radar Display Reason
- Public Display Review Status
- Phone

## Queues executed

- brand_normalization
- parent_company_normalization
- core_identity_quality
- core_identity_source_lookup
- key_field_completion
- clean_core_classification
- market_geography_completion
- address_confirmation
- coordinate_completion
- phone_number_enrichment
- rooms_keys

## Phases

- **Reconfirm Brand / Core Identity**: updates=0
- **Clean Core Classification**: updates=0
- **Market Geography**: updates=3
- **Address Completion (Clean Core)**: updates=0
- **Coordinate Completion (Clean Core + High Address)**: updates=8
- **Phone Completion (Clean Core / official only)**: updates=11
- **Rooms Completion (Clean Core / official only)**: updates=0
- **Final Readiness Classification**: updates=0

## Safety stops

- (none)

## Next recommended action

Official sources exhausted for some Level 2 fields — dirty partner labels remain parked; do not invent address/phone/rooms/coords or promote Brand Setup automatically.
