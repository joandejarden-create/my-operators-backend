# GDI PE Test/Fixture Cleanup — Dry Run

Generated: 2026-09-24T08:21:56.097Z  
Apply: **NO**

## Classification table

| Record ID | Opportunity ID | Title | Hotel | Type | Family | Venue | Class | Action |
|-----------|----------------|-------|-------|------|--------|-------|-------|--------|
| `recUD78wICYV2Ec17` | `gdi_pe_781f12393f8117e7` | Woman's Club of Bethesda — Preferred Lodging Partnership | `recLuxvwwxID7U2B8` | VENUE_PARTNERSHIP | PRIVATE_EVENTS | `pev_d7991905b5642dc4` | **PRODUCTION_REAL** | **KEEP** |

## Tallies

{
  "TOTAL_PE_GDI_ROWS": 1,
  "PRODUCTION_REAL": 1,
  "TEST": 0,
  "FIXTURE": 0,
  "SAMPLE": 0,
  "TEMP_VALIDATION": 0,
  "AMBIGUOUS": 0,
  "LIVE_CANARY_REAL_BUT_NOT_PROMOTED": 0
}

## Fixture venues

FIXTURE: 0 · REAL: 11 · AMBIGUOUS: 0

## Verdict

**CUSTOMER VISIBILITY GUARD ADDED — NO CLEANUP NEEDED**
