/**
 * Old Home — How Dealality Works timeline (v20260812c)
 * Eyebrow: A Clear Path to Agreement.
 * Path-gated to /, /es, /old-home. CTAs match live Explore / Request Demo behavior.
 * v12c: localize marketing chrome + offline step copy on /es (platform titles/UI stay EN).
 */
(function () {
  "use strict";
  try {
  ...
  /* Platform titles + mock UI stay English on /es.
     Offline relationship steps + marketing chrome localize. */
  if (isEs) {
    var OFFLINE_ES = {
      "04": {
    ...
    for (var _si = 0; _si < STEPS.length; _si++) {
      ...
      var _es = OFFLINE_ES[STEPS[_si].n];
      if (!_es) continue;
  ...
  var CSS = [
    ...
    "@media (max-width:900px){#" + ROOT_ID + "{padding:44px 1rem 40px;--ptm-card-h:185px;--ptm-path-h:56px}#" + ROOT_ID + " .ptm-eyebrow{flex-wrap:wrap;padding:4px 10px 4px 4px}#" + ROOT_ID + " .ptm-eyebrow-right{white-space:normal;margin:6px 2px 2px 8px}#" + ROOT_ID + " .ptm-detail{width:min(420px,70%);bottom:4px;top:auto}}"
  ].join("");
  ...
  function mount(host) {
    ...
    track("process_section_view", { section: "how-we-do-it", variant: "timeline-12c" });
  }
