/**
 * Old Home Boot Guard (v20260808c)
 * Path-gated to /, /es, and /old-home (homepage + Spanish locale).
 * 08c: Title Case section H2s (FAQs/testimonials).
 * 08b: FAQs sentence-case H2 (EN/ES) via v20260808b.
 * 08a: testimonials 02e — LATAM polish (acuerdo / compensaciones) on /es.
 * 02m: testimonials 02d — Lead Asset Manager → Gerente Principal de Activos on /es.
 * 02l: many-futures i18n 02e + testimonials 02c — Spanish Title Case function words.
 * 02k: many-futures i18n 02d — keep .mf-ui / Deal Compare platform mock UI in English on /es.
 * 02j: footer-oh 02b — hoist orPath/t for iframe opener on /es — locale OR iframe URLs + Spanish reader chrome.
 * 02f: many-futures i18n 02b — Desired/Decision outcomes hardened on /es.
 * 02e: Spanish Coming Soon + scroll-cue 02a.
 * 02d: many-futures i18n 02a — /es questions, opportunity, purpose/benefit.
 * 02c: testimonials 02a — /es keeps Justin + Gustavo (not static placeholders).
 * 02a: unlock path gate for Spanish home `/es` (globe + critical CSS).
 * 01x: Many Futures mobile — after question select, scroll title under sticky nav.
 * 01w: FAQs 01d — mobile open scrolls question top into view.
 * 01v: freeform-head 01f — footer Platform/Learn/Company top-align (not end).
 * 01t: footer — Insights → /insights; dim Brand/Operator Intelligence,
 *      Case Studies, About with " - Coming Soon".
 * 01s: globe 01e — remove MENA pins (Casablanca, Cairo, Riyadh, Dubai).
 * 01q: hero scroll-cue 01c — eyebrow snug under sticky chrome (no dead nav gap).
...
(function () {
  try {
    ...
    if (window.__ohBootGuard && window.__ohBootGuard >= 2026080801) return;
    window.__ohBootGuard = 2026080801;
    ...
    function ensureFooterSoonCss() {
      if (document.getElementById("oh-footer-soon-css")) return;
      var st = document.createElement("style");
      st.id = "oh-footer-soon-css";
      st.textContent =
        "#footer-new a.oh-footer-link-soon," +
        "#footer a.oh-footer-link-soon," +
        "footer a.oh-footer-link-soon{" +
        "opacity:.38!important;" +
        "color:rgba(232,236,245,.38)!important;" +
        "cursor:default!important;" +
        "pointer-events:none!important;" +
        "text-decoration:none!important" +
        "}" +
        "#footer-new a.oh-footer-link-soon:hover," +
        "#footer a.oh-footer-link-soon:hover," +
        "footer a.oh-footer-link-soon:hover{" +
        "opacity:.38!important;" +
        "color:rgba(232,236,245,.38)!important" +
        "}";
    ...
    function dimFooterSoonLink(a, baseLabel) {
      if (!a) return;
      a.classList.add("oh-footer-link-soon");
      a.setAttribute("aria-disabled", "true");
      a.setAttribute("tabindex", "-1");
      a.setAttribute("role", "text");
      a.setAttribute("title", "Coming soon");
      a.setAttribute("data-oh-footer-soon", "1");
      try {
        a.removeAttribute("href");
      } catch (_e) {}
      var soon = (path === "/es" || path.indexOf("/es/") === 0) ? " - Próximamente" : " - Coming Soon";
      a.textContent = baseLabel + soon;
      if (a.getAttribute("data-oh-footer-soon-bound") !== "1") {
        a.setAttribute("data-oh-footer-soon-bound", "1");
        a.addEventListener(
          "click",
          function (e) {
            e.preventDefault();
    ...
    function footerLinkBaseLabel(text) {
      return String(text || "")
        .replace(/\s*[-–—]\s*Coming Soon\s*$/i, "")
        .replace(/\s*[-–—]\s*Próximamente\s*$/i, "")
        .replace(/\s+/g, " ")
        .trim();
    }
    ...
    function rewriteFooterLinks() {
      ...
      var whoDest = "https://www.dealality.com/who-its-for";
      var insightsDest = "https://www.dealality.com/insights";
      var soonExact = {
        "Brand Intelligence": true,
        "Operator Intelligence": true,
        "Case Studies": true,
        About: true,
      };
      ...
      roots.forEach(function (root) {
        ...
        for (var i = 0; i < links.length; i++) {
          var a = links[i];
          var text = footerLinkBaseLabel(a.textContent);
          if (!text) continue;

          if (
            /^The Dealality Method$/i.test(text) ||
            /^Dealality Method$/i.test(text)
          ) {
            a.textContent = "Who It's For";
            a.setAttribute("href", whoDest);
            a.setAttribute("data-oh-footer-who-its-for", "1");
            continue;
          }

          if (/^Insights$/i.test(text)) {
            a.setAttribute("href", insightsDest);
            a.setAttribute("data-oh-footer-insights", "1");
            a.classList.remove("oh-footer-link-soon");
            a.removeAttribute("aria-disabled");
            a.removeAttribute("data-oh-footer-soon");
            continue;
          }

          if (soonExact[text]) {
            dimFooterSoonLink(a, text);
          }
        }
      });
      ...
      for (var j = 0; j < all.length; j++) {
        all[j].setAttribute("href", whoDest);
      }
    }

    rewriteFooterLinks();
    if (document.readyState === "loading") {
      document.addEventListener("DOMContentLoaded", rewriteFooterLinks);
    }
    [0, 400, 1200, 2500].forEach(function (ms) {
      setTimeout(rewriteFooterLinks, ms);
    });
    ...
    var GLOBE_JS =
      "6a6e1f82590d5f90320c98e3_dealality-old-home-hero-globe-bg.v20260801e.js";
    ...
    [
      ...
      "6a6e49a3ab735f9c93cd83c8_dealality-old-home-freeform-head.v20260801f.css",
    ...
    if (
      ...
      !document.querySelector('script[data-oh-globe="01e"]')
    ) {
      var globeEarly = document.createElement("script");
      globeEarly.src = b + GLOBE_JS;
      globeEarly.async = true;
      globeEarly.setAttribute("data-oh-globe", "01e");
      head.appendChild(globeEarly);
    }

    // Below-fold / secondary scripts — deferred, after globe kickoff.
    [
      "6a6efedf4d84af73afe2bf13_dealality-old-home-platform-video-launcher.v20260802b.js",
      "6a7771403a753f3186be3b3f_dealality-old-home-testimonials.v20260808c.js",
      "6a777140157ed9bc2f0a1496_dealality-old-home-faqs.v20260808c.js",
      "6a6f1089b514b0b0c32c27f0_dealality-old-home-many-futures-i18n.v20260802e.js",
      "6a6e6f0c582749ce2c2c9de6_dealality-old-home-many-futures-mobile-scroll.v20260801a.js",
      "6a6f051416dc67b7fb50ad98_old-home-footer-oh.v20260802b.js",
      // Quiet hero tertiary: See How Dealality Works → How We Do It eyebrow
      "6a6efedf34b8bcac93ea315e_dealality-old-home-hero-scroll-cue.v20260802b.js",
    ].forEach(function (f) {
      var s = document.createElement("script");
      s.defer = true;
      s.src = b + f;
      if (f.indexOf("platform-video-launcher") !== -1 || f.indexOf("PVL_02A") !== -1) {
        s.setAttribute("data-oh-pvl", "02b");
      }
      if (f.indexOf("hero-scroll-cue") !== -1) {
        s.setAttribute("data-oh-hero-scroll-cue", "02b");
      }
      head.appendChild(s);
    });
  } catch (e) {
