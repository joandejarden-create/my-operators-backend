/**
 * Old Home section order (v20260731b)
 * Path-gated to /old-home.
 * Hero → Problem → How Dealality Works → Owner Benefits → Features →
 * Full Ecosystem → Perspectives → About → FAQ → Insights → Pricing → CTA → Footer
 */
(function () {
  try {
    var path = (window.location && window.location.pathname) || "";
    if (path !== "/old-home") return;
    if (window.__ohSectionOrder >= 202607312) return;
    window.__ohSectionOrder = 202607312;
