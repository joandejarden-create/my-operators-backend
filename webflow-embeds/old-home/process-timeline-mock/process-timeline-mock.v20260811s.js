/**
 * How Dealality Works timeline mock (v20260811r)
 * Step detail sits below the timeline so steps stay clickable while previewing.
 * Does not replace #oh-how-we-do-it.
 */
(function () {
  "use strict";
  var ROOT_ID = "oh-process-timeline-mock";
  var VERSION = "v20260811r";
  ...
  var CSS = [
    ...
    "#" + ROOT_ID + " .ptm-detail{position:relative;left:auto;top:auto;z-index:5;width:100%;max-width:806px;margin:18px auto 0;max-height:0;overflow:hidden;border-radius:16px;border:1px solid transparent;background:rgba(8,15,37,.97);padding:0 22px;box-shadow:none;opacity:0;visibility:hidden;pointer-events:none;transform:translateY(-6px);transition:opacity .22s var(--ptm-ease),transform .22s var(--ptm-ease),visibility .22s,max-height .28s var(--ptm-ease),padding .22s,border-color .22s,margin .22s,box-shadow .22s}",
    "#" + ROOT_ID + " .ptm-detail.is-open{opacity:1;visibility:visible;pointer-events:auto;transform:translateY(0);max-height:min(78vh,720px);overflow:auto;padding:20px 22px 18px;border-color:rgba(255,255,255,.14);box-shadow:0 18px 48px rgba(0,0,0,.4),0 0 0 1px rgba(108,114,255,.12)}",
    "#" + ROOT_ID + " .ptm-detail.is-swapping{opacity:0;transform:translateY(4px)}",
    "#" + ROOT_ID + " .ptm-detail.is-offline{border-color:rgba(215,142,44,.35);box-shadow:0 18px 48px rgba(0,0,0,.4),0 0 0 1px rgba(215,142,44,.18)}",
    ...
    "@media (max-width:980px){#" + ROOT_ID + " .ptm-detail{max-width:min(720px,100%);max-height:0}#" + ROOT_ID + " .ptm-detail.is-open{max-height:min(72vh,640px)}#" + ROOT_ID + " .ptm-d-paths,#" + ROOT_ID + " .ptm-d-moments{grid-template-columns:1fr}}",
    "@media (max-width:900px){#" + ROOT_ID + "{padding:44px 1rem 40px;--ptm-card-h:175px;--ptm-path-h:56px}#" + ROOT_ID + " .ptm-eyebrow{flex-wrap:wrap;padding:4px 10px 4px 4px}#" + ROOT_ID + " .ptm-eyebrow-right{white-space:normal;margin:6px 2px 2px 8px}#" + ROOT_ID + " .ptm-detail{max-width:100%}}"
  ].join("");
  ...
  function mount(host) {
    ...
    host.innerHTML =
      ...
      '<p class="ptm-hint">Hover a step to preview the detail below the timeline</p>' +
