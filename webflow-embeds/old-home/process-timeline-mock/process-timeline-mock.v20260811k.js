/**
 * How Dealality Works timeline mock (v20260811k)
 * Still taller dual-track cards + larger Outside/In Platform gap so step copy never clips.
 * Does not replace #oh-how-we-do-it.
 */
(function () {
  "use strict";
  var ROOT_ID = "oh-process-timeline-mock";
  var VERSION = "v20260811k";
  ...
  var CSS = [
    ...
    "#" + ROOT_ID + " .ptm-grid{display:grid;grid-template-columns:repeat(8,minmax(0,1fr));grid-template-rows:auto minmax(var(--ptm-card-h),auto) var(--ptm-band-h);width:100%;min-width:980px;row-gap:var(--ptm-track-gap)}",
    "#" + ROOT_ID + " .ptm-grid-head{grid-column:1/-1;grid-row:1;padding:14px 14px 0;text-align:left}",
    "#" + ROOT_ID + " .ptm-grid-head span{font-size:12px;font-weight:800;letter-spacing:.11em;text-transform:uppercase;color:#F0B86A}",
    "#" + ROOT_ID + " .ptm-col-offline{grid-row:2;padding:0 6px 8px;min-height:var(--ptm-card-h);height:auto;display:flex;align-items:stretch;justify-content:center;min-width:0}",
    ...
    "#" + ROOT_ID + " .ptm-col-platform{padding:72px 6px 24px;display:flex;align-items:flex-end;justify-content:center;position:relative;z-index:2;height:var(--ptm-band-h);min-width:0}",
    ...
    "#" + ROOT_ID + " .ptm-card{display:flex;flex-direction:column;width:100%;max-width:156px;height:auto;min-height:var(--ptm-card-h);max-height:none;padding:18px 14px 20px;border-radius:12px;overflow:visible;cursor:pointer;transition:border-color .2s,box-shadow .2s,transform .2s var(--ptm-ease);animation:ptmCardIn .55s var(--ptm-ease) both;animation-delay:calc(var(--ptm-i,0)*.05s)}",
    ...
    "#" + ROOT_ID + " .ptm-card h4{font-size:15px;font-weight:800;line-height:1.3;letter-spacing:-.02em;margin:0 0 10px;flex:0 0 auto}",
    "#" + ROOT_ID + " .ptm-card p{font-size:13px;line-height:1.5;margin:0;flex:0 0 auto;min-height:0;overflow:visible;display:block;-webkit-line-clamp:unset;line-clamp:unset}",
    ...
    "#" + ROOT_ID + " .ptm-offline-card::after{content:\"\";position:absolute;left:50%;bottom:calc(-1 * var(--ptm-track-gap));width:2px;height:var(--ptm-track-gap);transform:translateX(-50%);background:linear-gradient(180deg,#E8A84A,rgba(232,168,74,.12));border-radius:2px}",
    ...
    "@media (max-width:900px){#" + ROOT_ID + "{padding:44px 1rem 40px;--ptm-card-h:320px;--ptm-band-h:480px;--ptm-track-gap:72px}#" + ROOT_ID + " .ptm-eyebrow{flex-wrap:wrap;padding:4px 10px 4px 4px}#" + ROOT_ID + " .ptm-eyebrow-right{white-space:normal;margin:6px 2px 2px 8px}#" + ROOT_ID + " .ptm-detail{top:50%;width:min(640px,calc(100% - 16px))}#" + ROOT_ID + " .ptm-col-platform{padding:64px 6px 20px}}"
  ].join("");
