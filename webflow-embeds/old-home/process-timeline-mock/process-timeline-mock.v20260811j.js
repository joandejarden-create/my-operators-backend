/**
 * How Dealality Works timeline mock (v20260811j)
 * Extra-tall dual-track cards and band spacing so step copy is fully readable.
 * Does not replace #oh-how-we-do-it.
 */
(function () {
  "use strict";
  var ROOT_ID = "oh-process-timeline-mock";
  var VERSION = "v20260811j";
  ...
  var CSS = [
    "#" + ROOT_ID + "{--ptm-font:\"Inter Tight\",\"Plus Jakarta Sans\",system-ui,sans-serif;--ptm-display:\"Plus Jakarta Sans\",\"Inter Tight\",system-ui,sans-serif;--ptm-ease:cubic-bezier(.22,1,.36,1);--ptm-card-h:252px;--ptm-band-h:380px;position:relative;overflow:hidden;box-sizing:border-box;font-family:var(--ptm-font);color:#e8ecf5;background:#080F25;padding:52px 1.5rem 44px;border-top:1px solid rgba(255,255,255,.07);border-bottom:1px solid rgba(255,255,255,.07)}",
    ...
    "#" + ROOT_ID + " .ptm-grid{display:grid;grid-template-columns:repeat(8,minmax(0,1fr));grid-template-rows:auto var(--ptm-card-h) var(--ptm-band-h);width:100%;min-width:980px;row-gap:44px}",
    ...
    "#" + ROOT_ID + " .ptm-col-offline{grid-row:2;padding:0 6px 8px;height:var(--ptm-card-h);display:flex;align-items:stretch;justify-content:center;min-width:0}",
    ...
    "#" + ROOT_ID + " .ptm-col-platform{padding:48px 6px 18px;display:flex;align-items:flex-end;justify-content:center;position:relative;z-index:2;height:var(--ptm-band-h);min-width:0}",
    ...
    "#" + ROOT_ID + " .ptm-card{display:flex;flex-direction:column;width:100%;max-width:148px;height:var(--ptm-card-h);min-height:var(--ptm-card-h);max-height:var(--ptm-card-h);padding:16px 13px;border-radius:12px;cursor:pointer;transition:border-color .2s,box-shadow .2s,transform .2s var(--ptm-ease);animation:ptmCardIn .55s var(--ptm-ease) both;animation-delay:calc(var(--ptm-i,0)*.05s)}",
    ...
    "#" + ROOT_ID + " .ptm-card h4{font-size:15px;font-weight:800;line-height:1.25;letter-spacing:-.02em;margin:0 0 10px;flex:0 0 auto}",
    "#" + ROOT_ID + " .ptm-card p{font-size:13px;line-height:1.45;margin:0;flex:1 1 auto;min-height:0;overflow:visible}",
    ...
    "#" + ROOT_ID + " .ptm-offline-card::after{content:\"\";position:absolute;left:50%;bottom:-52px;width:2px;height:52px;transform:translateX(-50%);background:linear-gradient(180deg,#E8A84A,rgba(232,168,74,.12));border-radius:2px}",
    ...
    "@media (max-width:900px){#" + ROOT_ID + "{padding:44px 1rem 40px;--ptm-card-h:260px;--ptm-band-h:360px}#" + ROOT_ID + " .ptm-eyebrow{flex-wrap:wrap;padding:4px 10px 4px 4px}#" + ROOT_ID + " .ptm-eyebrow-right{white-space:normal;margin:6px 2px 2px 8px}#" + ROOT_ID + " .ptm-detail{top:50%;width:min(640px,calc(100% - 16px))}#" + ROOT_ID + " .ptm-grid{row-gap:36px}}"
  ].join("");
