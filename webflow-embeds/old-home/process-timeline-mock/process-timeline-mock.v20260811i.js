/**
 * How Dealality Works timeline mock (v20260811i)
 * Taller dual-track timeline with more space between bands; wider hover detail overlay.
 * Does not replace #oh-how-we-do-it.
 */
(function () {
  "use strict";
  var ROOT_ID = "oh-process-timeline-mock";
  var VERSION = "v20260811i";
  ...
  var CSS = [
    "#" + ROOT_ID + "{--ptm-font:\"Inter Tight\",\"Plus Jakarta Sans\",system-ui,sans-serif;--ptm-display:\"Plus Jakarta Sans\",\"Inter Tight\",system-ui,sans-serif;--ptm-ease:cubic-bezier(.22,1,.36,1);--ptm-card-h:196px;--ptm-band-h:320px;position:relative;overflow:hidden;box-sizing:border-box;font-family:var(--ptm-font);color:#e8ecf5;background:#080F25;padding:52px 1.5rem 44px;border-top:1px solid rgba(255,255,255,.07);border-bottom:1px solid rgba(255,255,255,.07)}",
    ...
    "#" + ROOT_ID + " .ptm-grid{display:grid;grid-template-columns:repeat(8,minmax(0,1fr));grid-template-rows:auto var(--ptm-card-h) var(--ptm-band-h);width:100%;min-width:980px;row-gap:18px}",
    "#" + ROOT_ID + " .ptm-grid-head{grid-column:1/-1;grid-row:1;padding:12px 14px 2px;text-align:left}",
    "#" + ROOT_ID + " .ptm-grid-head span{font-size:12px;font-weight:800;letter-spacing:.11em;text-transform:uppercase;color:#F0B86A}",
    "#" + ROOT_ID + " .ptm-col-offline{grid-row:2;padding:0 6px 12px;height:var(--ptm-card-h);display:flex;align-items:stretch;justify-content:center;min-width:0}",
    "#" + ROOT_ID + " .ptm-band-label--platform{position:absolute;top:12px;left:14px;right:auto;z-index:3;text-align:left;font-size:12px;font-weight:800;letter-spacing:.11em;text-transform:uppercase;color:rgba(255,255,255,.42);pointer-events:none;margin:0}",
    "#" + ROOT_ID + " .ptm-col-platform-wrap{grid-column:1/-1;grid-row:3;display:grid;grid-template-columns:subgrid;position:relative;height:var(--ptm-band-h);background:linear-gradient(180deg,rgba(17,27,58,.55),rgba(8,15,37,.72));border-top:1px solid rgba(255,255,255,.08);border-bottom:1px solid rgba(255,255,255,.06)}",
    ...
    "#" + ROOT_ID + " .ptm-col-platform{padding:0 6px;display:flex;align-items:center;justify-content:center;position:relative;z-index:2;height:var(--ptm-band-h);min-width:0}",
    ...
    "#" + ROOT_ID + " .ptm-path-layer text.ptm-node-label{fill:#F0B86A;font-size:12px;font-weight:800;font-family:var(--ptm-font);text-anchor:middle;dominant-baseline:central;pointer-events:none}",
    "#" + ROOT_ID + " .ptm-path-layer text.ptm-node-label-platform{fill:#a8abff}",
    "#" + ROOT_ID + " .ptm-card{display:flex;flex-direction:column;width:100%;max-width:136px;height:var(--ptm-card-h);min-height:var(--ptm-card-h);max-height:var(--ptm-card-h);padding:14px 12px;border-radius:12px;cursor:pointer;transition:border-color .2s,box-shadow .2s,transform .2s var(--ptm-ease);animation:ptmCardIn .55s var(--ptm-ease) both;animation-delay:calc(var(--ptm-i,0)*.05s)}",
    "#" + ROOT_ID + " .ptm-card:hover,#" + ROOT_ID + " .ptm-card.is-active{transform:translateY(-2px)}",
    "#" + ROOT_ID + " .ptm-card .ptm-num{font-size:12px;letter-spacing:.08em;text-transform:uppercase;margin-bottom:8px;flex:0 0 auto;line-height:1.2;font-weight:700}",
    "#" + ROOT_ID + " .ptm-card h4{font-size:15px;font-weight:800;line-height:1.25;letter-spacing:-.02em;margin:0 0 8px;min-height:2.5em;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden;flex:0 0 auto}",
    "#" + ROOT_ID + " .ptm-card p{font-size:13px;line-height:1.4;margin:0;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:5;overflow:hidden;flex:1 1 auto;min-height:0}",
    ...
    "#" + ROOT_ID + " .ptm-offline-card::after{content:\"\";position:absolute;left:50%;bottom:-36px;width:2px;height:36px;transform:translateX(-50%);background:linear-gradient(180deg,#E8A84A,rgba(232,168,74,.12));border-radius:2px}",
    ...
    "#" + ROOT_ID + " .ptm-platform-card{margin-top:8px;background:rgba(12,18,42,.88);border:1px solid rgba(108,114,255,.28);backdrop-filter:blur(8px);box-shadow:0 10px 24px rgba(0,0,0,.28),0 1px 0 rgba(255,255,255,.06) inset}",
    ...
    "#" + ROOT_ID + " .ptm-detail{position:absolute;left:50%;top:46%;transform:translate(-50%,-46%) scale(.98);z-index:20;width:min(806px,calc(100% - 32px));max-height:min(78vh,640px);overflow:auto;border-radius:16px;border:1px solid rgba(255,255,255,.14);background:rgba(8,15,37,.97);padding:20px 22px 18px;box-shadow:0 28px 80px rgba(0,0,0,.55),0 0 0 1px rgba(108,114,255,.12);opacity:0;visibility:hidden;pointer-events:none;transition:opacity .22s var(--ptm-ease),transform .22s var(--ptm-ease),visibility .22s}",
    "#" + ROOT_ID + " .ptm-detail.is-open{opacity:1;visibility:visible;pointer-events:auto;transform:translate(-50%,-50%) scale(1)}",
    "#" + ROOT_ID + " .ptm-detail.is-swapping{opacity:0;transform:translate(-50%,-46%) scale(.98)}",
    "#" + ROOT_ID + " .ptm-detail.is-offline{border-color:rgba(215,142,44,.35);box-shadow:0 28px 80px rgba(0,0,0,.55),0 0 0 1px rgba(215,142,44,.18)}",
    "#" + ROOT_ID + " .ptm-detail-backdrop{position:absolute;inset:0;z-index:15;background:rgba(4,8,20,.45);opacity:0;visibility:hidden;pointer-events:none;transition:opacity .22s var(--ptm-ease),visibility .22s;border-radius:14px}",
    "#" + ROOT_ID + " .ptm-stage.is-previewing .ptm-detail-backdrop{opacity:1;visibility:visible;pointer-events:auto}",
    ...
    "@media (max-width:980px){#" + ROOT_ID + " .ptm-detail{width:min(720px,calc(100% - 20px));max-height:min(72vh,600px)}#" + ROOT_ID + " .ptm-d-paths,#" + ROOT_ID + " .ptm-d-moments{grid-template-columns:1fr}}",
    "@media (max-width:900px){#" + ROOT_ID + "{padding:44px 1rem 40px;--ptm-card-h:200px;--ptm-band-h:300px}#" + ROOT_ID + " .ptm-eyebrow{flex-wrap:wrap;padding:4px 10px 4px 4px}#" + ROOT_ID + " .ptm-eyebrow-right{white-space:normal;margin:6px 2px 2px 8px}#" + ROOT_ID + " .ptm-detail{top:50%;width:min(100% - 16px,640px)}}"
  ].join("");
  ...
  function mount(host) {
    ...
    host.innerHTML =
      ...
      '<p class="ptm-hint">Hover a step to preview the detail above the timeline</p>' +
      '<div class="ptm-scroll"><div class="ptm-chart" id="ptm-chart"></div></div>' +
      '<div class="ptm-detail-backdrop" id="ptm-detail-backdrop" aria-hidden="true"></div>' +
    ...
    var backdrop = document.getElementById("ptm-detail-backdrop");
    var active = -1;
    var swapTimer = null;
    var hideTimer = null;
    var locked = false;
