/**
 * How Dealality Works timeline mock (v20260811h)
 * Full-width dual-track timeline; step detail floats as a hover overlay above the chart.
 * Does not replace #oh-how-we-do-it.
 */
(function () {
  "use strict";
  var ROOT_ID = "oh-process-timeline-mock";
  var VERSION = "v20260811h";
  ...
  var CSS = [
    "#" + ROOT_ID + "{--ptm-font:\"Inter Tight\",\"Plus Jakarta Sans\",system-ui,sans-serif;--ptm-display:\"Plus Jakarta Sans\",\"Inter Tight\",system-ui,sans-serif;--ptm-ease:cubic-bezier(.22,1,.36,1);--ptm-card-h:168px;position:relative;overflow:hidden;box-sizing:border-box;font-family:var(--ptm-font);color:#e8ecf5;background:#080F25;padding:52px 1.5rem 44px;border-top:1px solid rgba(255,255,255,.07);border-bottom:1px solid rgba(255,255,255,.07)}",
    ...
    "#" + ROOT_ID + " .ptm-grid{display:grid;grid-template-columns:repeat(8,minmax(0,1fr));grid-template-rows:auto var(--ptm-card-h) 220px;width:100%;min-width:980px}",
    ...
    "#" + ROOT_ID + " .ptm-band-label--platform{position:absolute;top:8px;left:12px;right:auto;z-index:3;text-align:left;font-size:12px;font-weight:800;letter-spacing:.11em;text-transform:uppercase;color:rgba(255,255,255,.42);pointer-events:none;margin:0}",
    ...
    "#" + ROOT_ID + " .ptm-col-platform{padding:0 4px;display:flex;align-items:center;justify-content:center;position:relative;z-index:2;height:220px;min-width:0}",
    ...
    "#" + ROOT_ID + " .ptm-card{display:flex;flex-direction:column;width:100%;max-width:128px;height:var(--ptm-card-h);min-height:var(--ptm-card-h);max-height:var(--ptm-card-h);padding:12px 10px;border-radius:12px;cursor:pointer;transition:border-color .2s,box-shadow .2s,transform .2s var(--ptm-ease);animation:ptmCardIn .55s var(--ptm-ease) both;animation-delay:calc(var(--ptm-i,0)*.05s)}",
    "#" + ROOT_ID + " .ptm-card:hover,#" + ROOT_ID + " .ptm-card.is-active{transform:translateY(-2px)}",
    "#" + ROOT_ID + " .ptm-card .ptm-num{font-size:12px;letter-spacing:.08em;text-transform:uppercase;margin-bottom:6px;flex:0 0 auto;line-height:1.2;font-weight:700}",
    "#" + ROOT_ID + " .ptm-card h4{font-size:15px;font-weight:800;line-height:1.2;letter-spacing:-.02em;margin:0 0 6px;height:2.4em;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden;flex:0 0 auto}",
    "#" + ROOT_ID + " .ptm-card p{font-size:13px;line-height:1.35;margin:0;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:4;overflow:hidden;flex:1 1 auto;min-height:0}",
    ...
    "#" + ROOT_ID + " .ptm-detail{position:absolute;left:50%;top:42%;transform:translate(-50%,-46%) scale(.98);z-index:20;width:min(460px,calc(100% - 24px));max-height:min(70vh,560px);overflow:auto;border-radius:16px;border:1px solid rgba(255,255,255,.14);background:rgba(8,15,37,.96);padding:18px 18px 16px;box-shadow:0 28px 80px rgba(0,0,0,.55),0 0 0 1px rgba(108,114,255,.12);opacity:0;visibility:hidden;pointer-events:none;transition:opacity .22s var(--ptm-ease),transform .22s var(--ptm-ease),visibility .22s}",
    ...
    "#" + ROOT_ID + " .ptm-detail-close{position:absolute;top:10px;right:10px;width:28px;height:28px;border-radius:8px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.04);color:rgba(255,255,255,.7);cursor:pointer;font-size:16px;line-height:1}",
    ...
    "@media (max-width:980px){#" + ROOT_ID + " .ptm-detail{width:min(420px,calc(100% - 16px));max-height:min(68vh,520px)}#" + ROOT_ID + " .ptm-d-paths,#" + ROOT_ID + " .ptm-d-moments{grid-template-columns:1fr}}",
    "@media (max-width:900px){#" + ROOT_ID + "{padding:44px 1rem 40px}#" + ROOT_ID + " .ptm-eyebrow{flex-wrap:wrap;padding:4px 10px 4px 4px}#" + ROOT_ID + " .ptm-eyebrow-right{white-space:normal;margin:6px 2px 2px 8px}#" + ROOT_ID + "{--ptm-card-h:176px}#" + ROOT_ID + " .ptm-detail{top:50%}}"
  ].join("");
