/**
 * Staging review — How Dealality Works timeline mock (v20260811b)
...
(function () {
  ...
  var CSS = [
    "#" + ROOT_ID + "{--ptm-font:\"Inter Tight\",\"Plus Jakarta Sans\",system-ui,sans-serif;--ptm-display:\"Plus Jakarta Sans\",\"Inter Tight\",system-ui,sans-serif;--ptm-ease:cubic-bezier(.22,1,.36,1);--ptm-card-h:148px;position:relative;overflow:hidden;box-sizing:border-box;font-family:var(--ptm-font);color:#e8ecf5;background:#080F25;padding:52px 1.5rem 44px;border-top:1px solid rgba(255,255,255,.07);border-bottom:1px solid rgba(255,255,255,.07)}",
    ...
    "#" + ROOT_ID + " .ptm-badge{display:inline-flex;align-items:center;margin:0 0 14px;padding:5px 10px;border-radius:999px;background:rgba(215,142,44,.12);border:1px solid rgba(215,142,44,.28);color:#F0B86A;font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase}",
    ...
    "#" + ROOT_ID + " .ptm-scroll{overflow-x:auto;-webkit-overflow-scrolling:touch;margin:0;padding:0 0 4px;scrollbar-color:rgba(255,255,255,.18) transparent}",
    "#" + ROOT_ID + " .ptm-chart{position:relative;width:100%;min-width:980px;border-radius:16px;overflow:hidden;background:rgba(10,16,36,.65);border:1px solid rgba(255,255,255,.1);box-shadow:0 18px 48px rgba(0,0,0,.28)}",
    "#" + ROOT_ID + " .ptm-grid{display:grid;grid-template-columns:repeat(8,minmax(112px,1fr));grid-template-rows:auto auto 228px auto}",
    "#" + ROOT_ID + " .ptm-grid-head{grid-column:1/-1;grid-row:1;padding:12px 14px 4px;background:rgba(8,15,37,.55)}",
    "#" + ROOT_ID + " .ptm-grid-head span{font-size:9.5px;font-weight:800;letter-spacing:.11em;text-transform:uppercase;color:#F0B86A}",
    "#" + ROOT_ID + " .ptm-col-offline{grid-row:2;padding:8px 8px 0;min-height:calc(var(--ptm-card-h) + 36px);display:flex;align-items:flex-end;justify-content:center}",
    "#" + ROOT_ID + " .ptm-band-label--platform{position:absolute;top:10px;left:0;right:0;z-index:3;text-align:center;font-size:9.5px;font-weight:800;letter-spacing:.11em;text-transform:uppercase;color:rgba(255,255,255,.42);pointer-events:none;margin:0}",
    "#" + ROOT_ID + " .ptm-col-platform-wrap{grid-column:1/-1;grid-row:3;display:grid;grid-template-columns:subgrid;position:relative;min-height:228px;background:linear-gradient(180deg,rgba(17,27,58,.92),rgba(8,15,37,.98));border-top:1px solid rgba(255,255,255,.08);border-bottom:1px solid rgba(255,255,255,.06)}",
    ...
    "#" + ROOT_ID + " .ptm-col-platform{padding:0 8px;display:flex;align-items:center;justify-content:center;position:relative;z-index:2;min-height:228px}",
    ...
    "#" + ROOT_ID + " .ptm-path-layer circle.ptm-node{fill:#0d1530;stroke:#d4a853;stroke-width:3.5}",
    "#" + ROOT_ID + " .ptm-path-layer circle.ptm-node-offline{fill:#1a1620;stroke:#E8A84A;stroke-width:3}",
    "#" + ROOT_ID + " .ptm-path-layer text.ptm-node-label{fill:#a8abff;font-size:11px;font-weight:800;font-family:var(--ptm-font);text-anchor:middle;dominant-baseline:central}",
    "#" + ROOT_ID + " .ptm-card{display:flex;flex-direction:column;width:100%;max-width:128px;height:var(--ptm-card-h);padding:12px 10px;border-radius:12px;animation:ptmCardIn .55s var(--ptm-ease) both;animation-delay:calc(var(--ptm-i,0)*.05s)}",
    "#" + ROOT_ID + " .ptm-card .ptm-num{font-size:10px;letter-spacing:.08em;text-transform:uppercase;margin-bottom:6px;flex:0 0 auto}",
    "#" + ROOT_ID + " .ptm-card h4{font-size:12.5px;font-weight:800;line-height:1.22;letter-spacing:-.02em;margin:0 0 6px;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden;flex:0 0 auto}",
    "#" + ROOT_ID + " .ptm-card p{font-size:11px;line-height:1.45;margin:0;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:4;overflow:hidden;flex:1 1 auto}",
    "#" + ROOT_ID + " .ptm-offline-card{position:relative;background:linear-gradient(165deg,rgba(215,142,44,.16),rgba(32,26,18,.72));border:1px solid rgba(215,142,44,.32);box-shadow:0 10px 24px rgba(0,0,0,.22)}",
    "#" + ROOT_ID + " .ptm-offline-card::after{content:\"\";position:absolute;left:50%;bottom:-28px;width:2px;height:28px;transform:translateX(-50%);background:linear-gradient(180deg,#E8A84A,rgba(232,168,74,.12));border-radius:2px}",
    ...
    "#" + ROOT_ID + " .ptm-platform-card{margin-top:-28px;background:rgba(12,18,42,.88);border:1px solid rgba(108,114,255,.28);backdrop-filter:blur(8px);box-shadow:0 12px 28px rgba(0,0,0,.35),0 1px 0 rgba(255,255,255,.06) inset}",
    ...
    "#" + ROOT_ID + " .ptm-node-pass{width:36px;height:36px;border-radius:50%;background:#0d1530;border:3.5px solid #E8A84A;box-shadow:0 4px 14px rgba(0,0,0,.28);flex-shrink:0}",
    "#" + ROOT_ID + " .ptm-col-trigger-wrap{grid-column:1/-1;grid-row:4;display:grid;grid-template-columns:subgrid;background:rgba(8,15,37,.85);border-top:1px solid rgba(255,255,255,.06)}",
    ...
    "#" + ROOT_ID + " .ptm-col-trigger{padding:8px 10px 14px;border-right:1px solid rgba(255,255,255,.06);min-height:78px}",
    ...
    "#" + ROOT_ID + " .ptm-note{margin:16px 0 0;font-size:12px;color:rgba(255,255,255,.38)}",
    "@keyframes ptmCardIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}",
    "@media (max-width:900px){#" + ROOT_ID + "{padding:44px 1rem 40px}#" + ROOT_ID + " .ptm-eyebrow{flex-wrap:wrap;padding:4px 10px 4px 4px}#" + ROOT_ID + " .ptm-eyebrow-right{white-space:normal;margin:6px 2px 2px 8px}#" + ROOT_ID + " .ptm-scroll-hint{display:block}#" + ROOT_ID + "{--ptm-card-h:156px}}"
  ].join("");
  ...
  function buildChart(chart) {
    ...
    STEPS.forEach(function (s, i) {
      ...
      circle.setAttribute("r", s.mode === "offline" ? 13 : 15);
