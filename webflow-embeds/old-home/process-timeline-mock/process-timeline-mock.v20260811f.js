/**
 * How Dealality Works timeline mock (v20260811f)
 * Dual-track timeline + hover detail panel (live step visuals on the right).
 * Does not replace #oh-how-we-do-it.
 */
(function () {
  "use strict";
  var ROOT_ID = "oh-process-timeline-mock";
  var VERSION = "v20260811f";
  ...
  var CSS = [
    "#" + ROOT_ID + "{--ptm-font:\"Inter Tight\",\"Plus Jakarta Sans\",system-ui,sans-serif;--ptm-display:\"Plus Jakarta Sans\",\"Inter Tight\",system-ui,sans-serif;--ptm-ease:cubic-bezier(.22,1,.36,1);--ptm-card-h:132px;--ptm-trigger-h:64px;position:relative;overflow:hidden;box-sizing:border-box;font-family:var(--ptm-font);color:#e8ecf5;background:#080F25;padding:52px 1.5rem 44px;border-top:1px solid rgba(255,255,255,.07);border-bottom:1px solid rgba(255,255,255,.07)}",
    ...
    "#" + ROOT_ID + " .ptm-stage{display:grid;grid-template-columns:minmax(0,1.05fr) minmax(280px,.95fr);gap:22px;align-items:stretch}",
    "#" + ROOT_ID + " .ptm-stage-left{min-width:0}",
    ...
    "#" + ROOT_ID + " .ptm-grid{display:grid;grid-template-columns:repeat(8,minmax(0,1fr));grid-template-rows:auto var(--ptm-card-h) 200px auto;width:100%;min-width:720px}",
    "#" + ROOT_ID + " .ptm-grid-head{grid-column:1/-1;grid-row:1;padding:10px 12px 4px;text-align:left}",
    "#" + ROOT_ID + " .ptm-grid-head span{font-size:9px;font-weight:800;letter-spacing:.11em;text-transform:uppercase;color:#F0B86A}",
    "#" + ROOT_ID + " .ptm-col-offline{grid-row:2;padding:0 4px;height:var(--ptm-card-h);display:flex;align-items:stretch;justify-content:center;min-width:0}",
    "#" + ROOT_ID + " .ptm-band-label--platform{position:absolute;top:8px;left:12px;right:auto;z-index:3;text-align:left;font-size:9px;font-weight:800;letter-spacing:.11em;text-transform:uppercase;color:rgba(255,255,255,.42);pointer-events:none;margin:0}",
    "#" + ROOT_ID + " .ptm-col-platform-wrap{grid-column:1/-1;grid-row:3;display:grid;grid-template-columns:subgrid;position:relative;height:200px;background:linear-gradient(180deg,rgba(17,27,58,.55),rgba(8,15,37,.72));border-top:1px solid rgba(255,255,255,.08);border-bottom:1px solid rgba(255,255,255,.06)}",
    ...
    "#" + ROOT_ID + " .ptm-col-platform{padding:0 4px;display:flex;align-items:center;justify-content:center;position:relative;z-index:2;height:200px;min-width:0}",
    ...
    "#" + ROOT_ID + " .ptm-path-layer circle.ptm-node{fill:#0d1530;stroke:#E8A84A;stroke-width:3.5;pointer-events:auto;cursor:pointer;transition:transform .2s var(--ptm-ease),filter .2s}",
    "#" + ROOT_ID + " .ptm-path-layer circle.ptm-node:hover,#" + ROOT_ID + " .ptm-path-layer circle.ptm-node.is-active{filter:drop-shadow(0 0 8px rgba(232,168,74,.55));transform-origin:center;transform:scale(1.12)}",
    "#" + ROOT_ID + " .ptm-path-layer text.ptm-node-label{fill:#F0B86A;font-size:10px;font-weight:800;font-family:var(--ptm-font);text-anchor:middle;dominant-baseline:central;pointer-events:none}",
    "#" + ROOT_ID + " .ptm-path-layer text.ptm-node-label-platform{fill:#a8abff}",
    "#" + ROOT_ID + " .ptm-card{display:flex;flex-direction:column;width:100%;max-width:108px;height:var(--ptm-card-h);min-height:var(--ptm-card-h);max-height:var(--ptm-card-h);padding:10px 8px;border-radius:12px;cursor:pointer;transition:border-color .2s,box-shadow .2s,transform .2s var(--ptm-ease);animation:ptmCardIn .55s var(--ptm-ease) both;animation-delay:calc(var(--ptm-i,0)*.05s)}",
    "#" + ROOT_ID + " .ptm-card:hover,#" + ROOT_ID + " .ptm-card.is-active{transform:translateY(-2px)}",
    "#" + ROOT_ID + " .ptm-card .ptm-num{font-size:9px;letter-spacing:.08em;text-transform:uppercase;margin-bottom:4px;flex:0 0 auto;line-height:1.2}",
    "#" + ROOT_ID + " .ptm-card h4{font-size:11px;font-weight:800;line-height:1.22;letter-spacing:-.02em;margin:0 0 4px;height:2.44em;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden;flex:0 0 auto}",
    "#" + ROOT_ID + " .ptm-card p{font-size:10px;line-height:1.35;margin:0;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:4;overflow:hidden;flex:1 1 auto;min-height:0}",
    ...
    "#" + ROOT_ID + " .ptm-offline-card::after{content:\"\";position:absolute;left:50%;bottom:-18px;width:2px;height:18px;transform:translateX(-50%);background:linear-gradient(180deg,#E8A84A,rgba(232,168,74,.12));border-radius:2px}",
    "#" + ROOT_ID + " .ptm-offline-card .ptm-num{color:#F0B86A}",
    "#" + ROOT_ID + " .ptm-offline-card h4,#" + ROOT_ID + " .ptm-platform-card h4{color:#fff}",
    "#" + ROOT_ID + " .ptm-offline-card p,#" + ROOT_ID + " .ptm-platform-card p{color:rgba(255,255,255,.68)}",
    "#" + ROOT_ID + " .ptm-offline-card.is-active,#" + ROOT_ID + " .ptm-offline-card:hover{border-color:rgba(232,168,74,.7);box-shadow:0 10px 24px rgba(0,0,0,.28),0 0 0 1px rgba(232,168,74,.25)}",
    "#" + ROOT_ID + " .ptm-platform-card{margin-top:-18px;background:rgba(12,18,42,.88);border:1px solid rgba(108,114,255,.28);backdrop-filter:blur(8px);box-shadow:0 10px 24px rgba(0,0,0,.28),0 1px 0 rgba(255,255,255,.06) inset}",
    "#" + ROOT_ID + " .ptm-platform-card .ptm-num{color:#a8abff}",
    "#" + ROOT_ID + " .ptm-platform-card.is-active,#" + ROOT_ID + " .ptm-platform-card:hover{border-color:rgba(139,144,255,.7);box-shadow:0 10px 24px rgba(0,0,0,.35),0 0 0 1px rgba(108,114,255,.28)}",
    ...
    "#" + ROOT_ID + " .ptm-col-trigger-wrap > .ptm-band-label{grid-column:1/-1;font-size:9px;font-weight:800;letter-spacing:.11em;text-transform:uppercase;color:rgba(255,255,255,.38);padding:10px 12px 0;margin:0}",
    "#" + ROOT_ID + " .ptm-col-trigger{padding:6px 6px 12px;border-right:1px solid rgba(255,255,255,.06);height:calc(var(--ptm-trigger-h) + 28px);min-width:0}",
    "#" + ROOT_ID + " .ptm-col-trigger:last-child{border-right:0}",
    "#" + ROOT_ID + " .ptm-col-trigger strong{display:block;font-size:8px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:rgba(255,255,255,.35);margin-bottom:4px}",
    "#" + ROOT_ID + " .ptm-col-trigger p{font-size:10px;line-height:1.35;color:rgba(255,255,255,.58);margin:0;height:var(--ptm-trigger-h);display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:4;overflow:hidden}",
    "#" + ROOT_ID + " .ptm-loi-badge{position:absolute;right:10px;bottom:12px;z-index:4;display:flex;align-items:center;gap:6px;padding:6px 10px;border-radius:999px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.14);color:rgba(255,255,255,.82);font-size:9px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;backdrop-filter:blur(6px)}",
    "#" + ROOT_ID + " .ptm-loi-badge svg{width:12px;height:12px;stroke:#6ee7a8;fill:none;stroke-width:2.5}",
    "#" + ROOT_ID + " .ptm-detail{position:relative;min-width:0;border-radius:14px;border:1px solid rgba(255,255,255,.08);background:rgba(10,16,36,.45);padding:16px 16px 14px;overflow:auto;max-height:min(72vh,680px);opacity:1;transform:none;transition:opacity .28s var(--ptm-ease),transform .28s var(--ptm-ease)}",
    "#" + ROOT_ID + " .ptm-detail.is-swapping{opacity:0;transform:translateY(8px)}",
    "#" + ROOT_ID + " .ptm-detail.is-offline{border-color:rgba(215,142,44,.22)}",
    ...
    "@media (max-width:980px){#" + ROOT_ID + " .ptm-stage{grid-template-columns:1fr}#" + ROOT_ID + " .ptm-detail{max-height:none}#" + ROOT_ID + " .ptm-d-paths,#" + ROOT_ID + " .ptm-d-moments{grid-template-columns:1fr}}",
    "@media (max-width:900px){#" + ROOT_ID + "{padding:44px 1rem 40px}#" + ROOT_ID + " .ptm-eyebrow{flex-wrap:wrap;padding:4px 10px 4px 4px}#" + ROOT_ID + " .ptm-eyebrow-right{white-space:normal;margin:6px 2px 2px 8px}#" + ROOT_ID + "{--ptm-card-h:140px}}"
  ].join("");
  ...
  function visualHtml(step) {
    ...
    if (step.visual === "meet") {
      return (
        ...
        '<div class="ptm-d-panel"><div class="ptm-d-panel-head"><span class="ptm-d-chip">Meet and align</span><span class="ptm-d-meta">Where trust begins</span></div>' +
    ...
    if (step.visual === "diligence") {
      return (
        ...
        '<div class="ptm-d-panel"><div class="ptm-d-panel-head"><span class="ptm-d-chip">Collaborate and review</span><span class="ptm-d-meta">Where judgment happens</span></div>' +
        '<div class="ptm-d-moments">' +
    ...
    return (
      ...
      '<div class="ptm-d-panel"><div class="ptm-d-panel-head"><span class="ptm-d-chip">Align and hand off</span><span class="ptm-d-meta">Where the deal closes</span></div>' +
      '<div class="ptm-d-moments">' +
  ...
  function buildChart(chart, onSelect) {
    ...
    STEPS.forEach(function (s, i) {
      ...
      if (s.mode === "offline") {
        offlineCol.innerHTML =
          '<article class="ptm-card ptm-offline-card" data-ptm-step="' + i + '" tabindex="0" role="button" aria-label="Step ' + s.n + ": " + s.title + '" style="--ptm-i:' + i + '">' +
    ...
    STEPS.forEach(function (s, i) {
      ...
      if (s.mode === "platform") {
        pCol.innerHTML =
          '<article class="ptm-card ptm-platform-card" data-ptm-step="' + i + '" tabindex="0" role="button" aria-label="Step ' + s.n + ": " + s.title + '" style="--ptm-i:' + i + '">' +
          '<span class="ptm-num">Step ' + s.n + "</span>" +
          "<h4>" + s.title + "</h4>" +
          "<p>" + s.short + "</p></article>";
      }
  ...
  function mount(host) {
    ...
    host.innerHTML =
      ...
      "</div>" +
      '<div class="ptm-stage">' +
      '<div class="ptm-stage-left">' +
      '<p class="ptm-hint">Hover a step to preview the detail on the right</p>' +
      '<div class="ptm-scroll"><div class="ptm-chart" id="ptm-chart"></div></div>' +
      "</div>" +
      '<aside class="ptm-detail" id="ptm-detail" aria-live="polite"></aside>' +
      "</div>" +
      "</div>";
    ...
    function setActiveUI(i) {
      host.querySelectorAll("[data-ptm-step]").forEach(function (el) {
        var on = Number(el.getAttribute("data-ptm-step")) === i;
    ...
    function showStep(i, lock) {
      ...
      swapTimer = setTimeout(function () {
        ...
        detail.innerHTML = detailHtml(step);
        detail.classList.remove("is-swapping");
      }, 140);
      if (lock) detail.setAttribute("data-locked", "1");
    }
