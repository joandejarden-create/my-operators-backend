/**
 * How Dealality Works timeline mock (v20260811n)
 * Full-chart continuous gold path with symmetric sine arches across both tracks.
 * Does not replace #oh-how-we-do-it.
 */
(function () {
  "use strict";
  var ROOT_ID = "oh-process-timeline-mock";
  var VERSION = "v20260811n";
  ...
  var STEPS = [
    ...
    {
      n: "07", mode: "platform", title: "Choose a Path & Move Forward",
  ...
  var CSS = [
    ...
    "#" + ROOT_ID + " .ptm-chart{position:relative;width:100%;border-radius:14px;overflow:visible;background:rgba(10,16,36,.35);border:1px solid rgba(255,255,255,.08)}",
    "#" + ROOT_ID + " .ptm-grid{display:grid;grid-template-columns:repeat(8,minmax(0,1fr));grid-template-rows:auto var(--ptm-card-h) var(--ptm-band-h);width:100%;min-width:980px;row-gap:var(--ptm-track-gap);position:relative}",
    "#" + ROOT_ID + " .ptm-grid-head{grid-column:1/-1;grid-row:1;padding:14px 14px 0;text-align:left;position:relative;z-index:2}",
    "#" + ROOT_ID + " .ptm-grid-head span{font-size:12px;font-weight:800;letter-spacing:.11em;text-transform:uppercase;color:#F0B86A}",
    "#" + ROOT_ID + " .ptm-col-offline{grid-row:2;padding:0 6px 8px;height:var(--ptm-card-h);min-height:var(--ptm-card-h);display:flex;align-items:stretch;justify-content:center;min-width:0;position:relative;z-index:2}",
    "#" + ROOT_ID + " .ptm-band-label--platform{position:absolute;top:14px;left:14px;right:auto;z-index:3;text-align:left;font-size:12px;font-weight:800;letter-spacing:.11em;text-transform:uppercase;color:rgba(255,255,255,.42);pointer-events:none;margin:0}",
    "#" + ROOT_ID + " .ptm-col-platform-wrap{grid-column:1/-1;grid-row:3;display:grid;grid-template-columns:subgrid;position:relative;z-index:2;height:var(--ptm-band-h);background:linear-gradient(180deg,rgba(17,27,58,.55),rgba(8,15,37,.72));border-top:1px solid rgba(255,255,255,.08);border-bottom:1px solid rgba(255,255,255,.06)}",
    ...
    "#" + ROOT_ID + " .ptm-col-platform{padding:56px 6px 18px;display:flex;align-items:flex-end;justify-content:center;position:relative;z-index:2;height:var(--ptm-band-h);min-width:0}",
    "#" + ROOT_ID + " .ptm-path-layer{grid-column:1/-1;grid-row:2/4;width:100%;height:100%;z-index:1;pointer-events:none;overflow:visible}",
    "#" + ROOT_ID + " .ptm-path-layer path.ptm-rail{fill:none;stroke:url(#ptmGoldGrad);stroke-width:4.5;stroke-linecap:round;stroke-linejoin:round;filter:drop-shadow(0 1px 2px rgba(0,0,0,.35))}",
    ...
    "#" + ROOT_ID + " .ptm-path-layer circle.ptm-node-platform{stroke:#8B90FF}",
    ...
    "#" + ROOT_ID + " .ptm-card h4{font-size:14px;font-weight:800;line-height:1.25;letter-spacing:-.02em;margin:0 0 8px;flex:0 0 auto}",
    ...
    "@media (max-width:900px){#" + ROOT_ID + "{padding:44px 1rem 40px;--ptm-card-h:175px;--ptm-band-h:320px;--ptm-track-gap:72px}#" + ROOT_ID + " .ptm-eyebrow{flex-wrap:wrap;padding:4px 10px 4px 4px}#" + ROOT_ID + " .ptm-eyebrow-right{white-space:normal;margin:6px 2px 2px 8px}#" + ROOT_ID + " .ptm-detail{top:50%;width:min(640px,calc(100% - 16px))}#" + ROOT_ID + " .ptm-col-platform{padding:48px 6px 16px}}"
  ].join("");
  ...
  function buildChart(chart, onSelect) {
    ...
    var CARD_H = cssPx("--ptm-card-h", 175);
    var TRACK_GAP = cssPx("--ptm-track-gap", 96);
    var BAND_H = cssPx("--ptm-band-h", 340);
    var VB_H = CARD_H + TRACK_GAP + BAND_H;
    var PAD = 70;
    var colW = (VB_W - PAD * 2) / (COLS - 1);
    // Offline peaks sit in the gap under Outside cards; platform valleys sit above In Platform cards.
    var offlineY = CARD_H + TRACK_GAP * 0.42;
    var platformY = CARD_H + TRACK_GAP + Math.min(78, BAND_H * 0.22);
    ...
    function stepY(i) {
      return STEPS[i].mode === "offline" ? offlineY : platformY;
    }

    function buildPath() {
      var d = "M " + colX(0).toFixed(1) + " " + stepY(0).toFixed(1);
      for (var i = 1; i < COLS; i++) {
        ...
        var y1 = stepY(i);
        var dx = x1 - x0;
        // Symmetric cubic: equal horizontal thirds, hold start/end Y → smooth sine-like S-curve.
        d += " C " + (x0 + dx / 3).toFixed(1) + " " + y0.toFixed(1);
        d += " " + (x1 - dx / 3).toFixed(1) + " " + y1.toFixed(1);
        d += " " + x1.toFixed(1) + " " + y1.toFixed(1);
      }
      return d;
    }
