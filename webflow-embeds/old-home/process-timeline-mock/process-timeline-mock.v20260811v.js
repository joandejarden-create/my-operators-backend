/**
 * How Dealality Works timeline mock (v20260811v)
 * Live-site intro copy (eyebrow, H2, lead, principle callout) above the timeline.
 * Does not replace #oh-how-we-do-it.
 */
(function () {
  "use strict";
  var ROOT_ID = "oh-process-timeline-mock";
  var VERSION = "v20260811v";
  var NODE_R = 14;
  ...
  var CSS = [
    ...
    "#" + ROOT_ID + " .ptm-detail{position:absolute;top:36px;z-index:8;width:min(520px,54%);max-height:0;overflow:hidden;border-radius:16px;border:1px solid transparent;background:rgba(8,15,37,.97);padding:0 20px;box-shadow:none;opacity:0;visibility:hidden;pointer-events:none;transition:opacity .22s var(--ptm-ease),visibility .22s,max-height .28s var(--ptm-ease),padding .22s,border-color .22s,box-shadow .22s,left .28s var(--ptm-ease),right .28s var(--ptm-ease),transform .22s var(--ptm-ease)}",
    ...
    "#" + ROOT_ID + " .ptm-detail.is-open{opacity:1;visibility:visible;pointer-events:auto;max-height:min(68vh,580px);overflow:auto;padding:18px 20px 16px;border-color:rgba(255,255,255,.14);box-shadow:0 22px 56px rgba(0,0,0,.5),0 0 0 1px rgba(108,114,255,.12)}",
    ...
    "@media (max-width:980px){#" + ROOT_ID + " .ptm-detail{width:min(480px,58%)}#" + ROOT_ID + " .ptm-detail.is-open{max-height:min(64vh,520px)}#" + ROOT_ID + " .ptm-d-paths,#" + ROOT_ID + " .ptm-d-moments{grid-template-columns:1fr}}",
    "@media (max-width:900px){#" + ROOT_ID + "{padding:44px 1rem 40px;--ptm-card-h:175px;--ptm-path-h:56px}#" + ROOT_ID + " .ptm-eyebrow{flex-wrap:wrap;padding:4px 10px 4px 4px}#" + ROOT_ID + " .ptm-eyebrow-right{white-space:normal;margin:6px 2px 2px 8px}#" + ROOT_ID + " .ptm-detail{width:min(420px,70%);top:48px}}"
  ].join("");
