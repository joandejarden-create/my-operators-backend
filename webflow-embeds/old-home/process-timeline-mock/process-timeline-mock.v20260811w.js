/**
 * How Dealality Works timeline mock (v20260811w)
 * Compact hover panel bottom-aligned to the timeline; no detail scrollbar.
 * Does not replace #oh-how-we-do-it.
 */
(function () {
  "use strict";
  var ROOT_ID = "oh-process-timeline-mock";
  var VERSION = "v20260811w";
  var NODE_R = 14;
  ...
  var STEPS = [
    {
      ...
      primary: "Clarify what the owner wants before any relationship shapes the answer.",
      support: "Hotel, market, objectives, constraints, and criteria in one brief.",
      bridge: "Shared story ready for the first offline conversations.",
      visual: "define"
    },
    {
      ...
      support: "Brand, operator, conversion, and partner paths worth evaluating.",
      bridge: "Shortlists who is worth meeting before outreach.",
    ...
    },
    {
      ...
      support: "Participants, confidentiality, outreach, and responses in one place.",
      bridge: "Clean brief and status into the first relationship step.",
      visual: "engage"
    },
    {
      ...
      support: "Introductions happen offline; Dealality prepares both sides and tracks outcomes.",
      bridge: "Met offline · facilitated & tracked in Dealality.",
      visual: "meet"
    },
    {
      ...
      support: "Fees, control, support, timing, and gaps side by side.",
      bridge: "Shared comparison ready for offline diligence.",
    ...
    },
    {
      ...
      primary: "Take the comparison into the real world—calls, walkthroughs, tech review.",
      support: "Offline diligence; Dealality facilitates context and tracks coverage.",
      bridge: "Met offline · facilitated & tracked in Dealality.",
      visual: "diligence"
    },
    {
      ...
      primary: "Hold finalists, materials, and feasibility in one place.",
      support: "Deal Room open; feasibility synced between offline alignment moments.",
      bridge: "Record ready for final alignment—still tracked.",
      visual: "pursue"
    },
    {
      ...
      primary: "Close alignment between people—then hand into legal with a clean exit.",
      support: "Vision, cost, and ops confirmed offline; LOI intent tracked through handoff.",
      bridge: "Aligned offline · tracked through handoff.",
      visual: "handoff"
    }
  ];

  var CSS = [
    "#" + ROOT_ID + "{--ptm-font:\"Inter Tight\",\"Plus Jakarta Sans\",system-ui,sans-serif;--ptm-display:\"Plus Jakarta Sans\",\"Inter Tight\",system-ui,sans-serif;--ptm-ease:cubic-bezier(.22,1,.36,1);--ptm-card-h:175px;--ptm-path-h:64px;position:relative;overflow:visible;box-sizing:border-box;font-family:var(--ptm-font);color:#e8ecf5;background:#080F25;padding:52px 1.5rem 44px;border-top:1px solid rgba(255,255,255,.07);border-bottom:1px solid rgba(255,255,255,.07)}",
    ...
    "#" + ROOT_ID + " .ptm-lead{margin:0;max-width:42rem;font-size:1.05rem;font-weight:500;line-height:1.65;color:rgba(255,255,255,.62)}",
    "#" + ROOT_ID + " .ptm-lead em{font-style:normal;color:#E8A84A;font-weight:650}",
    "#" + ROOT_ID + " .ptm-principle{display:flex;align-items:stretch;gap:14px;margin:14px 0 18px;padding:14px 16px;border-radius:12px;border:1px solid rgba(215,142,44,.32);background:rgba(215,142,44,.06);max-width:46rem}",
    "#" + ROOT_ID + " .ptm-principle-mark{width:4px;border-radius:999px;background:linear-gradient(180deg,#F0B86A,#D78E2C);flex:0 0 auto}",
    "#" + ROOT_ID + " .ptm-principle p{margin:0;font-size:.95rem;line-height:1.55;color:rgba(255,255,255,.62)}",
    "#" + ROOT_ID + " .ptm-principle strong{color:#F0B86A;font-weight:700}",
    ...
    "#" + ROOT_ID + " .ptm-hint{margin:0 0 10px;font-size:12px;font-weight:600;color:rgba(255,255,255,.4)}",
    ...
    "#" + ROOT_ID + " .ptm-chart{position:relative;width:100%;border-radius:14px;overflow:visible;background:linear-gradient(180deg,rgba(17,27,58,.45),rgba(8,15,37,.72));border:1px solid rgba(255,255,255,.08);padding:18px 10px 20px}",
    ...
    "#" + ROOT_ID + " .ptm-loi-badge{position:absolute;right:12px;bottom:12px;z-index:4;display:flex;align-items:center;gap:6px;padding:6px 10px;border-radius:999px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.14);color:rgba(255,255,255,.82);font-size:9px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;backdrop-filter:blur(6px)}",
    "#" + ROOT_ID + " .ptm-loi-badge svg{width:12px;height:12px;stroke:#6ee7a8;fill:none;stroke-width:2.5}",
    "#" + ROOT_ID + " .ptm-detail{position:absolute;left:auto;right:10px;bottom:4px;top:auto;z-index:8;width:min(440px,48%);max-height:0;overflow:hidden;border-radius:14px;border:1px solid transparent;background:rgba(8,15,37,.98);padding:0 16px;box-shadow:none;opacity:0;visibility:hidden;pointer-events:none;transition:opacity .22s var(--ptm-ease),visibility .22s,max-height .28s var(--ptm-ease),padding .22s,border-color .22s,box-shadow .22s,left .28s var(--ptm-ease),right .28s var(--ptm-ease),transform .22s var(--ptm-ease)}",
    ...
    "#" + ROOT_ID + " .ptm-detail.is-open{opacity:1;visibility:visible;pointer-events:auto;max-height:100%;overflow:hidden;padding:14px 16px 12px;border-color:rgba(255,255,255,.14);box-shadow:0 18px 40px rgba(0,0,0,.48),0 0 0 1px rgba(108,114,255,.12)}",
    ...
    "#" + ROOT_ID + " .ptm-detail-close{position:absolute;top:8px;right:8px;width:28px;height:28px;border-radius:8px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.04);color:rgba(255,255,255,.7);cursor:pointer;font-size:16px;line-height:1}",
    "#" + ROOT_ID + " .ptm-detail-close:hover{background:rgba(255,255,255,.08);color:#fff}",
    "#" + ROOT_ID + " .ptm-d-kicker{margin:0 0 4px;font-size:.7rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#8B90FF}",
    "#" + ROOT_ID + " .ptm-detail.is-offline .ptm-d-kicker{color:#E8A84A}",
    "#" + ROOT_ID + " .ptm-d-title{margin:0 0 6px;padding-right:28px;font-size:clamp(1.05rem,1.8vw,1.28rem);font-weight:800;letter-spacing:-.02em;color:#fff;line-height:1.2;text-transform:capitalize}",
    "#" + ROOT_ID + " .ptm-d-pill{display:inline-flex;align-items:center;gap:7px;margin:0 0 8px;padding:4px 9px;border-radius:999px;border:1px solid rgba(255,255,255,.12);font-size:10px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;color:rgba(255,255,255,.7)}",
    ...
    "#" + ROOT_ID + " .ptm-d-primary{margin:0 0 6px;font-size:.9rem;font-weight:650;line-height:1.4;color:#fff}",
    "#" + ROOT_ID + " .ptm-d-support{display:none}",
    "#" + ROOT_ID + " .ptm-d-bridge{margin:0 0 10px;padding:8px 10px;border-radius:8px;border:1px dashed rgba(255,255,255,.16);font-size:.78rem;line-height:1.4;color:rgba(255,255,255,.78)}",
    ...
    "#" + ROOT_ID + " .ptm-d-chrome{display:none}",
    ...
    "#" + ROOT_ID + " .ptm-d-panel{padding:8px 10px;display:grid;gap:6px}",
    ...
    "#" + ROOT_ID + " .ptm-d-row{display:grid;grid-template-columns:1fr auto;gap:10px;align-items:baseline;padding:4px 0;border-bottom:1px solid rgba(255,255,255,.06)}",
    ...
    "#" + ROOT_ID + " .ptm-d-moment{padding:7px 8px;border-radius:8px;border:1px solid rgba(215,142,44,.28);background:rgba(215,142,44,.06)}",
    "#" + ROOT_ID + " .ptm-d-moment strong{display:block;color:#F0B86A;font-size:.78rem;margin:0 0 4px}",
    "#" + ROOT_ID + " .ptm-d-moment p{margin:0;color:rgba(255,255,255,.58);font-size:.7rem;line-height:1.35}",
    "#" + ROOT_ID + " .ptm-d-shot{display:none}",
    ...
    "@media (max-width:980px){#" + ROOT_ID + " .ptm-detail{width:min(400px,56%)}#" + ROOT_ID + " .ptm-detail.is-open{max-height:100%}#" + ROOT_ID + " .ptm-d-paths,#" + ROOT_ID + " .ptm-d-moments{grid-template-columns:1fr}}",
    "@media (max-width:900px){#" + ROOT_ID + "{padding:44px 1rem 40px;--ptm-card-h:175px;--ptm-path-h:56px}#" + ROOT_ID + " .ptm-eyebrow{flex-wrap:wrap;padding:4px 10px 4px 4px}#" + ROOT_ID + " .ptm-eyebrow-right{white-space:normal;margin:6px 2px 2px 8px}#" + ROOT_ID + " .ptm-detail{width:min(380px,72%);bottom:4px;top:auto}}"
  ].join("");
  ...
  function visualHtml(step) {
    if (step.visual === "define") {
      return (
        ...
        '<div class="ptm-d-panel"><div class="ptm-d-panel-head"><span class="ptm-d-chip">Opportunity Brief</span></div>' +
        '<div class="ptm-d-rows">' +
        '<div class="ptm-d-row"><strong>Type</strong><span>Brand + operator review</span></div>' +
        '<div class="ptm-d-row"><strong>Objectives</strong><span>Stabilize NOI · Preserve control</span></div>' +
        '<div class="ptm-d-row"><strong>Criteria</strong><span>Control · fees · timeline</span></div>' +
        "</div></div></div>"
      );
    }
    if (step.visual === "paths") {
      return (
        '<div class="ptm-d-visual">' +
        '<div class="ptm-d-panel"><div class="ptm-d-panel-head"><span class="ptm-d-chip">Credible Paths</span></div>' +
        '<div class="ptm-d-paths">' +
        '<div class="ptm-d-path"><strong>Soft Brand</strong><span>Fit · Capex moderate</span></div>' +
        '<div class="ptm-d-path"><strong>Operator</strong><span>Lift · Control trade-offs</span></div>' +
        '<div class="ptm-d-path"><strong>Conversion</strong><span>Positioning · Evidence</span></div>' +
        "</div></div></div>"
      );
    }
    if (step.visual === "engage") {
      return (
        '<div class="ptm-d-visual">' +
        '<div class="ptm-d-panel"><div class="ptm-d-panel-head"><span class="ptm-d-chip">Controlled Outreach</span></div>' +
        '<div class="ptm-d-rows">' +
        '<div class="ptm-d-row"><strong>Participants</strong><span>3 brands · 2 operators</span></div>' +
        '<div class="ptm-d-row"><strong>Confidentiality</strong><span>NDA active · gated</span></div>' +
        '<div class="ptm-d-row"><strong>Next</strong><span>Intro call · offline</span></div>' +
        "</div></div></div>"
      );
    }
    if (step.visual === "meet") {
      return (
        '<div class="ptm-d-visual">' +
        '<div class="ptm-d-panel"><div class="ptm-d-panel-head"><span class="ptm-d-chip">Meet &amp; align</span></div>' +
        '<div class="ptm-d-moments">' +
        '<div class="ptm-d-moment"><strong>Introduction</strong><p>Fit, intent, and chemistry—live.</p></div>' +
        '<div class="ptm-d-moment"><strong>Clarification</strong><p>Questions answered before terms harden.</p></div>' +
        "</div></div></div>"
      );
    }
    if (step.visual === "compare") {
      return (
        '<div class="ptm-d-visual">' +
        '<div class="ptm-d-panel"><div class="ptm-d-panel-head"><span class="ptm-d-chip">Shared Comparison</span></div>' +
        '<div class="ptm-d-compare" role="table">' +
        '<div class="is-head">Category</div><div class="is-head">A</div><div class="is-head">B</div><div class="is-head">C</div>' +
        "<div>Fees</div><div>4.5%+2%</div><div>5.0%+1%</div><div>3.8%+3%</div>" +
        "<div>Control</div><div>High</div><div>Medium</div><div>Shared</div>" +
        '<div>Gaps</div><div class="is-gap">Capex TBD</div><div>—</div><div class="is-gap">Exit TBD</div>' +
        "</div></div></div>"
      );
    }
    if (step.visual === "diligence") {
      return (
        '<div class="ptm-d-visual">' +
        '<div class="ptm-d-panel"><div class="ptm-d-panel-head"><span class="ptm-d-chip">Collaborate &amp; review</span></div>' +
        '<div class="ptm-d-moments">' +
        '<div class="ptm-d-moment"><strong>Collaboration</strong><p>Walk economics and fit—live.</p></div>' +
        '<div class="ptm-d-moment"><strong>Site / Tech</strong><p>Walkthrough or remote review.</p></div>' +
        "</div></div></div>"
      );
    }
    if (step.visual === "pursue") {
      return (
        '<div class="ptm-d-visual">' +
        '<div class="ptm-d-panel"><div class="ptm-d-panel-head"><span class="ptm-d-chip">Preferred Direction</span></div>' +
        '<div class="ptm-d-rows">' +
        '<div class="ptm-d-row"><strong>Direction</strong><span>Soft brand + operator</span></div>' +
        '<div class="ptm-d-row"><strong>Deal Room</strong><span>Open for finalists</span></div>' +
        '<div class="ptm-d-row"><strong>Next</strong><span>Final alignment · offline</span></div>' +
        "</div></div></div>"
      );
    }
    return (
      '<div class="ptm-d-visual">' +
      '<div class="ptm-d-panel"><div class="ptm-d-panel-head"><span class="ptm-d-chip">Align &amp; hand off</span></div>' +
      '<div class="ptm-d-moments">' +
      '<div class="ptm-d-moment"><strong>Final Alignment</strong><p>Vision, cost, and ops confirmed.</p></div>' +
      '<div class="ptm-d-moment"><strong>Legal Start</strong><p>MA / FA drafting · clean exit.</p></div>' +
      "</div></div></div>"
    );
  }
  ...
  function mount(host) {
    ...
    host.innerHTML =
      ...
      "</div>" +
      '<h2 class="ptm-title" id="ptm-h2">Turn One Hotel Opportunity into a Clear Decision Process.</h2>' +
      ...
      '<div class="ptm-principle">' +
      '<span class="ptm-principle-mark" aria-hidden="true"></span>' +
      ...
      '<p class="ptm-hint">Hover a step to preview detail on the opposite side—other steps stay selectable</p>' +
