/**
 * How Dealality Works timeline mock (v20260811o)
 * Single-row steps with gold path above (no Outside/In Platform split).
 * Does not replace #oh-how-we-do-it.
 */
(function () {
  "use strict";
  var ROOT_ID = "oh-process-timeline-mock";
  var VERSION = "v20260811o";
  ...
  var CSS = [
    ...
    "#" + ROOT_ID + " .ptm-col{grid-row:2;padding:0 5px;height:var(--ptm-card-h);display:flex;align-items:stretch;justify-content:center;min-width:0;position:relative;z-index:2}",
    "#" + ROOT_ID + " .ptm-col::before{content:\"\";position:absolute;left:50%;top:-14px;width:2px;height:14px;transform:translateX(-50%);background:linear-gradient(180deg,rgba(232,168,74,.55),rgba(232,168,74,.12));border-radius:2px;pointer-events:none}",
    "#" + ROOT_ID + " .ptm-col.is-platform::before{background:linear-gradient(180deg,rgba(139,144,255,.55),rgba(139,144,255,.12))}",
    ...
    "@media (max-width:900px){#" + ROOT_ID + "{padding:44px 1rem 40px;--ptm-card-h:175px;--ptm-path-h:56px}#" + ROOT_ID + " .ptm-eyebrow{flex-wrap:wrap;padding:4px 10px 4px 4px}#" + ROOT_ID + " .ptm-eyebrow-right{white-space:normal;margin:6px 2px 2px 8px}#" + ROOT_ID + " .ptm-detail{top:50%;width:min(640px,calc(100% - 16px))}}"
  ].join("");
  ...
  function buildChart(chart, onSelect) {
    ...
    // Single horizontal rail centered in the top path row, with a soft wave for motion.
    var baseY = PATH_H * 0.5;
    var wave = Math.min(10, PATH_H * 0.16);
    ...
    function stepY(i) {
      // Gentle crest on offline steps so mode stays readable without a split layout.
      return STEPS[i].mode === "offline" ? baseY - wave : baseY + wave * 0.35;
    }
