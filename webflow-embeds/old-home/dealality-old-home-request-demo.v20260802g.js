/**
 * Old Home — Request a Demo (v20260802g)
 * 02g: /es — email→correo, walkthrough→demostración (LATAM).
 * 02f: /es — modal title Solicitud de Demo (Title Case parity with Platform Demo Request).
 * 02e: /es — modal submit "Solicitar Demo" (Title Case parity with EN).
...
(function () {
  ...
  try {
    ...
    if (window.__ohRequestDemo >= 2026080206) return;
    window.__ohRequestDemo = 2026080206;
    ...
    function ensureModal() {
      ...
      overlay.innerHTML = isEs
        ? '<div id="oh-demo-panel" role="dialog" aria-modal="true" aria-labelledby="oh-demo-title">' +
          ...
          '<p id="oh-demo-lead">Deja tu correo — te contactaremos para agendar una breve demostración.</p>' +
          '<form id="oh-demo-form" novalidate>' +
          '<label id="oh-demo-label" for="oh-demo-email">Correo de trabajo</label>' +
    ...
    async function onSubmit(e) {
      ...
      if (!EMAIL_RE.test(email)) {
        setError(isEs ? "Ingresa un correo válido." : "Enter a valid email address.");
        if (input) input.focus();
        return;
      }
      if (btn) {
