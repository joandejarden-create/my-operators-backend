 ...
 * Version: v20260802b
 * 02b: localize hero "Scroll to explore" cue on `/es`.
 * 01h: Benefits-style hover lift (`oh-m-lift`) on Problem 6 boxes
 *      (.dmp-card + .dmp-problem) and Ecosystem cards + process steps.
 * 02a: unlock path gate for Spanish home `/es`.
...
(function () {
  ...
  function ensureScrollCue() {
    ...
    // EN is sentence case ("Scroll to explore") — match that casing in ES.
    var cueLabel = isEs ? "Desplázate para explorar" : "Scroll to explore";
    var cue = document.createElement("div");
    cue.id = "oh-scroll-cue";
    cue.setAttribute("aria-hidden", "true");
    cue.innerHTML =
      '<div class="oh-scroll-cue-inner"><span>' +
      cueLabel +
      '</span><span class="oh-scroll-cue-chev"></span></div>';
  ...
  function decorate() {
    ...
    if (dmp) {
      ...
      // Same Benefits hover bump on journey cards + bottom problem cards (6 boxes)
      markLift(qs(".dmp-card, .dmp-problem", dmp));
    } else {
      markSequence("about", [
        qs("#about-badge"),
        qs("#about-h2"),
    ...
    // Benefits-style hover bump on all Ecosystem boxes (role cards + process steps)
    markLift(
      qs(
        "#ecosystem article.oh-eco-card-1, #ecosystem .oh-eco-card-1, #ecosystem .oh-eco-step, #ecosystem .oh-eco-close"
      )
    );
