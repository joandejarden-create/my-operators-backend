/**
 * Old Home — How We Do It compact one-screen process (v20260808d)
 ...
 *
 * v20260808d: Process Steps column label; Step 01 labels; animated progress rail.
 * v20260802g: Spanish Title Case — lowercase mid-phrase de/del/al/en/un/por/y.
* v20260802f: Spanish Title Case matches EN (lowercase de/la/al in eyebrows).
  ...
  try {
    ...
    var STEPS = [
      ...
      },
      {
        n: "07",
        mode: "platform",
        title: "Choose a path and move forward",
        primary:
    ...
    if (isEs) {
      var TITLE_ES = {
        "Meet and align": "Reúnete y alinea",
        "Collaborate and review": "Colabora y revisa",
        "Align and hand off": "Alinea y traspasa"
      };
      var LABEL_ES = {
    ...
    function injectNavChromeCss() {
      if (document.getElementById("oh-how-nav-chrome")) return;
      var st = document.createElement("style");
      st.id = "oh-how-nav-chrome";
      st.textContent = [
        "#oh-how-we-do-it .dealality-process_nav-heading{",
        "margin:0 0 10px!important;padding:0!important;",
        "font-family:\"Plus Jakarta Sans\",\"Inter Tight\",system-ui,sans-serif!important;",
        "font-size:.72rem!important;font-weight:750!important;letter-spacing:.08em!important;",
        "text-transform:uppercase!important;color:rgba(255,255,255,.72)!important;",
        "}",
        "#oh-how-we-do-it .dealality-process_nav-num{",
        "min-width:4.6em!important;white-space:nowrap!important;",
        "font-variant-numeric:tabular-nums!important;",
        "}",
        "#oh-how-we-do-it .dealality-process_nav-rail{",
        ...
        "transition:height .38s cubic-bezier(.22,.61,.36,1)!important;",
        "will-change:height;",
        "}",
        "@media (prefers-reduced-motion:reduce){",
        "#oh-how-we-do-it .dealality-process_progress > span{transition:none!important}",
        "}",
      ].join("");
      (document.head || document.documentElement).appendChild(st);
    }
    ...
    function visualHtml(step) {
      ...
      if (step.visual === "meet") {
        return (
          ...
          '<div class="dealality-process_panel">' +
          '<div class="dealality-process_panel-head">' +
          '<span class="dealality-process_chip">Meet and align</span>' +
      ...
      if (step.visual === "diligence") {
        return (
          ...
          '<div class="dealality-process_panel">' +
          '<div class="dealality-process_panel-head">' +
          '<span class="dealality-process_chip">Collaborate and review</span>' +
      ...
      return (
        ...
        '<div class="dealality-process_panel">' +
        '<div class="dealality-process_panel-head">' +
        '<span class="dealality-process_chip">Align and hand off</span>' +
    ...
    function buildSection() {
      var nav = STEPS.map(function (step, i) {
        ...
        return (
          ...
          esc(step.title) +
          ", " +
          (offline ? t("Offline", "Offline") : t("Platform", "Plataforma")) +
          '">' +
      ...
      return (
        ...
        '<span class="dealality-process_eyebrow-right">' + t("One Clear Process from Opportunity to Agreement.", "Un Proceso Conectado de la Oportunidad al Acuerdo.") + "</span>" +
        "</div>" +
        '<h2 class="dealality-process_h2" id="dealality-process-h2">' + t("Turn One Hotel Opportunity into a Clear Decision Process.", "Convierte una Oportunidad Hotelera en un Proceso de Decisión Claro.") + "</h2>" +
        ...
        '">' +
        '<div class="dealality-process_nav-heading" id="dealality-process-nav-heading">' +
        t("Process Steps", "Pasos del Proceso") +
        "</div>" +
    ...
    function bind(section) {
      ...
      function setActive(index, fromUser) {
        ...
        if (progress) {
          var rail = section.querySelector(".dealality-process_nav-rail");
          var activeBtn = navItems[active];
          var pct =
            STEPS.length <= 1
              ? 100
              : Math.round(((active + 1) / STEPS.length) * 100);
          if (rail && activeBtn && rail.getBoundingClientRect().height > 0) {
            var railRect = rail.getBoundingClientRect();
            var btnRect = activeBtn.getBoundingClientRect();
            var mid = (btnRect.top + btnRect.bottom) / 2 - railRect.top;
            pct = Math.max(
              8,
              Math.min(100, Math.round((mid / railRect.height) * 100))
            );
          }
