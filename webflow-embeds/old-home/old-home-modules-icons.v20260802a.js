/**
 * Old Home — modules tile SVG icons (v20260802a)
 * Path-gated to /, /es, and /old-home (homepage + Spanish locale).
 * 02a: unlock `/es` so Spanish Benefits cards get the same SVG icons as English.
 */
(function () {
  try {
    var path = (location.pathname || "").replace(/\/+$/, "").toLowerCase() || "/";
    if (path !== "/" && path !== "/old-home" && path !== "/es") return;
    if (window.__ohModulesIcons >= 202608021) return;
    window.__ohModulesIcons = 202608021;
