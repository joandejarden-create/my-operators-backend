/**
 * Old Home Many Futures Spanish i18n (v20260802a)
 * Path-gated to /es.
 *
...
(function () {
  ...
  try {
    ...
    if (window.__ohMfI18n >= 202608021) return;
  } catch (ePath) {
    return;
  }
  ...
  var TEXT = {
    ...
    "How Do I Maintain Confidentiality and Control?":
      "¿Cómo mantengo la confidencialidad y el control?",
  ...
  function tryBoot() {
    if (started && window.__ohMfI18n >= 202608021) return;
    var root = document.getElementById("dealality-many-futures");
    if (!root || !root.querySelector(".mf-q-title")) {
    ...
    apply(root);
    started = true;
    window.__ohMfI18n = 202608021;

    /* Re-apply lightly if MF swaps panel chrome after interaction. */
    if (!root.getAttribute("data-oh-mf-i18n")) {
      root.setAttribute("data-oh-mf-i18n", "1");
