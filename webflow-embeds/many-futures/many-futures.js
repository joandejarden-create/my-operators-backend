(function () {
  ...
  /* Mobile: after relocating the workspace under the selected question, keep
     that question's top readable under sticky nav (avoid scroll-up to start). */
  function scrollActiveQuestionIntoView(id) {
    if (!isMobile() || !id) return;
    var btn = root.querySelector('.mf-q[data-q="' + id + '"]');
    if (!btn) return;
  ...
  function setActive(id, fromUser) {
    ...
    if (id === current) {
      ...
      drawConnectors(id);
      if (fromUser) scrollActiveQuestionIntoView(id);
      return;
    }
    current = id;
