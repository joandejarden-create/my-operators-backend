#!/usr/bin/env node
/**
 * Nine-question local extension review package.
 * Does NOT modify Webflow. Local preview only.
 */
...
/* Transfer weights: default (Q1) then after visiting all nine */
{
  ...
  const defaultAssets = transferred.map((t) => ({ ...t }));
  ...
  const uniqueTotal = Object.values(unique).reduce((s, n) => s + n, 0);
    ...
    JSON.stringify(
      {
        ...
        uniqueAssetBytes: uniqueTotal,
        uniqueAssetKB: Math.round((uniqueTotal / 1024) * 10) / 10,
        defaultAssets: defaultAssets.filter((a) => a.bytes > 0),
        allUniqueAssets: Object.entries(unique)
          .map(([url, bytes]) => ({ url, bytes }))
          .sort((a, b) => b.bytes - a.bytes),
        newlyIntroducedImageAssets: [],
        note: "Q5–Q7 use HTML/CSS reconstructions; Q6 reuses existing Radar images. No new raster assets introduced.",
      },
      null,
      2
    )
  );
  console.log("transfer default KB", Math.round(defaultBytes / 1024), "all nine KB", Math.round(totalBytes / 1024));
  await context.close();
}

/* Desktop Q5–Q7 + height compare */
{
  const page = await openPage(1440, 1200);
  await activate(page, "confidential");
  await save(page, "01-desktop-1440-q5-confidential.png");
  await activate(page, "market");
  await save(page, "02-desktop-1440-q6-market.png");
  await activate(page, "actions");
  await save(page, "03-desktop-1440-q7-actions.png");
  ...
  console.log("height compare", heights);

  /* Crop hotel + questions columns for height visual */
  const boxes = await page.evaluate(() => {
    ...
    return {
      clip: {
        x: Math.max(0, rootBox.left + left - 8),
        y: Math.max(0, rootBox.top + top - 8),
  ...
  await page.screenshot({
    path: join(outDir, "04-desktop-height-hotel-vs-questions.png"),
    clip: boxes.clip,
  });
  console.log("wrote height crop");
  await page.close();
}
...
def make_sheet(items, title, outfile, cols=3, cell_w=520, label_h=40, pad=14, bg=(12, 18, 36), max_h=720):
    ...
    draw.text((pad, 40), "Local extension review — Webflow not modified", font=font(11), fill=(160, 170, 200))
