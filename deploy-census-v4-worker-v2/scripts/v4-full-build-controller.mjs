...
async function main() {
  ...
  console.log(
    `[controller] start live=${startingLive} iterations<=${cfg.max_iterations_per_process} runtime<=${cfg.max_runtime_ms}ms`
  );
  ...
  for (let iter = 1; iter <= cfg.max_iterations_per_process; iter++) {
    ...
    // ========== LANE: OFFICIAL DIRECTORY ==========
      ...
      if (!queue.length) {
        ...
        // Partial network/source remaining: do NOT permanently exhaust — allow resume next run
        const partial =
        ...
        laneResult.note = `directory_no_new discovered=${discovered.length} new_class=${neu.length} status=${laneStatus || "complete"}`;
      } else {
        const r = await insertCandidates(baseId, token, queue, {
          keys,
          urls,
    ...
    // ========== LANE: INDEPENDENT REDISCOVERY ==========
      ...
      if (!cachedDiscovered) {
        // Need directory evidence — do not stop build; mark and let next iter discover
        exhaustedLanes = exhaustedLanes.filter((l) => l !== LANES.OFFICIAL_DIRECTORY_DISCOVERY);
        directoryWaveExhausted = false;
        laneResult.note = "deferred_need_directory_cache";
        // Don't add INDEPENDENT to exhausted — retry after discovery
      } else {
